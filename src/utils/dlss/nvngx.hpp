...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_CONV hooked_NVSDK_NGX_D3D12_CreateFeature(ID3D12GraphicsCommandList* InCmdList, NVSDK_NGX_Feature InFeatureID, NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle) {
  auto* original_cmd_list = InCmdList;
  const bool unwrapped = utils::directx::NativeFromReShadeProxy(&InCmdList);
#ifdef DEBUG_LEVEL_2
  if (unwrapped) {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_CreateFeature() - Using native command list ",
        utils::log::AsPtr(original_cmd_list),
        " => ",
        utils::log::AsPtr(InCmdList));
  } else {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_CreateFeature() - Command list already native: ",
        utils::log::AsPtr(InCmdList));
  }
#endif
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_CONV hooked_NVSDK_NGX_D3D12_EvaluateFeature(ID3D12GraphicsCommandList* InCmdList, const NVSDK_NGX_Handle* InFeatureHandle, const NVSDK_NGX_Parameter* InParameters, PFN_NVSDK_NGX_ProgressCallback InCallback = NULL) {
  ...
  if (unwrapped) {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_EvaluateFeature() - Using native command list ",
        utils::log::AsPtr(original_cmd_list),
        " => ",
        utils::log::AsPtr(InCmdList));
  } else {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_EvaluateFeature() - Command list already native: ",
        utils::log::AsPtr(InCmdList));
  }
#endif
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_CONV hooked_NVSDK_NGX_D3D12_EvaluateFeature_C(ID3D12GraphicsCommandList* InCmdList, const NVSDK_NGX_Handle* InFeatureHandle, const NVSDK_NGX_Parameter* InParameters, PFN_NVSDK_NGX_ProgressCallback_C InCallback) {
  ...
  if (unwrapped) {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_EvaluateFeature_C() - Using native command list ",
        utils::log::AsPtr(original_cmd_list),
        " => ",
        utils::log::AsPtr(InCmdList));
  } else {
    utils::log::d(
        "utils::dlss::nvngx::hooked_NVSDK_NGX_D3D12_EvaluateFeature_C() - Command list already native: ",
        utils::log::AsPtr(InCmdList));
  }
#endif
