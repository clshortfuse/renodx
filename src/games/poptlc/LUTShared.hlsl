...
float3 GetSceneColor(float2 inCoords, Texture2D<float4> SourceTexture, SamplerState sampler0)
{
  ...
  return SourceTexture.SampleBias(sampler0, inCoords.xy, sampleBias).rgb;
}

float3 ApplyBloom(float2 inCoords, float3 color, Texture2D<float4> BloomTexture, SamplerState sampler0)
{
  const float sampleBias = cb0[21].x; // Expected to be zero, though it could be used by the game to do a very ugly game blur effect
...
float3 ApplyLUT(float2 inCoords, float3 color, Texture2D<float4> InternalGradingLUT, SamplerState sampler0)
{
  float3 outColor;
  ...
  if (injectedData.toneMapType == 0) {
    ...
    if (injectedData.lutExtrapolation) {
        ...
        outColor.rgb = RestorePostProcess(color, saturate(color), postLutColor);
    }
...
float3 ApplyVignette(float2 inCoords, float3 color, Texture2D<float4> VignetteTexture, SamplerState sampler0)
{
  const float sampleBias = cb0[21].x; // Expected to be zero, though it could be used by the game to do a very ugly game blur effect

  float4 vignette;
  vignette.rgba = VignetteTexture.SampleBias(sampler0, inCoords.xy, sampleBias).rgba;
  vignette.rgb = vignette.rgb * cb0[144].rgb + -color.rgb;
  float alpha = cb0[144].w * vignette.w;
