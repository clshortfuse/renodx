...
// ---- Created with 3Dmigoto v1.3.16 on Thu Nov 27 12:47:39 2025
...
cbuffer _Globals : register(b0)
{
  ...
  float fLensFlareWeight : packoffset(c6) = {0.300000012};
  float2 SimulateHDRParams : packoffset(c6.y);
  float fSaturationScaleEx : packoffset(c6.w) = {1};
  float4 vLightShaftPower : packoffset(c7);
  float3 vColorScale : packoffset(c8) = {1,1,1};
  float3 vSaturationScale : packoffset(c9) = {1,1,1};
  float2 vScreenSize : packoffset(c10) = {1280,720};
  float4 vSpotParams : packoffset(c11) = {640,360,300,400};
  float fLimbDarkening : packoffset(c12) = {755364.125};
  float fLimbDarkeningWeight : packoffset(c12.y) = {0};
  float fGamma : packoffset(c12.z) = {1};
}
...
SamplerState smplFlare_s : register(s8);
SamplerState smplLightShaftLinWork2_s : register(s9);
...
Texture2D<float4> smplFlare_Tex : register(t8);
Texture2D<float4> smplLightShaftLinWork2_Tex : register(t9);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.xyz = smplFlare_Tex.Sample(smplFlare_s, v1.xy).xyz;
  r0.xyz = r0.xyz * fLensFlareWeight + r1.yzw;
