...
// ---- Created with 3Dmigoto v1.3.16 on Thu Nov 27 12:47:37 2025

cbuffer _Globals : register(b0)
{
  float4 fChromaAberraParam : packoffset(c0) = {0,0,1,0};
  int nChromaAberraTypeIndex : packoffset(c1) = {0};
  float fLensFlareWeight : packoffset(c1.y) = {0.300000012};
  float2 SimulateHDRParams : packoffset(c1.z);
  float fSaturationScaleEx : packoffset(c2) = {1};
  float3 vColorScale : packoffset(c2.y) = {1,1,1};
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r1.yzw = smplFlare_Tex.Sample(smplFlare_s, v1.xy).xyz;
  r1.yzw = fLensFlareWeight * r1.yzw;
