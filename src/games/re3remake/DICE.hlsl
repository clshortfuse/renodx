...
float rangeCompress(float X, float Max = renodx::math::FLT_MAX) {
...
float luminanceCompress(float InValue, float OutMaxValue,
                        ...
                        float InMaxValue = renodx::math::FLT_MAX) {
  ...
  const float possibleOutValue =
      ShoulderStart + compressedRange * rangeCompress(compressableValue / compressedRange, ConsiderMaxValue ? (compressableRange / compressedRange) : renodx::math::FLT_MAX);
#if 1
