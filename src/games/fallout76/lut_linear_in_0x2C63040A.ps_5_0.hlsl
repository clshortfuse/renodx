...
void main(float4 v0 : SV_POSITION0, float2 v1 : TEXCOORD0, out float4 o0 : SV_Target0) {
  ...
  config.reno_drt_saturation = 2.0f;
  config.reno_drt_dechroma = injectedData.colorGradeBlowout;
  config.reno_drt_flare = 0.5f;
  ...
  if (injectedData.colorGradeLUTStrength == 0.f || config.type == 1.f) {
    ...
    r2.xyz = renodx::lut::Sample(t3, lut_config, r1.xyz);
    ...
    if (config.type == 0.f) {
      outputColor = lerp(outputColor, o0.xyz, lut_config.strength);
    } else {
      outputColor = renodx::tonemap::UpgradeToneMap(hdrColor, sdrColor, o0.xyz, lut_config.strength);
    }
  }
  o0.rgb = outputColor;
