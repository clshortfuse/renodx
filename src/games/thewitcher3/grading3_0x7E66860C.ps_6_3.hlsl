...
float4 main(
  ...
  //_19.rgb = ungraded_sdr;
  float4 ungraded_sdr = ColorGradingSDR(ungraded);

  float3 lut_sampled_sdr1 = LUTSampling(ungraded_sdr.xyz, ungraded_sdr.xyz, t1, s1);
  float3 lut_sampled_sdr2 = LUTSampling(lut_sampled_sdr1, ungraded_sdr.xyz, t3, s3);

  float3 graded_sdr = VanillaOutput3(lut_sampled_sdr1, lut_sampled_sdr2, ungraded_sdr.xyz);
