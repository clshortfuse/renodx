...
float4 main(
  ...
  //float3 ungraded_sdr_gamma = renodx::color::gamma::EncodeSafe(ungraded_sdr.xyz);
  //float4 lut_coords = VanillaSamplingCoords(ungraded_sdr_gamma);

  float3 lut_sampled_sdr = LUTSampling(ungraded_sdr.xyz, ungraded_sdr.xyz, t1, s1);
  //float3 graded_sdr = (((CustomPixelConsts_016.z * lut_sampled_sdr.xyz) - ungraded_sdr.xyz) * CustomPixelConsts_016.y) + ungraded_sdr.xyz;
  float3 graded_sdr = VanillaOutput1(lut_sampled_sdr, ungraded_sdr.xyz);
