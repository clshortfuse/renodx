...
float3 ApplyColorGradingLUTs(
  ...
  if (COLOR_GRADE_LUT_SCALING > 0.f) {
    float3 lut_zero_output = BlendLUTs(
        0.f,
        fTextureSize,
        fTextureBlendRate,
        fTextureBlendRate2,
...
void ApplyColorGrading(
  ...
  if (RENODX_TONE_MAP_TYPE == 0.f) {
    ...
    compression_scale = ApplyAnchoredCInfinityShoulderMaxChannelScale(color_input);
  }
...
float3 ApplyUserGradingAndToneMap(float3 color_bt709, float2 grain_uv) {
  ...
  {
    color_bt2020 = color_bt2020 * ApplyAnchoredCInfinityShoulderMaxChannelScale(color_bt2020, PEAK_RATIO);
  }
