...
void main(
  ...
  if (CUSTOM_SHARPENING == 0.f) {
    ...
    OutputImage[int2(_119, _16)] = float4(SampleScaledSource(SrcImage, int2(_119, _16), tex_max, const0), 1.f);
    OutputImage[int2(_119, _220)] = float4(SampleScaledSource(SrcImage, int2(_119, _220), tex_max, const0), 1.f);
    OutputImage[int2(_15, _220)] = float4(SampleScaledSource(SrcImage, int2(_15, _220), tex_max, const0), 1.f);
    return;
    ...
    // Run RCAS using bilinear-scaled samples (keeps fractional coords to avoid aliasing).
    OutputImage[int2(_15, _16)] = float4(ApplyLiliumRCASScaled(SrcImage, int2(_15, _16), sharpness_strength, tex_max, const0), 1.f);
    OutputImage[int2(_119, _16)] = float4(ApplyLiliumRCASScaled(SrcImage, int2(_119, _16), sharpness_strength, tex_max, const0), 1.f);
    OutputImage[int2(_119, _220)] = float4(ApplyLiliumRCASScaled(SrcImage, int2(_119, _220), sharpness_strength, tex_max, const0), 1.f);
    OutputImage[int2(_15, _220)] = float4(ApplyLiliumRCASScaled(SrcImage, int2(_15, _220), sharpness_strength, tex_max, const0), 1.f);
    return;
  }
