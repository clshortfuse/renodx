...
void main(
  ...
  if (CUSTOM_SHARPENING == 0) {
    OutputImage[int2(_15, _16)] = SrcImage.Load(int3(_15, _16, 0));
    OutputImage[int2(_15 | 8, _16)] = SrcImage.Load(int3(_15 | 8, _16, 0));
    OutputImage[int2(_15 | 8, _16 | 8)] = SrcImage.Load(int3(_15 | 8, _16 | 8, 0));
    OutputImage[int2(_15, _16 | 8)] = SrcImage.Load(int3(_15, _16 | 8, 0));
    return;
  } else if (CUSTOM_SHARPENING == 2) {       // Lilium RCAS
    const float sharpness_strength = 0.75f;  // asfloat(const1.x)
    uint2 tex_max;
