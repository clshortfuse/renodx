...
float4 main(
  ...
  float4 SV_Target;
  float _25 = EyeAdaptationBuffer[0].x;
  float4 _37 = ColorTexture.Sample(ColorSampler, float2(min(max(TEXCOORD.x, Color_UVViewportBilinearMin.x), Color_UVViewportBilinearMax.x), min(max(TEXCOORD.y, Color_UVViewportBilinearMin.y), Color_UVViewportBilinearMax.y)));
  float4 _61 = BloomTexture.Sample(BloomSampler, float2(min(max(((ColorToBloom.x * TEXCOORD.x) + ColorToBloom.z), BloomUVViewportBilinearMin.x), BloomUVViewportBilinearMax.x), min(max(((ColorToBloom.y * TEXCOORD.y) + ColorToBloom.w), BloomUVViewportBilinearMin.y), BloomUVViewportBilinearMax.y)));
  float4 _80 = BloomDirtMaskTexture.Sample(BloomDirtMaskSampler, float2(((((LensPrincipalPointOffsetScale.z * TEXCOORD_3.x) + LensPrincipalPointOffsetScale.x) * 0.5f) + 0.5f), (0.5f - (((LensPrincipalPointOffsetScale.w * TEXCOORD_3.y) + LensPrincipalPointOffsetScale.y) * 0.5f))));
  float _100 = SceneColorApplyParamaters[0].x;
  float _101 = SceneColorApplyParamaters[0].y;
  float _102 = SceneColorApplyParamaters[0].z;
  float _103 = _25 * View_OneOverPreExposure;
  float _119 = (((_37.x * _103) * ColorScale0.x) * _100) + ((_61.x * _103) * ((BloomDirtMaskTint.x * _80.x) + 1.0f));
  float _120 = (((_37.y * _103) * ColorScale0.y) * _101) + ((_61.y * _103) * ((BloomDirtMaskTint.y * _80.y) + 1.0f));
  float _121 = (((_37.z * _103) * ColorScale0.z) * _102) + ((_61.z * _103) * ((BloomDirtMaskTint.z * _80.z) + 1.0f));
  float _172;
  float _183;
  float _194;
  float _396;
  ...
  float _474;
  if (ToneMapImpl == 0) {
    float4 _152 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * saturate((log2(_119 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2(_120 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2(_121 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset)));
    _396 = (_152.x * 1.0499999523162842f);
    _397 = (_152.y * 1.0499999523162842f);
    _398 = (_152.z * 1.0499999523162842f);
  } else {
    if (ToneMapImpl == 1) {
      if (_119 < 0.0031306699384003878f) {
        _172 = (_119 * 12.920000076293945f);
      } else {
        _172 = (((pow(_119, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
      }
      if (_120 < 0.0031306699384003878f) {
        _183 = (_120 * 12.920000076293945f);
      } else {
        _183 = (((pow(_120, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
      }
      if (_121 < 0.0031306699384003878f) {
        _194 = (_121 * 12.920000076293945f);
      } else {
        _194 = (((pow(_121, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
      }
      _396 = saturate(_172);
      _397 = saturate(_183);
      _398 = saturate(_194);
      ...
      if ((ToneMapImpl & -2) == 2) {
        ...
        float _226 = max(0.0f, _119);
        float _227 = max(0.0f, _120);
        float _228 = max(0.0f, _121);
        float _231 = mad(0.18048079311847687f, _228, mad(0.3575843274593353f, _227, (_226 * 0.412390798330307f)));
        float _234 = mad(0.07219231873750687f, _228, mad(0.7151686549186707f, _227, (_226 * 0.2126390039920807f)));
        float _237 = mad(0.9505321383476257f, _228, mad(0.11919478327035904f, _227, (_226 * 0.019330818206071854f)));
        float _259 = min(max(log2(mad((AgX_CompressionMatrixInv[0].z), _237, mad((AgX_CompressionMatrixInv[0].y), _234, (_231 * (AgX_CompressionMatrixInv[0].x)))) * 5.55555534362793f), -10.0f), 6.5f) + 10.0f;
        float _260 = min(max(log2(mad((AgX_CompressionMatrixInv[1].z), _237, mad((AgX_CompressionMatrixInv[1].y), _234, (_231 * (AgX_CompressionMatrixInv[1].x)))) * 5.55555534362793f), -10.0f), 6.5f) + 10.0f;
        float _261 = min(max(log2(mad((AgX_CompressionMatrixInv[2].z), _237, mad((AgX_CompressionMatrixInv[2].y), _234, (_231 * (AgX_CompressionMatrixInv[2].x)))) * 5.55555534362793f), -10.0f), 6.5f) + 10.0f;
        float _262 = _259 * 0.06060606241226196f;
        float _263 = _260 * 0.06060606241226196f;
        float _264 = _261 * 0.06060606241226196f;
        float _265 = _262 * _262;
        float _266 = _263 * _263;
        float _267 = _264 * _264;
        float _268 = _265 * _265;
        float _269 = _266 * _266;
        float _270 = _267 * _267;
        float _331 = exp2(log2(saturate((((((0.0028570001013576984f - (_259 * 0.010412122122943401f)) + (_265 * 4.361000061035156f)) + (_268 * 92.05999755859375f)) - ((_259 * 1.7406060695648193f) * _265)) - ((_259 * 7.678787708282471f) * _268)) + ((_268 * _265) * (78.01000213623047f - (_259 * 1.082424283027649f))))) * 2.200000047683716f);
        float _332 = exp2(log2(saturate((((((0.0028570001013576984f - (_260 * 0.010412122122943401f)) + (_266 * 4.361000061035156f)) + (_269 * 92.05999755859375f)) - ((_260 * 1.7406060695648193f) * _266)) - ((_260 * 7.678787708282471f) * _269)) + ((_269 * _266) * (78.01000213623047f - (_260 * 1.082424283027649f))))) * 2.200000047683716f);
        float _333 = exp2(log2(saturate((((((0.0028570001013576984f - (_261 * 0.010412122122943401f)) + (_267 * 4.361000061035156f)) + (_270 * 92.05999755859375f)) - ((_261 * 1.7406060695648193f) * _267)) - ((_261 * 7.678787708282471f) * _270)) + ((_270 * _267) * (78.01000213623047f - (_261 * 1.082424283027649f))))) * 2.200000047683716f);
        float _336 = mad((AgX_CompressionMatrix[0].z), _333, mad((AgX_CompressionMatrix[0].y), _332, (_331 * (AgX_CompressionMatrix[0].x))));
        float _339 = mad((AgX_CompressionMatrix[1].z), _333, mad((AgX_CompressionMatrix[1].y), _332, (_331 * (AgX_CompressionMatrix[1].x))));
        float _342 = mad((AgX_CompressionMatrix[2].z), _333, mad((AgX_CompressionMatrix[2].y), _332, (_331 * (AgX_CompressionMatrix[2].x))));
        float _358 = exp2(log2(mad(-0.4986107647418976f, _342, mad(-1.5373831987380981f, _339, (_336 * 3.2409698963165283f)))) * 0.45454543828964233f);
        float _359 = exp2(log2(mad(0.04155505821108818f, _342, mad(1.8759675025939941f, _339, (_336 * -0.9692436456680298f)))) * 0.45454543828964233f);
        float _360 = exp2(log2(mad(1.056971549987793f, _342, mad(-0.20397695899009705f, _339, (_336 * 0.05563008040189743f)))) * 0.45454543828964233f);
        if (ToneMapImpl == 3) {
          float _374 = saturate(pow(_358, AgX_PostContrast));
          float _375 = saturate(pow(_359, AgX_PostContrast));
          float _376 = saturate(pow(_360, AgX_PostContrast));
          float _378 = dot(float3(_374, _375, _376), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f));
          _396 = saturate(lerp(_378, _374, AgX_PostSaturation));
          _397 = saturate(lerp(_378, _375, AgX_PostSaturation));
          _398 = saturate(lerp(_378, _376, AgX_PostSaturation));
        } else {
          _396 = saturate(_358);
          _397 = saturate(_359);
          _398 = saturate(_360);
        ...
        float3 sdr_final = renodx::color::srgb::DecodeSafe(float3(_396, _397, _398));
  ...
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _409 = (pow(_396, 0.012683313339948654f));
    float _410 = (pow(_397, 0.012683313339948654f));
    float _411 = (pow(_398, 0.012683313339948654f));
    float _444 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_409 + -0.8359375f)) / (18.8515625f - (_409 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _445 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_410 + -0.8359375f)) / (18.8515625f - (_410 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _446 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_411 + -0.8359375f)) / (18.8515625f - (_411 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _472 = min((_444 * 12.920000076293945f), ((exp2(log2(max(_444, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _473 = min((_445 * 12.920000076293945f), ((exp2(log2(max(_445, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _474 = min((_446 * 12.920000076293945f), ((exp2(log2(max(_446, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _472 = _396;
    _473 = _397;
    _474 = _398;
  }
  float _481 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _484 = min(max((_481 * +1.#INF), -1.0f), 1.0f);
  float _493 = (_484 - (sqrt(saturate(1.0f - abs(_481))) * _484)) * BackbufferQuantizationDithering;
  SV_Target.x = (_493 + _472);
  SV_Target.y = (_493 + _473);
  SV_Target.z = (_493 + _474);
  SV_Target.w = 0.0f;
  return SV_Target;
}
