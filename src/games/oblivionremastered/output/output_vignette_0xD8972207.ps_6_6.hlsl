...
OutputSignature main(
  ...
  HandleLocalExposure(_33 * View_OneOverPreExposure, _181, _182);

  float4 _224 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * saturate((log2((((((BloomDirtMaskTint.x * _89.x) + 1.0f) * _70.x) * _181) + 0.002667719265446067f) + (((ColorScale0.x * _46.x) * _120) * _182)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.y * _89.y) + 1.0f) * _70.y) * _181) + 0.002667719265446067f) + (((ColorScale0.y * _46.y) * _121) * _182)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.z * _89.z) + 1.0f) * _70.z) * _181) + 0.002667719265446067f) + (((ColorScale0.z * _46.z) * _122) * _182)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset)));
  float _228 = _224.x * 1.0499999523162842f;
  float _229 = _224.y * 1.0499999523162842f;
  float _230 = _224.z * 1.0499999523162842f;

  SV_Target_1 = dot(float3(_228, _229, _230), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
  HandleLUTOutput(_228, _229, _230, SV_Target_1, TEXCOORD.xy, true);
  ...
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _242 = (pow(_228, 0.012683313339948654f));
    float _243 = (pow(_229, 0.012683313339948654f));
    float _244 = (pow(_230, 0.012683313339948654f));
    float _277 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_242 + -0.8359375f)) / (18.8515625f - (_242 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _278 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_243 + -0.8359375f)) / (18.8515625f - (_243 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _279 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_244 + -0.8359375f)) / (18.8515625f - (_244 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _305 = min((_277 * 12.920000076293945f), ((exp2(log2(max(_277, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _306 = min((_278 * 12.920000076293945f), ((exp2(log2(max(_278, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _307 = min((_279 * 12.920000076293945f), ((exp2(log2(max(_279, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _305 = _228;
    _306 = _229;
    _307 = _230;
  ...
  if (CUSTOM_GRAIN_TYPE == 0.f) {
    float _314 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
    float _317 = min(max((_314 * +1.#INF), -1.0f), 1.0f);
    _326 = (_317 - (sqrt(saturate(1.0f - abs(_314))) * _317)) * BackbufferQuantizationDithering;
  }
  SV_Target.x = (_326 + _305);
  SV_Target.y = (_326 + _306);
  SV_Target.z = (_326 + _307);
  SV_Target.w = 0.0f;
  // SV_Target_1 = dot(float3(_228, _229, _230), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
