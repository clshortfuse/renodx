...
void main(
  ...
  if (!(max(abs(((_29 + float((uint)(int)(Output_ViewportMin.x))) * Output_ExtentInverse.x) + -0.5f), abs(((_31 + float((uint)(int)(Output_ViewportMin.y))) * Output_ExtentInverse.y) + -0.5f)) >= 0.5f)) {
    ...
    float4 _169 = ColorTexture.SampleLevel(ColorSampler, float2(min(max(_49, Color_UVViewportBilinearMin.x), Color_UVViewportBilinearMax.x), min(max(_50, Color_UVViewportBilinearMin.y), Color_UVViewportBilinearMax.y)), 0.0f);
    ...
    float _231 = log2(max(dot(float3((_157.x * View_OneOverPreExposure), (_163.y * View_OneOverPreExposure), (_169.z * View_OneOverPreExposure)), float3(0.3333333432674408f, 0.3333333432674408f, 0.3333333432674408f)), EyeAdaptation_LuminanceMin));
    float _236 = log2((_75 * 0.18000000715255737f) * LocalExposure_MiddleGreyExposureCompensation);
    float4 _251 = LumBilateralGrid.SampleLevel(LumBilateralGridSampler, float3((LocalExposure_BilateralGridUVScale.x * _49), (LocalExposure_BilateralGridUVScale.y * _50), ((((EyeAdaptation_HistogramScale * _231) + EyeAdaptation_HistogramBias) * 0.96875f) + 0.015625f)), 0.0f);
    float4 _257 = BlurredLogLum.SampleLevel(BlurredLogLumSampler, float2(_49, _50), 0.0f);
    float _260 = select((_251.y < 0.0010000000474974513f), _257.x, (_251.x / _251.y));
    float _263 = log2(_74);
    float _265 = (_260 + _263) + ((_257.x - _260) * LocalExposure_BlurredLuminanceBlend);
    float _269 = _263 + _231;
    float _271 = _265 - _236;
    float _280 = _74 * View_OneOverPreExposure;
    float _281 = _280 * exp2(((_236 - _269) + ((_269 - _265) * LocalExposure_DetailStrength)) + (select((_271 > 0.0f), LocalExposure_HighlightContrastScale, LocalExposure_ShadowContrastScale) * _271));
    // float _309 = exp2(log2(((((ColorScale0.x * _157.x) * _221) * _281) + ((_191.x * _280) * ((BloomDirtMaskTint.x * _201.x) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
    // float _310 = exp2(log2(((((ColorScale0.y * _163.y) * _222) * _281) + ((_191.y * _280) * ((BloomDirtMaskTint.y * _201.y) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
    // float _311 = exp2(log2(((((ColorScale0.z * _169.z) * _223) * _281) + ((_191.z * _280) * ((BloomDirtMaskTint.z * _201.z) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
    // float4 _350 = ColorGradingLUT.SampleLevel(ColorGradingLUTSampler, float3(((LUTScale * exp2(log2((1.0f / ((_309 * 18.6875f) + 1.0f)) * ((_309 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_310 * 18.6875f) + 1.0f)) * ((_310 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_311 * 18.6875f) + 1.0f)) * ((_311 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset)), 0.0f);

    float3 bloom_extra = ((((BloomDirtMaskTint.rgb * _201.rgb) + 1.0f) * _191.rgb) * _280) * CUSTOM_BLOOM;
    float autoexposure = lerp(1.f, _281, CUSTOM_AUTO_EXPOSURE);
    float3 scaled_color = (((ColorScale0.rgb * float3(_157.x, _163.y, _169.z)) * SceneColorApplyParamaters[0].rgb) * autoexposure);
    ...
    float4 _350 = ColorGradingLUT.SampleLevel(ColorGradingLUTSampler, lut_coordinates, 0.0f);

    float _354 = _350.x * 1.0499999523162842f;
    float _355 = _350.y * 1.0499999523162842f;
    float _356 = _350.z * 1.0499999523162842f;
    float _357 = dot(float3(_354, _355, _356), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));

    HandleLUTOutput(_354, _355, _356, _357, float2(_49, _50), true);
    ...
    if (!(bOutputInHDR == 0)) {
      ...
      float _404 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_369 + -0.8359375f)) / (18.8515625f - (_369 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
      float _405 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_370 + -0.8359375f)) / (18.8515625f - (_370 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    ...
    if (CUSTOM_GRAIN_TYPE == 0.f) {
      float _440 = (frac(sin((_49 + GrainRandomFull.x) + ((_50 + GrainRandomFull.y) * 543.3099975585938f)) * 493013.0f) * 2.0f) + -1.0f;
      float _443 = min(max((_440 * +1.#INF), -1.0f), 1.0f);
      _452 = (_443 - (sqrt(saturate(1.0f - abs(_440))) * _443)) * BackbufferQuantizationDithering;
    }
    uint _459 = (int)(Output_ViewportMin.x) + SV_DispatchThreadID.x;
    uint _460 = (int)(Output_ViewportMin.y) + SV_DispatchThreadID.y;
    RWOutputTexture[int2(_459, _460)] = float4((_452 + _431), (_452 + _432), (_452 + _433), 0.0f);
    RWOutputLuminance[int2(_459, _460)] = _357;
  }
}
