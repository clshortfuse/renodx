...
float3 GenerateOutput(float3 untonemapped_ap1, float2 uv, uint output_mode) {
  ...
  if (TONE_MAP_TYPE == 1.f) {  // RenoDX (Enhanced)
    ...
    const float highlight_contrast = 46.f / 50.f;
    const float shadow_contrast = 1.f;
    const float shadows = 1.f;
    const float flare = 0.93f;
    const float compression = 1.5f;
#else  // Match HDR
    const float mid_gray_in = 0.18f;
    const float mid_gray_out = 0.0994752f;
    const float cone_response_exponent = 1.344681f;
    const float highlight_contrast = 46.f / 50.f;
    const float shadow_contrast = 1.75f;
    const float shadows = 1.f;
    const float flare = 0.7f;
    const float compression = 100.f;
#endif
    const int gamut_compression_mode = output_mode == 0u
    ...
    float3 tonemapped_bt709 = renodx::tonemap::psychov::custom_psychotm_test31(
        ...
        shadows * RENODX_TONE_MAP_SHADOWS,
