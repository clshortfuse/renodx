...
// decode type: 0 = PQ, 1 = sRGB, 2 = Gamma 2.2, 3 = Linear
float3 CustomDecode(float3 color, int decode_type, float pq_scaling) {
  if (decode_type == 0) {
    color.xyz = renodx::color::pq::DecodeSafe(color.xyz, pq_scaling);
    color.xyz = renodx::color::bt709::from::BT2020(color.xyz);
  }
  else if (decode_type == 1) {
    color.xyz = renodx::color::srgb::Decode(color.xyz);
  }
  else if (decode_type == 2) {
    color.xyz = renodx::color::gamma::Decode(color.xyz);
  }
