...
void main(
  ...
  if (CUSTOM_SHARPENING == 1.f) {  // Lilium RCAS
    ...
    if (sharpness_strength == 0.f) {
      ...
      OutputImage[int2(_15 | 8, _16)] = float4(SrcImage.Load(int3(_15 | 8, _16, 0)).rgb, 1.f);
      OutputImage[int2(_15 | 8, _16 | 8)] = float4(SrcImage.Load(int3(_15 | 8, _16 | 8, 0)).rgb, 1.f);
      OutputImage[int2(_15, _16 | 8)] = float4(SrcImage.Load(int3(_15, _16 | 8, 0)).rgb, 1.f);
    } else {
      uint2 tex_max;
      ...
      OutputImage[int2(_15 | 8, _16)] = float4(ApplyLiliumRCAS(SrcImage, int2(_15 | 8, _16), sharpness_strength, tex_max), 1.f);
      OutputImage[int2(_15 | 8, _16 | 8)] = float4(ApplyLiliumRCAS(SrcImage, int2(_15 | 8, _16 | 8), sharpness_strength, tex_max), 1.f);
      OutputImage[int2(_15, _16 | 8)] = float4(ApplyLiliumRCAS(SrcImage, int2(_15, _16 | 8), sharpness_strength, tex_max), 1.f);
    }
