...
float4 main(
    ...
    : SV_Target {
  return HDRComposite(TEXCOORD, TEXCOORD_1, SV_Position, true);
}
...
float4 main(
  ...
  // 8333.33 = LIGHT_UNIT_SCALE / 1.2
  // 3030 = log2(fIlluminance * LIGHT_UNIT_SCALE * 100.0 / 330.0)?
  ...
  // vignetting_x_exposure * lerp(cScene.xyz, cBloom.xyz, saturate(HDRBloomColor.rgb))
  ...
  float _91 = (PER_BATCH_004x) * 0.2199999988079071f;   // 0.22 * HDRFilmCurve.x
  float _93 = (PER_BATCH_004y) * 0.30000001192092896f;  // LinStren = 0.3 * HDRFilmCurve.y
  float _95 = _91 * _85;
  float _96 = _91 * _86;
  float _97 = _91 * _87;
  float _98 = _91 * (PER_BATCH_004w);
  float _99 = (PER_BATCH_004y) * 0.030000001192092896f;    // LinStren = 0.3 * HDRFilmCurve.y
  float _108 = (PER_BATCH_004z) * 0.0020000000949949026f;  // ToeStren*ToeNum
  float _129 = (PER_BATCH_004z) * 0.03333333134651184f;    // (ToeNum/ToeDenom)?
  // ToeStren*ToeDenom = 0.06000000238418579f
  float _133 = ((((_98 + _99) * (PER_BATCH_004w)) + _108) / (((_98 + _93) * (PER_BATCH_004w)) + 0.06000000238418579f)) - _129;  // compressedCol.w?
  ...
  float3 testSDR = float3(_168, _168, _169);
  ...
  testSDR = renodx::color::srgb::DecodeSafe(testSDR);

  testSDR = renodx::lut::Sample(
      testSDR,
      lut_config,
      colorChartTex);
  // SV_Target.rgb = Tonemap(SV_Target.rgb, untonemapped);
  // SV_Target.rgb = testSDR;
