// ---- Created with 3Dmigoto v1.4.1 on Sun Apr 20 14:49:53 2025
  ...
  out float4 o1 : SV_Target1)
{
  ...
  r3.z = ColorSampler.SampleLevel(Clamp_s, r2.xy, 0, int2(0, 0)).x;
  r3.z = max(0, r3.z);
  r4.x = min(65000, r3.z);
  r3.z = ColorSampler.SampleLevel(Clamp_s, r2.zw, 0, int2(0, 0)).y;
  r3.z = max(0, r3.z);
  r4.y = min(65000, r3.z);
  r3.z = ColorSampler.SampleLevel(Clamp_s, r3.xy, 0, int2(0, 0)).z;
  r3.z = max(0, r3.z);
  r4.z = min(65000, r3.z);
  r2.x = ColorAndMaskTexture.SampleLevel(Clamp_s, r2.xy, 0).x;
  r2.x = max(0, r2.x);
  r5.x = min(5000, r2.x);
  r2.x = ColorAndMaskTexture.SampleLevel(Clamp_s, r2.zw, 0).y;
  r2.x = max(0, r2.x);
  r5.y = min(5000, r2.x);
  r2.x = ColorAndMaskTexture.SampleLevel(Clamp_s, r3.xy, 0).z;
  r2.x = max(0, r2.x);
  r5.z = min(5000, r2.x);
