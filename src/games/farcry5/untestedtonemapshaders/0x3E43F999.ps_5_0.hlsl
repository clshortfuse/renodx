// ---- Created with 3Dmigoto v1.4.1 on Sun Apr 20 14:49:29 2025
...
Texture2D<float4> ColorSeparationMaskTexture : register(t5);
Texture2D<float4> ColorMaskTexture : register(t6);
  ...
  out float4 o1 : SV_Target1)
{
  ...
  r2.xy = v1.xy * ColorSeparationUVScaleBias0.xy + ColorSeparationUVScaleBias0.zw;
  r2.zw = v1.xy * ColorSeparationUVScaleBias1.xy + ColorSeparationUVScaleBias1.zw;
  r3.xy = v1.xy * ColorSeparationUVScaleBias2.xy + ColorSeparationUVScaleBias2.zw;
  r1.w = ColorSampler.SampleLevel(Clamp_s, r2.xy, 0, int2(0, 0)).x;
  r1.w = max(0, r1.w);
  r4.x = min(65000, r1.w);
  r1.w = ColorSampler.SampleLevel(Clamp_s, r2.zw, 0, int2(0, 0)).y;
  r1.w = max(0, r1.w);
  r4.y = min(65000, r1.w);
  r1.w = ColorSampler.SampleLevel(Clamp_s, r3.xy, 0, int2(0, 0)).z;
  r1.w = max(0, r1.w);
  r4.z = min(65000, r1.w);
  r1.w = ColorAndMaskTexture.SampleLevel(Clamp_s, r2.xy, 0).x;
  r1.w = max(0, r1.w);
  r5.x = min(5000, r1.w);
  r1.w = ColorAndMaskTexture.SampleLevel(Clamp_s, r2.zw, 0).y;
  r1.w = max(0, r1.w);
  r5.y = min(5000, r1.w);
  r1.w = ColorAndMaskTexture.SampleLevel(Clamp_s, r3.xy, 0).z;
  r1.w = max(0, r1.w);
  r5.z = min(5000, r1.w);
  r2.xyz = ColorSeparationMaskTexture.SampleLevel(Clamp_s, v1.xy, 0).xyz;
  r2.xyz = ColorSeparationOpacity.xxx * r2.xyz;
