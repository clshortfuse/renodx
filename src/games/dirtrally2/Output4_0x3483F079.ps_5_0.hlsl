#include "./common.hlsl"
// ---- Created with 3Dmigoto v1.3.2 on Fri Apr 18 14:22:22 2025
...
SamplerState sampler2Sampler_s : register(s5);
SamplerState bloomSampler_s : register(s6);
SamplerState lensFlareSampler_s : register(s7);
SamplerState warpMapSampler_s : register(s8);
SamplerState overlayMapSampler_s : register(s9);
SamplerState bokehMapSampler_s : register(s10);
SamplerState dustMapSampler_s : register(s11);
SamplerState lensRainSampler_s : register(s12);
SamplerState lensMudMapSampler_s : register(s13);
SamplerState decodeLUTSampler_s : register(s14);
Texture2D<float4> sampler0 : register(t0);
Texture2D<float4> sampler1 : register(t1);
Texture2D<float4> sampler2 : register(t2);
Texture2D<float4> bloom : register(t3);
Texture2D<float4> lensFlare : register(t4);
Texture2D<float4> warpMap : register(t5);
Texture2D<float4> overlayMap : register(t6);
Texture2D<float4> motionBlur : register(t7);
Texture2D<float4> bokehMap : register(t8);
Texture2D<float4> dustMap : register(t9);
Texture2D<float4> lensRain : register(t10);
Texture2D<float4> lensMudMap : register(t11);
Texture3D<float4> decodeLUT : register(t12);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.xyzw = warpMap.Sample(warpMapSampler_s, v1.xy).xyzw;
  r0.xy = r0.xy + -r0.zw;
  r0.xy = r0.xy * distortionParams.xy + v1.xy;
  r1.xyz = sampler0.Sample(sampler0Sampler_s, r0.xy).xyz;
  r0.z = cmp(0 != blurEnabled);
  if (r0.z != 0) {
    r2.xyz = motionBlur.Sample(g_bilinearClamped_s, r0.xy).xyz;
    r0.z = sampler1.Sample(g_pointClamped_s, r0.xy).w;
    r0.w = r0.z * r0.z;
    r0.z = r0.z * r0.w + -motionBlurVelocityMin;
    r0.z = saturate(motionBlurVelocityRcpRange * r0.z);
    r2.xyz = r2.xyz + -r1.xyz;
    r1.xyz = r0.zzz * r2.xyz + r1.xyz;
  }
  r0.z = cmp(0 != dofEnabled);
  if (r0.z != 0) {
    r2.xyzw = sampler2.Sample(sampler2Sampler_s, r0.xy).xyzw;
    r0.z = saturate(r2.w * 2 + -1);
    r0.w = saturate(dofBlurScale);
    r0.z = r0.z * (r0.w * CUSTOM_DOF / 2);
    r2.xyz = r2.xyz + -r1.xyz;
    r1.xyz = r0.zzz * r2.xyz + r1.xyz;
  }
  r0.z = cmp(0 != bokehEnabled);
  if (r0.z != 0) {
    r0.xyz = bokehMap.Sample(bokehMapSampler_s, r0.xy).xyz;
  ...
  r0.xyz = bloom.Sample(bloomSampler_s, v1.xy).xyz;
  r1.xyz = (r0.xyz * enabledBlendBloom) * CUSTOM_BLOOM + r1.xyz;
  r2.xyz = lensFlare.Sample(lensFlareSampler_s, v1.xy).xyz;
  r1.xyz = r2.xyz * enabledBlendLensFlare + r1.xyz;
  r1.xyz = v3.xxx * r1.xyz;

  float3 untonemapped = r1.rgb;
  float y_in = renodx::color::y::from::BT709(untonemapped);

  r0.w = dot(r1.xyz, float3(0.212656006,0.715157986,0.0721860006));
  ...
  r0.w = r1.w / r0.w;
  r1.xyz = (r1.xyz * r0.www); // r1.xyz = saturate(r1.xyz * r0.www);
  float mid_gray = renodx::color::y::from::BT709(r1.xyz);

  r1.xyz = TonemappedUngraded(untonemapped, tonemapped, tonemapped_y, mid_gray);
  ...
  r0.xyz = lensdust_params1.www * r0.xyz;
  r0.xyz = lensdust_params3.xyz * r0.www + r0.xyz;
  r0.xyz = r0.xyz * v3.yyy + lensdust_params4.zzz;
  r0.w = lensdust_params1.y + -lensdust_params1.x;
  r0.xyz = saturate(r0.xyz * r0.www + lensdust_params1.xxx);
  r0.xyz = r3.xyz * r0.xyz;
  r0.xyz = lensdust_params5.xyz * r0.xyz;
  ...
  if (r0.w != 0) {
    ...
    r0.xyz = r2.xyz * lensdust_params5.www + r3.xyz;
  }
  ...
  r0.xyz = TonemappedGraded(ungraded, sdr_color, r0.xyz);
