...
vec3 DivideSafe(vec3 a, vec3 b, vec3 fallback) {
  return vec3(DivideSafe(a.x, b.x, fallback.x),
              DivideSafe(a.y, b.y, fallback.y),
              DivideSafe(a.z, b.z, fallback.z));
}
...
vec3 ApplyACES(vec3 untonemapped, float peak_nits, float paper_white) {
  const float ACES_MIN = 0.0001f;
  const float ACES_EXPOSURE = 48.f;
  float aces_min = ACES_MIN / paper_white;
  float aces_max = (peak_nits / paper_white);
  ...
  if (USE_PER_CHANNEL_CORRECTION == 1) {
    ...
    // ODT by luminance
    float scale = (y_out / y_in) * step(0.f, y_in);
    vec3 luminance_tonemapped_ap1 = untonemapped_graded_ap1 * scale;

    // Match per channel chrominance
