#ifndef SRC_TEMPLATE_SHARED_H_
#define SRC_TEMPLATE_SHARED_H_

// #define RENODX_PEAK_WHITE_NITS                 1000.f
// #define RENODX_DIFFUSE_WHITE_NITS              renodx::color::bt2408::REFERENCE_WHITE
// #define RENODX_GRAPHICS_WHITE_NITS             renodx::color::bt2408::GRAPHICS_WHITE
// #define RENODX_COLOR_GRADE_STRENGTH            1.f
// #define RENODX_TONE_MAP_TYPE                   TONE_MAP_TYPE_RENO_DRT
// #define RENODX_TONE_MAP_EXPOSURE               1.f
// #define RENODX_TONE_MAP_HIGHLIGHTS             1.f
// #define RENODX_TONE_MAP_SHADOWS                1.f
// #define RENODX_TONE_MAP_CONTRAST               1.f
// #define RENODX_TONE_MAP_SATURATION             1.f
// #define RENODX_TONE_MAP_HIGHLIGHT_SATURATION   1.f
// #define RENODX_TONE_MAP_BLOWOUT                0
// #define RENODX_TONE_MAP_FLARE                  0
// #define RENODX_TONE_MAP_HUE_CORRECTION         1.f
// #define RENODX_TONE_MAP_HUE_SHIFT              0
// #define RENODX_TONE_MAP_WORKING_COLOR_SPACE    color::convert::COLOR_SPACE_BT709
// #define RENODX_TONE_MAP_CLAMP_COLOR_SPACE      color::convert::COLOR_SPACE_NONE
// #define RENODX_TONE_MAP_CLAMP_PEAK             color::convert::COLOR_SPACE_BT709
// #define RENODX_TONE_MAP_HUE_PROCESSOR          HUE_PROCESSOR_OKLAB
// #define RENODX_TONE_MAP_PER_CHANNEL            0
// #define RENODX_GAMMA_CORRECTION                GAMMA_CORRECTION_GAMMA_2_2
// #define RENODX_INTERMEDIATE_SCALING            (RENODX_DIFFUSE_WHITE_NITS / RENODX_GRAPHICS_WHITE_NITS)
// #define RENODX_INTERMEDIATE_ENCODING           (RENODX_GAMMA_CORRECTION + 1.f)
// #define RENODX_INTERMEDIATE_COLOR_SPACE        color::convert::COLOR_SPACE_BT709
// #define RENODX_SWAP_CHAIN_DECODING             RENODX_INTERMEDIATE_ENCODING
// #define RENODX_SWAP_CHAIN_DECODING_COLOR_SPACE RENODX_INTERMEDIATE_COLOR_SPACE
// #define RENODX_SWAP_CHAIN_CUSTOM_COLOR_SPACE   COLOR_SPACE_CUSTOM_BT709D65
// #define RENODX_SWAP_CHAIN_SCALING_NITS         RENODX_GRAPHICS_WHITE_NITS
// #define RENODX_SWAP_CHAIN_CLAMP_NITS           RENODX_PEAK_WHITE_NITS
// #define RENODX_SWAP_CHAIN_CLAMP_COLOR_SPACE    color::convert::COLOR_SPACE_UNKNOWN
// #define RENODX_SWAP_CHAIN_ENCODING             ENCODING_SCRGB
// #define RENODX_SWAP_CHAIN_ENCODING_COLOR_SPACE color::convert::COLOR_SPACE_BT709

// Must be 32bit aligned
// Should be 4x32
struct ShaderInjectData {
  float peak_white_nits;  // 1
  float diffuse_white_nits;
  float graphics_white_nits;
  float color_grade_strength;
  float tone_map_type;  // 2
  float tone_map_exposure;
  float tone_map_highlights;
  float tone_map_shadows;
  float tone_map_contrast;  // 3
  float tone_map_saturation;
  float tone_map_highlight_saturation;
  float tone_map_blowout;
  float tone_map_flare;  // 4
  float tone_map_hue_correction;
  float tone_map_hue_shift;
  float tone_map_working_color_space;
  float tone_map_clamp_color_space;  // 5
  float tone_map_clamp_peak;
  float tone_map_hue_processor;
  float tone_map_per_channel;
  float gamma_correction;  // 6
  float intermediate_scaling;
  float intermediate_encoding;
  float intermediate_color_space;
  float swap_chain_decoding;  // 7
  float swap_chain_gamma_correction;
  //  float swap_chain_decoding_color_space;
  float swap_chain_custom_color_space;
  // float swap_chain_scaling_nits;
  // float swap_chain_clamp_nits;
  float swap_chain_clamp_color_space;
  float swap_chain_encoding; //
  float swap_chain_encoding_color_space;

  float custom_sprites;  

  float custom_hudbrightness_healthbar;
  float custom_hudbrightness_healthbardelta; //
  float custom_hudbrightness_progressbar;
  float custom_hudbrightness_noteresponse;  
  float custom_hudbrightness_holdcombobg;

  // float custom_dice_maxvalue;
  // float custom_dice_shoulderstart;
  // float custom_dice_power;

  // float custom_tonemap_debug;
  // float custom_tonemap_identify;
  float callback_tonemap_isdrawn; //

  // float custom_bgsprites_debug;
  float custom_bgsprites_isinvtonemap;
  float custom_bgsprites_exposure;  

  float custom_mov_multiplier;
  float custom_mov_isupscale; //

  float custom_preexposure;
  float custom_preexposure_ratio;  
  // float custom_preexposure_complexfudge;

  // float custom_bloom_sdr;
  // float custom_bloom_sdr_saturation;
  // float custom_bloom_hdr;
  // float custom_bloom_hdr_power;
  // float custom_bloom_hdr_dodge;
  float custom_bloom;

  float custom_grade_chroma; //
  float custom_grade_luma;

  float custom_tonemap2nd_mode;  
  float custom_tonemap2nd_inv_pow;
  float custom_tonemap2nd_inv_exposure; //
  float custom_tonemap2nd_inv_preexposure;
  float padding2;
  float padding3;
  // float padding4;
};

#ifndef __cplusplus
#if ((__SHADER_TARGET_MAJOR == 5 && __SHADER_TARGET_MINOR >= 1) || __SHADER_TARGET_MAJOR >= 6)
cbuffer shader_injection : register(b13, space50) {
#elif (__SHADER_TARGET_MAJOR < 5) || ((__SHADER_TARGET_MAJOR == 5) && (__SHADER_TARGET_MINOR < 1))
cbuffer shader_injection : register(b13) {
#endif
  ShaderInjectData shader_injection : packoffset(c0);
}

#define RENODX_TONE_MAP_TYPE                 shader_injection.tone_map_type
#define RENODX_PEAK_WHITE_NITS               shader_injection.peak_white_nits
#define RENODX_DIFFUSE_WHITE_NITS            shader_injection.diffuse_white_nits
#define RENODX_GRAPHICS_WHITE_NITS           shader_injection.graphics_white_nits
#define RENODX_GAMMA_CORRECTION              shader_injection.gamma_correction
#define RENODX_TONE_MAP_PER_CHANNEL          shader_injection.tone_map_per_channel
#define RENODX_TONE_MAP_WORKING_COLOR_SPACE  shader_injection.tone_map_working_color_space
#define RENODX_TONE_MAP_HUE_PROCESSOR        shader_injection.tone_map_hue_processor
#define RENODX_TONE_MAP_HUE_CORRECTION       shader_injection.tone_map_hue_correction
#define RENODX_TONE_MAP_HUE_SHIFT            shader_injection.tone_map_hue_shift
#define RENODX_TONE_MAP_CLAMP_COLOR_SPACE    shader_injection.tone_map_clamp_color_space
#define RENODX_TONE_MAP_CLAMP_PEAK           shader_injection.tone_map_clamp_peak
#define RENODX_TONE_MAP_EXPOSURE             shader_injection.tone_map_exposure
#define RENODX_TONE_MAP_HIGHLIGHTS           shader_injection.tone_map_highlights
#define RENODX_TONE_MAP_SHADOWS              shader_injection.tone_map_shadows
#define RENODX_TONE_MAP_CONTRAST             shader_injection.tone_map_contrast
#define RENODX_TONE_MAP_SATURATION           shader_injection.tone_map_saturation
#define RENODX_TONE_MAP_HIGHLIGHT_SATURATION shader_injection.tone_map_highlight_saturation
#define RENODX_TONE_MAP_BLOWOUT              shader_injection.tone_map_blowout
#define RENODX_TONE_MAP_FLARE                shader_injection.tone_map_flare
// #define RENODX_RENO_DRT_WHITE_CLIP           shader_injection.tone_map_white_clip
#define RENODX_COLOR_GRADE_STRENGTH        shader_injection.color_grade_strength
#define RENODX_INTERMEDIATE_ENCODING       shader_injection.intermediate_encoding
#define RENODX_SWAP_CHAIN_DECODING         shader_injection.swap_chain_decoding
#define RENODX_SWAP_CHAIN_GAMMA_CORRECTION shader_injection.swap_chain_gamma_correction
// #define RENODX_SWAP_CHAIN_DECODING_COLOR_SPACE shader_injection.swap_chain_decoding_color_space
#define RENODX_SWAP_CHAIN_CUSTOM_COLOR_SPACE shader_injection.swap_chain_custom_color_space
// #define RENODX_SWAP_CHAIN_SCALING_NITS         shader_injection.swap_chain_scaling_nits
// #define RENODX_SWAP_CHAIN_CLAMP_NITS           shader_injection.swap_chain_clamp_nits
#define RENODX_SWAP_CHAIN_CLAMP_COLOR_SPACE shader_injection.swap_chain_clamp_color_space
#define RENODX_SWAP_CHAIN_ENCODING             shader_injection.swap_chain_encoding
#define RENODX_SWAP_CHAIN_ENCODING_COLOR_SPACE shader_injection.swap_chain_encoding_color_space
#define RENODX_RENO_DRT_TONE_MAP_METHOD renodx::tonemap::renodrt::config::tone_map_method::REINHARD

#define CUSTOM_SPRITES_DRAW                 shader_injection.custom_sprites
#define CUSTOM_HUDBRIGHTNESS_HEALTHBAR      shader_injection.custom_hudbrightness_healthbar
#define CUSTOM_HUDBRIGHTNESS_HEALTHBARDELTA shader_injection.custom_hudbrightness_healthbardelta
#define CUSTOM_HUDBRIGHTNESS_PROGRESSBAR    shader_injection.custom_hudbrightness_progressbar
#define CUSTOM_HUDBRIGHTNESS_NOTERESPONSE   shader_injection.custom_hudbrightness_noteresponse
#define CUSTOM_HUDBRIGHTNESS_HOLDCOMBOBG    shader_injection.custom_hudbrightness_holdcombobg

// #define CUSTOM_MOV_SHOULDERPOW shader_injection.custom_mov_shoulderpow

// #define CUSTOM_DICE_SHOULDERTHRESHOLD shader_injection.custom_dice_shoulderstart
// #define CUSTOM_DICE_POWER             shader_injection.custom_dice_power

// #define CUSTOM_TONEMAP_DEBUG     shader_injection.custom_tonemap_debug
// #define CUSTOM_TONEMAP_IDENTIFY  shader_injection.custom_tonemap_identify
#define CALLBACK_TONEMAP_ISDRAWN shader_injection.callback_tonemap_isdrawn

// #define CUSTOM_BGSPRITES_DEBUG        shader_injection.custom_bgsprites_debug
#define CUSTOM_BGSPRITES_ISINVTONEMAP shader_injection.custom_bgsprites_isinvtonemap
#define CUSTOM_BGSPRITES_EXPOSURE     shader_injection.custom_bgsprites_exposure

#define CUSTOM_MOV_MULTIPLIER shader_injection.custom_mov_multiplier
#define CUSTOM_MOV_ISUPSCALE  shader_injection.custom_mov_isupscale

#define CUSTOM_PREEXPOSURE       shader_injection.custom_preexposure
#define CUSTOM_PREEXPOSURE_RATIO shader_injection.custom_preexposure_ratio
// #define CUSTOM_PREEXPOSURE_COMPLEXFUDGE shader_injection.custom_preexposure_complexfudge

// #define CUSTOM_BLOOM_SDR              shader_injection.custom_bloom_sdr
// #define CUSTOM_BLOOM_SDR_SATURATION   shader_injection.custom_bloom_sdr_saturation
// #define CUSTOM_BLOOM_HDR              shader_injection.custom_bloom_hdr
// #define CUSTOM_BLOOM_HDR_POWER        shader_injection.custom_bloom_hdr_power
// #define CUSTOM_BLOOM_HDR_DODGE        shader_injection.custom_bloom_hdr_dodge
#define CUSTOM_BLOOM shader_injection.custom_bloom

#define CUSTOM_GRADE_CHROMA shader_injection.custom_grade_chroma
#define CUSTOM_GRADE_LUMA   shader_injection.custom_grade_luma

#define CUSTOM_TONEMAP2ND_MODE       shader_injection.custom_tonemap2nd_mode
#define CUSTOM_TONEMAP2ND_INV_PREEXP shader_injection.custom_tonemap2nd_inv_preexposure
#define CUSTOM_TONEMAP2ND_INV_POW    shader_injection.custom_tonemap2nd_inv_pow
#define CUSTOM_TONEMAP2ND_INV_EXP    shader_injection.custom_tonemap2nd_inv_exposure

#include "../../shaders/renodx.hlsl"

#endif

#endif  // SRC_TEMPLATE_SHARED_H_
