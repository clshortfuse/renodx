...
float4 main(
  ...
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _270 = (pow(_257, 0.012683313339948654f));
    float _271 = (pow(_258, 0.012683313339948654f));
    float _272 = (pow(_259, 0.012683313339948654f));
    float _305 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_270 + -0.8359375f)) / (18.8515625f - (_270 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    ...
    _333 = min((_305 * 12.920000076293945f), ((exp2(log2(max(_305, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  ...
  float _342 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _345 = min(max((_342 * +1.#INF), -1.0f), 1.0f);
  float _354 = (_345 - (sqrt(saturate(1.0f - abs(_342))) * _345)) * BackbufferQuantizationDithering;
  SV_Target.x = (_354 + _333);
  SV_Target.y = (_354 + _334);
  SV_Target.z = (_354 + _335);
  SV_Target.w = 0.0f;
  return SV_Target;
}
