...
float4 main(
  ...
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _281 = (pow(_268, 0.012683313339948654f));
    float _282 = (pow(_269, 0.012683313339948654f));
    float _283 = (pow(_270, 0.012683313339948654f));
    float _316 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_281 + -0.8359375f)) / (18.8515625f - (_281 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    ...
    _344 = min((_316 * 12.920000076293945f), ((exp2(log2(max(_316, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  ...
  float _353 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _356 = min(max((_353 * +1.#INF), -1.0f), 1.0f);
  float _365 = (_356 - (sqrt(saturate(1.0f - abs(_353))) * _356)) * BackbufferQuantizationDithering;
  SV_Target.x = (_365 + _344);
  SV_Target.y = (_365 + _345);
  SV_Target.z = (_365 + _346);
  SV_Target.w = 0.0f;
  return SV_Target;
}
