...
float4 main(
  ...
  float _131 = exp2(log2(((((_37.x * _103) * ColorScale0.x) * _100) + ((_61.x * _103) * ((BloomDirtMaskTint.x * _80.x) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _132 = exp2(log2(((((_37.y * _103) * ColorScale0.y) * _101) + ((_61.y * _103) * ((BloomDirtMaskTint.y * _80.y) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _133 = exp2(log2(((((_37.z * _103) * ColorScale0.z) * _102) + ((_61.z * _103) * ((BloomDirtMaskTint.z * _80.z) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float4 _172 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * exp2(log2((1.0f / ((_131 * 18.6875f) + 1.0f)) * ((_131 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_132 * 18.6875f) + 1.0f)) * ((_132 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_133 * 18.6875f) + 1.0f)) * ((_133 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset)));
  float _176 = _172.x * 1.0499999523162842f;
  float _177 = _172.y * 1.0499999523162842f;
  float _178 = _172.z * 1.0499999523162842f;
  float _252;
  float _253;
  float _254;
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _189 = (pow(_176, 0.012683313339948654f));
    float _190 = (pow(_177, 0.012683313339948654f));
    float _191 = (pow(_178, 0.012683313339948654f));
    float _224 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_189 + -0.8359375f)) / (18.8515625f - (_189 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    ...
    _252 = min((_224 * 12.920000076293945f), ((exp2(log2(max(_224, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  ...
  float _261 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _264 = min(max((_261 * +1.#INF), -1.0f), 1.0f);
  float _273 = (_264 - (sqrt(saturate(1.0f - abs(_261))) * _264)) * BackbufferQuantizationDithering;
  SV_Target.x = (_273 + _252);
  SV_Target.y = (_273 + _253);
  SV_Target.z = (_273 + _254);
  SV_Target.w = 0.0f;
  return SV_Target;
}
