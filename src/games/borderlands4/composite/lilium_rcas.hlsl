...
#define SHARPENING_NORMALIZATION_POINT 125
  ...
  //    h
  float3 b =
      renodx::color::pq::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(0, -1) * texel_size, 0).rgb, RENODX_DIFFUSE_WHITE_NITS);
  float3 d =
      renodx::color::pq::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(-1, 0) * texel_size, 0).rgb, RENODX_DIFFUSE_WHITE_NITS);
  float3 e =
      center_color;
  float3 f =
      renodx::color::pq::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(1, 0) * texel_size, 0).rgb, RENODX_DIFFUSE_WHITE_NITS);
