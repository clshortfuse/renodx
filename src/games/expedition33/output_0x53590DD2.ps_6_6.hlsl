...
OutputSignature main(
  ...
  float _154 = _148.z * 1.0499999523162842f;
  float _229;
  float _230;
  float _231;
  [branch]
  if (!((uint)(bOutputInHDR) == 0)) {
    float _166 = (pow(_152, 0.012683313339948654f));
    float _167 = (pow(_153, 0.012683313339948654f));
    float _168 = (pow(_154, 0.012683313339948654f));
    float _201 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_166 + -0.8359375f)) / (18.8515625f - (_166 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _202 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_167 + -0.8359375f)) / (18.8515625f - (_167 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _203 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_168 + -0.8359375f)) / (18.8515625f - (_168 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _229 = min((_201 * 12.920000076293945f), ((exp2(log2(max(_201, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _230 = min((_202 * 12.920000076293945f), ((exp2(log2(max(_202, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _231 = min((_203 * 12.920000076293945f), ((exp2(log2(max(_203, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _229 = _152;
    _230 = _153;
    _231 = _154;
  }
  float _238 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _241 = min(max((_238 * 0x7FF0000000000000), -1.0f), 1.0f);
  float _250 = (_241 - (sqrt(saturate(1.0f - abs(_238))) * _241)) * BackbufferQuantizationDithering;
  SV_Target.x = (_250 + _229);
  SV_Target.y = (_250 + _230);
  SV_Target.z = (_250 + _231);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_152, _153, _154), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
