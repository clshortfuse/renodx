...
SamplerState samNoise0_s : register(s1);
SamplerState samColor1_s : register(s2);
SamplerState samColor2_s : register(s3);
SamplerState samColor3_s : register(s4);
Texture2D<float4> sColor0 : register(t0);
Texture2D<float4> sNoise0 : register(t1);
Texture2D<float4> sColor1 : register(t2);
Texture2D<float4> sColor2 : register(t3);
Texture2D<float4> sColor3 : register(t4);
...
void main( 
  ...
  linear centroid float2 v3 : TEXCOORD3,
  linear centroid float2 w3 : TEXCOORD4,
  linear centroid float4 v4 : TEXCOORD5,
  linear centroid float3 v5 : TEXCOORD6,
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.xyzw = sNoise0.Sample(samNoise0_s, v4.xy).xyzw;
  r0.x = r0.x * CONST_4.x + CONST_4.y;
  r0.x = v2.x + r0.x;
  r0.y = v2.y;
  r0.xyzw = sColor0.Sample(samColor0_s, r0.xy).xyzw;
  r0.xyz = vSunColor.xyz * r0.www;
  float SunDiskIntensity = FakeHDRGain::Apply(fHDRLightsIntensity, pow(CUSTOM_SKY_LUMINARIES_GLOW, 3.3), CUSTOM_SKY_LUMINARIES_GLOW_CONTRAST, CUSTOM_SKY_LUMINARIES_GLOW_SATURATION);  // Sun disk, * 10
  r0.xyz = fHDRSunIntensity * SunDiskIntensity * r0.xyz;
  r1.xyz = CAMERA_POS_WS.xyz + -v1.xyz;
  r0.w = dot(r1.xyz, r1.xyz);
  r0.w = rsqrt(r0.w);
  r1.xyz = r0.www * r1.yxz;
  r0.w = saturate(dot(-r1.yxz, vSunDir.xyz));
  r1.x = saturate(-r1.x);
  r1.y = SCATTERING[1].y * r0.w + SCATTERING[1].x;
  r0.w = r0.w * r0.w + 1;
  r1.y = max(9.99999975e-05, r1.y);
  r1.z = 1 / r1.y;
  r1.y = rsqrt(r1.y);
  r2.xyz = SCATTERING[3].xyz  * r1.zzz;
  r1.yzw = r2.xyz * r1.yyy;
  r1.yzw = SCATTERING[2].xyz * r0.www + r1.yzw;
  r1.yzw = FakeHDRGain::Apply(r1.yzw, CUSTOM_SKY_SKYBOX_GLOW, CUSTOM_SKY_SKYBOX_GLOW_CONTRAST, CUSTOM_SKY_SKYBOX_GLOW_SATURATION); // Sky, * 1
  r2.xyz = float3(1,1,1) + -v5.xyz;
  r0.xyz = r1.yzw * r2.xyz + r0.xyz; 
  r1.yzw = max(0.0, vSunColor.xyz * fHDRSkyIntensity + -r0.xyz); 
  r2.xyzw = sColor1.Sample(samColor1_s, w2.xy).xyzw;
  r3.xyzw = sColor2.Sample(samColor2_s, v3.xy).xyzw;
  r0.w = r3.w + -r2.w;
  r0.w = r3.w * r0.w + r2.w;
  r2.xyzw = sColor3.Sample(samColor3_s, w3.xy).xyzw;
  r2.x = r2.w + -r0.w;
  r0.w = r2.w * r2.x + r0.w;
  r0.w = (r0.w * r1.x);
  r0.w = FakeHDRGain::Apply(r0.w, CUSTOM_SKY_CLOUDS_GLOW, CUSTOM_SKY_CLOUDS_GLOW_CONTRAST, 1.0);  // Clouds, * 2
  o0.xyz = max(0.0, r0.www * r1.yzw + r0.xyz);
  o0.w = 1000000;
  return;
}