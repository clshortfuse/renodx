...
// ---- Created with 3Dmigoto v1.2.45 on Fri Jan 30 22:51:58 2026
...
SamplerState samColor2_s : register(s2);
SamplerState samColor3_s : register(s3);
Texture2D<float4> sColor0 : register(t0);
Texture2D<float4> sColor1 : register(t1);
Texture2D<float4> sColor2 : register(t2);
Texture2D<float4> sColor3 : register(t3);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.xyzw = sColor1.Sample(samColor1_s, w2.xy).xyzw;
  r1.xyzw = sColor2.Sample(samColor2_s, v3.xy).xyzw;
  r0.x = r1.w + -r0.w;
  r0.x = r1.w * r0.x + r0.w;
  r1.xyzw = sColor3.Sample(samColor3_s, w3.xy).xyzw;
  r0.y = r1.w + -r0.x;
  r0.x = r1.w * r0.y + r0.x;
  ...
  r0.yzw = FakeHDRGain::Apply(r0.yzw, CUSTOM_SKY_SKYBOX_GLOW, CUSTOM_SKY_SKYBOX_GLOW_CONTRAST, CUSTOM_SKY_SKYBOX_GLOW_SATURATION); // Sky, * 1
  r1.xyzw = sColor0.Sample(samColor0_s, v2.xy).xyzw;
  r1.xyz = vSunColor.xyz * r1.www;
  r1.xyz = FakeHDRGain::Apply(r1.xyz, pow(CUSTOM_SKY_LUMINARIES_GLOW, 3.3), CUSTOM_SKY_LUMINARIES_GLOW_CONTRAST, CUSTOM_SKY_SKYBOX_GLOW_SATURATION); // Luminaries intensity, * 10
  r1.xyz = fHDRSunIntensity * r1.xyz;
  r2.xyz = float3(1,1,1) + -v4.xyz;
