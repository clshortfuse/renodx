...
void main(uint3 SV_DispatchThreadID: SV_DispatchThreadID) {
  float3 lutInput;
  float _15 = (HDRMapping_000z) * 0.009999999776482582f;  // displayMaxNits?
  float _18 = _15 * (HDRMapping_009w);                    // shoulderStartPoint
  ...
  // XYZ are now float3(_164, _177, _190)
  float _205 = exp2(((log2((saturate(_192)))) * 0.012683313339948654f));                                                                 // Toe stuff
  float _214 = (exp2(((log2(((max(0.0f, (_205 + -0.8359375f))) / (18.8515625f - (_205 * 18.6875f))))) * 6.277394771575928f))) * 100.0f;  // PQ decode
  _250 = _214;
  // Toe calculations
  if (!((_198 == 0.0f))) {         // If toe end is not 0
    float _217 = max(_200, 0.0f);  // blackpoint
    float _221 = saturate(((_214 - _217) / (_198 - _217)));
    _242 = 0.0f;
    do {
      if ((!(_214 <= _200))) {  // PQ toe <= blackpoint, I guess blackpoint is defined in PQ not linear (but then gets decoded)
        if (!(!(_200 >= 0.0f))) {
      ...
      // new toe
      _250 = ((((exp2(((log2(_242)) * (HDRMapping_009y)))) - _214) * (1.0f - ((_221 * _221) * (3.0f - (_221 * 2.0f))))) + _214);  // toeStrength
    } while (false);
  }

  // Shoulder calculations
  _300 = _15;  // overall brightness
  // Optimization checks
  if (!(((bool)((_18 == _15))) && ((bool)((_250 > _15))))) {  // _18 = shoulder start, _15 = white point, _250 = new toe
    float _258 = (1.0f - (HDRMapping_009w)) * _15;            // shoulderStartPoint
    float _259 = _15 - _258;
    float _260 = exp2((HDRMapping_010x));  // shoulderStrength
