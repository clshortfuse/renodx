...
float3 DisplayMap(float3 color) {
  ...
  if (RENODX_SWAP_CHAIN_OUTPUT_PRESET != 0.f) {
    ...
    float diffuse_white_nits = config.swap_chain_scaling_nits / renodx::color::srgb::REFERENCE_WHITE;  // Normalizes game brightness
   ...
   if (config.gamma_correction > 0.f) {
     tonemap_peak = renodx::color::correct::GammaSafe(tonemap_peak, config.gamma_correction > 0.f, abs(config.gamma_correction) == 1.f ? 2.2f : 2.4f);
   }
   ...
   if (RENODX_TONE_MAP_TYPE == 1.f) {
     if (RENODX_SWAP_CHAIN_OUTPUT_PRESET != 0.f) {
       ...
       outputColor = renodx::tonemap::neutwo::PerChannel(color, tonemap_peak);
     }
  }
...
float3 CustomTonemap(float3 untonemapped, float2 uv) {
  if (RENODX_TONE_MAP_TYPE == 0) {
    return renodx::color::srgb::EncodeSafe(saturate(untonemapped));
  }
  ...
  tonemapped = ApplyRenoDXPostProcessing(tonemapped, uv);
  return renodx::color::srgb::EncodeSafe(tonemapped);
}