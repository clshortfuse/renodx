...
// Tonemapper 2 (0x3953C72A) - Ghosts-faithful RenoDX HDR conversion.
//
// Vanilla mode:
...
//
// IMPORTANT:
// The original SDR tonemapper begins immediately after this stage:
//
//   20  square scene
//   21-29 rational tonemap
//   30-32 3D LUT + saturate
//   33-39 sRGB encode
//   40-41 output-range transform + final saturate
//
// RenoDX therefore branches from the scene signal HERE, before instruction 20.
// This preserves the HDR range that the original SDR path would otherwise lose.
// ============================================================================
...
float2 ComputeOriginalSceneUV(float2 texcoord)
{
    ...
    // Disassembly instruction 0:
    //   mul r0.x, cb2[22].x, cb2[33].x
    float horizontalOffset =
        cb2[22].x * cb2[33].x;

    // Instructions 1-4:
    //
    //   r0.z = v1.y - cb2[22].w * cb2[33].z
    //   lookupUV = float2(
    //       v1.x + horizontalOffset,
    //       r0.z + 1
    //   )
    float adjustedY =
        texcoord.y
    ...
    float2 halfDistortionUV =
        GhostsFiniteSigned2(
            distortionUV * 0.5f
        );

    // Instructions 6-7:
    // sample t4.xy at the full coordinate and scale by cb2[22].y.
    float2 fullResolutionDistortion =
        GhostsFiniteSigned2(
            t4.Sample(
                s4_s,
                distortionUV
    ...
    // Instructions 8-10:
    // sample t4.zw at the half coordinate, scale by cb2[22].x,
    // and multiply by 0.5.
    float2 halfResolutionDistortion =
        GhostsFiniteSigned2(
            t4.Sample(
                s4_s,
                halfDistortionUV
    ...
    // Instruction 11:
    //   r0.xy = firstSample.xy * 0.5 + secondSample.zw
    float2 combinedDistortion =
        GhostsFiniteSigned2(
            fullResolutionDistortion * 0.5f
            + halfResolutionDistortion
        );

    // Instructions 12-15:
    //
    //   mask = saturate(v1.y - 2 * cb2[34].w + 1)
    //   mask = mask^2 * cb2[33].w
    ...
    // Instruction 16:
    //   mad_sat r0.xy, distortion, mask, texcoord
    return saturate(
        GhostsFiniteSigned2(
            texcoord
            + combinedDistortion * distortionMask
        )
    );
}
...
{
    // Reconstruct instructions 0-16 first.
    float2 sceneUV =
...
// Instruction 18.
    //
    // This is the ACTUAL pre-tonemap scene signal. RenoDX must branch here.
    //
    ...
    // Instructions 20-32 reconstructed for the Vanilla reference and for
    // preserving the game's authored SDR grade in the HDR path.
