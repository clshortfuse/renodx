...
//   _36 = SV_Position.x - float((uint)(uint)(ViewportDestination_ViewportMin.x));
//   _37 = SV_Position.y - float((uint)(uint)(ViewportDestination_ViewportMin.y));
//   _43 = saturate(_36 * ViewportDestination_ViewportSizeInverse.x);
//   _44 = saturate(_37 * ViewportDestination_ViewportSizeInverse.y);
//   _57 = select(_20, saturate(ViewportDestination_ViewportSizeInverse.x * ((floor(_36 * 0.5f) * 2.0f) + 1.0f)), _43);
//   _58 = select(_20, saturate(((floor(_37 * 0.5f) * 2.0f) + 1.0f) * ViewportDestination_ViewportSizeInverse.y), _44);
//   _127 = ColorTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(min(max((((ViewportColor_ViewportSize.x * _57) + float((uint)(uint)(ViewportColor_ViewportMin.x))) * ViewportColor_ExtentInverse.x), ViewportColor_UVViewportBilinearMin.x), ViewportColor_UVViewportBilinearMax.x), min(max((((ViewportColor_ViewportSize.y * _58) + float((uint)(uint)(ViewportColor_ViewportMin.y))) * ViewportColor_ExtentInverse.y), ViewportColor_UVViewportBilinearMin.y), ViewportColor_UVViewportBilinearMax.y)), 0.0f);
//   _139 = (1.0f / max(0.0010000000474974513f, VignetteContext.y)) * TEXCOORD_1.z;
//   _144 = (_139 * _139) * (1.0f / max(9.999999747378752e-06f, dot(float3(TEXCOORD_1.x, TEXCOORD_1.y, _139), float3(TEXCOORD_1.x, TEXCOORD_1.y, _139))));
//   _148 = (((_144 * _144) + -1.0f) * VignetteContext.x) + 1.0f;
//   _149 = _148 * min(_127.x, 64512.0f);
//   _150 = _148 * min(_127.y, 64512.0f);
//   _151 = _148 * min(_127.z, 64512.0f);
//   _153 = GlareTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(min(max((((ViewportGlare_ViewportSize.x * _57) + float((uint)(uint)(ViewportGlare_ViewportMin.x))) * ViewportGlare_ExtentInverse.x), ViewportGlare_UVViewportBilinearMin.x), ViewportGlare_UVViewportBilinearMax.x), min(max((((ViewportGlare_ViewportSize.y * _58) + float((uint)(uint)(ViewportGlare_ViewportMin.y))) * ViewportGlare_ExtentInverse.y), ViewportGlare_UVViewportBilinearMin.y), ViewportGlare_UVViewportBilinearMax.y)), 0.0f);
//   _182 = exp2(log2(saturate(((((min(_153.x, 64512.0f) - _149) + (GlareContext.y * _149)) * GlareContext.x) + _149) * 0.009999999776482582f)) * 0.1593017578125f);
//   _198 = exp2(log2(saturate(((((min(_153.y, 64512.0f) - _150) + (GlareContext.y * _150)) * GlareContext.x) + _150) * 0.009999999776482582f)) * 0.1593017578125f);
//   _214 = exp2(log2(saturate(((((min(_153.z, 64512.0f) - _151) + (GlareContext.y * _151)) * GlareContext.x) + _151) * 0.009999999776482582f)) * 0.1593017578125f);
//   _233 = BT709PQToBT2020PQLUT.SampleLevel(View_SharedBilinearClampedSampler, float3(((saturate(exp2(log2(max(0.0f, (((_182 * 18.8515625f) + 0.8359375f) * (1.0f / ((_182 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_198 * 18.8515625f) + 0.8359375f) * (1.0f / ((_198 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_214 * 18.8515625f) + 0.8359375f) * (1.0f / ((_214 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f)), 0.0f);
//   _240 = exp2(log2(saturate(_233.x)) * 0.012683313339948654f);
//   _249 = exp2(log2(max(0.0f, ((_240 + -0.8359375f) * (1.0f / (18.8515625f - (_240 * 18.6875f)))))) * 6.277394771575928f);
//   _250 = _249 * 10000.0f;
//   _254 = exp2(log2(saturate(_233.y)) * 0.012683313339948654f);
//   _263 = exp2(log2(max(0.0f, ((_254 + -0.8359375f) * (1.0f / (18.8515625f - (_254 * 18.6875f)))))) * 6.277394771575928f);
//   _264 = _263 * 10000.0f;
//   _268 = exp2(log2(saturate(_233.z)) * 0.012683313339948654f);
//   _277 = exp2(log2(max(0.0f, ((_268 + -0.8359375f) * (1.0f / (18.8515625f - (_268 * 18.6875f)))))) * 6.277394771575928f);
//   _278 = _277 * 10000.0f;
//   _288 = max(9.999999747378752e-05f, HDRCompositionContextColor.x);
//   _289 = max(9.999999747378752e-05f, HDRCompositionContextColor.y);
//   _290 = max(9.999999747378752e-05f, HDRCompositionContextColor.z);
//   _292 = 1.0f / dot(float3(_288, _289, _290), float3(0.2125999927520752f, 0.7152000069618225f, 0.0722000002861023f));
//   _293 = dot(float3(_250, _264, _278), float3(0.2125999927520752f, 0.7152000069618225f, 0.0722000002861023f)) * 9.999999747378752e-05f;
//   _299 = exp2(log2(saturate((_293 * _288) * _292)) * 0.1593017578125f);
//   _316 = exp2(log2(saturate((_293 * _289) * _292)) * 0.1593017578125f);
//   _333 = exp2(log2(saturate((_293 * _290) * _292)) * 0.1593017578125f);
//   _352 = BT2020PQTosRGBLUT.SampleLevel(View_SharedBilinearClampedSampler, float3(((saturate(exp2(log2(max(0.0f, (((_299 * 18.8515625f) + 0.8359375f) * (1.0f / ((_299 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_316 * 18.8515625f) + 0.8359375f) * (1.0f / ((_316 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_333 * 18.8515625f) + 0.8359375f) * (1.0f / ((_333 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f)), 0.0f);
...
//   }
//   _391 = (1.0f / max(0.0010000000474974513f, HDRCompositionContext.z)) * TEXCOORD_1.w;
//   _396 = (_391 * _391) * (1.0f / max(9.999999747378752e-06f, dot(float3(TEXCOORD_1.x, TEXCOORD_1.y, _391), float3(TEXCOORD_1.x, TEXCOORD_1.y, _391))));
//   _400 = (saturate(_396 * _396) + -1.0f) * HDRCompositionContext.y;
//   _401 = -0.0f - _400;
//   _405 = CompositeSDRTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(((min(((ViewportDestination_ViewportSize.x * 0.5625f) * ViewportDestination_ViewportSizeInverse.y), 1.0f) * (_43 + -0.5f)) + 0.5f), ((min(((ViewportDestination_ViewportSize.y * 1.7777777910232544f) * ViewportDestination_ViewportSizeInverse.x), 1.0f) * (_44 + -0.5f)) + 0.5f)), 0.0f);
//   _419 = (((_366 * HDRCompositionContext.x) * _401) * _405.w) + _405.x;
//   _420 = (((_377 * HDRCompositionContext.x) * _401) * _405.w) + _405.y;
//   _421 = (((_388 * HDRCompositionContext.x) * _401) * _405.w) + _405.z;
//   _422 = _405.w * ((_400 * HDRCompositionContext.x) + 1.0f);
//   if (_419 < 0.0031308000907301903f) {
//     _433 = (_419 * 12.920000076293945f);
//   } else {
//     _433 = (((pow(_419, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   if (_420 < 0.0031308000907301903f) {
//     _444 = (_420 * 12.920000076293945f);
//   } else {
//     _444 = (((pow(_420, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   if (_421 < 0.0031308000907301903f) {
//     _455 = (_421 * 12.920000076293945f);
//   } else {
//     _455 = (((pow(_421, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   _456 = _422 * _422;
//   _463 = 1.0f / ((_249 * 40.0f) + 1.0f);
//   _464 = 1.0f / ((_263 * 40.0f) + 1.0f);
//   _465 = 1.0f / ((_277 * 40.0f) + 1.0f);
//   _490 = saturate(pow(_433, 2.200000047683716f));
//   _491 = saturate(pow(_444, 2.200000047683716f));
//   _492 = saturate(pow(_455, 2.200000047683716f));
//   _506 = exp2(log2(saturate(((dot(float3(0.6274039149284363f, 0.3292829990386963f, 0.043313100934028625f), float3(_490, _491, _492)) * 250.0f) + ((_422 * _250) * (lerp(_463, 1.0f, _456)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _517 = saturate(exp2(log2(max(0.0f, (((_506 * 18.8515625f) + 0.8359375f) * (1.0f / ((_506 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _522 = exp2(log2(saturate(((dot(float3(0.06909730285406113f, 0.9195405840873718f, 0.011362300254404545f), float3(_490, _491, _492)) * 250.0f) + ((_422 * _264) * (lerp(_464, 1.0f, _456)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _533 = saturate(exp2(log2(max(0.0f, (((_522 * 18.8515625f) + 0.8359375f) * (1.0f / ((_522 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _538 = exp2(log2(saturate(((dot(float3(0.01639140024781227f, 0.08801329880952835f, 0.8955953121185303f), float3(_490, _491, _492)) * 250.0f) + ((_422 * _278) * (lerp(_465, 1.0f, _456)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _549 = saturate(exp2(log2(max(0.0f, (((_538 * 18.8515625f) + 0.8359375f) * (1.0f / ((_538 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _551 = ((((float4)(View_SpatiotemporalBlueNoiseVolumeTexture.Load(int4((int(SV_Position.x) & 127), (int(SV_Position.y) & 127), (View_StateFrameIndex & 63), 0)))).x) * 2.0f) + -1.0f;
//   _568 = ((1.0f - sqrt(1.0f - abs(_551))) * float((int)(((int)(uint)((int)(_551 > 0.0f))) - ((int)(uint)((int)(_551 < 0.0f)))))) * 0.0009775171056389809f;
//   SV_Target.x = saturate(select(((abs((_517 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_568 + _517), _517));
//   SV_Target.y = saturate(select(((abs((_533 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_568 + _533), _533));
//   SV_Target.z = saturate(select(((abs((_549 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_568 + _549), _549));
//   SV_Target.w = 0.0f;
//   return SV_Target;
// }