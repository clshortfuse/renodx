...
//   float _533;
//   float _550;
//   _19 = (DeviceCorrectorContext.z != 0.0f);
//   _35 = SV_Position.x - float((uint)(uint)(ViewportDestination_ViewportMin.x));
//   _36 = SV_Position.y - float((uint)(uint)(ViewportDestination_ViewportMin.y));
//   _42 = saturate(_35 * ViewportDestination_ViewportSizeInverse.x);
//   _43 = saturate(_36 * ViewportDestination_ViewportSizeInverse.y);
//   _56 = select(_19, saturate(ViewportDestination_ViewportSizeInverse.x * ((floor(_35 * 0.5f) * 2.0f) + 1.0f)), _42);
//   _57 = select(_19, saturate(((floor(_36 * 0.5f) * 2.0f) + 1.0f) * ViewportDestination_ViewportSizeInverse.y), _43);
//   _118 = min(((ViewportDestination_ViewportSizeInverse.y * ViewportDestination_ViewportSize.x) * 0.5625f), 1.0f);
//   _119 = min(((ViewportDestination_ViewportSizeInverse.x * ViewportDestination_ViewportSize.y) * 1.7777777910232544f), 1.0f);
//   _126 = ColorTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(min(max((((ViewportColor_ViewportSize.x * _56) + float((uint)(uint)(ViewportColor_ViewportMin.x))) * ViewportColor_ExtentInverse.x), ViewportColor_UVViewportBilinearMin.x), ViewportColor_UVViewportBilinearMax.x), min(max((((ViewportColor_ViewportSize.y * _57) + float((uint)(uint)(ViewportColor_ViewportMin.y))) * ViewportColor_ExtentInverse.y), ViewportColor_UVViewportBilinearMin.y), ViewportColor_UVViewportBilinearMax.y)), 0.0f);
//   _138 = (1.0f / max(0.0010000000474974513f, VignetteContext.y)) * TEXCOORD_1.z;
//   _143 = (_138 * _138) * (1.0f / max(9.999999747378752e-06f, dot(float3(TEXCOORD_1.x, TEXCOORD_1.y, _138), float3(TEXCOORD_1.x, TEXCOORD_1.y, _138))));
//   _147 = (((_143 * _143) + -1.0f) * VignetteContext.x) + 1.0f;
//   _148 = _147 * min(_126.x, 64512.0f);
//   _149 = _147 * min(_126.y, 64512.0f);
//   _150 = _147 * min(_126.z, 64512.0f);
//   _152 = GlareTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(min(max((((ViewportGlare_ViewportSize.x * _56) + float((uint)(uint)(ViewportGlare_ViewportMin.x))) * ViewportGlare_ExtentInverse.x), ViewportGlare_UVViewportBilinearMin.x), ViewportGlare_UVViewportBilinearMax.x), min(max((((ViewportGlare_ViewportSize.y * _57) + float((uint)(uint)(ViewportGlare_ViewportMin.y))) * ViewportGlare_ExtentInverse.y), ViewportGlare_UVViewportBilinearMin.y), ViewportGlare_UVViewportBilinearMax.y)), 0.0f);
//   _181 = exp2(log2(saturate(((((min(_152.x, 64512.0f) - _148) + (GlareContext.y * _148)) * GlareContext.x) + _148) * 0.009999999776482582f)) * 0.1593017578125f);
//   _197 = exp2(log2(saturate(((((min(_152.y, 64512.0f) - _149) + (GlareContext.y * _149)) * GlareContext.x) + _149) * 0.009999999776482582f)) * 0.1593017578125f);
//   _213 = exp2(log2(saturate(((((min(_152.z, 64512.0f) - _150) + (GlareContext.y * _150)) * GlareContext.x) + _150) * 0.009999999776482582f)) * 0.1593017578125f);
//   _232 = BT709PQToBT2020PQLUT.SampleLevel(View_SharedBilinearClampedSampler, float3(((saturate(exp2(log2(max(0.0f, (((_181 * 18.8515625f) + 0.8359375f) * (1.0f / ((_181 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_197 * 18.8515625f) + 0.8359375f) * (1.0f / ((_197 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f), ((saturate(exp2(log2(max(0.0f, (((_213 * 18.8515625f) + 0.8359375f) * (1.0f / ((_213 * 18.6875f) + 1.0f))))) * 78.84375f)) * 0.96875f) + 0.015625f)), 0.0f);
//   _239 = exp2(log2(saturate(_232.x)) * 0.012683313339948654f);
//   _253 = exp2(log2(saturate(_232.y)) * 0.012683313339948654f);
//   _267 = exp2(log2(saturate(_232.z)) * 0.012683313339948654f);
//   [branch]
//   if (CompositeSurfaceContext.x != 0.0f) {
//     _294 = CompositeSurfaceTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(((_118 * (_56 + -0.5f)) + 0.5f), ((_119 * (_57 + -0.5f)) + 0.5f)), 0.0f);
//     [branch]
//     if (CompositeSurfaceContext.y != 0.0f) {
//       [branch]
//       if (CompositeSurfaceContext.z != 0.0f) {
//         _303 = exp2(log2(saturate(_294.x)) * 0.012683313339948654f);
//         _317 = exp2(log2(saturate(_294.y)) * 0.012683313339948654f);
//         _331 = exp2(log2(saturate(_294.z)) * 0.012683313339948654f);
//         _395 = (exp2(log2(max(0.0f, ((_303 + -0.8359375f) * (1.0f / (18.8515625f - (_303 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//         _396 = (exp2(log2(max(0.0f, ((_317 + -0.8359375f) * (1.0f / (18.8515625f - (_317 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//         _397 = (exp2(log2(max(0.0f, ((_331 + -0.8359375f) * (1.0f / (18.8515625f - (_331 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//       } else {
//         _395 = _294.x;
//         _396 = _294.y;
//         _397 = _294.z;
//       }
//     } else {
//       if (_294.x < 0.0031308000907301903f) {
//         _353 = (_294.x * 12.920000076293945f);
//       } else {
//         _353 = (((pow(_294.x, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//       }
//       if (_294.y < 0.0031308000907301903f) {
//         _364 = (_294.y * 12.920000076293945f);
//       } else {
//         _364 = (((pow(_294.y, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//       }
//       if (_294.z < 0.0031308000907301903f) {
//         _375 = (_294.z * 12.920000076293945f);
//       } else {
//         _375 = (((pow(_294.z, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//       }
//       _385 = saturate(pow(_353, 2.200000047683716f));
//       _386 = saturate(pow(_364, 2.200000047683716f));
//       _387 = saturate(pow(_375, 2.200000047683716f));
//       _395 = (dot(float3(0.6274039149284363f, 0.3292829990386963f, 0.043313100934028625f), float3(_385, _386, _387)) * 250.0f);
//       _396 = (dot(float3(0.06909730285406113f, 0.9195405840873718f, 0.011362300254404545f), float3(_385, _386, _387)) * 250.0f);
//       _397 = (dot(float3(0.01639140024781227f, 0.08801329880952835f, 0.8955953121185303f), float3(_385, _386, _387)) * 250.0f);
//     }
//   } else {
//     _395 = (exp2(log2(max(0.0f, ((_239 + -0.8359375f) * (1.0f / (18.8515625f - (_239 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//     _396 = (exp2(log2(max(0.0f, ((_253 + -0.8359375f) * (1.0f / (18.8515625f - (_253 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//     _397 = (exp2(log2(max(0.0f, ((_267 + -0.8359375f) * (1.0f / (18.8515625f - (_267 * 18.6875f)))))) * 6.277394771575928f) * 10000.0f);
//   }
//   _400 = CompositeSDRTexture.SampleLevel(View_SharedBilinearClampedSampler, float2(((_118 * (_42 + -0.5f)) + 0.5f), ((_119 * (_43 + -0.5f)) + 0.5f)), 0.0f);
//   if (_400.x < 0.0031308000907301903f) {
//     _415 = (_400.x * 12.920000076293945f);
//   } else {
//     _415 = (((pow(_400.x, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   if (_400.y < 0.0031308000907301903f) {
//     _426 = (_400.y * 12.920000076293945f);
//   } else {
//     _426 = (((pow(_400.y, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   if (_400.z < 0.0031308000907301903f) {
//     _437 = (_400.z * 12.920000076293945f);
//   } else {
//     _437 = (((pow(_400.z, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f);
//   }
//   _438 = _400.w * _400.w;
//   _445 = 1.0f / ((_395 * 0.004000000189989805f) + 1.0f);
//   _446 = 1.0f / ((_396 * 0.004000000189989805f) + 1.0f);
//   _447 = 1.0f / ((_397 * 0.004000000189989805f) + 1.0f);
//   _472 = saturate(pow(_415, 2.200000047683716f));
//   _473 = saturate(pow(_426, 2.200000047683716f));
//   _474 = saturate(pow(_437, 2.200000047683716f));
//   _488 = exp2(log2(saturate(((dot(float3(0.6274039149284363f, 0.3292829990386963f, 0.043313100934028625f), float3(_472, _473, _474)) * 250.0f) + ((_400.w * _395) * (lerp(_445, 1.0f, _438)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _499 = saturate(exp2(log2(max(0.0f, (((_488 * 18.8515625f) + 0.8359375f) * (1.0f / ((_488 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _504 = exp2(log2(saturate(((dot(float3(0.06909730285406113f, 0.9195405840873718f, 0.011362300254404545f), float3(_472, _473, _474)) * 250.0f) + ((_400.w * _396) * (lerp(_446, 1.0f, _438)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _515 = saturate(exp2(log2(max(0.0f, (((_504 * 18.8515625f) + 0.8359375f) * (1.0f / ((_504 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _520 = exp2(log2(saturate(((dot(float3(0.01639140024781227f, 0.08801329880952835f, 0.8955953121185303f), float3(_472, _473, _474)) * 250.0f) + ((_400.w * _397) * (lerp(_447, 1.0f, _438)))) * 9.999999747378752e-05f)) * 0.1593017578125f);
//   _531 = saturate(exp2(log2(max(0.0f, (((_520 * 18.8515625f) + 0.8359375f) * (1.0f / ((_520 * 18.6875f) + 1.0f))))) * 78.84375f));
//   _533 = ((((float4)(View_SpatiotemporalBlueNoiseVolumeTexture.Load(int4((int(SV_Position.x) & 127), (int(SV_Position.y) & 127), (View_StateFrameIndex & 63), 0)))).x) * 2.0f) + -1.0f;
//   _550 = ((1.0f - sqrt(1.0f - abs(_533))) * float((int)(((int)(uint)((int)(_533 > 0.0f))) - ((int)(uint)((int)(_533 < 0.0f)))))) * 0.0009775171056389809f;
//   SV_Target.x = saturate(select(((abs((_499 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_550 + _499), _499));
//   SV_Target.y = saturate(select(((abs((_515 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_550 + _515), _515));
//   SV_Target.z = saturate(select(((abs((_531 * 2.0f) + -1.0f) + -0.9980449676513672f) < 0.0f), (_550 + _531), _531));
//   SV_Target.w = 0.0f;
//   return SV_Target;
// }