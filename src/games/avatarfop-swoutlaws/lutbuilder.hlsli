...
float3 GenerateOutput(float3 ungraded_bt709, float shadow_contrast, float highlight_contrast,
...
float3 GenerateOutputAvatar(float3 ungraded_bt709, float contrast) {
  // `pow(c, contrast) * exposure_adjustment` per channel gives an uncapped approximation of vanilla.
  // Lower highlight contrast keeps the image from becoming overly harsh while allowing the sun to reach 10,000 nits.
  return GenerateOutput(ungraded_bt709, contrast, contrast * 0.681f, 2.f);
}

float3 GenerateOutputOutlaws(float3 ungraded_bt709, float contrast) {
  return GenerateOutput(ungraded_bt709, contrast, contrast * 0.7375f, 1.7f);
}
