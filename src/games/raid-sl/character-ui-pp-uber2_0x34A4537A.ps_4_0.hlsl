...
// ---- Created with 3Dmigoto v1.4.1 on Wed Jan  7 05:32:37 2026
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.xyzw = cb0[34].yyyy * r0.xyzw; // Normalize weight

  // Prepare glow/bloom color
  r1.xyzw = float4(0.0625,0.0625,0.0625,1) * r0.xyzw; 
  r0.xyzw = float4(0.0625,0.0625,0.0625,0.0625) * r0.xyzw; 
  r2.xyz = cb0[35].xyz * r1.xyz; // Scale bloom color
  r2.w = 0.0625 * r1.w;

  // --- Vignette / Mask (t1) & sRGB to Linear ---
  // Calculates linear vignette/mask color
  ...
  // --- Scene Composition (t2 + t1 + Bloom) ---
  r4.xyzw = t2.Sample(s2_s, v1.xy).xyzw; // Sample Scene (or mask?)
  r1.xyz = r4.xxx * r3.xyz;            // Modulate Scene (masked?) by Vignette Color
  r1.xyzw = r1.xyzw + r2.xyzw;         // Add Bloom (r2)

  // --- Overlay / Dirt (t4) ---
  r2.xy = v1.xy * cb0[33].xy + cb0[33].zw; // Calculate UVs
  r2.xyzw = t4.Sample(s4_s, r2.xy).xyzw;   // Sample Overlay
  r2.xyz = cb0[34].zzz * r2.xyz;           // Scale Overlay Color
  r2.w = 0;
  r0.xyzw = r2.xyzw * r0.xyzw + r1.xyzw;   // Combine: Overlay (weighted by blur r0?) + Scene/Bloom

  float3 linearInput = r0.xyz;

  // --- Linear to sRGB Conversion (Output) ---
  ...
  // --- Dithering / ToneMap Pass ---
  if (RENODX_TONE_MAP_TYPE != 0.f) {
    float3 sdrColor = renodx::color::srgb::DecodeSafe(r0.xyz);
