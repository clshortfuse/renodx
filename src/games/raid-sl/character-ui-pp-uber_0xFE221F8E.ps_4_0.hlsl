...
// ---- Created with 3Dmigoto v1.4.1 on Wed Jan  7 02:25:59 2026
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r3.xyz = r5.xyz ? r4.xyz : r3.xyz; // Linearized Vignette Color

  // --- Scene Color (t2) Composition ---
  r4.xyzw = t2.Sample(s2_s, v1.xy).xyzw;
  r1.xyz = r4.xxx * r3.xyz; // Combine with vignette/mask
  ...
  // Capture Linear Input before saturation/clamping
  float4 combinedColor = r2.xyzw * r0.xyzw + r1.xyzw;
  float3 linearInput = combinedColor.xyz; 

  r0.xyzw = saturate(combinedColor);

  // --- Convert Linear to sRGB (Preparation for LUT) ---
  ...
  o0.w = r0.w;
  r0.xyz = r0.xyz ? r2.xyz : r1.xyz; // r0 is now sRGB

  // --- Color Grading LUT Lookup (t5) ---
  ...
  r0.xyz = r0.xyz ? r2.xyz : r1.xyz; // r0 is now Linear
  ...
  */
  ...
  float3 lutColor = r0.xyz; // Capture final SDR color (sRGB encoded)
