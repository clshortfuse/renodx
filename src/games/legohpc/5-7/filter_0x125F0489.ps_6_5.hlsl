...
float4 main(
  ...
  float4 fullColor = g_textures2D[(uint)fullColor_tex_t].Sample(g_samplers[(uint)fullColor_tex], TEXCOORD.xy);
  float4 mipColor  = g_textures2D[(uint)mipColor1_tex_t].Sample(g_samplers[(uint)mipColor1_tex], TEXCOORD.zw);
  float4 blurColor = g_textures2D[(uint)blur_tex_t].Sample(g_samplers[(uint)blur_tex], TEXCOORD.zw);

  // lerp(mipColor.rgb, fullColor.rgb, mipColor.a), then add half the blur on top
  float3 blended = (fullColor.rgb - mipColor.rgb) * mipColor.a + mipColor.rgb;
  blended = lerp(fullColor.rgb, blended, CUSTOM_DOF);
  float3 color = (blended + blurColor.rgb * (TEXCOORD_2.x * 0.5f) * CUSTOM_BLOOM) * TEXCOORD_2.z;

  float3 sdr = saturate(color);
  if (RENODX_TONE_MAP_TYPE != 0) sdr = ColourCorrect(color, sdr);
  color = renodx::color::srgb::DecodeSafe(sdr);
