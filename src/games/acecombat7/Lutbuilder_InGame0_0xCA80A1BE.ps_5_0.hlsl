...
void main(
  ...
  r0.z = cmp(asuint(cb0[56].z) >= 3);  // checks if output device is under 3 (SDR)
  r2.xy = log2(r0.xy);
  r2.z = log2(r1.z);
  ...
  float FilmBlackClip = cb0[27].w;
  float FilmToe = cb0[27].y;
  float FilmWhiteClip = cb0[28].x;
  float FilmShoulder = cb0[27].z;
  ...
  float3 film_graded_color = r2.rgb;
