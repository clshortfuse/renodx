#ifndef ASSCREEDSHADOWS_MACLEOD_BOYNTON_HLSL_
#define ASSCREEDSHADOWS_MACLEOD_BOYNTON_HLSL_
...
float3 CorrectHueAndPurityMBBT709(
  ...
  float target_purity01 =
      renodx_custom::color::macleod_boynton::ApplyBT709(target_color_bt709, 1.f, 1.f,
                                                        mb_white_override, t_min)
          .purityCur01;
  ...
  if (purity_strength > 0.f) {
    float reference_purity01 =
        renodx_custom::color::macleod_boynton::ApplyBT709(reference_color_bt709, 1.f, 1.f,
                                                          mb_white_override, t_min)
            .purityCur01;
    applied_purity01 = lerp(target_purity01, reference_purity01, saturate(purity_strength));

    if (clamp_purity_gain > 0.f && applied_purity01 > target_purity01) {
      float clamp_strength = saturate(clamp_purity_gain);
      // Only clamp purity increases: pull toward target by clamp strength.
      applied_purity01 = lerp(applied_purity01, target_purity01, clamp_strength);
    }

    if (clamp_purity_loss > 0.f && applied_purity01 < target_purity01) {
      float clamp_strength = saturate(clamp_purity_loss);
      // Only clamp purity reductions: pull toward target by clamp strength.
      applied_purity01 = lerp(applied_purity01, target_purity01, clamp_strength);
    }
  }

  if (hue_strength <= 0.f) {
    return renodx_custom::color::macleod_boynton::ApplyBT709(
               target_color_bt709, applied_purity01, curve_gamma, mb_white_override, t_min)
        .rgbOut;
  }

  float3 target_lms = mul(renodx_custom::color::macleod_boynton::XYZ_TO_LMS_2006,
                          mul(renodx::color::BT709_TO_XYZ_MAT, target_color_bt709));
  ...
  float3 reference_lms = mul(renodx_custom::color::macleod_boynton::XYZ_TO_LMS_2006,
                             mul(renodx::color::BT709_TO_XYZ_MAT, reference_color_bt709));
  ...
  if (target_len_sq < renodx_custom::color::macleod_boynton::MB_NEAR_WHITE_EPSILON &&
      reference_len_sq < renodx_custom::color::macleod_boynton::MB_NEAR_WHITE_EPSILON) {
    return renodx_custom::color::macleod_boynton::ApplyBT709(
               target_color_bt709, applied_purity01, curve_gamma, mb_white_override, t_min)
        .rgbOut;
  }
  ...
  return renodx_custom::color::macleod_boynton::ApplyBT709(
             seed_bt709, applied_purity01, curve_gamma, mb_white_override, t_min)
      .rgbOut;
}
...
#endif  // ASSCREEDSHADOWS_MACLEOD_BOYNTON_HLSL_
