...
float3 ApplyToneMapEncodePQ(float3 untonemapped_ap1, float peak_nits, float diffuse_white_nits) {
  ...
  if (RENODX_TONE_MAP_TYPE == 3.f) {
    ...
    float3 aces_tonemap = renodx::tonemap::aces::RRTAndODT(untonemapped_bt709, aces_min * 48.f, aces_max * 48.f);
    aces_tonemap /= 48.f;
    ...
    float renoDRTDechroma = 0.93f;

    // use ACES for hue correction
    tm_config.hue_correction_type = renodx::tonemap::renodrt::config::hue_correction_type::CUSTOM;
    tm_config.hue_correction_color = aces_tonemap;
...
#if TONEMAP_BLEND
    tonemapped = lerp(tonemapped, aces_tonemap, saturate(renodx::color::y::from::BT709(tonemapped)));
#endif
