...
float3 applyDICE(float3 untonemapped, float diffuse_white, float peak_white, float shoulder_start = 0.5f) {
  ...
  config.Type = 3u;
  config.ShoulderStart = shoulder_start;
  config.DesaturationAmount = 0.f;
  config.DarkeningAmount = 0.f;

  const float dicePaperWhite = diffuse_white / renodx::color::srgb::REFERENCE_WHITE;
  const float dicePeakWhite = peak_white / renodx::color::srgb::REFERENCE_WHITE;
