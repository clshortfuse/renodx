...
void main(
  ...
  if (TONE_MAP_TYPE != 0.f) {
    ...
    if (RENODX_GAMMA_CORRECTION != 0.f) {
      ...
      if (RENODX_GAMMA_CORRECTION == 2.f) {
        lut_output = ApplyGammaCorrectionByLuminosity(lut_output);
      } else {
  ...
  float _583 = exp2(log2(mad(-0.25336629152297974f, _570, mad(-0.3556708097457886f, _567, (_564 * 1.716651201248169f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _596 = exp2(log2(mad(0.015768500044941902f, _570, mad(1.6164811849594116f, _567, (_564 * -0.6666843891143799f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _609 = exp2(log2(mad(0.9421030879020691f, _570, mad(-0.04277060180902481f, _567, (_564 * 0.017639899626374245f))) * 0.009999999776482582f) * 0.1593017578125f);
  OutLUT[int3((uint)(SV_DispatchThreadID.x), (uint)(SV_DispatchThreadID.y), (uint)(SV_DispatchThreadID.z))] = float4(saturate(exp2(log2(((_583 * 18.8515625f) + 0.8359375f) / ((_583 * 18.6875f) + 1.0f)) * 78.84375f)), saturate(exp2(log2(((_596 * 18.8515625f) + 0.8359375f) / ((_596 * 18.6875f) + 1.0f)) * 78.84375f)), saturate(exp2(log2(((_609 * 18.8515625f) + 0.8359375f) / ((_609 * 18.6875f) + 1.0f)) * 78.84375f)), 1.0f);
}
