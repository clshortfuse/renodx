...
float3 PostTonemapSliders(float3 hdr_color) {
  ...
  float3 outputColor = ApplySaturationBlowoutHighlightSaturation(hdr_color, y, config);
...
float3 CustomTonemap(float3 color) {
  if (RENODX_TONE_MAP_TYPE == 0.f) return saturate(renodx::color::srgb::Encode(color));
  renodx::draw::Config config = renodx::draw::BuildConfig();  // Pulls config values
  ...
  if (RENODX_SWAP_CHAIN_OUTPUT_PRESET > 0) {
    ...
    tonemap_peak = RENODX_PEAK_WHITE_NITS / RENODX_DIFFUSE_WHITE_NITS;
  ...
  if (RENODX_TONE_MAP_TYPE == 1.f) {
    ...
    outputColor = ApplyHDRBoost(outputColor, CUSTOM_HDR_BOOST);
    outputColor = psychotm_test11(
        outputColor,  // mid-gray adjusted
        tonemap_peak,
