...
/*
 * DXBC checksum table for the Alien Isolation passes Alias Isolation depends on.
 *
 * These are the four 32-bit checksum words stored in the DXBC header, matching
 * the original ASI's shaderHooks.cpp checks. They are independent of RenoDX's
 * dumped-file CRC32 names and only identify original game shaders/passes.
 */
...
inline bool MatchesDxbcChecksum(std::span<const uint8_t> code, const DxbcChecksum& checksum) {
  ...
  // DXBC bytes 4..19 are the checksum Alias Isolation used for shader matching.
