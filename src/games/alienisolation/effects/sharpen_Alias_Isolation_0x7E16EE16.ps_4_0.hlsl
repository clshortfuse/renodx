...
PSOutput main(in PSInput IN) {
  ...
  float3 center = renodx::color::srgb::DecodeSafe(colorTex.SampleLevel(PointSampler, IN.param1 + float2(0, 0) * texelSize, 0).xyz);
  float3 outColor;
  ...
  if (sharpenEnabled) {
    ...
    [unroll]
    for (uint i = 0; i < 4; ++i) {
      neighborDiff += renodx::color::y::from::BT709(neighbors[i] - center);  // replace BT.601 luma
    }
