// Mechanically reconstructed from 0xC5CFC1CF.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(-1.0f, 1.0f, 0.0f, 0.200000003f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c4 = float4(4.0f, -2.0f, 0.25f, 0.0009765625f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[32]);
    r0 = (v6.xxxx) * (c[31]) + (r0);
    r0 = (v6.zzzz) * (c[33]) + (r0);
    r1 = (r0) + (c[34]);
    r3.xy = (r1.ww) * (c[35].xy) + (r1.xy);
    r3.zw = r1.zw;
    r0 = tex2Dproj(s1, r3);
    r2.zw = r3.zw;
    r1.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[35].zw)) + (r1.xy);
    r4.zw = r1.zw;
    r4 = tex2Dproj(s1, r4);
    r0.w = r4.x;
    r2.xy = (r3.ww) * (-(c[35].xy)) + (r1.xy);
    r1.xy = (r3.ww) * (c[35].zw) + (r1.xy);
    r2 = tex2Dproj(s1, r2);
    r0.y = r2.x;
    r2 = tex2Dproj(s1, r1);
    r1 = (v6.xyzx) * (c0.yyyz) + (c0.zzzy);
    r0.z = dot(r1, c[28]);
    r2.w = 1.0f / (r0.z);
    r3.x = dot(r1, c[25]);
    r3.y = dot(r1, c[26]);
    r0.z = r2.x;
    r3.xy = (r2.ww) * (r3.xy);
    r2.x = dot(r1, c[27]);
    r1.xy = (r3.xy) * (c3.yy) + (c3.yy);
    r1 = tex2D(s2, r1.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[23].yz, r2.xy) + (c[23].x);
    r3.xy = saturate((r2.xx) * (c[24].xy) + (c[24].zw));
    r2.w = saturate(1.0f / (r1.w));
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c0.z) : (r2.w));
    r7.xyz = (-(v6.xyz)) + (c[20].xyz);
    r2.xy = (r3.xy) * (r3.xy);
    r2.w = dot(r7.xyz, r7.xyz);
    r3.xy = (r3.xy) * (c14.xx) + (c14.yy);
    r5.w = rsqrt(r2.w);
    r2.z = (r2.x) * (r3.x);
    r4.xyz = (r7.xyz) * (r5.www);
    r2.y = (r2.y) * (-(r3.y)) + (c0.y);
    r2.w = dot(r4.xyz, c[29].xyz);
    r1.w = (r1.w) * (r2.z);
    r2.z = saturate((r2.w) * (c[30].x) + (c[30].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c14.x) + (c14.y);
    r1.w = (r2.y) * (r1.w);
    r2.w = (r2.w) * (r2.z);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r1.w) * (r2.w);
    r0.w = dot(r0, c4.zzzz);
    r0.xyz = (r1.xyz) * (r1.www);
    r6.xyz = (r0.www) * (r0.xyz);
    r0.xy = (v1.zw) * (c3.xy);
    r1 = tex2D(s13, r0.xy);
    r0.xy = (v1.zw) * (c3.xy) + (c3.zy);
    r3 = tex2D(s13, r0.xy);
    r1.w = r3.y;
    r0 = tex2D(s5, v7.zw);
    r5.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s4, v7.zw);
    r3.w = (r0.w) * (v0.z);
    r2.xy = (r1.yw) * (c4.xx) + (c4.yy);
    r11.xy = (r5.xy) * (r3.ww);
    r1.w = dot(r2.xy, r2.xy) + (c0.z);
    r0.w = dot(r11.xy, r11.xy) + (c0.z);
    r1.w = exp2(-(r1.w));
    r0.w = exp2(-(r0.w));
    r1.w = (r1.w) * (c2.x) + (c2.y);
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r1.y = (r1.w) * (r0.w);
    r1.w = dot(r2.xy, r11.xy) + (c0.z);
    r2 = tex2D(s14, v1.zw);
    r8.xy = (r2.xy) * (c3.ww);
    r2.w = saturate((r1.w) * (r1.y) + (r1.y));
    r3.xy = (r3.xz) * (r8.yy);
    r1.w = (r2.y) * (c3.w) + (-(r3.x));
    r5.xz = (r3.xy) * (c4.xx);
    r1.w = (r3.z) * (-(r8.y)) + (r1.w);
    r1.xy = (r1.xz) * (r8.xx);
    r5.y = (r1.w) + (r1.w);
    r1.w = (r2.x) * (c3.w) + (-(r1.x));
    r2.xyz = (r2.www) * (r5.xyz);
    r1.w = (r1.z) * (-(r8.x)) + (r1.w);
    r5.xz = (r1.xy) * (c4.xx);
    r5.y = (r1.w) + (r1.w);
    r1 = tex2D(s6, v7.zw);
    r1.yzw = (r1.wyy) + (c0.xzx);
    r3.xyz = (r5.xyz) * (r0.www) + (r2.xyz);
    r12.xy = (r3.ww) * (r1.yw) + (c0.yy);
    r2 = tex2D(s3, v7.xy);
    r2.xyz = (r2.xyz) + (c0.xxx);
    r3.xyz = (r3.xyz) * (r12.yyy);
    r9.xyz = (v0.yyy) * (r2.xyz) + (c0.yyy);
    r2 = tex2D(s0, v1.xy);
    r0.xyz = (r2.xyz) * (-(r9.xyz)) + (r0.xyz);
    r10.xyz = (r3.www) * (r0.xyz);
    r0 = v2;
    r8.xyz = (r11.xxx) * (v5.xyz) + (r0.xyz);
    r0.xyz = (r2.xyz) * (r9.xyz) + (r10.xyz);
    r2.xyz = (r11.yyy) * (v4.xyz) + (r8.xyz);
    r10.xyz = normalize(r2.xyz);
    r2.xyz = normalize(v6.xyz);
    r1.x = c0.w;
    r1.w = dot(r2.xyz, r10.xyz);
    r11.xz = (r3.ww) * (r1.xz);
    r1.z = (r1.w) + (r1.w);
    r1.w = (r12.x) * (c2.z);
    r1.xyz = (r10.xyz) * (-(r1.zzz)) + (r2.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r11.zzz) * (r1.xyz);
    r2.w = (r11.x) * (r11.x);
    r6.w = saturate(dot(r10.xyz, -(r2.xyz)));
    r1.w = (r11.x) * (-(r11.x)) + (c0.y);
    r3.w = (-(r6.w)) + (c0.y);
    r8.xy = (r12.xx) * (c12.xy) + (c12.zw);
    r4.w = (r3.w) * (r3.w);
    r7.w = 1.0f / (r8.x);
    r3.w = (r3.w) * (r4.w);
    r4.w = exp2(r8.y);
    r3.w = (r7.w) * (r3.w);
    r2.xyz = (r7.xyz) * (r5.www) + (-(r2.xyz));
    r3.w = (r1.w) * (r3.w) + (r2.w);
    r1.xyz = (r1.xyz) * (r3.www);
    r7.z = (r12.x) * (c2.w);
    r3.w = (r12.x) * (-(c13.x)) + (c13.y);
    r5.w = saturate(dot(r4.xyz, r10.xyz));
    r6.w = (r6.w) * (r3.w) + (r7.z);
    r3.w = (r5.w) * (r3.w) + (r7.z);
    r1.xyz = (r5.xyz) * (r1.xyz);
    r3.w = (r3.w) * (r6.w) + (c4.w);
    r1.xyz = (r1.xyz) * (c2.zzz);
    r3.w = 1.0f / (r3.w);
    r7.xyz = (r0.xyz) * (r3.xyz) + (r1.xyz);
    r3.w = (r5.w) * (r3.w);
    r8.xyz = (r5.www) * (c[21].xyz);
    r1.xyz = normalize(r2.xyz);
    r2.z = (r4.w) * (c13.z) + (c13.w);
    r2.x = saturate(dot(r1.xyz, r4.xyz));
    r2.y = saturate(dot(r10.xyz, r1.xyz));
    r1.z = (-(r2.x)) + (c0.y);
    r1.x = pow(abs(r2.y), r4.w);
    r1.y = (r1.z) * (r1.z);
    r1.x = (r2.z) * (r1.x);
    r1.y = (r1.y) * (r1.y);
    r5.w = (r3.w) * (r1.x);
    r1.z = (r1.z) * (r1.y);
    r5.z = (r1.w) * (r1.z) + (r2.w);
    r4 = (-(v6.yyyy)) + (c[6]);
    r1 = (r4) * (r4);
    r3 = (-(v6.xxxx)) + (c[5]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v6.zzzz)) + (c[7]);
    r5.w = (r5.w) * (r5.z);
    r1 = (r2) * (r2) + (r1);
    r9.xyz = (r5.www) * (c[22].xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r9.xyz = (r11.zzz) * (r9.xyz);
    r4 = (r4) * (r5);
    r4 = (r10.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r10.xxxx) + (r4);
    r4.z = c0.y;
    r1 = saturate((r1) * (c[8]) + (r4.zzzz));
    r2 = saturate((r2) * (r10.zzzz) + (r3));
    r3.xyz = (r0.xyz) * (r8.xyz) + (r9.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r6.xyz) * (r3.xyz) + (r7.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[36].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.y;

    return oC0;
}
