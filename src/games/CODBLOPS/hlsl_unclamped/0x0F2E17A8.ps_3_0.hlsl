// Mechanically reconstructed from 0x0F2E17A8.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(2.0f, -1.0f, 0.200000003f, 0.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c12 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c13 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 oC0 = 0.0f;

    r8.xyz = (-(v5.xyz)) + (c[20].xyz);
    r2.y = dot(r8.xyz, r8.xyz);
    r7.w = rsqrt(r2.y);
    r2.x = 1.0f / (r7.w);
    r1.xy = saturate((r2.xx) * (c[24].xy) + (c[24].zw));
    r0.xy = (r1.xy) * (r1.xy);
    r1.xy = (r1.xy) * (c14.xx) + (c14.yy);
    r0.w = dot(c[23].yz, r2.xy) + (c[23].x);
    r0.xy = (r0.xy) * (r1.xy);
    r0.w = (r0.w) * (r0.x);
    r2.w = (r0.y) * (r0.w);
    r1 = tex2D(s12, v0.zw);
    r0 = tex2D(s1, v0.xy);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s3, v6.zw);
    r3.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = (c1.xxxx) * (v7) + (c1.yyyy);
    r0.x = dot(r3.xy, r0.xy) + (c1.w);
    r0.y = dot(r3.xy, r0.zw) + (c1.w);
    r3 = tex2D(s2, v6.zw);
    r3.w = (r3.w) * (v6.y);
    r15.xy = lerp(r2.xy, r0.xy, r3.ww);
    r0 = v1;
    r0.xyz = (r15.xxx) * (v4.xyz) + (r0.xyz);
    r8.w = (r2.w) * (r1.y);
    r0.xyz = (r15.yyy) * (v3.xyz) + (r0.xyz);
    r7.xyz = (r8.xyz) * (r7.www);
    r9.xyz = normalize(r0.xyz);
    r5 = tex2D(s5, v6.zw);
    r2 = tex2D(s4, v0.xy);
    r6.w = r2.w;
    r1 = tex2D(s0, v0.xy);
    r6.xyz = lerp(r1.xyz, c1.zzz, r2.xxx);
    r2.z = saturate(dot(r9.xyz, r7.xyz));
    r4 = lerp(r6, r5, r3.wwww);
    r1.w = (r4.w) * (c2.w);
    r12.xyz = normalize(v5.xyz);
    r0.y = (r4.w) * (-(c13.z)) + (c13.w);
    r0.x = saturate(dot(r9.xyz, -(r12.xyz)));
    r0.z = (r2.z) * (r0.y) + (r1.w);
    r0.y = (r0.x) * (r0.y) + (r1.w);
    r2.w = (-(r0.x)) + (-(c1.y));
    r0.z = (r0.z) * (r0.y) + (c12.x);
    r0.z = 1.0f / (r0.z);
    r5.xyz = (r8.xyz) * (r7.www) + (-(r12.xyz));
    r1.w = (r2.z) * (r0.z);
    r0.xyz = normalize(r5.xyz);
    r13.xyz = (r2.zzz) * (c[21].xyz);
    r5.w = saturate(dot(r9.xyz, r0.xyz));
    r0.z = saturate(dot(r0.xyz, r7.xyz));
    r0.xy = (r4.ww) * (c3.xy) + (c3.zw);
    r0.y = exp2(r0.y);
    r7.w = 1.0f / (r0.x);
    r2.z = pow(abs(r5.w), r0.y);
    r0.z = (-(r0.z)) + (-(c1.y));
    r0.x = (r0.y) * (c12.y) + (c12.z);
    r0.y = (r0.z) * (r0.z);
    r0.x = (r2.z) * (r0.x);
    r0.y = (r0.y) * (r0.y);
    r2.z = (r1.w) * (r0.x);
    r0.z = (r0.z) * (r0.y);
    r11.xyz = (r4.xyz) * (r4.xyz);
    r10.xyz = (r4.xyz) * (-(r4.xyz)) + (-(c1.yyy));
    r1.w = (r4.w) * (c2.z);
    r0.xyz = (r10.xyz) * (r0.zzz) + (r11.xyz);
    r0.xyz = (r2.zzz) * (r0.xyz);
    r7.xyz = lerp(r2.yyy, c[25].xyw, r3.www);
    r0.xyz = (r0.xyz) * (r7.zzz);
    r4.xy = (v0.zw) * (c4.xy);
    r4 = tex2D(s13, r4.xy);
    r5.xy = (v0.zw) * (c4.xy) + (c4.zy);
    r6 = tex2D(s13, r5.xy);
    r4.w = r6.y;
    r14.xyz = (r0.xyz) * (c[22].xyz);
    r0.xy = (r4.yw) * (c13.xx) + (c13.yy);
    r2.z = dot(r15.xy, r15.xy) + (c1.w);
    r0.z = dot(r0.xy, r0.xy) + (c1.w);
    r2.z = exp2(-(r2.z));
    r0.z = exp2(-(r0.z));
    r2.z = (r2.z) * (c2.x) + (c2.y);
    r2.y = (r0.z) * (c2.x) + (c2.y);
    r0.z = dot(r0.xy, r15.xy) + (c1.w);
    r0.y = (r2.z) * (r2.y);
    r5 = tex2D(s14, v0.zw);
    r8.xy = (r5.xy) * (c4.ww);
    r4.w = saturate((r0.z) * (r0.y) + (r0.y));
    r0.xy = (r6.xz) * (r8.yy);
    r2.y = (r5.y) * (c4.w) + (-(r0.x));
    r0.xz = (r0.xy) * (c13.xx);
    r0.y = (r6.z) * (-(r8.y)) + (r2.y);
    r4.xy = (r4.xz) * (r8.xx);
    r0.y = (r0.y) + (r0.y);
    r2.y = (r5.x) * (c4.w) + (-(r4.x));
    r0.xyz = (r4.www) * (r0.xyz);
    r2.y = (r4.z) * (-(r8.x)) + (r2.y);
    r8.xz = (r4.xy) * (c13.xx);
    r8.y = (r2.y) + (r2.y);
    r4.xyz = (r8.www) * (r14.xyz);
    r0.xyz = (r8.xyz) * (r2.zzz) + (r0.xyz);
    r5.xyz = (r7.yyy) * (r0.xyz);
    r0.xyz = (r1.xyz) * (-(r2.xxx)) + (r3.xyz);
    r3.xyz = (r8.www) * (r13.xyz) + (r5.xyz);
    r0.xyz = (r3.www) * (r0.xyz);
    r0.xyz = (r1.xyz) * (r2.xxx) + (r0.xyz);
    r1.z = dot(r12.xyz, r9.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.z = (r1.z) + (r1.z);
    r6.xyz = (r0.xyz) * (r3.xyz) + (r4.xyz);
    r1.xyz = (r9.xyz) * (-(r1.zzz)) + (r12.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r7.xxx) * (r1.xyz);
    r1.w = (r2.w) * (r2.w);
    r5.w = (r2.w) * (r1.w);
    r4 = (-(v5.yyyy)) + (c[6]);
    r1 = (r4) * (r4);
    r3 = (-(v5.xxxx)) + (c[5]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v5.zzzz)) + (c[7]);
    r5.w = (r7.w) * (r5.w);
    r1 = (r2) * (r2) + (r1);
    r10.xyz = (r10.xyz) * (r5.www) + (r11.xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r7.xyz = (r7.xyz) * (r10.xyz);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r4.z = c1.y;
    r1 = saturate((r1) * (c[8]) + (-(r4.zzzz)));
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.xyz = (r8.xyz) * (r7.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c2.zzz) + (r6.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[26].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = -(c1.y);

    return oC0;
}
