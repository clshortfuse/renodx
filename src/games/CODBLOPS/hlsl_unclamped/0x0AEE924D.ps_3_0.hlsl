// Mechanically reconstructed from 0x0AEE924D.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD2;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    const float4 c0 = float4(-2.0f, 3.0f, 0.0f, 0.75f);
    const float4 c1 = float4(1.0f, 0.38647899f, 0.392856985f, 0.364796013f);
    const float4 c2 = float4(6.0f, 13.0f, 8.0f, 12.0f);
    const float4 c3 = float4(31.875f, 0.5f, -28.0f, 30.0f);
    const float4 c4 = float4(9.0f, 14.0f, 4.0f, 11.0f);
    const float4 c12 = float4(24.0f, 0.0166666675f, 0.419999987f, 9.99999994e-09f);
    const float4 c13 = float4(-0.589999974f, -0.5f, 0.589999974f, 0.5f);
    const float4 c14 = float4(1.22077894f, 2.0f, 0.857142985f, 0.0f);
    const float4 c15 = float4(30.0f, 10.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xyz = (c[6].xyz) + (-(v1.xyz));
    r0.w = dot(r0.xyz, r0.xyz);
    r0.w = rsqrt(r0.w);
    r1.x = 1.0f / (r0.w);
    r0.xyz = (r0.xyz) * (r0.www);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[10].yz, r1.xy) + (c[10].x);
    r1.xy = saturate((r1.xx) * (c[11].xy) + (c[11].zw));
    r1.zw = (r1.xy) * (r1.xy);
    r1.xy = (r1.xy) * (c0.xx) + (c0.yy);
    r1.xy = (r1.zw) * (r1.xy);
    r0.w = (r0.w) * (r1.x);
    r0.w = (r1.y) * (r0.w);
    r1.xyz = normalize(v2.xyz);
    r2.x = max(abs(r1.y), abs(r1.z));
    r3.x = max(abs(r1.x), r2.x);
    r1.w = 1.0f / (r3.x);
    r2.xyz = (r1.xyz) * (c[5].xyz);
    r2.xyz = (r2.xyz) * (r1.www) + (v4.xyz);
    r2 = tex3D(s11, r2.xyz);
    r0.w = (r0.w) * (r2.w);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r3.xyz = (r0.www) * (c[7].xyz);
    r4.xyz = (r0.www) * (c[8].xyz);
    r4.xyz = (r4.xyz) * (c[9].yyy);
    r3.xyz = (r3.xyz) * (c[9].xxx);
    r0.w = saturate(dot(r1.xyz, r0.xyz));
    r3.xyz = (r3.xyz) * (r0.www);
    r5.x = log2(abs(r3.x));
    r5.y = log2(abs(r3.y));
    r5.z = log2(abs(r3.z));
    r5.xyz = (r5.xyz) * (c0.www);
    r6.x = exp2(r5.x);
    r6.y = exp2(r5.y);
    r6.z = exp2(r5.z);
    r2.xyz = (r2.xyz) * (c3.xxx) + (r6.xyz);
    r5.x = c[25].x;
    r7 = (-(r5.xxxx)) + (c2);
    r3.xyz = float3(((-abs(r7.x)) >= 0.0f ? (r3.x) : (c0.z)), ((-abs(r7.x)) >= 0.0f ? (r3.y) : (c0.z)), ((-abs(r7.x)) >= 0.0f ? (r3.z) : (c0.z)));
    r3.xyz = float3(((-abs(r7.y)) >= 0.0f ? (r6.x) : (r3.x)), ((-abs(r7.y)) >= 0.0f ? (r6.y) : (r3.y)), ((-abs(r7.y)) >= 0.0f ? (r6.z) : (r3.z)));
    r3.xyz = float3(((-abs(r7.z)) >= 0.0f ? (r2.x) : (r3.x)), ((-abs(r7.z)) >= 0.0f ? (r2.y) : (r3.y)), ((-abs(r7.z)) >= 0.0f ? (r2.z) : (r3.z)));
    r0.w = dot(-(r0.xyz), r1.xyz);
    r0.w = (r0.w) + (r0.w);
    r5.yzw = (r1.xyz) * (-(r0.www)) + (-(r0.xyz));
    r6.xyz = normalize(v1.xyz);
    r0.w = saturate(dot(r5.yzw, -(r6.xyz)));
    r0.x = dot(r0.xyz, r6.xyz);
    r0.x = saturate((r0.x) * (c3.y) + (c3.y));
    r0.y = (r0.x) * (c3.z) + (c3.w);
    r0.x = (r0.x) + (c1.x);
    r1.w = pow(abs(r0.w), r0.y);
    r0.yzw = (r4.xyz) * (r1.www);
    r0.xyz = (r0.xxx) * (r0.yzw);
    r3.xyz = float3(((-abs(r7.w)) >= 0.0f ? (r0.x) : (r3.x)), ((-abs(r7.w)) >= 0.0f ? (r0.y) : (r3.y)), ((-abs(r7.w)) >= 0.0f ? (r0.z) : (r3.z)));
    r4.xyz = (r0.xyz) * (c1.yzw);
    r7 = (-(r5.xxxx)) + (c4);
    r3.xyz = float3(((-abs(r7.x)) >= 0.0f ? (r4.x) : (r3.x)), ((-abs(r7.x)) >= 0.0f ? (r4.y) : (r3.y)), ((-abs(r7.x)) >= 0.0f ? (r4.z) : (r3.z)));
    r0.w = dot(r6.xyz, r1.xyz);
    r0.w = (r0.w) + (r0.w);
    r1.xyz = (r1.xyz) * (-(r0.www)) + (r6.xyz);
    r1 = texCUBE(s15, r1.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r1.xyz) * (c2.zzz);
    r4 = tex3D(s11, v4.xyz);
    r4.xyz = (r4.xyz) * (r4.xyz);
    r1.xyz = (r1.xyz) * (r4.xyz);
    r1.xyz = (r1.xyz) * (c3.xxx);
    r3.xyz = float3(((-abs(r7.y)) >= 0.0f ? (r1.x) : (r3.x)), ((-abs(r7.y)) >= 0.0f ? (r1.y) : (r3.y)), ((-abs(r7.y)) >= 0.0f ? (r1.z) : (r3.z)));
    r1.xyz = (r1.xyz) * (c[26].xxx);
    r3.xyz = float3(((-abs(r7.z)) >= 0.0f ? (r1.x) : (r3.x)), ((-abs(r7.z)) >= 0.0f ? (r1.y) : (r3.y)), ((-abs(r7.z)) >= 0.0f ? (r1.z) : (r3.z)));
    r0.xyz = (r0.xyz) * (c1.yzw) + (r1.xyz);
    r1.xyz = float3(((-abs(r7.w)) >= 0.0f ? (r0.x) : (r3.x)), ((-abs(r7.w)) >= 0.0f ? (r0.y) : (r3.y)), ((-abs(r7.w)) >= 0.0f ? (r0.z) : (r3.z)));
    r3.x = c12.x;
    r0.w = (r3.x) * (c[20].w);
    r0.w = frac(r0.w);
    r0.w = (c[20].w) * (r3.x) + (-(r0.w));
    r3.y = (r0.w) * (c12.y);
    r3.x = c12.z;
    r3 = tex2D(s2, r3.xy);
    r4.xy = c[35].xy;
    r3.yz = (-(r4.xy)) + (c[27].xy);
    r3.xy = (r3.xx) * (r3.yz) + (c[35].xy);
    r3.zw = (c13.xy) + (v3.xy);
    r3.xy = (r3.zw) * (r3.xy) + (c13.zw);
    r0.w = c[20].w;
    r3.xy = (r0.ww) * (c[34].xy) + (r3.xy);
    r3 = tex2D(s3, r3.xy);
    r3.xyz = (r3.xyz) * (r3.xyz);
    r3.xyz = (r3.xyz) * (c14.xyz);
    r4 = tex2D(s4, v3.xy);
    r4.xyz = (r4.xyz) * (r4.xyz);
    r3.xyz = (r4.xyz) * (c[33].xxx) + (r3.xyz);
    r0.w = (r0.w) * (c[31].x);
    r1.w = frac(abs(r0.w));
    r0.w = ((r0.w) >= 0.0f ? (r1.w) : (-(r1.w)));
    r0.w = (-(r0.w)) + (v3.y);
    r4.y = (r0.w) * (c[32].x);
    r4.x = v3.x;
    r4 = tex2D(s1, r4.xy);
    r0.w = pow(abs(r4.y), c[28].x);
    r4.xyz = (r0.www) * (c[30].xyz);
    r3.xyz = (r4.xyz) * (c[29].xxx) + (r3.xyz);
    r4.xy = (-(r5.xx)) + (c15.xy);
    r1.xyz = float3(((-abs(r4.x)) >= 0.0f ? (r3.x) : (r1.x)), ((-abs(r4.x)) >= 0.0f ? (r3.y) : (r1.y)), ((-abs(r4.x)) >= 0.0f ? (r3.z) : (r1.z)));
    r5 = tex2D(s5, v3.xy);
    r4.xzw = (r5.xyz) * (r5.xyz);
    r3.xyz = (r3.xyz) * (r4.xzw);
    r0.xyz = (r2.xyz) * (r3.xyz) + (r0.xyz);
    r2.xyz = max(r0.xyz, c0.zzz);
    r0.xyz = float3(((-abs(r4.y)) >= 0.0f ? (r2.x) : (r1.x)), ((-abs(r4.y)) >= 0.0f ? (r2.y) : (r1.y)), ((-abs(r4.y)) >= 0.0f ? (r2.z) : (r1.z)));
    r0.w = abs(c[25].x);
    r0.xyz = float3(((-(r0.w)) >= 0.0f ? (r2.x) : (r0.x)), ((-(r0.w)) >= 0.0f ? (r2.y) : (r0.y)), ((-(r0.w)) >= 0.0f ? (r2.z) : (r0.z)));
    r0.xyz = (r2.xyz) * (c12.www) + (r0.xyz);
    r0.xyz = float3(((c[25].x) >= 0.0f ? (r0.x) : (r2.x)), ((c[25].x) >= 0.0f ? (r0.y) : (r2.y)), ((c[25].x) >= 0.0f ? (r0.z) : (r2.z)));
    r0.w = c1.x;
    r1.x = dot(r0, c[22]);
    r1.y = dot(r0, c[23]);
    r1.z = dot(r0, c[24]);
    r0.w = v1.w;
    r2.xyz = lerp(v0.xyz, r1.xyz, r0.www);
    r0.xyz = max(((r2.xyz) * (c[21].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    oC0.x = 1.0f / (r0.x);
    r0.x = rsqrt(r0.y);
    r0.y = rsqrt(r0.z);
    oC0.z = 1.0f / (r0.y);
    oC0.y = 1.0f / (r0.x);
    oC0.w = c1.x;

    return oC0;
}
