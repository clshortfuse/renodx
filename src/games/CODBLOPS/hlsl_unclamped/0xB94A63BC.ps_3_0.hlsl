// Mechanically reconstructed from 0xB94A63BC.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(-1.0f, 1.0f, 0.200000003f, 8.0f);
    const float4 c1 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 0.125f, 0.25f);
    const float4 c4 = float4(0.000244140625f, 0.0f, -0.000244140625f, 0.75f);
    const float4 c12 = float4(0.00048828125f, -0.000122070312f, 0.0f, -3.0f);
    const float4 c13 = float4(4.0f, -3.0f, -2.0f, 3.0f);
    const float4 c14 = float4(0.797884583f, 1.0f, 0.5f, 0.0f);
    const float4 c15 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 r16 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v4.xyz, v4.xyz);
    r12.w = rsqrt(r0.w);
    r1 = tex2D(s5, v0.xy);
    r0.xyz = (-(v4.xyz)) + (c[5].xyz);
    r0.w = dot(r0.xyz, r0.xyz);
    r0.w = rsqrt(r0.w);
    r12.xyz = (r12.www) * (v4.xyz);
    r3.xyz = (r0.xyz) * (r0.www) + (-(r12.xyz));
    r2.xyz = normalize(r3.xyz);
    r0.xyz = (r0.xyz) * (r0.www);
    r0.w = saturate(dot(r2.xyz, r0.xyz));
    r10.w = (r1.w) * (-(c14.x)) + (c14.y);
    r0.w = (-(r0.w)) + (c0.y);
    r1.x = dot(r0.xyz, c[22].xyz);
    r1.z = (r0.w) * (r0.w);
    r3.z = saturate((r1.x) * (c[23].x) + (c[23].y));
    r1.z = (r1.z) * (r1.z);
    r3.w = (r0.w) * (r1.z);
    r11.xyz = normalize(v1.xyz);
    r0.w = saturate(dot(r11.xyz, r2.xyz));
    r11.w = (r1.w) * (c14.x);
    r2.w = saturate(dot(r0.xyz, r11.xyz));
    r1.z = saturate(dot(r11.xyz, -(r12.xyz)));
    r0.z = (r2.w) * (r10.w) + (r11.w);
    r9.w = (r1.z) * (r10.w) + (r11.w);
    r0.z = (r0.z) * (r9.w) + (c1.w);
    r15.xy = (r1.ww) * (c15.xy) + (c15.zw);
    r0.z = 1.0f / (r0.z);
    r8.w = exp2(r15.y);
    r3.y = (r2.w) * (r0.z);
    r3.x = pow(abs(r0.w), r8.w);
    r0 = tex2D(s4, v5.zw);
    r0.xyz = (r0.xyz) + (c0.xxx);
    r1.x = (r8.w) * (c3.z) + (c3.w);
    r2.xyz = (v5.yyy) * (r0.xyz) + (c0.yyy);
    r0.w = (r3.x) * (r1.x);
    r0.xyz = (r2.xyz) * (c0.zzz);
    r9.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.yyy);
    r10.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r3.y) * (r0.w);
    r0.xyz = (r9.xyz) * (r3.www) + (r10.xyz);
    r3.w = (r3.z) * (c13.z) + (c13.w);
    r0.xyz = (r0.www) * (r0.xyz);
    r0.w = (r3.z) * (r3.z);
    r0.xyz = (r0.xyz) * (c[7].xyz);
    r6.w = (r3.w) * (r0.w);
    r4.xyz = (r1.yyy) * (r0.xyz);
    r0 = tex2D(s0, v0.xy);
    r2.xyz = (r2.xyz) * (r0.xyz);
    r0.xy = (v0.zw) * (c14.yz);
    r0 = tex2D(s13, r0.xy);
    r3 = tex2D(s14, v0.zw);
    r8.xy = (r3.xy) * (c1.xx);
    r7.xyz = (r2.xyz) * (r2.xyz);
    r6.xy = (r0.xz) * (r8.xx);
    r2.x = r0.y;
    r0.w = (r3.x) * (c1.x) + (-(r6.x));
    r2.z = (r0.z) * (-(r8.x)) + (r0.w);
    r0.xy = (v0.zw) * (c14.yz) + (c14.wz);
    r0 = tex2D(s13, r0.xy);
    r5.xy = (r8.yy) * (r0.xz);
    r2.y = r0.y;
    r0.w = (r3.y) * (c1.x) + (-(r5.x));
    r0.xy = (r2.xy) * (c1.yy) + (c1.zz);
    r0.z = (r0.z) * (-(r8.y)) + (r0.w);
    r0.w = dot(r0.xy, r0.xy) + (c14.w);
    r8.y = (r2.z) + (r2.z);
    r0.w = exp2(-(r0.w));
    r0.y = (r0.z) + (r0.z);
    r0.w = saturate((r0.w) * (c3.x) + (c3.y));
    r8.xz = (r6.xy) * (c1.yy);
    r0.xz = (r5.xy) * (c1.yy);
    r2.xyz = (r2.www) * (c[6].xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (r8.xyz);
    r6.xyz = (r7.xyz) * (r2.xyz) + (r4.xyz);
    r14.xyz = (r1.yyy) * (r0.xyz);
    r0 = tex2D(s12, v0.zw);
    r7.w = saturate(dot(r11.xyz, c[17].xyz));
    r0.w = ((-abs(r0.y)) >= 0.0f ? (c14.y) : (c14.w));
    r13.xyz = (r7.www) * (c[18].xyz);
    if ((r0.w) != (-(r0.w)))
    {
        r0.w = r0.y;
    }
    else
    {
        if ((c0.y) >= (v3.w))
        {
            r2 = (v3.xyzx) * (c14.yyyw);
            r0 = (r2) + (-(c12.xyzz));
            r0 = tex2Dlod(s1, r0);
            r0.w = r0.x;
            r3 = (r2) + (c4.xxyy);
            r3 = tex2Dlod(s1, r3);
            r0.x = r3.x;
            r3 = (r2) + (c4.zzyy);
            r3 = tex2Dlod(s1, r3);
            r0.y = r3.x;
            r2 = (r2) + (c12.xyzz);
            r2 = tex2Dlod(s1, r2);
            r0.z = r2.x;
            r5.w = dot(r0, c3.wwww);
            if ((c4.w) < (v3.w))
            {
                r5.xy = (v3.xy) * (c[2].ww) + (c[2].xy);
                r0.xy = (r5.xy) + (c4.xx);
                r0.zw = (v3.zx) * (c14.yw);
                r0 = tex2Dlod(s1, r0);
                r2.xy = (r5.xy) + (c4.zz);
                r2.zw = (v3.zx) * (c14.yw);
                r4 = tex2Dlod(s1, r2);
                r2.xy = (r5.xy) + (c12.xy);
                r2.zw = (v3.zx) * (c14.yw);
                r3 = tex2Dlod(s1, r2);
                r2.xy = (r5.xy) + (-(c12.xy));
                r2.zw = (v3.zx) * (c14.yw);
                r2 = tex2Dlod(s1, r2);
                r0.y = r4.x;
                r0.z = r3.x;
                r0.w = r2.x;
                r0.w = dot(r0, c3.wwww);
                r0.z = (-(r5.w)) + (r0.w);
                r0.w = (v3.w) * (c13.x) + (c13.y);
                r0.w = (r0.w) * (r0.z) + (r5.w);
            }
            else
            {
                r0.w = r5.w;
            }
        }
        else
        {
            r0.w = (v3.w) + (-(c1.y));
            r0.w = ((r0.w) >= 0.0f ? (c14.w) : (c14.y));
            if ((r0.w) != (-(r0.w)))
            {
                r16.xy = (v3.xy) * (c[2].ww) + (c[2].xy);
                r2.xy = (r16.xy) + (c4.xx);
                r2.zw = (v3.zz) * (c14.yw);
                r2 = tex2Dlod(s1, r2);
                r3.xy = (r16.xy) + (c4.zz);
                r3.zw = (v3.zz) * (c14.yw);
                r5 = tex2Dlod(s1, r3);
                r3.xy = (r16.xy) + (c12.xy);
                r3.zw = (v3.zz) * (c14.yw);
                r4 = tex2Dlod(s1, r3);
                r3.xy = (r16.xy) + (-(c12.xy));
                r3.zw = (v3.zz) * (c14.yw);
                r3 = tex2Dlod(s1, r3);
                r2.y = r5.x;
                r2.z = r4.x;
                r2.w = r3.x;
                r0.x = dot(r2, c3.wwww);
                r0.w = saturate((v3.w) + (c12.w));
                r0.z = (r0.y) + (-(r0.x));
                r0.w = (r0.w) * (r0.z) + (r0.x);
            }
            else
            {
                r0.w = r0.y;
            }
        }
    }
    r2.xyz = (v4.xyz) * (-(r12.www)) + (c[17].xyz);
    r0.xyz = normalize(r2.xyz);
    r2.w = saturate(dot(r0.xyz, c[17].xyz));
    r2.w = (-(r2.w)) + (c0.y);
    r2.z = (r2.w) * (r2.w);
    r2.z = (r2.z) * (r2.z);
    r2.w = (r2.w) * (r2.z);
    r2.z = (r7.w) * (r10.w) + (r11.w);
    r2.y = (r2.z) * (r9.w) + (c1.w);
    r2.z = saturate(dot(r11.xyz, r0.xyz));
    r0.y = 1.0f / (r2.y);
    r0.z = pow(abs(r2.z), r8.w);
    r0.y = (r7.w) * (r0.y);
    r0.z = (r1.x) * (r0.z);
    r1.x = (r0.y) * (r0.z);
    r0.xyz = (r9.xyz) * (r2.www) + (r10.xyz);
    r0.xyz = (r1.xxx) * (r0.xyz);
    r0.xyz = (r1.yyy) * (r0.xyz);
    r2.xyz = (r0.xyz) * (c[19].xyz);
    r0.xyz = (r0.www) * (r13.xyz) + (r14.xyz);
    r2.xyz = (r0.www) * (r2.xyz);
    r7.xyz = (r7.xyz) * (r0.xyz) + (r2.xyz);
    r0.w = (-(r1.z)) + (c0.y);
    r0.y = 1.0f / (r15.x);
    r0.z = (r0.w) * (r0.w);
    r0.z = (r0.w) * (r0.z);
    r0.w = dot(r12.xyz, r11.xyz);
    r4.w = (r0.y) * (r0.z);
    r0.z = (r0.w) + (r0.w);
    r0.w = (r1.w) * (c0.w);
    r0.xyz = (r11.xyz) * (-(r0.zzz)) + (r12.xyz);
    r2 = texCUBElod(s15, r0);
    r0 = (v4.yyyy) * (c[25]);
    r0 = (v4.xxxx) * (c[24]) + (r0);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r0 = (v4.zzzz) * (c[26]) + (r0);
    r1.xyz = (r1.yyy) * (r2.xyz);
    r3 = (r0) + (c[27]);
    r0.xyz = (r9.xyz) * (r4.www) + (r10.xyz);
    r2.zw = r3.zw;
    r0.xyz = (r1.xyz) * (r0.xyz);
    r4.zw = r2.zw;
    r5.xyz = (r8.xyz) * (r0.xyz);
    r1.zw = r4.zw;
    r0.xy = (r3.ww) * (-(c[28].zw)) + (r3.xy);
    r0.zw = r1.zw;
    r0 = tex2Dproj(s2, r0);
    r0.w = r0.x;
    r4.xy = (r2.ww) * (-(c[28].xy)) + (r3.xy);
    r4 = tex2Dproj(s2, r4);
    r0.y = r4.x;
    r2.xy = (r2.ww) * (c[28].xy) + (r3.xy);
    r4 = tex2Dproj(s2, r2);
    r0.x = r4.x;
    r1.xy = (r2.ww) * (c[28].zw) + (r3.xy);
    r2 = tex2Dproj(s2, r1);
    r1 = (v4.xyzx) * (c14.yyyw) + (c14.wwwy);
    r0.z = r2.x;
    r2.w = dot(r1, c[21]);
    r2.w = 1.0f / (r2.w);
    r3.x = dot(r1, c[20]);
    r2.x = dot(r1, c[10]);
    r3.y = (r3.x) * (r3.x);
    r2.y = dot(r1, c[11]);
    r1.w = dot(c[8].yz, r3.xy) + (c[8].x);
    r1.z = saturate(1.0f / (r1.w));
    r1.xy = saturate((r3.xx) * (c[9].xy) + (c[9].zw));
    r3.xy = (r1.xy) * (c13.zz) + (c13.ww);
    r1.xy = (r1.xy) * (r1.xy);
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c14.w) : (r1.z));
    r1.z = (r3.x) * (r1.x);
    r1.w = (r1.w) * (r1.z);
    r1.z = (r1.y) * (-(r3.y)) + (c0.y);
    r1.xy = (r2.ww) * (r2.xy);
    r1.w = (r1.w) * (r1.z);
    r2.w = (r6.w) * (r1.w);
    r1.xy = (r1.xy) * (c14.zz) + (c14.zz);
    r1 = tex2D(s3, r1.xy);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = dot(r0, c3.wwww);
    r0.xyz = (r2.www) * (r1.xyz);
    r1.xyz = (r5.xyz) * (c0.www) + (r7.xyz);
    r0.xyz = (r0.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (r6.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.w = v1.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.y;

    return oC0;
}
