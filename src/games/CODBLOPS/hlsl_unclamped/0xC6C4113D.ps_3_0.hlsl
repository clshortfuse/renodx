// Mechanically reconstructed from 0xC6C4113D.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler3D s11 : register(s11);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 1.0f, 0.0f, 31.875f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 oC0 = 0.0f;

    r5 = (-(v6.yyyy)) + (c[7]);
    r0 = (r5) * (r5);
    r4 = (-(v6.xxxx)) + (c[6]);
    r1 = (r4) * (r4) + (r0);
    r3 = (-(v6.zzzz)) + (c[8]);
    r0.xy = (v1.xy) * (c[21].xy);
    r0 = tex2D(s2, r0.xy);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r6.xy = (c[21].zz) * (r2.xy) + (r0.xy);
    r6.z = c1.y;
    r0 = tex2D(s0, v1.xy);
    r7.w = (r0.w) * (v0.w) + (c1.x);
    r2 = (r3) * (r3) + (r1);
    r7.xyz = float3(((r7.w) >= 0.0f ? (r6.x) : (c1.z)), ((r7.w) >= 0.0f ? (r6.y) : (c1.z)), ((r7.w) >= 0.0f ? (r6.z) : (c1.z)));
    r6.x = rsqrt(r2.x);
    r6.y = rsqrt(r2.y);
    r6.z = rsqrt(r2.z);
    r6.w = rsqrt(r2.w);
    r1 = v2;
    r1.xyz = (r7.xxx) * (v5.xyz) + (r1.xyz);
    r0.w = c1.y;
    r2 = saturate((r2) * (c[9]) + (r0.wwww));
    r1.xyz = (r7.yyy) * (v4.xyz) + (r1.xyz);
    r5 = (r5) * (r6);
    r8.xyz = normalize(r1.xyz);
    r5 = (r5) * (r8.yyyy);
    r4 = (r4) * (r6);
    r3 = (r3) * (r6);
    r4 = (r4) * (r8.xxxx) + (r5);
    r0 = (r0.xyzx) * (c1.yyyz) + (c1.zzzy);
    r3 = saturate((r3) * (r8.zzzz) + (r4));
    r0 = float4(((r7.w) >= 0.0f ? (r0.x) : (c1.z)), ((r7.w) >= 0.0f ? (r0.y) : (c1.z)), ((r7.w) >= 0.0f ? (r0.z) : (c1.z)), ((r7.w) >= 0.0f ? (r0.w) : (c1.z)));
    r2 = (r2) * (r3);
    r4.x = dot(c[10], r2);
    r4.y = dot(c[11], r2);
    r4.z = dot(c[20], r2);
    r2.w = max(abs(r8.y), abs(r8.z));
    r0.xyz = (r0.xyz) * (v0.xyz);
    r1.z = max(abs(r8.x), r2.w);
    r2.w = 1.0f / (r1.z);
    r1.xyz = (r8.xyz) * (c[5].xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r1.xyz) * (r2.www) + (v7.xyz);
    r2 = tex3D(s11, r1.xyz);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r2.z = saturate(dot(c[17].xyz, r8.xyz));
    r1.xyz = (r7.zzz) * (r1.xyz);
    r3.xyz = (r1.xyz) * (c1.www);
    r1.xyz = (r2.zzz) * (c[18].xyz);
    r2.xyz = (r4.xyz) * (r0.xyz);
    r1.xyz = (r2.www) * (r1.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[22].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
