// Mechanically reconstructed from 0x77D5396F.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(-0.5f, 8.0f, 31.875f, 1.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.797884583f, -2.0f, 3.0f, 1.0f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v6.ww) * (c[28].xy) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[28].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.y = r0.x;
    r0.xy = (v6.ww) * (c[28].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0.xy = (v6.ww) * (-(c[28].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0.xy = (v1.xy) * (c[30].xy);
    r0 = tex2D(s4, r0.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r3.xy = (c[30].zz) * (r3.xy) + (r0.xy);
    r0 = v2;
    r0.xyz = (r3.xxx) * (v5.xyz) + (r0.xyz);
    r0.xyz = (r3.yyy) * (v4.xyz) + (r0.xyz);
    r9.xyz = normalize(r0.xyz);
    r2.w = max(abs(r9.y), abs(r9.z));
    r1.w = r2.x;
    r0.z = max(abs(r9.x), r2.w);
    r2.w = 1.0f / (r0.z);
    r0.xyz = (r9.xyz) * (c[5].xyz);
    r4.z = dot(r1, c3.zzzz);
    r0.xyz = (r0.xyz) * (r2.www) + (v8.xyz);
    r1 = tex3D(s11, r0.xyz);
    r0.xyz = (-(v7.xyz)) + (c[21].xyz);
    r5.y = dot(r0.xyz, r0.xyz);
    r3.z = rsqrt(r5.y);
    r2.xyz = (r0.xyz) * (r3.zzz);
    r2.w = dot(r2.xyz, c[24].xyz);
    r2.w = saturate((r2.w) * (c[25].x) + (c[25].y));
    r5.x = 1.0f / (r3.z);
    r4.w = (r2.w) * (c[25].w);
    r4.xy = saturate((r5.xx) * (c[27].xy) + (c[27].zw));
    r3.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c2.yy) + (c2.zz);
    r3.w = dot(c[26].yz, r5.xy) + (c[26].x);
    r3.xy = (r3.xy) * (r4.xy);
    r2.w = lerp(r1.w, r4.z, r4.w);
    r1.w = (r3.w) * (r3.x);
    r8.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r3.y) * (r1.w);
    r5.w = (r2.w) * (r1.w);
    r3.w = saturate(dot(r9.xyz, r2.xyz));
    r1 = tex2D(s3, v1.xy);
    r4.z = (r1.w) * (c2.x);
    r7.xyz = normalize(v7.xyz);
    r3.y = (r1.w) * (-(c2.x)) + (c2.w);
    r3.x = saturate(dot(r9.xyz, -(r7.xyz)));
    r2.w = (r3.w) * (r3.y) + (r4.z);
    r3.y = (r3.x) * (r3.y) + (r4.z);
    r4.w = (-(r3.x)) + (c0.w);
    r2.w = (r2.w) * (r3.y) + (c3.x);
    r2.w = 1.0f / (r2.w);
    r3.xyz = (r0.xyz) * (r3.zzz) + (-(r7.xyz));
    r2.w = (r3.w) * (r2.w);
    r0.xyz = normalize(r3.xyz);
    r4.xyz = (r3.www) * (c[22].xyz);
    r3.w = saturate(dot(r9.xyz, r0.xyz));
    r0.z = saturate(dot(r0.xyz, r2.xyz));
    r2.xy = (r1.ww) * (c4.xy) + (c4.zw);
    r0.y = exp2(r2.y);
    r0.z = (-(r0.z)) + (c0.w);
    r2.z = pow(abs(r3.w), r0.y);
    r0.x = (r0.z) * (r0.z);
    r0.y = (r0.y) * (c3.y) + (c3.z);
    r0.x = (r0.x) * (r0.x);
    r0.y = (r2.z) * (r0.y);
    r0.z = (r0.z) * (r0.x);
    r10.xyz = (r1.xyz) * (r1.xyz);
    r6.xyz = (r1.xyz) * (-(r1.xyz)) + (c0.www);
    r1.z = (r2.w) * (r0.y);
    r0.xyz = (r6.xyz) * (r0.zzz) + (r10.xyz);
    r6.w = 1.0f / (r2.x);
    r0.xyz = (r1.zzz) * (r0.xyz);
    r1.w = (r1.w) * (c0.y);
    r0.xyz = (r0.xyz) * (c[29].www);
    r2 = tex2D(s6, v1.xy);
    r1.xyz = (r0.xyz) * (r2.www);
    r0.xy = (v1.xy) * (c[31].xy);
    r3 = tex2D(s5, r0.xy);
    r0.xyz = (r3.xyz) + (c0.xxx);
    r3 = tex2D(s0, v1.xy);
    r0.xyz = saturate((r0.xyz) * (r3.www) + (r3.xyz));
    r1.xyz = (r1.xyz) * (c[23].xyz);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r5.xyz = (r5.www) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r8.xyz) * (c[29].yyy);
    r3.w = dot(r7.xyz, r9.xyz);
    r1.xyz = (r2.www) * (r1.xyz);
    r2.w = (r3.w) + (r3.w);
    r3.xyz = (r1.xyz) * (c0.zzz);
    r1.xyz = (r9.xyz) * (-(r2.www)) + (r7.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r4.xyz = (r5.www) * (r4.xyz) + (r3.xyz);
    r3.xyz = (r1.xyz) * (c0.yyy);
    r1 = tex3D(s11, v8.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r0.xyz) * (r4.xyz) + (r5.xyz);
    r1.xyz = (r3.xyz) * (r1.xyz);
    r8.xyz = (r1.xyz) * (c0.zzz);
    r1.w = (r4.w) * (r4.w);
    r2.w = (r4.w) * (r1.w);
    r5 = (-(v7.yyyy)) + (c[7]);
    r1 = (r5) * (r5);
    r4 = (-(v7.xxxx)) + (c[6]);
    r1 = (r4) * (r4) + (r1);
    r3 = (-(v7.zzzz)) + (c[8]);
    r2.w = (r6.w) * (r2.w);
    r1 = (r3) * (r3) + (r1);
    r10.xyz = (r6.xyz) * (r2.www) + (r10.xyz);
    r6.x = rsqrt(r1.x);
    r6.y = rsqrt(r1.y);
    r6.z = rsqrt(r1.z);
    r6.w = rsqrt(r1.w);
    r8.xyz = (r8.xyz) * (r10.xyz);
    r5 = (r5) * (r6);
    r5 = (r9.yyyy) * (r5);
    r4 = (r4) * (r6);
    r3 = (r3) * (r6);
    r4 = (r4) * (r9.xxxx) + (r5);
    r2.w = c0.w;
    r1 = saturate((r1) * (c[9]) + (r2.wwww));
    r3 = saturate((r3) * (r9.zzzz) + (r4));
    r4.xyz = (r8.xyz) * (c[29].xxx);
    r1 = (r1) * (r3);
    r3.xyz = (r4.xyz) * (r2.xyz) + (r7.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.z = dot(c[20], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[32].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.w;

    return oC0;
}
