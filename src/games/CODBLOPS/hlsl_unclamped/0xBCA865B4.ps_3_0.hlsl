// Mechanically reconstructed from 0xBCA865B4.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s12 : register(s12);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD4;
    float4 v5 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(1.0f, 0.0f, 0.000244140625f, -0.000244140625f);
    const float4 c1 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.25f);
    const float4 c3 = float4(0.75f, 4.0f, -3.0f, -4.0f);
    const float4 c4 = float4(-2.0f, 3.0f, 0.5f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xyz = (-(v5.xyz)) + (c[5].xyz);
    r1.xyz = normalize(r0.xyz);
    r0.w = dot(r1.xyz, c[21].xyz);
    r0.w = saturate((r0.w) * (c[22].x) + (c[22].y));
    r0.z = (r0.w) * (c4.x) + (c4.y);
    r0.w = (r0.w) * (r0.w);
    r6.w = (r0.z) * (r0.w);
    r0.xyz = normalize(v2.xyz);
    r6.z = saturate(dot(r1.xyz, r0.xyz));
    r1.w = saturate(dot(c[17].xyz, r0.xyz));
    r0 = tex2D(s12, v1.zw);
    r0.w = ((-abs(r0.y)) >= 0.0f ? (c0.x) : (c0.y));
    r5.xyz = (r1.www) * (c[18].xyz);
    if ((r0.w) != (-(r0.w)))
    {
        r0.w = r0.y;
        r5.w = r0.w;
    }
    else
    {
        if ((c0.x) >= (v4.w))
        {
            r1 = (v4.xyzx) * (c0.xxxy);
            r0 = (r1) + (-(c1.xyzz));
            r0 = tex2Dlod(s1, r0);
            r0.w = r0.x;
            r2 = (r1) + (c0.zzyy);
            r2 = tex2Dlod(s1, r2);
            r0.x = r2.x;
            r2 = (r1) + (c0.wwyy);
            r2 = tex2Dlod(s1, r2);
            r0.y = r2.x;
            r1 = (r1) + (c1.xyzz);
            r1 = tex2Dlod(s1, r1);
            r0.z = r1.x;
            r5.w = dot(r0, c1.wwww);
            if ((c3.x) < (v4.w))
            {
                r4.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
                r0.xy = (r4.xy) + (c0.zz);
                r0.zw = (v4.zx) * (c0.xy);
                r0 = tex2Dlod(s1, r0);
                r1.xy = (r4.xy) + (c0.ww);
                r1.zw = (v4.zx) * (c0.xy);
                r3 = tex2Dlod(s1, r1);
                r1.xy = (r4.xy) + (c1.xy);
                r1.zw = (v4.zx) * (c0.xy);
                r2 = tex2Dlod(s1, r1);
                r1.xy = (r4.xy) + (-(c1.xy));
                r1.zw = (v4.zx) * (c0.xy);
                r1 = tex2Dlod(s1, r1);
                r0.y = r3.x;
                r0.z = r2.x;
                r0.w = r1.x;
                r0.w = dot(r0, c1.wwww);
                r0.z = (-(r5.w)) + (r0.w);
                r0.w = (v4.w) * (c3.y) + (c3.z);
                r5.w = (r0.w) * (r0.z) + (r5.w);
            }
        }
        else
        {
            r0.w = (v4.w) + (c3.w);
            r0.w = ((r0.w) >= 0.0f ? (c0.y) : (c0.x));
            if ((r0.w) != (-(r0.w)))
            {
                r6.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
                r1.xy = (r6.xy) + (c0.zz);
                r1.zw = (v4.zz) * (c0.xy);
                r1 = tex2Dlod(s1, r1);
                r2.xy = (r6.xy) + (c0.ww);
                r2.zw = (v4.zz) * (c0.xy);
                r4 = tex2Dlod(s1, r2);
                r2.xy = (r6.xy) + (c1.xy);
                r2.zw = (v4.zz) * (c0.xy);
                r3 = tex2Dlod(s1, r2);
                r2.xy = (r6.xy) + (-(c1.xy));
                r2.zw = (v4.zz) * (c0.xy);
                r2 = tex2Dlod(s1, r2);
                r1.y = r4.x;
                r1.z = r3.x;
                r1.w = r2.x;
                r0.x = dot(r1, c1.wwww);
                r0.w = saturate((v4.w) + (c3.z));
                r0.z = (r0.y) + (-(r0.x));
                r0.w = (r0.w) * (r0.z) + (r0.x);
            }
            else
            {
                r0.w = r0.y;
            }
            r5.w = r0.w;
        }
    }
    r0 = (v5.yyyy) * (c[24]);
    r0 = (v5.xxxx) * (c[23]) + (r0);
    r0 = (v5.zzzz) * (c[25]) + (r0);
    r3 = (r0) + (c[26]);
    r2.zw = r3.zw;
    r4.zw = r2.zw;
    r6.xyz = (r6.zzz) * (c[6].xyz);
    r0.zw = r4.zw;
    r1.xy = (r3.ww) * (-(c[27].zw)) + (r3.xy);
    r1.zw = r0.zw;
    r1 = tex2Dproj(s2, r1);
    r1.w = r1.x;
    r4.xy = (r2.ww) * (-(c[27].xy)) + (r3.xy);
    r4 = tex2Dproj(s2, r4);
    r1.y = r4.x;
    r2.xy = (r2.ww) * (c[27].xy) + (r3.xy);
    r4 = tex2Dproj(s2, r2);
    r1.x = r4.x;
    r0.xy = (r2.ww) * (c[27].zw) + (r3.xy);
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0 = (v5.xyzx) * (c0.xxxy) + (c0.yyyx);
    r2.w = dot(r1, c1.wwww);
    r1.w = dot(r0, c[20]);
    r1.w = 1.0f / (r1.w);
    r2.x = dot(r0, c[11]);
    r1.x = dot(r0, c[9]);
    r2.y = (r2.x) * (r2.x);
    r1.y = dot(r0, c[10]);
    r0.w = dot(c[7].yz, r2.xy) + (c[7].x);
    r0.z = saturate(1.0f / (r0.w));
    r0.xy = saturate((r2.xx) * (c[8].xy) + (c[8].zw));
    r2.xy = (r0.xy) * (c4.xx) + (c4.yy);
    r0.xy = (r0.xy) * (r0.xy);
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c0.y) : (r0.z));
    r0.z = (r2.x) * (r0.x);
    r0.w = (r0.w) * (r0.z);
    r0.z = (r0.y) * (-(r2.y)) + (c0.x);
    r0.xy = (r1.ww) * (r1.xy);
    r0.w = (r0.w) * (r0.z);
    r3.w = (r6.w) * (r0.w);
    r0.xy = (r0.xy) * (c4.zz) + (c4.zz);
    r0 = tex2D(s3, r0.xy);
    r1 = tex2D(s0, v1.xy);
    r1.xyz = (r1.xyz) * (v0.www);
    r2.xyz = (r1.www) * (r1.xyz);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r2.xyz) * (v0.xyz);
    r1.xyz = (r3.www) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r2.www) * (r1.xyz);
    r3.xyz = (r6.xyz) * (r0.xyz);
    r1.xyz = (r5.xyz) * (r5.www);
    r2.xyz = (r2.xyz) * (r3.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[28].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.y;

    return oC0;
}
