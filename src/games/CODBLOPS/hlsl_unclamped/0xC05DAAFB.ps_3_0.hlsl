// Mechanically reconstructed from 0xC05DAAFB.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c2 = float4(1.0f, 0.0f, 8.0f, 31.875f);
    const float4 c3 = float4(0.797884583f, 1.0f, 0.0009765625f, 0.25f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v7.xyzx) * (c2.xxxy) + (c2.yyyx);
    r0.w = dot(r1, c[32]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[29]);
    r0.y = dot(r1, c[30]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[33].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[34].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[34].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[31]);
    r0.z = (r2.x) * (c[35].x);
    r0.y = (r2.y) * (c[35].y) + (-(r0.z));
    r0.z = c[35].y;
    r0.z = (r2.y) * (r0.z) + (-(c[34].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[27];
    r1 = saturate((r2.zyxz) * (r1) + (c[28]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[35].zzzz) * (r3) + (c[35].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[34].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[26].yz, r2.zw) + (c[26].x);
    r0.z = (r0.y) * (r0.z);
    r1.w = (r1.x) * (r0.z);
    r1.xz = c[25].xz;
    r1.x = dot(r0.xw, c[36].xy) + (r1.x);
    r1.y = dot(r0.xw, c[36].zw) + (r1.z);
    r0 = tex2D(s3, r1.xy);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r5.xyz = (r1.www) * (r0.xyz);
    r0.xy = (v6.ww) * (c[37].xy) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.xy = (v6.ww) * (-(c[37].xy)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.y = r1.x;
    r1.xy = (v6.ww) * (c[37].zw) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.z = r1.x;
    r1.xy = (v6.ww) * (-(c[37].zw)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r2 = tex2D(s1, v1.xy);
    r6.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r2 = tex2D(s7, v1.xy);
    r4.xy = (v1.xy) + (c1.xx);
    r1.z = c1.y;
    r3.xy = (r4.xy) * (c[43].xx) + (r1.zz);
    r3 = tex2D(s5, r3.xy);
    r6.w = (v8.w) * (c1.z) + (-(r3.x));
    r2.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r4.w = saturate((r6.w) * (c1.w));
    r3.xy = lerp(r6.xy, r2.xy, r4.ww);
    r2.xyz = v2.xyz;
    r2.xyz = (r3.xxx) * (v5.xyz) + (r2.xyz);
    r2.xyz = (r3.yyy) * (v4.xyz) + (r2.xyz);
    r9.xyz = normalize(r2.xyz);
    r0.w = r1.x;
    r1.w = max(abs(r9.y), abs(r9.z));
    r2.w = dot(r0, c3.wwww);
    r0.w = max(abs(r9.x), r1.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r9.xyz) * (c[5].xyz);
    r7.xy = (r4.xy) * (c[44].xx) + (r1.zz);
    r0.xyz = (r0.xyz) * (r0.www) + (v8.xyz);
    r0 = tex3D(s11, r0.xyz);
    r3.xyz = (-(v7.xyz)) + (c[21].xyz);
    r1.z = dot(r3.xyz, r3.xyz);
    r1.w = lerp(r0.w, r2.w, c[25].w);
    r3.w = rsqrt(r1.z);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r4.xyz = (r3.xyz) * (r3.www);
    r1.xyz = (r5.xyz) * (r1.www);
    r1.w = saturate(dot(r9.xyz, r4.xyz));
    r0 = tex2D(s4, v1.xy);
    r6.y = (r0.w) * (c3.x);
    r6.z = (r0.w) * (-(c3.x)) + (c3.y);
    r5.xyz = normalize(v7.xyz);
    r2.w = (r1.w) * (r6.z) + (r6.y);
    r5.w = saturate(dot(r9.xyz, -(r5.xyz)));
    r6.z = (r5.w) * (r6.z) + (r6.y);
    r5.w = (-(r5.w)) + (c2.x);
    r2.w = (r2.w) * (r6.z) + (c3.z);
    r6.xyz = (r3.xyz) * (r3.www) + (-(r5.xyz));
    r2.w = 1.0f / (r2.w);
    r3.xyz = normalize(r6.xyz);
    r6.z = (r1.w) * (r2.w);
    r6.y = saturate(dot(r9.xyz, r3.xyz));
    r3.w = saturate(dot(r3.xyz, r4.xyz));
    r3.xy = (r0.ww) * (c4.xy) + (c4.zw);
    r2.w = exp2(r3.y);
    r3.w = (-(r3.w)) + (c2.x);
    r3.y = pow(abs(r6.y), r2.w);
    r3.z = (r3.w) * (r3.w);
    r2.w = (r2.w) * (c12.x) + (c12.y);
    r3.z = (r3.z) * (r3.z);
    r2.w = (r3.y) * (r2.w);
    r3.w = (r3.w) * (r3.z);
    r10.xyz = (r0.xyz) * (r0.xyz);
    r8.xyz = (r0.xyz) * (-(r0.xyz)) + (c2.xxx);
    r2.w = (r6.z) * (r2.w);
    r0.xyz = (r8.xyz) * (r3.www) + (r10.xyz);
    r8.w = 1.0f / (r3.x);
    r0.xyz = (r2.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c[38].www);
    r4.xy = c[24].xy;
    r3.xyz = (r4.yyy) * (c[23].xyz);
    r0.w = (r0.w) * (c2.z);
    r3.xyz = (r0.xyz) * (r3.xyz);
    r2.xyz = (r2.xyz) * (c[38].yyy);
    r0.xyz = (r4.xxx) * (c[22].xyz);
    r2.xyz = (r2.xyz) * (c2.www);
    r0.xyz = (r1.www) * (r0.xyz);
    r4.xyz = (r1.xyz) * (r3.xyz);
    r3.xyz = (r1.xyz) * (r0.xyz) + (r2.xyz);
    r1 = tex2D(s6, r7.xy);
    r2 = tex2D(s0, v1.xy);
    r0.xyz = (r2.xyz) * (v0.xyz);
    r2.z = dot(r5.xyz, r9.xyz);
    r2.w = (r2.w) * (v0.w);
    r3.w = (r2.z) + (r2.z);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r9.xyz) * (-(r3.www)) + (r5.xyz);
    r0 = texCUBElod(s15, r0);
    r5.xyz = (r0.xyz) * (r0.xyz);
    r0 = lerp(r2, r1, r4.wwww);
    r2.xyz = (r5.xyz) * (c2.zzz);
    r1 = tex3D(s11, v8.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.w = (-(r4.w)) + (c2.x);
    r1.xyz = (r2.xyz) * (r1.xyz);
    r6.xyz = (r0.xyz) * (r3.xyz) + (r4.xyz);
    r7.xyz = (r1.xyz) * (c2.www);
    r5.z = (r5.w) * (r5.w);
    r4 = (-(v7.yyyy)) + (c[7]);
    r1 = (r4) * (r4);
    r3 = (-(v7.xxxx)) + (c[6]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v7.zzzz)) + (c[8]);
    r5.w = (r5.w) * (r5.z);
    r1 = (r2) * (r2) + (r1);
    r8.w = (r8.w) * (r5.w);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r8.xyz = (r8.xyz) * (r8.www) + (r10.xyz);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r4.w = c2.x;
    r1 = saturate((r1) * (c[9]) + (r4.wwww));
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.xyz = (r7.xyz) * (r8.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c[38].xxx) + (r6.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.w = ((r6.w) >= 0.0f ? (r7.w) : (c2.y));
    r2.z = dot(c[20], r1);
    r1.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r1.w = (r1.w) * (c[45].w);
    r0.xyz = (c[45].xyz) * (r1.www) + (r0.xyz);
    r1.xyz = (r0.www) * (r0.xyz);
    r1.w = c2.x;
    r0.x = dot(r1, c[40]);
    r0.y = dot(r1, c[41]);
    r0.z = dot(r1, c[42]);
    r0.xyz = (v3.xyz) * (-(r0.www)) + (r0.xyz);
    r0.xyz = (r0.xyz) * (v2.www);
    r0.xyz = (v3.xyz) * (r0.www) + (r0.xyz);
    r0.xyz = max(((r0.xyz) * (c[39].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
