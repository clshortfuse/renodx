// Mechanically reconstructed from 0xF9E06D9E.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c2 = float4(-2.0f, 3.0f, 0.0009765625f, 0.25f);
    const float4 c3 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c4 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v6.ww) * (c[22].xy) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[22].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.y = r0.x;
    r0.xy = (v6.ww) * (c[22].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0.xy = (v6.ww) * (-(c[22].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0 = tex2D(s1, v1.xy);
    r3.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = v2;
    r0.xyz = (r3.xxx) * (v5.xyz) + (r0.xyz);
    r3.xyz = (r3.yyy) * (v4.xyz) + (r0.xyz);
    r0.xyz = normalize(r3.xyz);
    r3.w = max(abs(r0.y), abs(r0.z));
    r1.w = r2.x;
    r2.w = max(abs(r0.x), r3.w);
    r2.w = 1.0f / (r2.w);
    r2.xyz = (r0.xyz) * (c[5].xyz);
    r3.z = dot(r1, c2.wwww);
    r1.xyz = (r2.xyz) * (r2.www) + (v8.xyz);
    r1 = tex3D(s11, r1.xyz);
    r4.xyz = (-(v7.xyz)) + (c[6].xyz);
    r6.y = dot(r4.xyz, r4.xyz);
    r6.w = rsqrt(r6.y);
    r5.xyz = (r4.xyz) * (r6.www);
    r2.w = dot(r5.xyz, c[10].xyz);
    r2.w = saturate((r2.w) * (c[11].x) + (c[11].y));
    r6.x = 1.0f / (r6.w);
    r3.w = (r2.w) * (c[11].w);
    r3.xy = saturate((r6.xx) * (c[21].xy) + (c[21].zw));
    r2.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c2.xx) + (c2.yy);
    r2.z = dot(c[20].yz, r6.xy) + (c[20].x);
    r3.xy = (r2.xy) * (r3.xy);
    r2.w = lerp(r1.w, r3.z, r3.w);
    r1.w = (r2.z) * (r3.x);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r3.y) * (r1.w);
    r2.w = (r2.w) * (r1.w);
    r5.w = saturate(dot(r0.xyz, r5.xyz));
    r1 = tex2D(s3, v1.xy);
    r6.y = (r1.w) * (c1.w);
    r6.z = (r1.w) * (-(c1.w)) + (c1.z);
    r3.xyz = normalize(v7.xyz);
    r3.w = (r5.w) * (r6.z) + (r6.y);
    r4.w = saturate(dot(r0.xyz, -(r3.xyz)));
    r6.z = (r4.w) * (r6.z) + (r6.y);
    r4.w = (-(r4.w)) + (c1.z);
    r3.w = (r3.w) * (r6.z) + (c2.z);
    r6.xyz = (r4.xyz) * (r6.www) + (-(r3.xyz));
    r3.w = 1.0f / (r3.w);
    r4.xyz = normalize(r6.xyz);
    r6.w = (r5.w) * (r3.w);
    r6.z = saturate(dot(r0.xyz, r4.xyz));
    r3.w = saturate(dot(r4.xyz, r5.xyz));
    r4.xy = (r1.ww) * (c3.xy) + (c3.zw);
    r4.z = exp2(r4.y);
    r3.w = (-(r3.w)) + (c1.z);
    r5.z = pow(abs(r6.z), r4.z);
    r4.y = (r3.w) * (r3.w);
    r4.z = (r4.z) * (c4.x) + (c4.y);
    r4.y = (r4.y) * (r4.y);
    r4.z = (r5.z) * (r4.z);
    r3.w = (r3.w) * (r4.y);
    r7.xyz = (r1.xyz) * (r1.xyz);
    r6.xyz = (r1.xyz) * (-(r1.xyz)) + (c1.zzz);
    r4.z = (r6.w) * (r4.z);
    r1.xyz = (r6.xyz) * (r3.www) + (r7.xyz);
    r3.w = 1.0f / (r4.x);
    r1.xyz = (r4.zzz) * (r1.xyz);
    r1.xyz = (r1.xyz) * (c[23].www);
    r5.xy = c[9].xy;
    r4.xyz = (r5.yyy) * (c[8].xyz);
    r1.w = (r1.w) * (c1.x);
    r4.xyz = (r1.xyz) * (r4.xyz);
    r2.xyz = (r2.xyz) * (c[23].yyy);
    r1.xyz = (r5.xxx) * (c[7].xyz);
    r2.xyz = (r2.xyz) * (c1.yyy);
    r1.xyz = (r5.www) * (r1.xyz);
    r5.xyz = (r2.www) * (r4.xyz);
    r4.xyz = (r2.www) * (r1.xyz) + (r2.xyz);
    r2 = tex2D(s0, v1.xy);
    r1.z = dot(r3.xyz, r0.xyz);
    r1.z = (r1.z) + (r1.z);
    r2.xyz = (r2.xyz) * (v0.xyz);
    r1.xyz = (r0.xyz) * (-(r1.zzz)) + (r3.xyz);
    r1 = texCUBElod(s15, r1);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r3.xyz = (r2.xyz) * (r2.xyz);
    r2.xyz = (r0.xyz) * (c1.xxx);
    r1 = tex3D(s11, v8.xyz);
    r1.w = (r4.w) * (r4.w);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r4.w) * (r1.w);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r0.xyz) * (c1.yyy);
    r2.xyz = (r6.xyz) * (r1.www) + (r7.xyz);
    r1.xyz = (r3.xyz) * (r4.xyz) + (r5.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r0.xyz) * (c[23].xxx) + (r1.xyz);
    r1.w = c1.z;
    r0.x = dot(r1, c[25]);
    r0.y = dot(r1, c[26]);
    r0.z = dot(r1, c[27]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[24].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.z;

    return oC0;
}
