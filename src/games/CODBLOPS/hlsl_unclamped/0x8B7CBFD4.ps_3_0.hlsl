// Mechanically reconstructed from 0x8B7CBFD4.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(0.0f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c2 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c3 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    const float4 c4 = float4(1.0f, 0.797884583f, 0.5f, 0.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c4.xz);
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (c4.xz) + (c4.wz);
    r2 = tex2D(s13, r1.xy);
    r0.w = r2.y;
    r5.xy = (r0.yw) * (c2.yy) + (c2.zz);
    r1 = tex2D(s1, v1.xy);
    r3.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r0.w = dot(r5.xy, r5.xy) + (c1.x);
    r0.y = dot(r3.xy, r3.xy) + (c1.x);
    r0.w = exp2(-(r0.w));
    r0.y = exp2(-(r0.y));
    r0.w = (r0.w) * (c1.y) + (c1.z);
    r2.w = (r0.y) * (c1.y) + (c1.z);
    r1 = tex2D(s14, v1.zw);
    r4.xy = (r1.xy) * (c2.xx);
    r1.w = (r0.w) * (r2.w);
    r2.xy = (r2.xz) * (r4.yy);
    r0.w = dot(r5.xy, r3.xy) + (c1.x);
    r0.y = (r1.y) * (c2.x) + (-(r2.x));
    r0.w = saturate((r0.w) * (r1.w) + (r1.w));
    r0.y = (r2.z) * (-(r4.y)) + (r0.y);
    r2.xz = (r2.xy) * (c2.yy);
    r2.y = (r0.y) + (r0.y);
    r2.xyz = (r0.www) * (r2.xyz);
    r0.xy = (r0.xz) * (r4.xx);
    r6.xyz = (-(v6.xyz)) + (c[20].xyz);
    r0.w = (r1.x) * (c2.x) + (-(r0.x));
    r4.y = dot(r6.xyz, r6.xyz);
    r8.xz = (r0.xy) * (c2.yy);
    r5.w = rsqrt(r4.y);
    r0.w = (r0.z) * (-(r4.x)) + (r0.w);
    r4.x = 1.0f / (r5.w);
    r8.y = (r0.w) + (r0.w);
    r1.xy = saturate((r4.xx) * (c[24].xy) + (c[24].zw));
    r0.xy = (r1.xy) * (r1.xy);
    r1.xy = (r1.xy) * (c3.zz) + (c3.ww);
    r0.w = dot(c[23].yz, r4.xy) + (c[23].x);
    r1.xy = (r0.xy) * (r1.xy);
    r0.xyz = (r8.xyz) * (r2.www) + (r2.xyz);
    r0.w = (r0.w) * (r1.x);
    r4.xyz = (r0.xyz) * (c[26].yyy);
    r2.w = (r1.y) * (r0.w);
    r1 = tex2D(s12, v1.zw);
    r0 = v2;
    r0.xyz = (r3.xxx) * (v5.xyz) + (r0.xyz);
    r0.xyz = (r3.yyy) * (v4.xyz) + (r0.xyz);
    r5.xyz = (r6.xyz) * (r5.www);
    r9.xyz = normalize(r0.xyz);
    r4.w = saturate(dot(r9.xyz, r5.xyz));
    r3.w = (r2.w) * (r1.y);
    r3.xyz = (r4.www) * (c[21].xyz);
    r1 = tex2D(s2, v1.xy);
    r0.x = (r1.w) * (c4.y);
    r2.xyz = normalize(v6.xyz);
    r0.y = (r1.w) * (-(c4.y)) + (c4.x);
    r2.w = saturate(dot(r9.xyz, -(r2.xyz)));
    r0.z = (r4.w) * (r0.y) + (r0.x);
    r0.y = (r2.w) * (r0.y) + (r0.x);
    r0.z = (r0.z) * (r0.y) + (c2.w);
    r6.xyz = (r6.xyz) * (r5.www) + (-(r2.xyz));
    r5.w = 1.0f / (r0.z);
    r0.xyz = normalize(r6.xyz);
    r4.w = (r4.w) * (r5.w);
    r6.w = saturate(dot(r9.xyz, r0.xyz));
    r0.z = saturate(dot(r0.xyz, r5.xyz));
    r5.xy = (r1.ww) * (c12.xy) + (c12.zw);
    r0.y = exp2(r5.y);
    r0.z = (-(r0.z)) + (c4.x);
    r5.w = pow(abs(r6.w), r0.y);
    r0.x = (r0.z) * (r0.z);
    r0.y = (r0.y) * (c3.x) + (c3.y);
    r0.x = (r0.x) * (r0.x);
    r0.y = (r5.w) * (r0.y);
    r0.z = (r0.z) * (r0.x);
    r10.xyz = (r1.xyz) * (r1.xyz);
    r6.xyz = (r1.xyz) * (-(r1.xyz)) + (c4.xxx);
    r4.w = (r4.w) * (r0.y);
    r0.xyz = (r6.xyz) * (r0.zzz) + (r10.xyz);
    r1.xyz = (r3.www) * (r3.xyz) + (r4.xyz);
    r0.xyz = (r4.www) * (r0.xyz);
    r2.w = (-(r2.w)) + (c4.x);
    r0.xyz = (r0.xyz) * (c[26].www);
    r7.w = 1.0f / (r5.x);
    r0.xyz = (r0.xyz) * (c[22].xyz);
    r1.w = (r1.w) * (c1.w);
    r4.xyz = (r3.www) * (r0.xyz);
    r3 = tex2D(s0, v1.xy);
    r0.xyz = (r3.xyz) * (v0.xyz);
    r3.z = dot(r2.xyz, r9.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r3.z = (r3.z) + (r3.z);
    r7.xyz = (r0.xyz) * (r1.xyz) + (r4.xyz);
    r1.xyz = (r9.xyz) * (-(r3.zzz)) + (r2.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r3.xyz = (r1.xyz) * (c[26].xxx);
    r1.w = (r2.w) * (r2.w);
    r6.w = (r2.w) * (r1.w);
    r5 = (-(v6.yyyy)) + (c[6]);
    r1 = (r5) * (r5);
    r4 = (-(v6.xxxx)) + (c[5]);
    r1 = (r4) * (r4) + (r1);
    r2 = (-(v6.zzzz)) + (c[7]);
    r6.w = (r7.w) * (r6.w);
    r1 = (r2) * (r2) + (r1);
    r10.xyz = (r6.xyz) * (r6.www) + (r10.xyz);
    r6.x = rsqrt(r1.x);
    r6.y = rsqrt(r1.y);
    r6.z = rsqrt(r1.z);
    r6.w = rsqrt(r1.w);
    r3.xyz = (r3.xyz) * (r10.xyz);
    r5 = (r5) * (r6);
    r5 = (r9.yyyy) * (r5);
    r4 = (r4) * (r6);
    r2 = (r2) * (r6);
    r4 = (r4) * (r9.xxxx) + (r5);
    r5.w = c4.x;
    r1 = saturate((r1) * (c[8]) + (r5.wwww));
    r2 = saturate((r2) * (r9.zzzz) + (r4));
    r3.xyz = (r8.xyz) * (r3.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c1.www) + (r7.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r2.x = c[25].w;
    r1.xy = (c[27].xy) * (-(r2.xx)) + (v1.xy);
    r1 = tex2D(s4, r1.xy);
    r2.xy = (c[28].xy) * (-(r2.xx)) + (v1.xy);
    r2 = tex2D(s3, r2.xy);
    r0.xyz = (r1.xyz) * (-(r2.xyz)) + (r0.xyz);
    r0.xyz = (r3.www) * (r0.xyz);
    r0.xyz = (r1.xyz) * (r2.xyz) + (r0.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c4.x;

    return oC0;
}
