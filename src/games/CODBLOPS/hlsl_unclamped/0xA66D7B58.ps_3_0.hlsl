// Mechanically reconstructed from 0xA66D7B58.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-1.0f, 1.0f, 0.200000003f, -0.5f);
    const float4 c2 = float4(0.0f, 1.0f, 0.200000003f, 8.0f);
    const float4 c3 = float4(4.0f, -2.0f, 0.25f, 0.0009765625f);
    const float4 c4 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c12 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c15 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[25]);
    r0 = (v6.xxxx) * (c[24]) + (r0);
    r0 = (v6.zzzz) * (c[26]) + (r0);
    r0 = (r0) + (c[27]);
    r3.xy = (r0.ww) * (c[28].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[28].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s2, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[28].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[28].zw) + (r0.xy);
    r2 = tex2Dproj(s2, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s2, r0);
    r0 = (v6.xyzx) * (c2.yyyx) + (c2.xxxy);
    r2.w = dot(r0, c[21]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[10]);
    r2.y = dot(r0, c[11]);
    r3.w = dot(r1, c3.zzzz);
    r1.xy = (r2.ww) * (r2.xy);
    r2.x = dot(r0, c[20]);
    r0.xy = (r1.xy) * (-(c1.ww)) + (-(c1.ww));
    r0 = tex2D(s3, r0.xy);
    r2.y = (r2.x) * (r2.x);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.w = dot(c[8].yz, r2.xy) + (c[8].x);
    r2.xy = saturate((r2.xx) * (c[9].xy) + (c[9].zw));
    r0.z = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c2.x) : (r0.z));
    r10.xyz = (-(v6.xyz)) + (c[5].xyz);
    r0.xy = (r2.xy) * (r2.xy);
    r0.z = dot(r10.xyz, r10.xyz);
    r2.xy = (r2.xy) * (c15.xx) + (c15.yy);
    r6.w = rsqrt(r0.z);
    r1.w = (r0.x) * (r2.x);
    r9.xyz = (r10.xyz) * (r6.www);
    r0.x = (r0.y) * (-(r2.y)) + (c1.y);
    r0.z = dot(r9.xyz, c[22].xyz);
    r0.w = (r0.w) * (r1.w);
    r0.y = saturate((r0.z) * (c[23].x) + (c[23].y));
    r0.z = (r0.y) * (r0.y);
    r0.y = (r0.y) * (c15.x) + (c15.y);
    r0.w = (r0.x) * (r0.w);
    r0.z = (r0.z) * (r0.y);
    r1.w = (r0.w) * (r0.z);
    r0.xy = (v1.zw) * (-(c1.xw));
    r0 = tex2D(s13, r0.xy);
    r2.xy = (v1.zw) * (c12.xy) + (c12.zy);
    r2 = tex2D(s13, r2.xy);
    r0.w = r2.y;
    r3.xyz = (r1.xyz) * (r1.www);
    r4.xy = (r0.yw) * (c3.xx) + (c3.yy);
    r1 = tex2D(s1, v1.xy);
    r7.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r0.y = dot(r4.xy, r4.xy) + (c2.x);
    r0.w = dot(r7.xy, r7.xy) + (c2.x);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c4.x) + (c4.y);
    r0.w = (r0.w) * (c4.x) + (c4.y);
    r2.w = (r0.y) * (r0.w);
    r0.y = dot(r4.xy, r7.xy) + (c2.x);
    r1 = tex2D(s14, v1.zw);
    r5.xy = (r1.xy) * (c4.ww);
    r1.z = saturate((r0.y) * (r2.w) + (r2.w));
    r2.xy = (r2.xz) * (r5.yy);
    r0.y = (r1.y) * (c4.w) + (-(r2.x));
    r4.xz = (r2.xy) * (c12.ww);
    r1.w = (r2.z) * (-(r5.y)) + (r0.y);
    r0.xy = (r0.xz) * (r5.xx);
    r4.y = (r1.w) + (r1.w);
    r1.w = (r1.x) * (c4.w) + (-(r0.x));
    r1.xyz = (r1.zzz) * (r4.xyz);
    r0.z = (r0.z) * (-(r5.x)) + (r1.w);
    r6.xz = (r0.xy) * (c12.ww);
    r6.y = (r0.z) + (r0.z);
    r5.xyz = (r3.www) * (r3.xyz);
    r13.xyz = (r6.xyz) * (r0.www) + (r1.xyz);
    r3 = tex2D(s6, v1.xy);
    r2 = tex2D(s7, v7.zw);
    r1 = tex2D(s5, v7.zw);
    r5.w = (r1.w) * (v0.z) + (c1.w);
    r1.w = ((r5.w) >= 0.0f ? (r2.y) : (r3.y));
    r0 = v2;
    r0.xyz = (r7.xxx) * (v5.xyz) + (r0.xyz);
    r0.xyz = (r7.yyy) * (v4.xyz) + (r0.xyz);
    r2 = (r2.wwww) * (c2.xxxy) + (c2.zzzx);
    r8.xyz = normalize(r0.xyz);
    r4 = tex2D(s4, v7.xy);
    r0.xyz = (r4.xyz) + (c1.xxx);
    r11.xyz = normalize(v6.xyz);
    r0.xyz = (v0.yyy) * (r0.xyz) + (c1.yyy);
    r4.w = dot(r11.xyz, r8.xyz);
    r3.xyz = (r0.xyz) * (c1.zzz);
    r4.w = (r4.w) + (r4.w);
    r3 = float4(((r5.w) >= 0.0f ? (r2.x) : (r3.x)), ((r5.w) >= 0.0f ? (r2.y) : (r3.y)), ((r5.w) >= 0.0f ? (r2.z) : (r3.z)), ((r5.w) >= 0.0f ? (r2.w) : (r3.w)));
    r2.xyz = (r8.xyz) * (-(r4.www)) + (r11.xyz);
    r2.w = (r3.w) * (c2.w);
    r2 = texCUBElod(s15, r2);
    r4.w = saturate(dot(r8.xyz, -(r11.xyz)));
    r2.w = (-(r4.w)) + (c1.y);
    r4.z = (r2.w) * (r2.w);
    r14.xy = (r3.ww) * (c13.xy) + (c13.zw);
    r2.w = (r2.w) * (r4.z);
    r4.z = 1.0f / (r14.x);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r2.w) * (r4.z);
    r7.xyz = (r3.xyz) * (r3.xyz);
    r3.xyz = (r3.xyz) * (-(r3.xyz)) + (c1.yyy);
    r2.xyz = (r1.www) * (r2.xyz);
    r12.xyz = (r3.xyz) * (r2.www) + (r7.xyz);
    r4.xyz = (r13.xyz) * (r1.www);
    r2.xyz = (r2.xyz) * (r12.xyz);
    r10.xyz = (r10.xyz) * (r6.www) + (-(r11.xyz));
    r2.xyz = (r6.xyz) * (r2.xyz);
    r6.w = exp2(r14.y);
    r6.xyz = (r2.xyz) * (c2.www);
    r2 = tex2D(s0, v1.xy);
    r7.w = (r3.w) * (c4.z);
    r3.w = (r3.w) * (-(c14.x)) + (c14.y);
    r2.w = saturate(dot(r9.xyz, r8.xyz));
    r4.w = (r4.w) * (r3.w) + (r7.w);
    r3.w = (r2.w) * (r3.w) + (r7.w);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r2.z = (r3.w) * (r4.w) + (c3.w);
    r3.w = 1.0f / (r2.z);
    r2.xyz = normalize(r10.xyz);
    r3.w = (r2.w) * (r3.w);
    r4.w = saturate(dot(r2.xyz, r9.xyz));
    r2.y = saturate(dot(r8.xyz, r2.xyz));
    r2.z = (-(r4.w)) + (c1.y);
    r4.w = pow(abs(r2.y), r6.w);
    r2.x = (r2.z) * (r2.z);
    r2.y = (r6.w) * (c14.z) + (c14.w);
    r2.x = (r2.x) * (r2.x);
    r2.y = (r4.w) * (r2.y);
    r2.z = (r2.z) * (r2.x);
    r3.w = (r3.w) * (r2.y);
    r2.xyz = (r3.xyz) * (r2.zzz) + (r7.xyz);
    r0.xyz = float3(((r5.w) >= 0.0f ? (r1.x) : (r0.x)), ((r5.w) >= 0.0f ? (r1.y) : (r0.y)), ((r5.w) >= 0.0f ? (r1.z) : (r0.z)));
    r1.xyz = (r3.www) * (r2.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r1.xyz) * (c[7].xyz);
    r2.xyz = (r2.www) * (c[6].xyz);
    r3.xyz = (r1.www) * (r1.xyz);
    r1.xyz = (r0.xyz) * (r4.xyz) + (r6.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r5.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.y;

    return oC0;
}
