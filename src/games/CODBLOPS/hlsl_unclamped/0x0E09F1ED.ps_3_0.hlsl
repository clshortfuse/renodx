// Mechanically reconstructed from 0x0E09F1ED.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
    float4 v8 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(1.0f, -1.0f, 0.0f, 8.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v6.xyzx) * (c1.xxxz) + (c1.zzzx);
    r0.w = dot(r1, c[23]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[20]);
    r0.y = dot(r1, c[21]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[24].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[25].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[25].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[22]);
    r0.z = (r2.x) * (c[26].x);
    r0.y = (r2.y) * (c[26].y) + (-(r0.z));
    r0.z = c[26].y;
    r0.z = (r2.y) * (r0.z) + (-(c[25].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[10];
    r1 = saturate((r2.zyxz) * (r1) + (c[11]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[26].zzzz) * (r3) + (c[26].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[25].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[9].yz, r2.zw) + (c[9].x);
    r0.z = (r0.y) * (r0.z);
    r2.w = (r1.x) * (r0.z);
    r1 = c[27];
    r1.x = dot(r0.xw, r1.xy) + (c[8].x);
    r1.y = dot(r0.xw, r1.zw) + (c[8].z);
    r0 = tex2D(s2, r1.xy);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r5.xyz = (r2.www) * (r0.xyz);
    r0 = tex2D(s12, v1.zw);
    r1.xy = (v1.zw) * (c4.xy);
    r2 = tex2D(s13, r1.xy);
    r1.xy = (v1.zw) * (c4.xy) + (c4.zy);
    r4 = tex2D(s13, r1.xy);
    r2.w = r4.y;
    r1 = tex2D(s1, v1.xy);
    r6.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s3, v7.xy);
    r5.w = (r1.w) * (v0.y);
    r3.xy = (r2.yw) * (c12.xx) + (c12.yy);
    r8.xy = (r5.ww) * (-(r6.xy)) + (r6.xy);
    r0.z = dot(r3.xy, r3.xy) + (c1.z);
    r0.w = dot(r8.xy, r8.xy) + (c1.z);
    r0.z = exp2(-(r0.z));
    r0.w = exp2(-(r0.w));
    r0.z = (r0.z) * (c2.x) + (c2.y);
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r0.x = (r0.z) * (r0.w);
    r0.z = dot(r3.xy, r8.xy) + (c1.z);
    r3 = tex2D(s14, v1.zw);
    r7.xy = (r3.xy) * (c2.ww);
    r0.x = saturate((r0.z) * (r0.x) + (r0.x));
    r4.xy = (r4.xz) * (r7.yy);
    r0.z = (r3.y) * (c2.w) + (-(r4.x));
    r6.xz = (r4.xy) * (c4.ww);
    r0.z = (r4.z) * (-(r7.y)) + (r0.z);
    r2.xy = (r2.xz) * (r7.xx);
    r6.y = (r0.z) + (r0.z);
    r0.z = (r3.x) * (c2.w) + (-(r2.x));
    r3.xyz = (r0.xxx) * (r6.xyz);
    r0.z = (r2.z) * (-(r7.x)) + (r0.z);
    r4.xz = (r2.xy) * (c4.ww);
    r4.y = (r0.z) + (r0.z);
    r5.xyz = (r5.xyz) * (r0.yyy);
    r7.xyz = (r4.xyz) * (r0.www) + (r3.xyz);
    r0.w = c1.x;
    r2.xyz = (r0.www) + (-(c[28].xyw));
    r3.xyz = (-(v6.xyz)) + (c[5].xyz);
    r0 = v2;
    r0.xyz = (r8.xxx) * (v5.xyz) + (r0.xyz);
    r1.w = dot(r3.xyz, r3.xyz);
    r6.xyz = (r8.yyy) * (v4.xyz) + (r0.xyz);
    r3.w = rsqrt(r1.w);
    r0.xyz = normalize(r6.xyz);
    r6.xyz = (r3.xyz) * (r3.www);
    r9.xyz = (r5.www) * (r2.xyz) + (c[28].xyw);
    r4.w = saturate(dot(r0.xyz, r6.xyz));
    r2 = tex2D(s6, v1.xy);
    r8.z = (r2.w) * (c2.z);
    r10.xyz = normalize(v6.xyz);
    r7.w = (r2.w) * (-(c12.z)) + (c12.w);
    r1.w = saturate(dot(r0.xyz, -(r10.xyz)));
    r6.w = (r4.w) * (r7.w) + (r8.z);
    r7.w = (r1.w) * (r7.w) + (r8.z);
    r13.xyz = (r7.xyz) * (r9.yyy);
    r6.w = (r6.w) * (r7.w) + (c3.x);
    r11.xyz = (r4.www) * (c[6].xyz);
    r6.w = 1.0f / (r6.w);
    r6.w = (r4.w) * (r6.w);
    r7.xyz = (r3.xyz) * (r3.www) + (-(r10.xyz));
    r3.xyz = normalize(r7.xyz);
    r14.xy = (r2.ww) * (c13.xy) + (c13.zw);
    r7.w = saturate(dot(r0.xyz, r3.xyz));
    r4.w = exp2(r14.y);
    r3.y = saturate(dot(r3.xyz, r6.xyz));
    r3.z = pow(abs(r7.w), r4.w);
    r3.w = (r4.w) * (c3.y) + (c3.z);
    r7.w = (-(r3.y)) + (c1.x);
    r4.w = (r3.z) * (r3.w);
    r6.z = (r7.w) * (r7.w);
    r3 = tex2D(s4, v7.zw);
    r3.xyz = (r3.xyz) + (c1.yyy);
    r7.z = (r6.z) * (r6.z);
    r12.xyz = (v0.zzz) * (r3.xyz) + (c1.xxx);
    r3 = tex2D(s5, v8.xy);
    r3.xyz = (r3.xyz) + (c1.yyy);
    r2.xyz = (r2.xyz) * (r12.xyz);
    r6.xyz = (v0.www) * (r3.xyz) + (c1.xxx);
    r3.z = (r7.w) * (r7.z);
    r2.xyz = (r2.xyz) * (r6.xyz);
    r8.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.xxx);
    r3.w = (r6.w) * (r4.w);
    r2.xyz = (r7.xyz) * (r3.zzz) + (r8.xyz);
    r3.xyz = (r5.xyz) * (r11.xyz) + (r13.xyz);
    r2.xyz = (r3.www) * (r2.xyz);
    r4.w = (-(r1.w)) + (c1.x);
    r2.xyz = (r9.zzz) * (r2.xyz);
    r3.w = 1.0f / (r14.x);
    r2.xyz = (r2.xyz) * (c[7].xyz);
    r1.w = (r2.w) * (c1.w);
    r5.xyz = (r5.xyz) * (r2.xyz);
    r2 = tex2D(s0, v1.xy);
    r11.xyz = lerp(r2.xyz, r1.xyz, r5.www);
    r2.w = dot(r10.xyz, r0.xyz);
    r1.xyz = (r12.xyz) * (r11.xyz);
    r2.w = (r2.w) + (r2.w);
    r6.xyz = (r6.xyz) * (r1.xyz);
    r1.xyz = (r0.xyz) * (-(r2.www)) + (r10.xyz);
    r1 = texCUBElod(s15, r1);
    r0.z = (r4.w) * (r4.w);
    r1.w = (r4.w) * (r0.z);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r9.xxx) * (r0.xyz);
    r2.xyz = (r7.xyz) * (r1.www) + (r8.xyz);
    r1.xyz = (r6.xyz) * (r6.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz) + (r5.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c1.www) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[30].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[29].w;

    return oC0;
}
