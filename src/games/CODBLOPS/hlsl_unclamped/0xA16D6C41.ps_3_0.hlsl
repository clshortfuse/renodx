// Mechanically reconstructed from 0xA16D6C41.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 1.0f, -1.0f, 0.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c4 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (-(c1.zx));
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (-(c1.zx)) + (-(c1.wx));
    r2 = tex2D(s13, r1.xy);
    r0.w = r2.y;
    r3.xy = (r0.yw) * (c3.yy) + (c3.zz);
    r1 = tex2D(s1, v1.xy);
    r5.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r0.y = dot(r3.xy, r3.xy) + (c1.w);
    r0.w = dot(r5.xy, r5.xy) + (c1.w);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c2.x) + (c2.y);
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r2.w = (r0.y) * (r0.w);
    r0.y = dot(r3.xy, r5.xy) + (c1.w);
    r1 = tex2D(s14, v1.zw);
    r4.xy = (r1.xy) * (c3.xx);
    r1.z = saturate((r0.y) * (r2.w) + (r2.w));
    r2.xy = (r2.xz) * (r4.yy);
    r0.y = (r1.y) * (c3.x) + (-(r2.x));
    r3.xz = (r2.xy) * (c3.yy);
    r1.w = (r2.z) * (-(r4.y)) + (r0.y);
    r0.xy = (r0.xz) * (r4.xx);
    r3.y = (r1.w) + (r1.w);
    r1.w = (r1.x) * (c3.x) + (-(r0.x));
    r1.xyz = (r1.zzz) * (r3.xyz);
    r0.z = (r0.z) * (-(r4.x)) + (r1.w);
    r4.xz = (r0.xy) * (c3.yy);
    r4.y = (r0.z) + (r0.z);
    r0.xyz = (r4.xyz) * (r0.www) + (r1.xyz);
    r1 = tex2D(s2, v7.xy);
    r6.xyz = (-(v6.xyz)) + (c[5].xyz);
    r7.y = dot(r6.xyz, r6.xyz);
    r6.w = rsqrt(r7.y);
    r7.x = 1.0f / (r6.w);
    r1.w = (r1.w) * (v0.y) + (c1.x);
    r3.xy = saturate((r7.xx) * (c[9].xy) + (c[9].zw));
    r2.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c13.xx) + (c13.yy);
    r0.w = dot(c[8].yz, r7.xy) + (c[8].x);
    r2.xy = (r2.xy) * (r3.xy);
    r2.z = c1.y;
    r8.xyz = float3(((r1.w) >= 0.0f ? (r2.z) : (c[10].x)), ((r1.w) >= 0.0f ? (r2.z) : (c[10].y)), ((r1.w) >= 0.0f ? (r2.z) : (c[10].w)));
    r0.w = (r0.w) * (r2.x);
    r9.xyz = (r0.xyz) * (r8.yyy);
    r3.w = (r2.y) * (r0.w);
    r2 = tex2D(s12, v1.zw);
    r0 = v2;
    r0.xyz = (r5.xxx) * (v5.xyz) + (r0.xyz);
    r3.xyz = (r5.yyy) * (v4.xyz) + (r0.xyz);
    r7.xyz = (r6.xyz) * (r6.www);
    r0.xyz = normalize(r3.xyz);
    r4.w = (r3.w) * (r2.y);
    r5.w = saturate(dot(r0.xyz, r7.xyz));
    r2 = tex2D(s0, v1.xy);
    r3 = tex2D(s3, v7.zw);
    r3.xyz = (r3.xyz) + (c1.zzz);
    r1.xyz = float3(((r1.w) >= 0.0f ? (r1.x) : (r2.x)), ((r1.w) >= 0.0f ? (r1.y) : (r2.y)), ((r1.w) >= 0.0f ? (r1.z) : (r2.z)));
    r5.xyz = (v0.zzz) * (r3.xyz) + (c1.yyy);
    r2.xyz = (r5.www) * (c[6].xyz);
    r1.xyz = (r1.xyz) * (r5.xyz);
    r3.xyz = (r4.www) * (r2.xyz) + (r9.xyz);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r1 = tex2D(s4, v1.xy);
    r8.y = (r1.w) * (c2.w);
    r9.xyz = normalize(v6.xyz);
    r7.w = (r1.w) * (-(c4.x)) + (c4.y);
    r3.w = saturate(dot(r0.xyz, -(r9.xyz)));
    r2.w = (r5.w) * (r7.w) + (r8.y);
    r7.w = (r3.w) * (r7.w) + (r8.y);
    r3.w = (-(r3.w)) + (c1.y);
    r2.w = (r2.w) * (r7.w) + (c3.w);
    r2.w = 1.0f / (r2.w);
    r10.xyz = (r6.xyz) * (r6.www) + (-(r9.xyz));
    r7.w = (r5.w) * (r2.w);
    r6.xyz = normalize(r10.xyz);
    r8.w = saturate(dot(r0.xyz, r6.xyz));
    r10.xy = (r1.ww) * (c12.xy) + (c12.zw);
    r6.w = saturate(dot(r6.xyz, r7.xyz));
    r5.w = exp2(r10.y);
    r2.w = 1.0f / (r10.x);
    r6.z = pow(abs(r8.w), r5.w);
    r5.w = (r5.w) * (c4.z) + (c4.w);
    r6.w = (-(r6.w)) + (c1.y);
    r5.w = (r6.z) * (r5.w);
    r6.z = (r6.w) * (r6.w);
    r5.w = (r7.w) * (r5.w);
    r6.z = (r6.z) * (r6.z);
    r6.w = (r6.w) * (r6.z);
    r1.xyz = (r5.xyz) * (r1.xyz);
    r7.xyz = (r1.xyz) * (r1.xyz);
    r5.xyz = (r1.xyz) * (-(r1.xyz)) + (c1.yyy);
    r1.w = (r1.w) * (c2.z);
    r1.xyz = (r5.xyz) * (r6.www) + (r7.xyz);
    r1.xyz = (r5.www) * (r1.xyz);
    r5.w = dot(r9.xyz, r0.xyz);
    r1.xyz = (r8.zzz) * (r1.xyz);
    r5.w = (r5.w) + (r5.w);
    r6.xyz = (r1.xyz) * (c[7].xyz);
    r1.xyz = (r0.xyz) * (-(r5.www)) + (r9.xyz);
    r1 = texCUBElod(s15, r1);
    r0.z = (r3.w) * (r3.w);
    r1.w = (r3.w) * (r0.z);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r2.w) * (r1.w);
    r0.xyz = (r8.xxx) * (r0.xyz);
    r5.xyz = (r5.xyz) * (r1.www) + (r7.xyz);
    r1.xyz = (r4.www) * (r6.xyz);
    r0.xyz = (r0.xyz) * (r5.xyz);
    r1.xyz = (r2.xyz) * (r3.xyz) + (r1.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c2.zzz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[20].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[11].w;

    return oC0;
}
