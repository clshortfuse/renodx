// Mechanically reconstructed from 0xBA11392E.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(2.0f, -1.0f, -0.5f, -0.0f);
    const float4 c2 = float4(0.0f, 1.0f, 0.200000003f, 8.0f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 3.5f, 1.0f);
    const float4 c4 = float4(31.875f, 4.0f, -2.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s3, v6.zw);
    r4.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = (c1.xxxx) * (v7) + (c1.yyyy);
    r3.x = dot(r4.xy, r0.xy) + (c1.w);
    r1 = tex2D(s1, v0.xy);
    r0.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r2 = tex2D(s0, v0.xy);
    r5.w = (r2.w) * (v6.x) + (c1.z);
    r3.y = dot(r4.xy, r0.zw) + (c1.w);
    r1.xy = float2(((r5.w) >= 0.0f ? (r0.x) : (c1.w)), ((r5.w) >= 0.0f ? (r0.y) : (c1.w)));
    r0 = tex2D(s2, v6.zw);
    r4.w = (r0.w) * (v6.y) + (c1.z);
    r7.xy = float2(((r4.w) >= 0.0f ? (r3.x) : (r1.x)), ((r4.w) >= 0.0f ? (r3.y) : (r1.y)));
    r1 = v1;
    r1.xyz = (r7.xxx) * (v4.xyz) + (r1.xyz);
    r3.xyz = (r7.yyy) * (v3.xyz) + (r1.xyz);
    r1.xyz = normalize(r3.xyz);
    r5.xyz = normalize(v5.xyz);
    r2 = (r2.xyzx) * (-(c1.yyyw)) + (-(c1.wwwy));
    r3.w = saturate(dot(r1.xyz, -(r5.xyz)));
    r0 = (r0.xyzx) * (-(c1.yyyw)) + (-(c1.wwwy));
    r6.w = (-(r3.w)) + (-(c1.y));
    r3 = tex2D(s4, v0.xy);
    r3.xyz = (r3.wwy) * (c2.xyy) + (c2.zxx);
    r6.z = (r6.w) * (r6.w);
    r4.xyz = float3(((r5.w) >= 0.0f ? (r3.x) : (-(c1.w))), ((r5.w) >= 0.0f ? (r3.y) : (-(c1.y))), ((r5.w) >= 0.0f ? (r3.z) : (-(c1.w))));
    r3 = tex2D(s5, v6.zw);
    r3.xyz = (r3.wwy) * (c2.xyy) + (c2.zxx);
    r3.w = (r6.w) * (r6.z);
    r4.xyz = float3(((r4.w) >= 0.0f ? (r3.x) : (r4.x)), ((r4.w) >= 0.0f ? (r3.y) : (r4.y)), ((r4.w) >= 0.0f ? (r3.z) : (r4.z)));
    r2 = float4(((r5.w) >= 0.0f ? (r2.x) : (c1.w)), ((r5.w) >= 0.0f ? (r2.y) : (c1.w)), ((r5.w) >= 0.0f ? (r2.z) : (c1.w)), ((r5.w) >= 0.0f ? (r2.w) : (c1.w)));
    r3.z = (r4.y) * (c3.z) + (c3.w);
    r0 = float4(((r4.w) >= 0.0f ? (r0.x) : (r2.x)), ((r4.w) >= 0.0f ? (r0.y) : (r2.y)), ((r4.w) >= 0.0f ? (r0.z) : (r2.z)), ((r4.w) >= 0.0f ? (r0.w) : (r2.w)));
    r2.w = 1.0f / (r3.z);
    r2.z = (r3.w) * (r2.w);
    r2.w = (r4.x) * (-(r4.x)) + (-(c1.y));
    r2.z = (r2.z) * (r2.w);
    r2.w = dot(r5.xyz, r1.xyz);
    r4.w = (r4.x) * (r4.x) + (r2.z);
    r2.w = (r2.w) + (r2.w);
    r2.xyz = (r1.xyz) * (-(r2.www)) + (r5.xyz);
    r2.w = (r4.y) * (c2.w);
    r2 = texCUBElod(s15, r2);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r2.xy = (v0.zw) * (-(c1.yz));
    r2 = tex2D(s13, r2.xy);
    r3 = tex2D(s14, v0.zw);
    r4.xy = (r3.xy) * (c4.xx);
    r1.xyz = (r4.zzz) * (r1.xyz);
    r5.xy = (r2.xz) * (r4.xx);
    r6.xyz = (r4.www) * (r1.xyz);
    r1.z = (r3.x) * (c4.x) + (-(r5.x));
    r5.xz = (r5.xy) * (c4.yy);
    r1.z = (r2.z) * (-(r4.x)) + (r1.z);
    r1.x = r2.y;
    r2.xy = (v0.zw) * (-(c1.yz)) + (-(c1.wz));
    r2 = tex2D(s13, r2.xy);
    r8.xy = (r4.yy) * (r2.xz);
    r5.y = (r1.z) + (r1.z);
    r1.z = (r3.y) * (c4.x) + (-(r8.x));
    r3.xz = (r8.xy) * (c4.yy);
    r1.y = r2.y;
    r2.z = (r2.z) * (-(r4.y)) + (r1.z);
    r1.xy = (r1.xy) * (c4.yy) + (c4.zz);
    r2.w = dot(r1.xy, r1.xy) + (c1.w);
    r1.z = dot(r7.xy, r7.xy) + (c1.w);
    r2.w = exp2(-(r2.w));
    r1.z = exp2(-(r1.z));
    r2.y = (r2.w) * (c3.x) + (c3.y);
    r2.w = (r1.z) * (c3.x) + (c3.y);
    r1.z = dot(r1.xy, r7.xy) + (c1.w);
    r1.y = (r2.y) * (r2.w);
    r3.y = (r2.z) + (r2.z);
    r1.z = saturate((r1.z) * (r1.y) + (r1.y));
    r2.xyz = (r6.xyz) * (r5.xyz);
    r1.xyz = (r3.xyz) * (r1.zzz);
    r2.xyz = (r2.xyz) * (c2.www);
    r1.xyz = (r5.xyz) * (r2.www) + (r1.xyz);
    r1.xyz = (r4.zzz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[5].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
