// Mechanically reconstructed from 0x45A850BE.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s8 : register(s8);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
    float4 v8 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(2.0f, -1.0f, 0.0f, -0.200000003f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c3.xy);
    r2 = tex2D(s13, r0.xy);
    r0.xy = (v1.zw) * (c3.xy) + (c3.zy);
    r4 = tex2D(s13, r0.xy);
    r0 = tex2D(s1, v1.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s4, v6.xy);
    r1.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = (c14.xxxx) * (v7) + (c14.yyyy);
    r0.x = dot(r1.xy, r0.xy) + (c14.z);
    r0.y = dot(r1.xy, r0.zw) + (c14.z);
    r1 = tex2D(s2, v6.xy);
    r8.w = (r1.w) * (v0.y);
    r2.w = r4.y;
    r7.xy = lerp(r3.xy, r0.xy, r8.ww);
    r0 = tex2D(s5, v6.zw);
    r5.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = (c14.xxxx) * (v8) + (c14.yyyy);
    r3.x = dot(r5.xy, r0.xy) + (c14.z);
    r3.y = dot(r5.xy, r0.zw) + (c14.z);
    r0 = tex2D(s3, v6.zw);
    r7.w = (r0.w) * (v0.z);
    r5.xy = (r2.yw) * (c12.xx) + (c12.yy);
    r6.xy = lerp(r7.xy, r3.xy, r7.ww);
    r1.w = dot(r5.xy, r5.xy) + (c14.z);
    r0.w = dot(r6.xy, r6.xy) + (c14.z);
    r1.w = exp2(-(r1.w));
    r0.w = exp2(-(r0.w));
    r1.w = (r1.w) * (c2.x) + (c2.y);
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r2.w = (r1.w) * (r0.w);
    r3 = tex2D(s14, v1.zw);
    r8.xy = (r3.xy) * (c3.ww);
    r1.w = dot(r5.xy, r6.xy) + (c14.z);
    r4.xy = (r4.xz) * (r8.yy);
    r1.w = saturate((r1.w) * (r2.w) + (r2.w));
    r2.w = (r3.y) * (c3.w) + (-(r4.x));
    r5.xz = (r4.xy) * (c12.xx);
    r2.w = (r4.z) * (-(r8.y)) + (r2.w);
    r5.y = (r2.w) + (r2.w);
    r2.xy = (r2.xz) * (r8.xx);
    r4.xyz = (r1.www) * (r5.xyz);
    r1.w = (r3.x) * (c3.w) + (-(r2.x));
    r7.xz = (r2.xy) * (c12.xx);
    r1.w = (r2.z) * (-(r8.x)) + (r1.w);
    r7.y = (r1.w) + (r1.w);
    r2.xyw = c[8].xyw;
    r2.xyz = (-(r2.xyw)) + (c[9].xyw);
    r3.xyz = (r7.xyz) * (r0.www) + (r4.xyz);
    r5.xyz = (r8.www) * (r2.xyz) + (c[8].xyw);
    r2 = tex2D(s8, v6.zw);
    r4.xyz = v2.xyz;
    r4.xyz = (r6.xxx) * (v4.xyz) + (r4.xyz);
    r4.xyz = (r6.yyy) * (v3.xyz) + (r4.xyz);
    r9.xyz = lerp(r5.xyz, r2.yyy, r7.www);
    r10.xyz = normalize(r4.xyz);
    r4 = (r2.wwww) * (-(c14.zzzy)) + (-(c14.wwwz));
    r1.w = saturate(dot(r10.xyz, c[17].xyz));
    r12.xyz = (r3.xyz) * (r9.yyy);
    r11.xyz = (r1.www) * (c[18].xyz);
    r2 = tex2D(s12, v1.zw);
    r6 = tex2D(s6, v1.xy);
    r3 = tex2D(s7, v6.xy);
    r5 = lerp(r6, r3, r8.wwww);
    r0.w = dot(v5.xyz, v5.xyz);
    r3 = lerp(r5, r4, r7.wwww);
    r2.x = rsqrt(r0.w);
    r4.w = (r3.w) * (c2.w);
    r4.xyz = (r2.xxx) * (v5.xyz);
    r2.z = (r3.w) * (-(c12.z)) + (c12.w);
    r0.w = saturate(dot(r10.xyz, -(r4.xyz)));
    r2.w = (r1.w) * (r2.z) + (r4.w);
    r2.z = (r0.w) * (r2.z) + (r4.w);
    r6.xyz = (v5.xyz) * (-(r2.xxx)) + (c[17].xyz);
    r2.w = (r2.w) * (r2.z) + (c4.x);
    r2.w = 1.0f / (r2.w);
    r5.xyz = normalize(r6.xyz);
    r2.z = (r1.w) * (r2.w);
    r2.x = saturate(dot(r10.xyz, r5.xyz));
    r2.w = saturate(dot(r5.xyz, c[17].xyz));
    r13.xy = (r3.ww) * (c13.xy) + (c13.zw);
    r1.w = exp2(r13.y);
    r2.w = (-(r2.w)) + (-(c14.y));
    r4.w = pow(abs(r2.x), r1.w);
    r2.x = (r2.w) * (r2.w);
    r1.w = (r1.w) * (c4.y) + (c4.z);
    r2.x = (r2.x) * (r2.x);
    r1.w = (r4.w) * (r1.w);
    r2.w = (r2.w) * (r2.x);
    r8.xyz = (r3.xyz) * (r3.xyz);
    r6.xyz = (r3.xyz) * (-(r3.xyz)) + (-(c14.yyy));
    r1.w = (r2.z) * (r1.w);
    r5.xyz = (r6.xyz) * (r2.www) + (r8.xyz);
    r3.xyz = (r2.yyy) * (r11.xyz) + (r12.xyz);
    r5.xyz = (r1.www) * (r5.xyz);
    r4.w = (-(r0.w)) + (-(c14.y));
    r5.xyz = (r9.zzz) * (r5.xyz);
    r1.w = 1.0f / (r13.x);
    r5.xyz = (r5.xyz) * (c[19].xyz);
    r0.w = (r3.w) * (c2.z);
    r5.xyz = (r2.yyy) * (r5.xyz);
    r2 = tex2D(s0, v1.xy);
    r11.xyz = lerp(r2.xyz, r1.xyz, r8.www);
    r2.w = dot(r4.xyz, r10.xyz);
    r1.xyz = lerp(r11.xyz, r0.xyz, r7.www);
    r0.z = (r2.w) + (r2.w);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r10.xyz) * (-(r0.zzz)) + (r4.xyz);
    r0 = texCUBElod(s15, r0);
    r0.w = (r4.w) * (r4.w);
    r0.w = (r4.w) * (r0.w);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r1.w) * (r0.w);
    r0.xyz = (r9.xxx) * (r0.xyz);
    r1.xyz = (r6.xyz) * (r0.www) + (r8.xyz);
    r2.xyz = (r2.xyz) * (r3.xyz) + (r5.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz);
    r0.w = dot(c[5].xyz, r4.xyz);
    r1.xyz = (r7.xyz) * (r0.xyz);
    r0.w = saturate((c[7].y) * (r0.w) + (c[7].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[6].xyz);
    r1.xyz = (r1.xyz) * (c2.zzz) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[11].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[10].w;

    return oC0;
}
