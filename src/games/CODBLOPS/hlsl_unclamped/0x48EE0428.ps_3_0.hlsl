// Mechanically reconstructed from 0x48EE0428.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : TEXCOORD7;
    float4 v8 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(2.0f, -1.0f, 0.0f, -0.5f);
    const float4 c3 = float4(0.200000003f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c4 = float4(0.797884583f, 31.875f, 4.0f, -2.0f);
    const float4 c12 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    const float4 c13 = float4(0.797884583f, 1.0f, 0.0009765625f, 0.0f);
    const float4 c14 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v5.xyzx) * (-(c2.yyyz)) + (-(c2.zzzy));
    r1.w = dot(r0, c[21]);
    r1.w = 1.0f / (r1.w);
    r1.x = dot(r0, c[10]);
    r1.y = dot(r0, c[11]);
    r2.xy = (r1.ww) * (r1.xy);
    r1.x = dot(r0, c[20]);
    r0.xy = (r2.xy) * (-(c2.ww)) + (-(c2.ww));
    r0 = tex2D(s2, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[8].yz, r1.xy) + (c[8].x);
    r4.xyz = (r0.xyz) * (r0.xyz);
    r0.z = saturate(1.0f / (r0.w));
    r1.xy = saturate((r1.xx) * (c[9].xy) + (c[9].zw));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c2.z) : (r0.z));
    r0.xy = (r1.xy) * (r1.xy);
    r11.xyz = (-(v5.xyz)) + (c[5].xyz);
    r1.xy = (r1.xy) * (c12.zz) + (c12.ww);
    r1.w = dot(r11.xyz, r11.xyz);
    r0.z = (r0.x) * (r1.x);
    r5.w = rsqrt(r1.w);
    r0.y = (r0.y) * (-(r1.y)) + (-(c2.y));
    r7.xyz = (r11.xyz) * (r5.www);
    r0.z = (r0.w) * (r0.z);
    r0.w = dot(r7.xyz, c[22].xyz);
    r4.w = (r0.y) * (r0.z);
    r0.z = saturate((r0.w) * (c[23].x) + (c[23].y));
    r0.w = (r0.z) * (r0.z);
    r0.z = (r0.z) * (c12.z) + (c12.w);
    r6.w = (r0.w) * (r0.z);
    r0.xy = (v1.zw) * (-(c2.yw));
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (-(c2.yw)) + (-(c2.zw));
    r3 = tex2D(s13, r1.xy);
    r0.w = r3.y;
    r1 = tex2D(s1, v1.xy);
    r6.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = tex2D(s6, v6.xy);
    r2.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = (c2.xxxx) * (v8) + (c2.yyyy);
    r5.x = dot(r2.xy, r1.xy) + (c2.z);
    r5.y = dot(r2.xy, r1.zw) + (c2.z);
    r1 = tex2D(s3, v6.xy);
    r1.w = (r1.w) * (v0.y) + (c2.w);
    r2.xy = (r0.yw) * (c4.zz) + (c4.ww);
    r8.xy = float2(((r1.w) >= 0.0f ? (r5.x) : (r6.x)), ((r1.w) >= 0.0f ? (r5.y) : (r6.y)));
    r0.y = dot(r2.xy, r2.xy) + (c2.z);
    r0.w = dot(r8.xy, r8.xy) + (c2.z);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c3.y) + (c3.z);
    r0.w = (r0.w) * (c3.y) + (c3.z);
    r3.w = (r0.y) * (r0.w);
    r0.y = dot(r2.xy, r8.xy) + (c2.z);
    r2 = tex2D(s14, v1.zw);
    r6.xy = (r2.xy) * (c4.yy);
    r2.z = saturate((r0.y) * (r3.w) + (r3.w));
    r3.xy = (r3.xz) * (r6.yy);
    r0.y = (r2.y) * (c4.y) + (-(r3.x));
    r5.xz = (r3.xy) * (c4.zz);
    r2.w = (r3.z) * (-(r6.y)) + (r0.y);
    r0.xy = (r0.xz) * (r6.xx);
    r5.y = (r2.w) + (r2.w);
    r2.w = (r2.x) * (c4.y) + (-(r0.x));
    r2.xyz = (r2.zzz) * (r5.xyz);
    r0.z = (r0.z) * (-(r6.x)) + (r2.w);
    r10.xz = (r0.xy) * (c4.zz);
    r10.y = (r0.z) + (r0.z);
    r2.w = (r4.w) * (r6.w);
    r2.xyz = (r10.xyz) * (r0.www) + (r2.xyz);
    r0 = tex2D(s7, v1.xy);
    r12.xy = float2(((r1.w) >= 0.0f ? (-(c2.z)) : (r0.y)), ((r1.w) >= 0.0f ? (-(c2.y)) : (r0.y)));
    r4.xyz = (r4.xyz) * (r2.www);
    r6.xyz = (r2.xyz) * (r12.yyy);
    r2 = tex2D(s0, v1.xy);
    r3 = tex2D(s4, v6.zw);
    r3.xyz = (r3.xyz) + (c2.yyy);
    r0.xyz = float3(((r1.w) >= 0.0f ? (r1.x) : (r2.x)), ((r1.w) >= 0.0f ? (r1.y) : (r2.y)), ((r1.w) >= 0.0f ? (r1.z) : (r2.z)));
    r1.xyz = (v0.zzz) * (r3.xyz) + (-(c2.yyy));
    r0.xyz = (r0.xyz) * (r1.xyz);
    r2.xyz = (r1.xyz) * (c3.xxx);
    r1 = tex2D(s5, v7.xy);
    r1.xyz = (r1.xyz) + (c2.yyy);
    r3.xyz = v2.xyz;
    r3.xyz = (r8.xxx) * (v4.xyz) + (r3.xyz);
    r1.xyz = (v0.www) * (r1.xyz) + (-(c2.yyy));
    r3.xyz = (r8.yyy) * (v3.xyz) + (r3.xyz);
    r8.xyz = normalize(r3.xyz);
    r5.xyz = normalize(v5.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz);
    r1.w = dot(r5.xyz, r8.xyz);
    r9.xyz = (r2.xyz) * (r1.xyz);
    r1.w = (r1.w) + (r1.w);
    r1.xyz = (r8.xyz) * (-(r1.www)) + (r5.xyz);
    r1.w = (r0.w) * (c3.w);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r12.xxx) * (r1.xyz);
    r3.xyz = (r9.xyz) * (r9.xyz);
    r4.w = saturate(dot(r8.xyz, -(r5.xyz)));
    r1.xyz = (r9.xyz) * (-(r9.xyz)) + (-(c2.yyy));
    r1.w = (-(r4.w)) + (-(c2.y));
    r2.w = (r1.w) * (r1.w);
    r9.xy = (r0.ww) * (c14.xy) + (c14.zw);
    r1.w = (r1.w) * (r2.w);
    r2.w = 1.0f / (r9.x);
    r3.w = exp2(r9.y);
    r1.w = (r1.w) * (r2.w);
    r9.xyz = (r1.xyz) * (r1.www) + (r3.xyz);
    r11.xyz = (r11.xyz) * (r5.www) + (-(r5.xyz));
    r2.xyz = (r2.xyz) * (r9.xyz);
    r9.xyz = normalize(r11.xyz);
    r2.xyz = (r10.xyz) * (r2.xyz);
    r5.w = saturate(dot(r8.xyz, r9.xyz));
    r2.w = saturate(dot(r9.xyz, r7.xyz));
    r1.w = saturate(dot(r7.xyz, r8.xyz));
    r7.w = pow(abs(r5.w), r3.w);
    r6.w = (r3.w) * (c12.x) + (c12.y);
    r5.w = (r0.w) * (c4.x);
    r3.w = (r0.w) * (-(c13.x)) + (c13.y);
    r0.w = (r7.w) * (r6.w);
    r4.w = (r4.w) * (r3.w) + (r5.w);
    r3.w = (r1.w) * (r3.w) + (r5.w);
    r2.w = (-(r2.w)) + (-(c2.y));
    r3.w = (r3.w) * (r4.w) + (c13.z);
    r4.w = (r2.w) * (r2.w);
    r3.w = 1.0f / (r3.w);
    r4.w = (r4.w) * (r4.w);
    r3.w = (r1.w) * (r3.w);
    r2.w = (r2.w) * (r4.w);
    r0.w = (r0.w) * (r3.w);
    r1.xyz = (r1.xyz) * (r2.www) + (r3.xyz);
    r2.xyz = (r2.xyz) * (c3.www);
    r1.xyz = (r0.www) * (r1.xyz);
    r2.xyz = (r0.xyz) * (r6.xyz) + (r2.xyz);
    r3.xyz = (r1.xyz) * (c[7].xyz);
    r1.xyz = (r1.www) * (c[6].xyz);
    r3.xyz = (r12.xxx) * (r3.xyz);
    r0.w = dot(c[24].xyz, r5.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r3.xyz);
    r0.w = saturate((c[26].y) * (r0.w) + (c[26].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[25].xyz);
    r1.xyz = (r4.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[27].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = -(c2.y);

    return oC0;
}
