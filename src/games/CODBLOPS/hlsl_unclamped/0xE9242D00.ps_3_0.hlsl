// Mechanically reconstructed from 0xE9242D00.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    const float4 c15 = float4(2.0f, -1.0f, 1.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c3.xy);
    r1 = tex2D(s13, r0.xy);
    r0.xy = (v1.zw) * (c3.xy) + (c3.zy);
    r3 = tex2D(s13, r0.xy);
    r1.w = r3.y;
    r0 = tex2D(s1, v1.xy);
    r6.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s4, v6.zw);
    r4.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = (c15.xxxx) * (v7) + (c15.yyyy);
    r2.x = dot(r4.xy, r0.xy) + (c15.w);
    r2.y = dot(r4.xy, r0.zw) + (c15.w);
    r0 = tex2D(s3, v6.zw);
    r4.w = (r0.w) * (v0.z);
    r4.xy = (r1.yw) * (c12.xx) + (c12.yy);
    r5.xy = lerp(r6.xy, r2.xy, r4.ww);
    r0.w = dot(r4.xy, r4.xy) + (c15.w);
    r1.w = dot(r5.xy, r5.xy) + (c15.w);
    r0.w = exp2(-(r0.w));
    r1.w = exp2(-(r1.w));
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r1.w = (r1.w) * (c2.x) + (c2.y);
    r1.y = (r0.w) * (r1.w);
    r2 = tex2D(s14, v1.zw);
    r6.xy = (r2.xy) * (c3.ww);
    r0.w = dot(r4.xy, r5.xy) + (c15.w);
    r3.xy = (r3.xz) * (r6.yy);
    r0.w = saturate((r0.w) * (r1.y) + (r1.y));
    r1.y = (r2.y) * (c3.w) + (-(r3.x));
    r4.xz = (r3.xy) * (c12.xx);
    r1.y = (r3.z) * (-(r6.y)) + (r1.y);
    r4.y = (r1.y) + (r1.y);
    r1.xy = (r1.xz) * (r6.xx);
    r3.xyz = (r0.www) * (r4.xyz);
    r0.w = (r2.x) * (c3.w) + (-(r1.x));
    r8.xz = (r1.xy) * (c12.xx);
    r2.xyz = (-(v5.xyz)) + (c[20].xyz);
    r0.w = (r1.z) * (-(r6.x)) + (r0.w);
    r7.y = dot(r2.xyz, r2.xyz);
    r8.y = (r0.w) + (r0.w);
    r0.w = rsqrt(r7.y);
    r1.xyz = (r8.xyz) * (r1.www) + (r3.xyz);
    r7.x = 1.0f / (r0.w);
    r12.zw = c15.zw;
    r3.xyz = (r12.wzw) + (-(c[28].xyw));
    r6.xy = saturate((r7.xx) * (c[24].xy) + (c[24].zw));
    r4.xy = (r6.xy) * (r6.xy);
    r6.xy = (r6.xy) * (c14.xx) + (c14.yy);
    r1.w = dot(c[23].yz, r7.xy) + (c[23].x);
    r4.xy = (r4.xy) * (r6.xy);
    r3.xyz = (r4.www) * (r3.xyz) + (c[28].xyw);
    r1.w = (r1.w) * (r4.x);
    r6.xyz = (r1.xyz) * (r3.yyy);
    r3.w = (r4.y) * (r1.w);
    r1 = tex2D(s12, v1.zw);
    r4.xyz = v2.xyz;
    r4.xyz = (r5.xxx) * (v4.xyz) + (r4.xyz);
    r5.xyz = (r5.yyy) * (v3.xyz) + (r4.xyz);
    r4.xyz = (r2.xyz) * (r0.www);
    r9.xyz = normalize(r5.xyz);
    r2.w = saturate(dot(r9.xyz, r4.xyz));
    r3.y = (r3.w) * (r1.y);
    r5.xyz = (r2.www) * (c[21].xyz);
    r1 = tex2D(s5, v1.xy);
    r7.w = (r1.w) * (c2.w);
    r7.xyz = normalize(v5.xyz);
    r6.w = (r1.w) * (-(c12.z)) + (c12.w);
    r3.w = saturate(dot(r9.xyz, -(r7.xyz)));
    r5.w = (r2.w) * (r6.w) + (r7.w);
    r6.w = (r3.w) * (r6.w) + (r7.w);
    r6.xyz = (r3.yyy) * (r5.xyz) + (r6.xyz);
    r5.w = (r5.w) * (r6.w) + (c4.x);
    r3.w = (-(r3.w)) + (c15.z);
    r5.w = 1.0f / (r5.w);
    r5.w = (r2.w) * (r5.w);
    r5.xyz = (r2.xyz) * (r0.www) + (-(r7.xyz));
    r2.xyz = normalize(r5.xyz);
    r5.xy = (r1.ww) * (c13.xy) + (c13.zw);
    r5.z = saturate(dot(r9.xyz, r2.xyz));
    r0.w = exp2(r5.y);
    r2.z = saturate(dot(r2.xyz, r4.xyz));
    r2.w = pow(abs(r5.z), r0.w);
    r0.w = (r0.w) * (c4.y) + (c4.z);
    r4.z = (-(r2.z)) + (c15.z);
    r0.w = (r2.w) * (r0.w);
    r4.y = (r4.z) * (r4.z);
    r2 = tex2D(s2, v6.xy);
    r2.xyz = (r2.xyz) + (c15.yyy);
    r2.w = (r4.y) * (r4.y);
    r2.xyz = (v0.yyy) * (r2.xyz) + (c15.zzz);
    r2.w = (r4.z) * (r2.w);
    r1.xyz = (r1.xyz) * (r2.xyz);
    r10.xyz = (r1.xyz) * (r1.xyz);
    r4.xyz = (r1.xyz) * (-(r1.xyz)) + (c15.zzz);
    r0.w = (r5.w) * (r0.w);
    r1.xyz = (r4.xyz) * (r2.www) + (r10.xyz);
    r6.w = 1.0f / (r5.x);
    r1.xyz = (r0.www) * (r1.xyz);
    r0.w = (r1.w) * (c2.z);
    r11.xyz = (r3.zzz) * (r1.xyz);
    r1 = tex2D(s0, v1.xy);
    r5.xyz = (r1.xyz) * (-(r2.xyz)) + (r0.xyz);
    r0.xyz = (r11.xyz) * (c[22].xyz);
    r5.xyz = (r4.www) * (r5.xyz);
    r0.xyz = (r3.yyy) * (r0.xyz);
    r1.xyz = (r1.xyz) * (r2.xyz) + (r5.xyz);
    r5.xyz = (r1.xyz) * (r1.xyz);
    r1.w = dot(r7.xyz, r9.xyz);
    r6.xyz = (r5.xyz) * (r6.xyz) + (r0.xyz);
    r0.z = (r1.w) + (r1.w);
    r0.xyz = (r9.xyz) * (-(r0.zzz)) + (r7.xyz);
    r5.w = dot(c[25].xyz, r7.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r7.xyz = (r3.xxx) * (r0.xyz);
    r0.w = (r3.w) * (r3.w);
    r4.w = (r3.w) * (r0.w);
    r3 = (-(v5.yyyy)) + (c[6]);
    r0 = (r3) * (r3);
    r2 = (-(v5.xxxx)) + (c[5]);
    r0 = (r2) * (r2) + (r0);
    r1 = (-(v5.zzzz)) + (c[7]);
    r4.w = (r6.w) * (r4.w);
    r0 = (r1) * (r1) + (r0);
    r10.xyz = (r4.xyz) * (r4.www) + (r10.xyz);
    r4.x = rsqrt(r0.x);
    r4.y = rsqrt(r0.y);
    r4.z = rsqrt(r0.z);
    r4.w = rsqrt(r0.w);
    r7.xyz = (r7.xyz) * (r10.xyz);
    r3 = (r3) * (r4);
    r3 = (r9.yyyy) * (r3);
    r2 = (r2) * (r4);
    r1 = (r1) * (r4);
    r2 = (r2) * (r9.xxxx) + (r3);
    r0 = saturate((r0) * (c[8]) + (r12.zzzz));
    r1 = saturate((r1) * (r9.zzzz) + (r2));
    r2.xyz = (r8.xyz) * (r7.xyz);
    r0 = (r0) * (r1);
    r2.xyz = (r2.xyz) * (c2.zzz) + (r6.xyz);
    r1.x = dot(c[9], r0);
    r1.y = dot(c[10], r0);
    r1.z = dot(c[11], r0);
    r0.w = saturate((c[27].y) * (r5.w) + (c[27].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[26].xyz);
    r1.xyz = (r5.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c15.z;

    return oC0;
}
