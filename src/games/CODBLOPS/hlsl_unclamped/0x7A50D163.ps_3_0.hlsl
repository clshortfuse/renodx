// Mechanically reconstructed from 0x7A50D163.ps_3_0.cso.
...
float4 main(PS_INPUT input) : COLOR0
{
    ...
    const float4 c13 = float4(2.0f, -1.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    ...
    r8.x = (c[38].w) + (v4.x);
    r8.y = v4.y;
    r8.zw = frac(abs(r8.xy));
    r8.xy = float2(((r8.x) >= 0.0f ? (r8.z) : (-(r8.z))), ((r8.y) >= 0.0f ? (r8.w) : (-(r8.w))));
    r9 = tex2D(s4, r8.xy);
    ...
    r0.x = (r0.x) + (r0.x);
    r6.xyz = (r9.xyz) * (-(r0.xxx)) + (-(r7.xyz));
    r0.x = dot(r7.xyz, r5.xyz);
    r0.x = saturate((r0.x) * (c1.w) + (c1.w));
    r7.x = c4.x;
    r1.w = lerp(c[41].x, -(r7.x), r0.x);
    r0.x = (r0.x) + (c1.z);
    ...
    r0.xyz = max(((r1.xyz) * (c[39].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
