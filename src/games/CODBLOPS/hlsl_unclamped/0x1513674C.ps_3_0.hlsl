// Mechanically reconstructed from 0x1513674C.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(0.200000003f, 8.0f, 31.875f, 1.0f);
    const float4 c2 = float4(0.797884583f, -2.0f, 3.0f, 1.0f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r1.xyz = (-(v6.xyz)) + (c[6].xyz);
    r0 = tex2D(s1, v1.xy);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r9.y = dot(r1.xyz, r1.xyz);
    r0.xyz = v2.xyz;
    r0.xyz = (r2.xxx) * (v5.xyz) + (r0.xyz);
    r2.w = rsqrt(r9.y);
    r0.xyz = (r2.yyy) * (v4.xyz) + (r0.xyz);
    r2.xyz = (r1.xyz) * (r2.www);
    r7.xyz = normalize(r0.xyz);
    r4.w = saturate(dot(r7.xyz, r2.xyz));
    r0 = tex2D(s2, v1.xy);
    r3.z = (r0.w) * (c2.x);
    r1.w = (r0.w) * (-(c2.x)) + (c2.w);
    r8.xyz = normalize(v6.xyz);
    r0.z = (r4.w) * (r1.w) + (r3.z);
    r3.w = saturate(dot(r7.xyz, -(r8.xyz)));
    r1.w = (r3.w) * (r1.w) + (r3.z);
    r3.w = (-(r3.w)) + (c1.w);
    r0.z = (r0.z) * (r1.w) + (c3.x);
    r3.xyz = (r1.xyz) * (r2.www) + (-(r8.xyz));
    r0.z = 1.0f / (r0.z);
    r1.xyz = normalize(r3.xyz);
    r4.y = (r4.w) * (r0.z);
    r3.z = saturate(dot(r7.xyz, r1.xyz));
    r1.w = saturate(dot(r1.xyz, r2.xyz));
    r5.xy = (r0.ww) * (c4.xy) + (c4.zw);
    r0.z = exp2(r5.y);
    r1.w = (-(r1.w)) + (c1.w);
    r1.y = pow(abs(r3.z), r0.z);
    r1.z = (r1.w) * (r1.w);
    r0.z = (r0.z) * (c3.y) + (c3.z);
    r1.z = (r1.z) * (r1.z);
    r0.z = (r1.y) * (r0.z);
    r4.z = (r1.w) * (r1.z);
    r1 = tex2D(s0, v1.xy);
    r2.xyz = lerp(r1.xyz, c1.xxx, r0.xxx);
    r6.xyz = (r2.xyz) * (r2.xyz);
    r3.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.www);
    r0.z = (r4.y) * (r0.z);
    r2.xyz = (r3.xyz) * (r4.zzz) + (r6.xyz);
    r4.xyz = (r4.www) * (c[7].xyz);
    r2.xyz = (r0.zzz) * (r2.xyz);
    r9.x = 1.0f / (r2.w);
    r2.xyz = (r0.yyy) * (r2.xyz);
    r0.z = 1.0f / (r5.x);
    r5.xyz = (r2.xyz) * (c[8].xyz);
    r2.w = dot(c[9].yz, r9.xy) + (c[9].x);
    r9.xy = saturate((r9.xx) * (c[10].xy) + (c[10].zw));
    r2.xy = (r9.xy) * (r9.xy);
    r9.xy = (r9.xy) * (c2.yy) + (c2.zz);
    r9.xy = (r2.xy) * (r9.xy);
    r5.w = max(abs(r7.y), abs(r7.z));
    r4.w = (r2.w) * (r9.x);
    r2.w = max(abs(r7.x), r5.w);
    r2.w = 1.0f / (r2.w);
    r2.xyz = (r7.xyz) * (c[5].xyz);
    r4.w = (r9.y) * (r4.w);
    r2.xyz = (r2.xyz) * (r2.www) + (v7.xyz);
    r2 = tex3D(s11, r2.xyz);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r4.w) * (r2.w);
    r2.xyz = (r0.yyy) * (r2.xyz);
    r5.xyz = (r5.xyz) * (r2.www);
    r2.xyz = (r2.xyz) * (c1.zzz);
    r4.xyz = (r2.www) * (r4.xyz) + (r2.xyz);
    r1.xyz = (r0.xxx) * (r1.xyz);
    r2.w = (r1.w) * (v0.w);
    r0.x = dot(r8.xyz, r7.xyz);
    r2.xyz = (r1.xyz) * (v0.xyz);
    r0.x = (r0.x) + (r0.x);
    r1.xyz = (r7.xyz) * (-(r0.xxx)) + (r8.xyz);
    r1.w = (r0.w) * (c1.y);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r1.xyz) * (c1.yyy);
    r1 = tex3D(s11, v7.xyz);
    r0.w = (r3.w) * (r3.w);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = (r3.w) * (r0.w);
    r1.xyz = (r7.xyz) * (r1.xyz);
    r0.w = (r0.z) * (r0.w);
    r1.xyz = (r1.xyz) * (c1.zzz);
    r3.xyz = (r3.xyz) * (r0.www) + (r6.xyz);
    r2.xyz = (r2.xyz) * (r4.xyz) + (r5.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz);
    r0.xyz = (r1.xyz) * (r0.yyy) + (r2.xyz);
    r1.xyz = (r2.www) * (v3.xyz);
    r0.xyz = (r0.xyz) * (r2.www) + (-(r1.xyz));
    r0.xyz = (v2.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[11].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r2.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
