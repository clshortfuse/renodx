// Mechanically reconstructed from 0xDC18CB39.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(-0.5f, 1.0f, -0.0f, 8.0f);
    const float4 c1 = float4(31.875f, 0.797884583f, 1.0f, 0.0009765625f);
    const float4 c2 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c3 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = tex2D(s1, v1.xy);
    r0 = tex2D(s0, v1.xy);
    r5.z = (r0.w) * (v0.w) + (c0.x);
    r0 = (r0.xyzx) * (c0.yyyz) + (c0.zzzy);
    r1 = float4(((r5.z) >= 0.0f ? (r1.x) : (c0.z)), ((r5.z) >= 0.0f ? (r1.y) : (c0.z)), ((r5.z) >= 0.0f ? (r1.z) : (c0.z)), ((r5.z) >= 0.0f ? (r1.w) : (c0.y)));
    r3.x = (r1.w) * (c1.y);
    r2.z = dot(v4.xyz, v4.xyz);
    r2.w = (r1.w) * (-(c1.y)) + (c1.z);
    r3.y = rsqrt(r2.z);
    r9.xyz = (r3.yyy) * (v4.xyz);
    r2.xyz = normalize(v2.xyz);
    r3.w = saturate(dot(r2.xyz, -(r9.xyz)));
    r5.w = saturate(dot(r2.xyz, c[17].xyz));
    r3.z = (r3.w) * (r2.w) + (r3.x);
    r2.w = (r5.w) * (r2.w) + (r3.x);
    r4.xyz = (v4.xyz) * (-(r3.yyy)) + (c[17].xyz);
    r2.w = (r2.w) * (r3.z) + (c1.w);
    r4.w = (-(r3.w)) + (c0.y);
    r2.w = 1.0f / (r2.w);
    r5.y = (r5.w) * (r2.w);
    r3.xyz = normalize(r4.xyz);
    r5.x = saturate(dot(r2.xyz, r3.xyz));
    r4.xy = (r1.ww) * (c2.xy) + (c2.zw);
    r2.w = saturate(dot(r3.xyz, c[17].xyz));
    r4.z = exp2(r4.y);
    r3.w = 1.0f / (r4.x);
    r2.w = (-(r2.w)) + (c0.y);
    r3.x = pow(abs(r5.x), r4.z);
    r3.y = (r2.w) * (r2.w);
    r3.z = (r4.z) * (c3.x) + (c3.y);
    r3.y = (r3.y) * (r3.y);
    r3.z = (r3.x) * (r3.z);
    r2.w = (r2.w) * (r3.y);
    r8.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r1.xyz) * (-(r1.xyz)) + (c0.yyy);
    r3.z = (r5.y) * (r3.z);
    r1.xyz = (r7.xyz) * (r2.www) + (r8.xyz);
    r2.w = (r1.w) * (c0.w);
    r1.xyz = (r3.zzz) * (r1.xyz);
    r1.w = c0.z;
    r3.xyz = float3(((r5.z) >= 0.0f ? (c[7].x) : (r1.w)), ((r5.z) >= 0.0f ? (c[7].y) : (r1.w)), ((r5.z) >= 0.0f ? (c[7].w) : (r1.w)));
    r0 = float4(((r5.z) >= 0.0f ? (r0.x) : (c0.z)), ((r5.z) >= 0.0f ? (r0.y) : (c0.z)), ((r5.z) >= 0.0f ? (r0.z) : (c0.z)), ((r5.z) >= 0.0f ? (r0.w) : (c0.z)));
    r4.xyz = (r1.xyz) * (r3.zzz);
    r3.z = max(abs(r2.y), abs(r2.z));
    r10.xy = c[6].xy;
    r5.xyz = (r10.yyy) * (c[19].xyz);
    r1.w = max(abs(r2.x), r3.z);
    r1.w = 1.0f / (r1.w);
    r1.xyz = (r2.xyz) * (c[5].xyz);
    r4.xyz = (r4.xyz) * (r5.xyz);
    r1.xyz = (r1.xyz) * (r1.www) + (v5.xyz);
    r1 = tex3D(s11, r1.xyz);
    r6.xyz = (r4.xyz) * (r1.www);
    r4.xyz = (r0.xyz) * (v0.xyz);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r4.xyz = (r4.xyz) * (r4.xyz);
    r0.xyz = (r3.yyy) * (r0.xyz);
    r5.xyz = (r0.xyz) * (c1.xxx);
    r1.z = dot(r9.xyz, r2.xyz);
    r0.xyz = (r10.xxx) * (c[18].xyz);
    r3.z = (r1.z) + (r1.z);
    r1.xyz = (r5.www) * (r0.xyz);
    r2.xyz = (r2.xyz) * (-(r3.zzz)) + (r9.xyz);
    r2 = texCUBElod(s15, r2);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r5.xyz = (r1.www) * (r1.xyz) + (r5.xyz);
    r2.xyz = (r0.xyz) * (c0.www);
    r1 = tex3D(s11, v5.xyz);
    r1.w = (r4.w) * (r4.w);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r4.w) * (r1.w);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r0.xyz) * (c1.xxx);
    r2.xyz = (r7.xyz) * (r1.www) + (r8.xyz);
    r1.xyz = (r4.xyz) * (r5.xyz) + (r6.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r0.xyz) * (r3.xxx) + (r1.xyz);
    r1.w = c0.y;
    r0.x = dot(r1, c[9]);
    r0.y = dot(r1, c[10]);
    r0.z = dot(r1, c[11]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r1.x = v2.w;
    r0.xyz = (r1.xxx) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[8].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
