// Mechanically reconstructed from 0x84B694BF.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler3D s11 : register(s11);

struct PS_INPUT
{
    float4 v0 : TEXCOORD1;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD2;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
    float4 v6 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(1.0f, -2.0f, 3.0f, 0.25f);
    const float4 c2 = float4(0.5f, 0.449999988f, 0.330000013f, 0.0900000036f);
    const float4 c3 = float4(31.875f, 9.99999975e-05f, 0.100000001f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 oC0 = 0.0f;

    r1.xy = (v6.xy) * (c[22].xy);
    r0 = tex2D(s2, r1.xy);
    r3 = tex2D(s1, r1.xy);
    r4.w = (r0.w) + (-(c1.x));
    r4.xyz = normalize(-(v3.xyz));
    r1.xy = (v0.ww) * (c[20].xy) + (v0.xy);
    r1.zw = v0.zw;
    r1 = tex2Dproj(s0, r1);
    r2.xy = (v0.ww) * (-(c[20].xy)) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r1.y = r2.x;
    r2.xy = (v0.ww) * (c[20].zw) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r1.z = r2.x;
    r2.xy = (v0.ww) * (-(c[20].zw)) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r6.xy = (r3.wy) * (c0.xy) + (c0.zw);
    r3.xyz = v4.xyz;
    r5.xyz = (r3.zxy) * (v2.yzx);
    r6.xy = (r6.xy) * (c[23].xx);
    r3.xyz = (r3.yzx) * (v2.zxy) + (-(r5.xyz));
    r3.xyz = (r6.yyy) * (-(r3.xyz));
    r3.xyz = (r6.xxx) * (v2.xyz) + (r3.xyz);
    r3.xyz = (r3.xyz) + (v4.xyz);
    r6.xyz = normalize(r3.xyz);
    r2.w = max(abs(r6.y), abs(r6.z));
    r1.w = r2.x;
    r0.w = max(abs(r6.x), r2.w);
    r0.w = 1.0f / (r0.w);
    r2.xyz = (r6.xyz) * (c[5].xyz);
    r3.w = dot(r1, c1.wwww);
    r1.xyz = (r2.xyz) * (r0.www) + (v1.xyz);
    r1 = tex3D(s11, r1.xyz);
    r2.xyz = (-(v3.xyz)) + (c[6].xyz);
    r7.y = dot(r2.xyz, r2.xyz);
    r0.w = rsqrt(r7.y);
    r5.xyz = (r2.xyz) * (r0.www);
    r7.x = 1.0f / (r0.w);
    r0.w = dot(r5.xyz, c[8].xyz);
    r2.w = saturate((r0.w) * (c[9].x) + (c[9].y));
    r3.xy = saturate((r7.xx) * (c[11].xy) + (c[11].zw));
    r2.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c1.yy) + (c1.zz);
    r0.w = dot(c[10].yz, r7.xy) + (c[10].x);
    r2.xy = (r2.xy) * (r3.xy);
    r2.z = (r2.w) * (c[9].w);
    r2.w = (r0.w) * (r2.x);
    r0.w = lerp(r1.w, r3.w, r2.z);
    r1.w = (r2.y) * (r2.w);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = (r0.w) * (r1.w);
    r1.w = (c[24].x) * (c[24].x);
    r2.w = c[24].x;
    r2.xy = (r2.ww) * (r2.ww) + (c2.zw);
    r3.xy = (r1.ww) * (c2.xy);
    r7.x = 1.0f / (r2.x);
    r7.y = 1.0f / (r2.y);
    r2.xyz = (r0.www) * (c[7].xyz);
    r0.w = (r3.x) * (-(r7.x)) + (c1.x);
    r2.w = (r3.y) * (r7.y);
    r1.xyz = (r1.xyz) * (r0.www);
    r3.xyz = (r1.xyz) * (c3.xxx);
    r5.w = saturate(dot(r6.xyz, r5.xyz));
    r3.w = saturate(dot(r6.xyz, r4.xyz));
    r1.w = dot(r5.xyz, r4.xyz);
    r1.w = saturate((r5.w) * (-(r3.w)) + (r1.w));
    r5.y = max(abs(r4.y), abs(r4.z));
    r5.z = (r2.w) * (r1.w);
    r1.w = max(abs(r4.x), r5.y);
    r1.xyz = (r4.xyz) * (c[5].xyz);
    r1.w = 1.0f / (r1.w);
    r5.y = 1.0f / (r3.w);
    r1.xyz = (r1.xyz) * (r1.www) + (v1.xyz);
    r1 = tex3D(s11, r1.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = saturate((r5.w) * (r5.y));
    r1.xyz = (r3.xyz) * (r1.xyz);
    r1.w = (r5.z) * (r1.w);
    r1.xyz = (r1.xyz) * (c3.xxx);
    r0.w = (r0.w) * (r5.w) + (r1.w);
    r1.x = rsqrt(r1.x);
    r1.y = rsqrt(r1.y);
    r1.z = rsqrt(r1.z);
    r3.xyz = (r2.xyz) * (r0.www) + (r3.xyz);
    r1.x = 1.0f / (r1.x);
    r1.y = 1.0f / (r1.y);
    r1.z = 1.0f / (r1.z);
    r0.w = (r3.w) * (r3.w);
    r1.w = (r3.w) * (-(r3.w)) + (c1.x);
    r1.xyz = (r2.www) * (r1.xyz);
    r0.w = ((-(r0.w)) >= 0.0f ? (c1.x) : (r1.w));
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r1.xyz) * (r0.www) + (r3.xyz);
    r1.xyz = normalize(v4.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz) + (-(v5.xyz));
    r0.w = dot(r4.xyz, r1.xyz);
    r1.x = v2.w;
    r0.xyz = (r1.xxx) * (r0.xyz) + (v5.xyz);
    r1.w = max(c3.y, r0.w);
    r0.xyz = max(((r0.xyz) * (c[21].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = pow(abs(r1.w), c3.z);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = (r0.w) * (r4.w) + (c1.x);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
