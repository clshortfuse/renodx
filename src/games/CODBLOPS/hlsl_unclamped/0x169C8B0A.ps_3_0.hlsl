// Mechanically reconstructed from 0x169C8B0A.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(1.0f, 0.0f, 8.0f, 31.875f);
    const float4 c3 = float4(0.797884583f, 0.25f, 1.0f, 0.0009765625f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[33]);
    r0 = (v6.xxxx) * (c[32]) + (r0);
    r0 = (v6.zzzz) * (c[34]) + (r0);
    r1 = (r0) + (c[35]);
    r3.xy = (r1.ww) * (c[36].xy) + (r1.xy);
    r3.zw = r1.zw;
    r0 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r1.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[36].zw)) + (r1.xy);
    r4.zw = r1.zw;
    r4 = tex2Dproj(s2, r4);
    r0.w = r4.x;
    r2.xy = (r3.ww) * (-(c[36].xy)) + (r1.xy);
    r1.xy = (r3.ww) * (c[36].zw) + (r1.xy);
    r2 = tex2Dproj(s2, r2);
    r0.y = r2.x;
    r2 = tex2Dproj(s2, r1);
    r1 = (v6.xyzx) * (c2.xxxy) + (c2.yyyx);
    r0.z = dot(r1, c[29]);
    r2.w = 1.0f / (r0.z);
    r3.x = dot(r1, c[26]);
    r3.y = dot(r1, c[27]);
    r0.z = r2.x;
    r3.xy = (r2.ww) * (r3.xy);
    r2.x = dot(r1, c[28]);
    r1.xy = (r3.xy) * (c0.yy) + (c0.yy);
    r1 = tex2D(s3, r1.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[24].yz, r2.xy) + (c[24].x);
    r4.xy = saturate((r2.xx) * (c[25].xy) + (c[25].zw));
    r2.w = saturate(1.0f / (r1.w));
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c2.y) : (r2.w));
    r3.xyz = (-(v6.xyz)) + (c[21].xyz);
    r2.xy = (r4.xy) * (r4.xy);
    r2.w = dot(r3.xyz, r3.xyz);
    r4.xy = (r4.xy) * (c12.zz) + (c12.ww);
    r3.w = rsqrt(r2.w);
    r2.z = (r2.x) * (r4.x);
    r5.xyz = (r3.xyz) * (r3.www);
    r2.y = (r2.y) * (-(r4.y)) + (c2.x);
    r2.w = dot(r5.xyz, c[30].xyz);
    r1.w = (r1.w) * (r2.z);
    r2.z = saturate((r2.w) * (c[31].x) + (c[31].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c12.z) + (c12.w);
    r1.w = (r2.y) * (r1.w);
    r2.w = (r2.w) * (r2.z);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r1.w) * (r2.w);
    r0.w = dot(r0, c3.yyyy);
    r0.xyz = (r1.xyz) * (r1.www);
    r6.xyz = (r0.www) * (r0.xyz);
    r0 = tex3D(s11, v7.xyz);
    r1 = tex2D(s1, v1.xy);
    r4.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = tex2D(s7, v1.xy);
    r7.xy = (v1.xy) + (c0.xx);
    r4.w = c0.y;
    r2.xy = (r7.xy) * (c[42].xx) + (r4.ww);
    r2 = tex2D(s5, r2.xy);
    r6.w = (v7.w) * (c0.z) + (-(r2.x));
    r1.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r5.w = saturate((r6.w) * (c0.w));
    r2.xy = lerp(r4.xy, r1.xy, r5.ww);
    r1.xyz = v2.xyz;
    r1.xyz = (r2.xxx) * (v5.xyz) + (r1.xyz);
    r1.xyz = (r2.yyy) * (v4.xyz) + (r1.xyz);
    r10.xyz = normalize(r1.xyz);
    r4.xyz = normalize(v6.xyz);
    r0.w = dot(r4.xyz, r10.xyz);
    r0.w = (r0.w) + (r0.w);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r10.xyz) * (-(r0.www)) + (r4.xyz);
    r1 = tex2D(s4, v1.xy);
    r0.w = (r1.w) * (c2.z);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r11.xy = (r7.xy) * (c[43].xx) + (r4.ww);
    r0.xyz = (r0.xyz) * (c2.zzz);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r7.w = saturate(dot(r10.xyz, -(r4.xyz)));
    r2.xyz = (r0.xyz) * (c2.www);
    r0.w = (-(r7.w)) + (c2.x);
    r9.xyz = (r3.xyz) * (r3.www) + (-(r4.xyz));
    r0.z = (r0.w) * (r0.w);
    r0.w = (r0.w) * (r0.z);
    r0.xy = (r1.ww) * (c4.xy) + (c4.zw);
    r0.z = 1.0f / (r0.x);
    r4.w = exp2(r0.y);
    r0.w = (r0.w) * (r0.z);
    r4.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r1.xyz) * (-(r1.xyz)) + (c2.xxx);
    r2.w = max(abs(r10.y), abs(r10.z));
    r3.xyz = (r1.xyz) * (r0.www) + (r4.xyz);
    r0.w = max(abs(r10.x), r2.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r10.xyz) * (c[5].xyz);
    r2.xyz = (r2.xyz) * (r3.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (v7.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r7.xyz = (r2.xyz) * (c[37].xxx);
    r8.xyz = (r0.xyz) * (c[37].yyy);
    r2 = tex2D(s6, r11.xy);
    r0 = tex2D(s0, v1.xy);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r3.w = (r0.w) * (v0.w);
    r3.xyz = (r0.xyz) * (r0.xyz);
    r0 = lerp(r3, r2, r5.wwww);
    r2.y = (r1.w) * (c3.x);
    r2.w = (r1.w) * (-(c3.x)) + (c3.z);
    r1.w = saturate(dot(r5.xyz, r10.xyz));
    r2.z = (r7.w) * (r2.w) + (r2.y);
    r2.w = (r1.w) * (r2.w) + (r2.y);
    r7.w = (-(r5.w)) + (c2.x);
    r2.w = (r2.w) * (r2.z) + (c3.w);
    r2.xyz = (r8.xyz) * (r0.xyz);
    r2.w = 1.0f / (r2.w);
    r7.xyz = (r2.xyz) * (c2.www) + (r7.xyz);
    r3.w = (r1.w) * (r2.w);
    r8.xyz = (r1.www) * (c[22].xyz);
    r2.xyz = normalize(r9.xyz);
    r3.z = saturate(dot(r10.xyz, r2.xyz));
    r1.w = saturate(dot(r2.xyz, r5.xyz));
    r2.y = pow(abs(r3.z), r4.w);
    r1.w = (-(r1.w)) + (c2.x);
    r2.z = (r4.w) * (c12.x) + (c12.y);
    r2.w = (r1.w) * (r1.w);
    r2.z = (r2.y) * (r2.z);
    r2.w = (r2.w) * (r2.w);
    r5.w = (r3.w) * (r2.z);
    r1.w = (r1.w) * (r2.w);
    r5.xyz = (r1.xyz) * (r1.www) + (r4.xyz);
    r4 = (-(v6.yyyy)) + (c[7]);
    r1 = (r4) * (r4);
    r3 = (-(v6.xxxx)) + (c[6]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v6.zzzz)) + (c[8]);
    r5.xyz = (r5.www) * (r5.xyz);
    r1 = (r2) * (r2) + (r1);
    r9.xyz = (r5.xyz) * (c[23].xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r9.xyz = (r9.xyz) * (c[37].www);
    r4 = (r4) * (r5);
    r4 = (r10.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r10.xxxx) + (r4);
    r4.w = c2.x;
    r1 = saturate((r1) * (c[9]) + (r4.wwww));
    r2 = saturate((r2) * (r10.zzzz) + (r3));
    r3.xyz = (r0.xyz) * (r8.xyz) + (r9.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r6.xyz) * (r3.xyz) + (r7.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.w = ((r6.w) >= 0.0f ? (r7.w) : (c2.y));
    r2.z = dot(c[20], r1);
    r1.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r1.w = (r1.w) * (c[44].w);
    r0.xyz = (c[44].xyz) * (r1.www) + (r0.xyz);
    r1.xyz = (r0.www) * (r0.xyz);
    r1.w = c2.x;
    r0.x = dot(r1, c[39]);
    r0.y = dot(r1, c[40]);
    r0.z = dot(r1, c[41]);
    r0.xyz = (v3.xyz) * (-(r0.www)) + (r0.xyz);
    r0.xyz = (r0.xyz) * (v2.www);
    r0.xyz = (v3.xyz) * (r0.www) + (r0.xyz);
    r0.xyz = max(((r0.xyz) * (c[38].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
