// Mechanically reconstructed from 0x20199B53.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s8 : register(s8);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
    float4 v8 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-1.0f, 1.0f, 0.0f, 8.0f);
    const float4 c2 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v6.xyzx) * (c1.yyyz) + (c1.zzzy);
    r0.w = dot(r1, c[30]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[27]);
    r0.y = dot(r1, c[28]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[31].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[32].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[32].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[29]);
    r0.z = (r2.x) * (c[33].x);
    r0.y = (r2.y) * (c[33].y) + (-(r0.z));
    r0.z = c[33].y;
    r0.z = (r2.y) * (r0.z) + (-(c[32].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[25];
    r1 = saturate((r2.zyxz) * (r1) + (c[26]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[33].zzzz) * (r3) + (c[33].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[32].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[24].yz, r2.zw) + (c[24].x);
    r0.z = (r0.y) * (r0.z);
    r2.w = (r1.x) * (r0.z);
    r1 = c[34];
    r1.x = dot(r0.xw, r1.xy) + (c[23].x);
    r1.y = dot(r0.xw, r1.zw) + (c[23].z);
    r0 = tex2D(s2, r1.xy);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r7.xyz = (r2.www) * (r0.xyz);
    r4 = tex2D(s12, v1.zw);
    r0 = tex2D(s8, v7.zw);
    r3 = tex2D(s6, v1.xy);
    r1 = tex2D(s7, v7.xy);
    r2 = tex2D(s3, v7.xy);
    r7.w = (r2.w) * (v0.y);
    r5 = lerp(r3, r1, r7.wwww);
    r1 = tex2D(s4, v7.zw);
    r8.w = (r1.w) * (v0.z);
    r3 = lerp(r5, r0, r8.wwww);
    r5.w = (r3.w) * (c3.z);
    r4.x = (r3.w) * (-(c12.z)) + (c12.w);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r6.xyz = (-(v6.xyz)) + (c[20].xyz);
    r0.xy = (r7.ww) * (-(r0.xy)) + (r0.xy);
    r0.w = dot(r6.xyz, r6.xyz);
    r8.xy = (r8.ww) * (-(r0.xy)) + (r0.xy);
    r4.w = rsqrt(r0.w);
    r0 = v2;
    r0.xyz = (r8.xxx) * (v5.xyz) + (r0.xyz);
    r5.xyz = (r6.xyz) * (r4.www);
    r0.xyz = (r8.yyy) * (v4.xyz) + (r0.xyz);
    r9.xyz = normalize(r0.xyz);
    r12.xyz = normalize(v6.xyz);
    r1.w = saturate(dot(r9.xyz, r5.xyz));
    r2.w = saturate(dot(r9.xyz, -(r12.xyz)));
    r4.z = (r1.w) * (r4.x) + (r5.w);
    r4.x = (r2.w) * (r4.x) + (r5.w);
    r0.xyz = (r7.xyz) * (r4.yyy);
    r4.z = (r4.z) * (r4.x) + (c4.x);
    r2.w = (-(r2.w)) + (c1.y);
    r4.z = 1.0f / (r4.z);
    r5.w = (r1.w) * (r4.z);
    r6.xyz = (r6.xyz) * (r4.www) + (-(r12.xyz));
    r13.xyz = (r1.www) * (c[21].xyz);
    r4.xyz = normalize(r6.xyz);
    r6.z = saturate(dot(r9.xyz, r4.xyz));
    r6.xy = (r3.ww) * (c13.xy) + (c13.zw);
    r1.w = saturate(dot(r4.xyz, r5.xyz));
    r5.z = exp2(r6.y);
    r6.w = 1.0f / (r6.x);
    r4.z = pow(abs(r6.z), r5.z);
    r4.w = (r5.z) * (c4.y) + (c4.z);
    r1.w = (-(r1.w)) + (c1.y);
    r4.z = (r4.z) * (r4.w);
    r4.w = (r1.w) * (r1.w);
    r5.w = (r5.w) * (r4.z);
    r5.z = (r4.w) * (r4.w);
    r4 = tex2D(s5, v8.xy);
    r4.xyz = (r4.xyz) + (c1.xxx);
    r4.w = (r1.w) * (r5.z);
    r6.xyz = (v0.www) * (r4.xyz) + (c1.yyy);
    r3.xyz = (r3.xyz) * (r6.xyz);
    r1.w = (r3.w) * (c1.w);
    r11.xyz = (r3.xyz) * (r3.xyz);
    r10.xyz = (r3.xyz) * (-(r3.xyz)) + (c1.yyy);
    r4.xyz = (r10.xyz) * (r4.www) + (r11.xyz);
    r3.xyw = c[35].xyw;
    r3.xyz = (-(r3.xyw)) + (c[36].xyw);
    r4.xyz = (r5.www) * (r4.xyz);
    r3.xyz = (r7.www) * (r3.xyz) + (c[35].xyw);
    r7.xyz = lerp(r3.xyz, c[37].xyw, r8.www);
    r3.xy = (v1.zw) * (c2.xy);
    r3 = tex2D(s13, r3.xy);
    r5.xy = (v1.zw) * (c2.xy) + (c2.zy);
    r5 = tex2D(s13, r5.xy);
    r3.w = r5.y;
    r14.xyz = (r4.xyz) * (r7.zzz);
    r4.xy = (r3.yw) * (c12.xx) + (c12.yy);
    r3.w = dot(r8.xy, r8.xy) + (c1.z);
    r3.y = dot(r4.xy, r4.xy) + (c1.z);
    r3.w = exp2(-(r3.w));
    r3.y = exp2(-(r3.y));
    r3.w = (r3.w) * (c3.x) + (c3.y);
    r4.w = (r3.y) * (c3.x) + (c3.y);
    r3.y = dot(r4.xy, r8.xy) + (c1.z);
    r5.w = (r3.w) * (r4.w);
    r4 = tex2D(s14, v1.zw);
    r15.xy = (r4.xy) * (c3.ww);
    r4.z = saturate((r3.y) * (r5.w) + (r5.w));
    r5.xy = (r5.xz) * (r15.yy);
    r3.y = (r4.y) * (c3.w) + (-(r5.x));
    r8.xz = (r5.xy) * (c2.ww);
    r4.w = (r5.z) * (-(r15.y)) + (r3.y);
    r3.xy = (r3.xz) * (r15.xx);
    r8.y = (r4.w) + (r4.w);
    r4.w = (r4.x) * (c3.w) + (-(r3.x));
    r5.xyz = (r4.zzz) * (r8.xyz);
    r3.z = (r3.z) * (-(r15.x)) + (r4.w);
    r8.xz = (r3.xy) * (c2.ww);
    r8.y = (r3.z) + (r3.z);
    r4.xyz = (r14.xyz) * (c[22].xyz);
    r3.xyz = (r8.xyz) * (r3.www) + (r5.xyz);
    r4.xyz = (r0.xyz) * (r4.xyz);
    r14.xyz = (r7.yyy) * (r3.xyz);
    r3 = tex2D(s0, v1.xy);
    r5.xyz = lerp(r3.xyz, r2.xyz, r7.www);
    r2.xyz = (r0.xyz) * (r13.xyz) + (r14.xyz);
    r0.xyz = lerp(r5.xyz, r1.xyz, r8.www);
    r0.xyz = (r6.xyz) * (r0.xyz);
    r1.z = dot(r12.xyz, r9.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.z = (r1.z) + (r1.z);
    r6.xyz = (r0.xyz) * (r2.xyz) + (r4.xyz);
    r1.xyz = (r9.xyz) * (-(r1.zzz)) + (r12.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r7.xxx) * (r1.xyz);
    r1.w = (r2.w) * (r2.w);
    r5.w = (r2.w) * (r1.w);
    r4 = (-(v6.yyyy)) + (c[6]);
    r1 = (r4) * (r4);
    r3 = (-(v6.xxxx)) + (c[5]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v6.zzzz)) + (c[7]);
    r5.w = (r6.w) * (r5.w);
    r1 = (r2) * (r2) + (r1);
    r10.xyz = (r10.xyz) * (r5.www) + (r11.xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r7.xyz = (r7.xyz) * (r10.xyz);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r4.z = c1.y;
    r1 = saturate((r1) * (c[8]) + (r4.zzzz));
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.xyz = (r8.xyz) * (r7.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c1.www) + (r6.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[38].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.y;

    return oC0;
}
