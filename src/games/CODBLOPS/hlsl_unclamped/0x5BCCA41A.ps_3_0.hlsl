// Mechanically reconstructed from 0x5BCCA41A.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);

struct PS_INPUT
{
    float4 v0 : TEXCOORD1;
    float4 v1 : TEXCOORD3;
    float4 v2 : TEXCOORD4;
    float4 v3 : TEXCOORD5;
    float4 v4 : TEXCOORD6;
    float4 v5 : TEXCOORD7;
    float4 v6 : TEXCOORD8;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(1.0f, 0.0f, 0.25f, -0.5f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(1e-15f, 1.44269502f, 0.200000003f, 5.0f);
    const float4 c3 = float4(-2.0f, 3.0f, 0.100000001f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v3.xyzx) * (c0.xxxy) + (c0.yyyx);
    r0.w = dot(r1, c[22]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[11]);
    r0.y = dot(r1, c[20]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[23].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[24].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[24].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[21]);
    r0.z = (r2.x) * (c[25].x);
    r0.y = (r2.y) * (c[25].y) + (-(r0.z));
    r0.z = c[25].y;
    r0.z = (r2.y) * (r0.z) + (-(c[24].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[9];
    r1 = saturate((r2.zyxz) * (r1) + (c[10]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[25].zzzz) * (r3) + (c[25].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[24].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[8].yz, r2.zw) + (c[8].x);
    r0.z = (r0.y) * (r0.z);
    r3.w = (r1.x) * (r0.z);
    r1.xz = c[7].xz;
    r1.x = dot(r0.xw, c[26].xy) + (r1.x);
    r1.y = dot(r0.xw, c[26].zw) + (r1.z);
    r1 = tex2D(s1, r1.xy);
    r0.xy = (v0.ww) * (c[27].xy) + (v0.xy);
    r0.zw = v0.zw;
    r0 = tex2Dproj(s0, r0);
    r2.xy = (v0.ww) * (-(c[27].xy)) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r0.y = r2.x;
    r2.xy = (v0.ww) * (c[27].zw) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r0.z = r2.x;
    r2.xy = (v0.ww) * (-(c[27].zw)) + (v0.xy);
    r2.zw = v0.zw;
    r2 = tex2Dproj(s0, r2);
    r0.w = r2.x;
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = dot(r0, c0.zzzz);
    r0.xyz = (r3.www) * (r1.xyz);
    r0.w = (r0.w) * (c[7].w);
    r0.xyz = (r0.xyz) * (r0.www);
    r6.xyz = (r0.xyz) * (c[6].xyz);
    r0.xyz = (-(v3.xyz)) + (c[5].xyz);
    r2.xyz = normalize(v4.xyz);
    r5.xyz = normalize(r0.xyz);
    r0.w = dot(r5.xyz, r2.xyz);
    r0.xy = (v6.xy) + (c0.ww);
    r3.w = (r0.w) + (c2.z);
    r0.w = c0.w;
    r8.xy = (r0.xy) * (c[30].xx) + (-(r0.ww));
    r0.xy = ddy(v6.xy);
    r4.xy = ddx(r8.xy);
    r10.xy = ddx(v6.xy);
    r11.xy = (r0.xy) * (r4.yy);
    r7.xy = ddy(r8.yx);
    r0 = v1;
    r3.xyz = (r0.yzx) * (v4.zxy);
    r1 = tex2D(s2, v6.xy);
    r1.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r0.xyz = (v4.yzx) * (r0.zxy) + (-(r3.xyz));
    r9.xy = (r1.xy) * (c[29].xx);
    r3.xy = (r10.xy) * (r7.xx) + (-(r11.xy));
    r1.xyz = (-(r0.xyz)) * (r9.yyy);
    r0.xyz = (r3.yyy) * (v2.xyz);
    r1.xyz = (r9.xxx) * (v1.xyz) + (r1.xyz);
    r0.xyz = (v1.xyz) * (r3.xxx) + (r0.xyz);
    r3.xyz = (r1.xyz) + (v4.xyz);
    r1.y = (r4.y) * (r7.y);
    r1.z = dot(r3.xyz, r3.xyz);
    r1.w = (r4.x) * (r7.x) + (-(r1.y));
    r2.w = rsqrt(r1.z);
    r1.xyz = float3(((r1.w) >= 0.0f ? (r0.x) : (-(r0.x))), ((r1.w) >= 0.0f ? (r0.y) : (-(r0.y))), ((r1.w) >= 0.0f ? (r0.z) : (-(r0.z))));
    r0.xyz = (r3.xyz) * (r2.www);
    r3.w = saturate((r3.w) * (c2.w));
    r4.w = dot(r1.xyz, r0.xyz);
    r1.w = (r3.w) * (c3.x) + (c3.y);
    r1.xyz = (r4.www) * (-(r0.xyz)) + (r1.xyz);
    r3.w = (r3.w) * (r3.w);
    r4.xyz = normalize(r1.xyz);
    r3.w = (r1.w) * (r3.w);
    r7.xyz = (r0.zxy) * (r4.yzx);
    r1 = tex2D(s3, r8.xy);
    r9.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1.xyz = (r0.yzx) * (r4.zxy) + (-(r7.xyz));
    r7.xy = (r9.xy) * (c[31].xx);
    r0.xyz = (r6.xyz) * (r3.www);
    r1.xyz = (-(r1.xyz)) * (r7.yyy);
    r1.xyz = (r7.xxx) * (r4.xyz) + (r1.xyz);
    r1.w = dot(-(v3.xyz), -(v3.xyz));
    r1.xyz = (r3.xyz) * (r2.www) + (r1.xyz);
    r1.w = rsqrt(r1.w);
    r4.xyz = normalize(r1.xyz);
    r6.xyz = (-(v3.xyz)) * (r1.www) + (r5.xyz);
    r1.xyz = (r1.www) * (-(v3.xyz));
    r3.xyz = normalize(r6.xyz);
    r2.w = saturate(dot(r4.xyz, r5.xyz));
    r1.w = saturate(dot(r3.xyz, r4.xyz));
    r3.w = saturate(dot(r1.xyz, r4.xyz));
    r3.z = dot(r3.xyz, r1.xyz);
    r1.x = saturate(dot(r1.xyz, r2.xyz));
    r1.z = (r1.w) + (r1.w);
    r1.w = (r1.w) * (r1.w) + (c2.x);
    r1.y = 1.0f / (r3.z);
    r1.z = (r1.z) * (r1.y);
    r1.y = min(r3.w, r2.w);
    r2.y = (r3.w) + (r1.x);
    r1.z = saturate((r1.z) * (r1.y));
    r3.w = (r2.w) * (r1.z);
    r2.x = 1.0f / (r1.w);
    r1 = tex2D(s4, r8.xy);
    r1.z = (r1.x) * (r1.x);
    r1.w = (-(r2.x)) + (c0.x);
    r2.z = 1.0f / (r1.z);
    r1.z = rsqrt(r3.w);
    r1.w = (r1.w) * (r2.z);
    r1.z = 1.0f / (r1.z);
    r1.y = (r1.w) * (c2.y);
    r1.w = (r2.x) * (r2.x);
    r1.y = exp2(r1.y);
    r1.w = (r1.w) * (r1.y);
    r2.y = (r2.y) * (-(c0.w));
    r1.z = (r1.z) * (r1.w);
    r1.w = max(c3.z, r2.y);
    r1.xyz = (r0.xyz) * (r1.zzz);
    r1.w = 1.0f / (r1.w);
    r1.xyz = (r2.zzz) * (r1.xyz);
    r1.w = rsqrt(r1.w);
    r1.xyz = (r1.xyz) * (c0.zzz);
    r1.w = 1.0f / (r1.w);
    r0.xyz = (r0.xyz) * (r2.www);
    r3.xyz = (r1.xyz) * (r1.www);
    r2 = tex2D(s6, r8.xy);
    r1 = tex2D(s5, r8.xy);
    r2.xyz = (r2.xyz) * (c[32].xxx);
    r2.xyz = (r3.xyz) * (r2.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v5.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v5.xyz);
    r0.xyz = max(((r0.xyz) * (c[28].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.x;

    return oC0;
}
