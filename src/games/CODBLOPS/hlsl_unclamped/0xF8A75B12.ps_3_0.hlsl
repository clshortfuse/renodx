// Mechanically reconstructed from 0xF8A75B12.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, -1.0f, 1.0f, 0.200000003f);
    const float4 c2 = float4(0.0f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c3 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c4 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    const float4 c12 = float4(0.797884583f, 1.0f, 0.5f, 0.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[25]);
    r0 = (v6.xxxx) * (c[24]) + (r0);
    r0 = (v6.zzzz) * (c[26]) + (r0);
    r1 = (r0) + (c[27]);
    r3.xy = (r1.ww) * (c[28].xy) + (r1.xy);
    r3.zw = r1.zw;
    r0 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r1.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[28].zw)) + (r1.xy);
    r4.zw = r1.zw;
    r4 = tex2Dproj(s2, r4);
    r0.w = r4.x;
    r2.xy = (r3.ww) * (-(c[28].xy)) + (r1.xy);
    r1.xy = (r3.ww) * (c[28].zw) + (r1.xy);
    r2 = tex2Dproj(s2, r2);
    r0.y = r2.x;
    r2 = tex2Dproj(s2, r1);
    r1 = (v6.xyzx) * (c12.yyyw) + (c12.wwwy);
    r0.z = dot(r1, c[21]);
    r2.w = 1.0f / (r0.z);
    r3.x = dot(r1, c[10]);
    r3.y = dot(r1, c[11]);
    r0.z = r2.x;
    r3.xy = (r2.ww) * (r3.xy);
    r2.x = dot(r1, c[20]);
    r1.xy = (r3.xy) * (-(c1.xx)) + (-(c1.xx));
    r1 = tex2D(s3, r1.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[8].yz, r2.xy) + (c[8].x);
    r3.xy = saturate((r2.xx) * (c[9].xy) + (c[9].zw));
    r2.w = saturate(1.0f / (r1.w));
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c2.x) : (r2.w));
    r6.xyz = (-(v6.xyz)) + (c[5].xyz);
    r2.xy = (r3.xy) * (r3.xy);
    r2.w = dot(r6.xyz, r6.xyz);
    r3.xy = (r3.xy) * (c4.zz) + (c4.ww);
    r9.w = rsqrt(r2.w);
    r2.z = (r2.x) * (r3.x);
    r10.xyz = (r6.xyz) * (r9.www);
    r2.y = (r2.y) * (-(r3.y)) + (c1.z);
    r2.w = dot(r10.xyz, c[22].xyz);
    r1.w = (r1.w) * (r2.z);
    r2.z = saturate((r2.w) * (c[23].x) + (c[23].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c4.z) + (c4.w);
    r1.w = (r2.y) * (r1.w);
    r2.w = (r2.w) * (r2.z);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r1.w) * (r2.w);
    r2.w = dot(r0, c4.yyyy);
    r2.xyz = (r1.xyz) * (r1.www);
    r1 = tex2D(s0, v1.xy);
    r0 = tex2D(s4, v7.xy);
    r3.w = (r0.w) * (v0.y) + (c1.x);
    r3.xyz = (r2.www) * (r2.xyz);
    r1.xyz = float3(((r3.w) >= 0.0f ? (r0.x) : (r1.x)), ((r3.w) >= 0.0f ? (r0.y) : (r1.y)), ((r3.w) >= 0.0f ? (r0.z) : (r1.z)));
    r0 = tex2D(s5, v7.zw);
    r0.xyz = (r0.xyz) + (c1.yyy);
    r5.xyz = (v0.zzz) * (r0.xyz) + (c1.zzz);
    r0.xy = (v1.zw) * (-(c1.yx));
    r0 = tex2D(s13, r0.xy);
    r2.xy = (v1.zw) * (c12.yz) + (c12.wz);
    r2 = tex2D(s13, r2.xy);
    r0.w = r2.y;
    r7.xyz = (r1.xyz) * (r5.xyz);
    r4.xy = (r0.yw) * (c3.yy) + (c3.zz);
    r1 = tex2D(s1, v1.xy);
    r8.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r0.y = dot(r4.xy, r4.xy) + (c2.x);
    r0.w = dot(r8.xy, r8.xy) + (c2.x);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c2.y) + (c2.z);
    r0.w = (r0.w) * (c2.y) + (c2.z);
    r2.w = (r0.y) * (r0.w);
    r0.y = dot(r4.xy, r8.xy) + (c2.x);
    r1 = tex2D(s14, v1.zw);
    r9.xy = (r1.xy) * (c3.xx);
    r1.z = saturate((r0.y) * (r2.w) + (r2.w));
    r2.xy = (r2.xz) * (r9.yy);
    r0.y = (r1.y) * (c3.x) + (-(r2.x));
    r4.xz = (r2.xy) * (c3.yy);
    r1.w = (r2.z) * (-(r9.y)) + (r0.y);
    r0.xy = (r0.xz) * (r9.xx);
    r4.y = (r1.w) + (r1.w);
    r1.w = (r1.x) * (c3.x) + (-(r0.x));
    r1.xyz = (r1.zzz) * (r4.xyz);
    r0.z = (r0.z) * (-(r9.x)) + (r1.w);
    r4.xz = (r0.xy) * (c3.yy);
    r4.y = (r0.z) + (r0.z);
    r11.xyz = (r5.xyz) * (c1.www);
    r5.xyz = (r4.xyz) * (r0.www) + (r1.xyz);
    r1 = tex2D(s6, v1.xy);
    r0 = tex2D(s7, v7.xy);
    r12.xy = float2(((r3.w) >= 0.0f ? (r0.w) : (r1.w)), ((r3.w) >= 0.0f ? (r0.y) : (r1.y)));
    r2.xyz = (r7.xyz) * (r7.xyz);
    r7.xyz = (r5.xyz) * (r12.yyy);
    r1 = tex2D(s12, v1.zw);
    r0 = v2;
    r0.xyz = (r8.xxx) * (v5.xyz) + (r0.xyz);
    r1.w = dot(v6.xyz, v6.xyz);
    r0.xyz = (r8.yyy) * (v4.xyz) + (r0.xyz);
    r1.z = rsqrt(r1.w);
    r8.xyz = normalize(r0.xyz);
    r5.xyz = (v6.xyz) * (-(r1.zzz)) + (c[17].xyz);
    r1.x = saturate(dot(r8.xyz, c[17].xyz));
    r0.xyz = normalize(r5.xyz);
    r5.xyz = (r1.xxx) * (c[18].xyz);
    r1.w = saturate(dot(r0.xyz, c[17].xyz));
    r5.xyz = (r1.yyy) * (r5.xyz) + (r7.xyz);
    r1.w = (-(r1.w)) + (c1.z);
    r9.xyz = (r1.zzz) * (v6.xyz);
    r1.z = (r1.w) * (r1.w);
    r2.w = saturate(dot(r8.xyz, r0.xyz));
    r0.z = (r1.z) * (r1.z);
    r6.w = (r1.w) * (r0.z);
    r7.xyz = (r11.xyz) * (r11.xyz);
    r0.xyz = (r11.xyz) * (-(r11.xyz)) + (c1.zzz);
    r1.z = saturate(dot(r8.xyz, -(r9.xyz)));
    r5.w = (r12.x) * (c12.x);
    r3.w = (r12.x) * (-(c12.x)) + (c12.y);
    r4.w = (r1.z) * (r3.w) + (r5.w);
    r1.w = (r1.x) * (r3.w) + (r5.w);
    r1.w = (r1.w) * (r4.w) + (c3.w);
    r13.xy = (r12.xx) * (c13.xy) + (c13.zw);
    r7.w = 1.0f / (r1.w);
    r8.w = exp2(r13.y);
    r1.w = pow(abs(r2.w), r8.w);
    r2.w = (r8.w) * (c4.x) + (c4.y);
    r1.x = (r1.x) * (r7.w);
    r1.w = (r1.w) * (r2.w);
    r11.xyz = (r0.xyz) * (r6.www) + (r7.xyz);
    r1.w = (r1.x) * (r1.w);
    r7.w = (-(r1.z)) + (c1.z);
    r11.xyz = (r11.xyz) * (r1.www);
    r6.w = 1.0f / (r13.x);
    r11.xyz = (r12.yyy) * (r11.xyz);
    r11.xyz = (r11.xyz) * (c[19].xyz);
    r1.w = dot(r9.xyz, r8.xyz);
    r1.xyz = (r1.yyy) * (r11.xyz);
    r1.w = (r1.w) + (r1.w);
    r5.xyz = (r2.xyz) * (r5.xyz) + (r1.xyz);
    r1.xyz = (r8.xyz) * (-(r1.www)) + (r9.xyz);
    r11.xyz = (r6.xyz) * (r9.www) + (-(r9.xyz));
    r1.w = (r12.x) * (c2.w);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r7.w) * (r7.w);
    r1.xyz = (r12.yyy) * (r1.xyz);
    r1.w = (r7.w) * (r1.w);
    r6.w = (r6.w) * (r1.w);
    r1.w = saturate(dot(r10.xyz, r8.xyz));
    r6.xyz = (r0.xyz) * (r6.www) + (r7.xyz);
    r3.w = (r1.w) * (r3.w) + (r5.w);
    r3.w = (r3.w) * (r4.w) + (c3.w);
    r9.xyz = normalize(r11.xyz);
    r4.w = 1.0f / (r3.w);
    r3.w = saturate(dot(r9.xyz, r10.xyz));
    r4.w = (r1.w) * (r4.w);
    r3.w = (-(r3.w)) + (c1.z);
    r7.w = saturate(dot(r8.xyz, r9.xyz));
    r5.w = (r3.w) * (r3.w);
    r6.w = pow(abs(r7.w), r8.w);
    r5.w = (r5.w) * (r5.w);
    r2.w = (r2.w) * (r6.w);
    r3.w = (r3.w) * (r5.w);
    r2.w = (r4.w) * (r2.w);
    r0.xyz = (r0.xyz) * (r3.www) + (r7.xyz);
    r1.xyz = (r1.xyz) * (r6.xyz);
    r0.xyz = (r2.www) * (r0.xyz);
    r1.xyz = (r4.xyz) * (r1.xyz);
    r4.xyz = (r0.xyz) * (c[7].xyz);
    r0.xyz = (r1.www) * (c[6].xyz);
    r4.xyz = (r12.yyy) * (r4.xyz);
    r1.xyz = (r1.xyz) * (c2.www) + (r5.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz) + (r4.xyz);
    r0.xyz = (r3.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.z;

    return oC0;
}
