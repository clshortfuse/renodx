// Mechanically reconstructed from 0xF340CB5F.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD5;
    float4 v4 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    const float4 c0 = float4(-0.5f, 1.0f, 8.0f, 0.797884583f);
    const float4 c1 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c2 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c3 = float4(4.0f, -2.0f, 0.600000024f, 0.400000006f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = tex2D(s0, v0.xy);
    r0 = tex2D(s1, v4.zw);
    r0.w = (r0.w) * (v4.y) + (c0.x);
    r0.xyz = float3(((r0.w) >= 0.0f ? (r0.x) : (r1.x)), ((r0.w) >= 0.0f ? (r0.y) : (r1.y)), ((r0.w) >= 0.0f ? (r0.z) : (r1.z)));
    r1.z = c0.y;
    r11.xyz = float3(((r0.w) >= 0.0f ? (r1.z) : (c[5].x)), ((r0.w) >= 0.0f ? (r1.z) : (c[5].y)), ((r0.w) >= 0.0f ? (r1.z) : (c[5].w)));
    r0.w = dot(v3.xyz, v3.xyz);
    r5.xyz = (r0.xyz) * (r0.xyz);
    r0.w = rsqrt(r0.w);
    r12.xyz = (r0.www) * (v3.xyz);
    r1.xyz = normalize(v1.xyz);
    r3.xyz = (v3.xyz) * (-(r0.www)) + (c[17].xyz);
    r2.y = saturate(dot(r1.xyz, -(r12.xyz)));
    r0 = tex2D(s2, v0.xy);
    r2.x = (r0.w) * (c0.w);
    r1.w = (r0.w) * (-(c0.w)) + (c0.y);
    r2.w = saturate(dot(r1.xyz, c[17].xyz));
    r2.z = (r2.y) * (r1.w) + (r2.x);
    r1.w = (r2.w) * (r1.w) + (r2.x);
    r6.w = (-(r2.y)) + (c0.y);
    r1.w = (r1.w) * (r2.z) + (c2.x);
    r1.w = 1.0f / (r1.w);
    r2.xyz = normalize(r3.xyz);
    r3.w = (r2.w) * (r1.w);
    r4.w = saturate(dot(r1.xyz, r2.xyz));
    r2.z = saturate(dot(r2.xyz, c[17].xyz));
    r2.xy = (r0.ww) * (c4.xy) + (c4.zw);
    r1.w = exp2(r2.y);
    r2.z = (-(r2.z)) + (c0.y);
    r3.z = pow(abs(r4.w), r1.w);
    r2.y = (r2.z) * (r2.z);
    r1.w = (r1.w) * (c2.y) + (c2.z);
    r2.y = (r2.y) * (r2.y);
    r1.w = (r3.z) * (r1.w);
    r2.z = (r2.z) * (r2.y);
    r10.xyz = (r0.xyz) * (r0.xyz);
    r9.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.yyy);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r9.xyz) * (r2.zzz) + (r10.xyz);
    r7.xyz = (r2.www) * (c[18].xyz);
    r0.xyz = (r1.www) * (r0.xyz);
    r5.w = 1.0f / (r2.x);
    r0.xyz = (r11.zzz) * (r0.xyz);
    r1.w = (r0.w) * (c0.z);
    r3.xyz = (r0.xyz) * (c[19].xyz);
    r0 = tex2D(s12, v0.zw);
    r2.xy = (v0.zw) * (c1.xy);
    r2 = tex2D(s13, r2.xy);
    r4.xy = (v0.zw) * (c1.xy) + (c1.zy);
    r4 = tex2D(s13, r4.xy);
    r2.w = r4.y;
    r6.xy = (r2.yw) * (c3.xx) + (c3.yy);
    r0.w = dot(r6.xy, r6.xy) + (c1.z);
    r6.xyz = (r3.xyz) * (r0.yyy);
    r0.w = exp2(-(r0.w));
    r3 = tex2D(s14, v0.zw);
    r13.xy = (r3.xy) * (c1.ww);
    r0.z = saturate((r0.w) * (c3.z) + (c3.w));
    r4.xy = (r4.xz) * (r13.yy);
    r0.w = (r3.y) * (c1.w) + (-(r4.x));
    r8.xz = (r4.xy) * (c3.xx);
    r0.w = (r4.z) * (-(r13.y)) + (r0.w);
    r2.xy = (r2.xz) * (r13.xx);
    r8.y = (r0.w) + (r0.w);
    r0.w = (r3.x) * (c1.w) + (-(r2.x));
    r3.xz = (r2.xy) * (c3.xx);
    r0.w = (r2.z) * (-(r13.x)) + (r0.w);
    r3.y = (r0.w) + (r0.w);
    r0.w = dot(r12.xyz, r1.xyz);
    r2.xyz = (r8.xyz) * (r0.zzz) + (r3.xyz);
    r0.w = (r0.w) + (r0.w);
    r8.xyz = (r11.yyy) * (r2.xyz);
    r1.xyz = (r1.xyz) * (-(r0.www)) + (r12.xyz);
    r1 = texCUBElod(s15, r1);
    r0.w = (r6.w) * (r6.w);
    r0.w = (r6.w) * (r0.w);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = (r5.w) * (r0.w);
    r2.xyz = (r11.xxx) * (r1.xyz);
    r4.xyz = (r9.xyz) * (r0.www) + (r10.xyz);
    r1.xyz = (r0.yyy) * (r7.xyz) + (r8.xyz);
    r0.xyz = (r2.xyz) * (r4.xyz);
    r1.xyz = (r5.xyz) * (r1.xyz) + (r6.xyz);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c0.zzz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.w = v1.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[7].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[6].w;

    return oC0;
}
