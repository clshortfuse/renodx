// Mechanically reconstructed from 0x76EDA3DB.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD4;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(-0.5f, 8.0f, 31.875f, 1.0f);
    const float4 c1 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.75f);
    const float4 c3 = float4(0.125f, 0.25f, 0.000244140625f, 0.0f);
    const float4 c4 = float4(4.0f, -3.0f, -4.0f, 2.0f);
    const float4 c12 = float4(0.797884583f, 1.0f, 0.0009765625f, 0.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v5.xyz, v5.xyz);
    r5.w = rsqrt(r0.w);
    r0.xyz = (v5.xyz) * (-(r5.www)) + (c[17].xyz);
    r14.xyz = normalize(r0.xyz);
    r0.w = saturate(dot(r14.xyz, c[17].xyz));
    r0.w = (-(r0.w)) + (c0.w);
    r0.z = (r0.w) * (r0.w);
    r0.z = (r0.z) * (r0.z);
    r8.w = (r0.w) * (r0.z);
    r0.xy = (v1.xy) * (c[31].xy);
    r0 = tex2D(s5, r0.xy);
    r2.xyz = (r0.xyz) + (c0.xxx);
    r0 = tex2D(s0, v1.xy);
    r1 = (v5.yyyy) * (c[26]);
    r1 = (v5.xxxx) * (c[25]) + (r1);
    r1 = (v5.zzzz) * (c[27]) + (r1);
    r3 = (r1) + (c[28]);
    r0.xyz = saturate((r2.xyz) * (r0.www) + (r0.xyz));
    r2.zw = r3.zw;
    r0.xyz = (r0.xyz) * (v0.xyz);
    r4.zw = r2.zw;
    r8.xyz = (r0.xyz) * (r0.xyz);
    r0.zw = r4.zw;
    r1.xy = (r3.ww) * (-(c[29].zw)) + (r3.xy);
    r1.zw = r0.zw;
    r1 = tex2Dproj(s2, r1);
    r1.w = r1.x;
    r4.xy = (r2.ww) * (-(c[29].xy)) + (r3.xy);
    r4 = tex2Dproj(s2, r4);
    r1.y = r4.x;
    r2.xy = (r2.ww) * (c[29].xy) + (r3.xy);
    r4 = tex2Dproj(s2, r2);
    r1.x = r4.x;
    r0.xy = (r2.ww) * (c[29].zw) + (r3.xy);
    r2 = tex2Dproj(s2, r0);
    r4.xyz = (-(v5.xyz)) + (c[6].xyz);
    r0.w = dot(r4.xyz, r4.xyz);
    r0.w = rsqrt(r0.w);
    r3.xyz = (r5.www) * (v5.xyz);
    r5.xyz = (r4.xyz) * (r0.www) + (-(r3.xyz));
    r0.xyz = normalize(r5.xyz);
    r4.xyz = (r4.xyz) * (r0.www);
    r0.w = saturate(dot(r0.xyz, r4.xyz));
    r0.w = (-(r0.w)) + (c0.w);
    r1.z = (r0.w) * (r0.w);
    r1.z = (r1.z) * (r1.z);
    r13.xyz = normalize(v2.xyz);
    r3.w = (r0.w) * (r1.z);
    r1.z = saturate(dot(r13.xyz, r0.xyz));
    r0 = tex2D(s4, v1.xy);
    r11.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.www);
    r12.xyz = (r0.xyz) * (r0.xyz);
    r13.w = (r0.w) * (-(c12.x)) + (c12.y);
    r14.w = (r0.w) * (c12.x);
    r2.z = saturate(dot(r4.xyz, r13.xyz));
    r2.w = saturate(dot(r13.xyz, -(r3.xyz)));
    r0.z = (r2.z) * (r13.w) + (r14.w);
    r12.w = (r2.w) * (r13.w) + (r14.w);
    r0.z = (r0.z) * (r12.w) + (c12.z);
    r5.xy = (r0.ww) * (c13.xy) + (c13.zw);
    r0.y = 1.0f / (r0.z);
    r11.w = exp2(r5.y);
    r0.z = pow(abs(r1.z), r11.w);
    r9.w = (r11.w) * (c3.x) + (c3.y);
    r2.y = (r2.z) * (r0.y);
    r1.z = (r0.z) * (r9.w);
    r0.xyz = (r11.xyz) * (r3.www) + (r12.xyz);
    r2.y = (r2.y) * (r1.z);
    r1.z = r2.x;
    r0.xyz = (r0.xyz) * (r2.yyy);
    r6.w = dot(r1, c3.yyyy);
    r0.xyz = (r0.xyz) * (c[8].xyz);
    r1.xyz = (r0.xyz) * (c[30].www);
    r1.w = dot(r4.xyz, c[23].xyz);
    r0.xyz = (r2.zzz) * (c[7].xyz);
    r1.w = saturate((r1.w) * (c[24].x) + (c[24].y));
    r6.xyz = (r8.xyz) * (r0.xyz) + (r1.xyz);
    r0.x = (r1.w) * (-(c4.w)) + (-(c4.y));
    r0.y = (r1.w) * (r1.w);
    r0.z = (-(r2.w)) + (c0.w);
    r7.w = (r0.x) * (r0.y);
    r0.x = (r0.z) * (r0.z);
    r0.y = 1.0f / (r5.x);
    r0.z = (r0.z) * (r0.x);
    r1.w = (r0.y) * (r0.z);
    r0.z = dot(r3.xyz, r13.xyz);
    r1.z = max(abs(r13.y), abs(r13.z));
    r0.z = (r0.z) + (r0.z);
    r0.w = (r0.w) * (c0.y);
    r0.xyz = (r13.xyz) * (-(r0.zzz)) + (r3.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.w = max(abs(r13.x), r1.z);
    r1.xyz = (r0.xyz) * (c0.yyy);
    r0 = tex3D(s11, v6.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = 1.0f / (r2.w);
    r0.xyz = (r1.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c0.zzz);
    r1.xyz = (r11.xyz) * (r1.www) + (r12.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz);
    r0.xyz = (r13.xyz) * (c[5].xyz);
    r7.xyz = (r1.xyz) * (c[30].xxx);
    r0.xyz = (r0.xyz) * (r0.www) + (v6.xyz);
    r1 = tex3D(s11, r0.xyz);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r0.xyz) * (c[30].yyy);
    r10.w = saturate(dot(r13.xyz, c[17].xyz));
    r0 = tex2D(s6, v1.xy);
    r1.xyz = (r1.xyz) * (r0.www);
    r10.xyz = (r1.xyz) * (c0.zzz);
    r9.xyz = (r10.www) * (c[18].xyz);
    if ((c0.w) >= (v4.w))
    {
        r2 = (v4.xyzx) * (c12.yyyw);
        r1 = (r2) + (-(c1.xyzz));
        r1 = tex2Dlod(s1, r1);
        r1.w = r1.x;
        r3 = (r2) + (c3.zzww);
        r3 = tex2Dlod(s1, r3);
        r1.x = r3.x;
        r3 = (r2) + (-(c3.zzww));
        r3 = tex2Dlod(s1, r3);
        r1.y = r3.x;
        r2 = (r2) + (c1.xyzz);
        r2 = tex2Dlod(s1, r2);
        r1.z = r2.x;
        r5.w = dot(r1, c3.yyyy);
        if ((c1.w) < (v4.w))
        {
            r5.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
            r1.xy = (r5.xy) + (c3.zz);
            r1.zw = (v4.zx) * (c12.yw);
            r1 = tex2Dlod(s1, r1);
            r2.xy = (r5.xy) + (-(c3.zz));
            r2.zw = (v4.zx) * (c12.yw);
            r4 = tex2Dlod(s1, r2);
            r2.xy = (r5.xy) + (c1.xy);
            r2.zw = (v4.zx) * (c12.yw);
            r3 = tex2Dlod(s1, r2);
            r2.xy = (r5.xy) + (-(c1.xy));
            r2.zw = (v4.zx) * (c12.yw);
            r2 = tex2Dlod(s1, r2);
            r1.y = r4.x;
            r1.z = r3.x;
            r1.w = r2.x;
            r1.w = dot(r1, c3.yyyy);
            r1.z = (-(r5.w)) + (r1.w);
            r1.w = (v4.w) * (c4.x) + (c4.y);
            r2.w = (r1.w) * (r1.z) + (r5.w);
        }
        else
        {
            r2.w = r5.w;
        }
    }
    else
    {
        r1.z = (v4.w) + (c4.z);
        r1.z = ((r1.z) >= 0.0f ? (c12.w) : (c12.y));
        if ((r1.z) != (-(r1.z)))
        {
            r1.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
            r2.xy = (r1.xy) + (c3.zz);
            r2.zw = (v4.zz) * (c12.yw);
            r2 = tex2Dlod(s1, r2);
            r3.xy = (r1.xy) + (-(c3.zz));
            r3.zw = (v4.zz) * (c12.yw);
            r5 = tex2Dlod(s1, r3);
            r3.xy = (r1.xy) + (c1.xy);
            r3.zw = (v4.zz) * (c12.yw);
            r4 = tex2Dlod(s1, r3);
            r3.xy = (r1.xy) + (-(c1.xy));
            r3.zw = (v4.zz) * (c12.yw);
            r3 = tex2Dlod(s1, r3);
            r2.y = r5.x;
            r2.z = r4.x;
            r2.w = r3.x;
            r1.y = dot(r2, c3.yyyy);
            r1.z = saturate((v4.w) + (c4.y));
            r1.w = (r1.w) + (-(r1.y));
            r1.w = (r1.z) * (r1.w) + (r1.y);
        }
        r2.w = r1.w;
    }
    r1.w = (r10.w) * (r13.w) + (r14.w);
    r1.w = (r1.w) * (r12.w) + (c12.z);
    r2.z = saturate(dot(r13.xyz, r14.xyz));
    r1.z = 1.0f / (r1.w);
    r1.w = pow(abs(r2.z), r11.w);
    r1.z = (r10.w) * (r1.z);
    r1.w = (r9.w) * (r1.w);
    r1.w = (r1.z) * (r1.w);
    r1.xyz = (r11.xyz) * (r8.www) + (r12.xyz);
    r1.xyz = (r1.www) * (r1.xyz);
    r1.xyz = (r1.xyz) * (c[30].www);
    r3.xyz = (r2.www) * (r9.xyz) + (r10.xyz);
    r1.xyz = (r0.www) * (r1.xyz);
    r2.xyz = (r1.xyz) * (c[19].xyz);
    r1 = (v5.xyzx) * (c12.yyyw) + (c12.wwwy);
    r4.xyz = (r2.www) * (r2.xyz);
    r0.w = dot(r1, c[22]);
    r2.w = 1.0f / (r0.w);
    r5.x = dot(r1, c[21]);
    r2.x = dot(r1, c[11]);
    r5.y = (r5.x) * (r5.x);
    r2.y = dot(r1, c[20]);
    r0.w = dot(c[9].yz, r5.xy) + (c[9].x);
    r1.w = saturate(1.0f / (r0.w));
    r1.xy = saturate((r5.xx) * (c[10].xy) + (c[10].zw));
    r5.xy = (r1.xy) * (-(c4.ww)) + (-(c4.yy));
    r1.xy = (r1.xy) * (r1.xy);
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c12.w) : (r1.w));
    r1.w = (r5.x) * (r1.x);
    r0.w = (r0.w) * (r1.w);
    r1.w = (r1.y) * (-(r5.y)) + (c0.w);
    r1.xy = (r2.ww) * (r2.xy);
    r0.w = (r0.w) * (r1.w);
    r0.w = (r7.w) * (r0.w);
    r1.xy = (r1.xy) * (-(c0.xx)) + (-(c0.xx));
    r1 = tex2D(s3, r1.xy);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r8.xyz) * (r3.xyz) + (r4.xyz);
    r2.xyz = (r0.www) * (r2.xyz);
    r1.xyz = (r7.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r6.www) * (r2.xyz);
    r0.xyz = (r0.xyz) * (r6.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[32].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.w;

    return oC0;
}
