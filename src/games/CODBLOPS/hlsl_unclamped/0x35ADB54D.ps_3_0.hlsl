// Mechanically reconstructed from 0x35ADB54D.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c2 = float4(1.0f, -0.0f, -1.0f, 8.0f);
    const float4 c3 = float4(31.875f, 0.797884583f, -2.0f, 3.0f);
    const float4 c4 = float4(0.797884583f, 1.0f, 0.0009765625f, 0.25f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v6.ww) * (c[22].xy) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[22].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r2.y = r0.x;
    r0.xy = (v6.ww) * (c[22].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r2.z = r0.x;
    r0.xy = (v6.ww) * (-(c[22].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r3 = tex2Dproj(s2, r0);
    r0.xy = (v1.xy) * (c[24].xy);
    r0 = tex2D(s4, r0.xy);
    r1.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r6.xy = (c[24].zz) * (r1.xy) + (r0.xy);
    r1 = tex2D(s7, v1.xy);
    r4.xy = (v1.xy) + (c1.xx);
    r3.z = c1.y;
    r0.xy = (r4.xy) * (c[29].xx) + (r3.zz);
    r0 = tex2D(s5, r0.xy);
    r5.w = (v8.w) * (c1.z) + (-(r0.x));
    r1.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r8.w = saturate((r5.w) * (c1.w));
    r5.xy = lerp(r6.xy, r1.xy, r8.ww);
    r1 = v2;
    r1.xyz = (r5.xxx) * (v5.xyz) + (r1.xyz);
    r1.xyz = (r5.yyy) * (v4.xyz) + (r1.xyz);
    r8.xyz = normalize(r1.xyz);
    r2.w = r3.x;
    r1.z = max(abs(r8.y), abs(r8.z));
    r4.w = dot(r2, c4.wwww);
    r0.w = max(abs(r8.x), r1.z);
    r0.w = 1.0f / (r0.w);
    r1.xyz = (r8.xyz) * (c[5].xyz);
    r10.xy = (r4.xy) * (c[30].xx) + (r3.zz);
    r1.xyz = (r1.xyz) * (r0.www) + (v8.xyz);
    r2 = tex3D(s11, r1.xyz);
    r3.xyz = (-(v7.xyz)) + (c[6].xyz);
    r6.y = dot(r3.xyz, r3.xyz);
    r3.w = rsqrt(r6.y);
    r4.xyz = (r3.xyz) * (r3.www);
    r0.w = dot(r4.xyz, c[10].xyz);
    r0.w = saturate((r0.w) * (c[11].x) + (c[11].y));
    r6.x = 1.0f / (r3.w);
    r1.z = (r0.w) * (c[11].w);
    r5.xy = saturate((r6.xx) * (c[21].xy) + (c[21].zw));
    r1.xy = (r5.xy) * (r5.xy);
    r5.xy = (r5.xy) * (c3.zz) + (c3.ww);
    r0.z = dot(c[20].yz, r6.xy) + (c[20].x);
    r5.xy = (r1.xy) * (r5.xy);
    r0.w = lerp(r2.w, r4.w, r1.z);
    r0.z = (r0.z) * (r5.x);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r0.z = (r5.y) * (r0.z);
    r0.w = (r0.w) * (r0.z);
    r0.z = saturate(dot(r8.xyz, r4.xyz));
    r2 = tex2D(s3, v1.xy);
    r5.y = (r2.w) * (c3.y);
    r4.w = (r2.w) * (-(c4.x)) + (c4.y);
    r9.xyz = normalize(v7.xyz);
    r0.x = (r0.z) * (r4.w) + (r5.y);
    r5.z = saturate(dot(r8.xyz, -(r9.xyz)));
    r4.w = (r5.z) * (r4.w) + (r5.y);
    r7.w = (-(r5.z)) + (c2.x);
    r0.x = (r0.x) * (r4.w) + (c4.z);
    r5.xyz = (r3.xyz) * (r3.www) + (-(r9.xyz));
    r0.x = 1.0f / (r0.x);
    r3.xyz = normalize(r5.xyz);
    r4.w = (r0.z) * (r0.x);
    r5.z = saturate(dot(r8.xyz, r3.xyz));
    r3.w = saturate(dot(r3.xyz, r4.xyz));
    r3.xy = (r2.ww) * (c12.xy) + (c12.zw);
    r0.x = exp2(r3.y);
    r3.w = (-(r3.w)) + (c2.x);
    r3.y = pow(abs(r5.z), r0.x);
    r3.z = (r3.w) * (r3.w);
    r0.x = (r0.x) * (c13.x) + (c13.y);
    r3.z = (r3.z) * (r3.z);
    r0.x = (r3.y) * (r0.x);
    r3.w = (r3.w) * (r3.z);
    r6.xyz = (r2.xyz) * (r2.xyz);
    r5.xyz = (r2.xyz) * (-(r2.xyz)) + (c2.xxx);
    r0.x = (r4.w) * (r0.x);
    r2.xyz = (r5.xyz) * (r3.www) + (r6.xyz);
    r6.w = 1.0f / (r3.x);
    r2.xyz = (r0.xxx) * (r2.xyz);
    r2.xyz = (r2.xyz) * (c[23].www);
    r4.xy = c[9].xy;
    r3.xyz = (r4.yyy) * (c[8].xyz);
    r2.w = (r2.w) * (c2.w);
    r3.xyz = (r2.xyz) * (r3.xyz);
    r2.xyz = (r1.xyz) * (c[23].yyy);
    r1.xyz = (r4.xxx) * (c[7].xyz);
    r2.xyz = (r2.xyz) * (c3.xxx);
    r1.xyz = (r0.zzz) * (r1.xyz);
    r7.xyz = (r0.www) * (r3.xyz);
    r1.xyz = (r0.www) * (r1.xyz) + (r2.xyz);
    r3 = tex2D(s6, r10.xy);
    r4 = tex2D(s0, v1.xy);
    r4.xyz = (r4.xyz) * (v0.xyz);
    r2.xyz = (r4.xyz) * (-(r4.xyz)) + (r3.xyz);
    r0.w = (r3.w) + (c2.z);
    r3.xyz = (r4.xyz) * (r4.xyz);
    r0.xyz = (r0.yyy) * (r2.xyz);
    r3.w = c2.x;
    r2.z = dot(r9.xyz, r8.xyz);
    r0 = (r8.wwww) * (r0) + (r3);
    r2.z = (r2.z) + (r2.z);
    r3.w = (-(r8.w)) + (c2.x);
    r2.xyz = (r8.xyz) * (-(r2.zzz)) + (r9.xyz);
    r2 = texCUBElod(s15, r2);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r7.xyz);
    r3.xyz = (r2.xyz) * (c2.www);
    r2 = tex3D(s11, v8.xyz);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r7.w) * (r7.w);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r2.w = (r7.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c3.xxx);
    r2.w = (r6.w) * (r2.w);
    r2.xyz = (r5.xyz) * (r2.www) + (r6.xyz);
    r2.w = ((r5.w) >= 0.0f ? (r3.w) : (c2.y));
    r0.xyz = (r0.xyz) * (r2.xyz);
    r2.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c[23].xxx) + (r1.xyz);
    r1.z = (r2.w) * (c[31].w);
    r2.xyz = (c[31].xyz) * (r1.zzz) + (r0.xyz);
    r2.w = c2.x;
    r0.x = dot(r2, c[26]);
    r0.y = dot(r2, c[27]);
    r0.z = dot(r2, c[28]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[25].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
