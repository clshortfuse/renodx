// Mechanically reconstructed from 0x19092132.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 1.0f, 0.0f, 8.0f);
    const float4 c2 = float4(4.0f, -2.0f, 0.25f, 0.0009765625f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[25]);
    r0 = (v6.xxxx) * (c[24]) + (r0);
    r0 = (v6.zzzz) * (c[26]) + (r0);
    r0 = (r0) + (c[27]);
    r3.xy = (r0.ww) * (c[28].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[28].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s2, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[28].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[28].zw) + (r0.xy);
    r2 = tex2Dproj(s2, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s2, r0);
    r0 = (v6.xyzx) * (c1.yyyz) + (c1.zzzy);
    r2.w = dot(r0, c[21]);
    r1.z = r2.x;
    r2.z = 1.0f / (r2.w);
    r2.x = dot(r0, c[10]);
    r2.y = dot(r0, c[11]);
    r2.w = dot(r1, c2.zzzz);
    r2.xy = (r2.zz) * (r2.xy);
    r1.x = dot(r0, c[20]);
    r0.xy = (r2.xy) * (-(c1.xx)) + (-(c1.xx));
    r0 = tex2D(s3, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[8].yz, r1.xy) + (c[8].x);
    r2.xy = saturate((r1.xx) * (c[9].xy) + (c[9].zw));
    r1.w = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c1.z) : (r1.w));
    r10.xyz = (-(v6.xyz)) + (c[5].xyz);
    r1.xy = (r2.xy) * (r2.xy);
    r1.w = dot(r10.xyz, r10.xyz);
    r2.xy = (r2.xy) * (c14.xx) + (c14.yy);
    r4.w = rsqrt(r1.w);
    r1.z = (r1.x) * (r2.x);
    r9.xyz = (r10.xyz) * (r4.www);
    r1.y = (r1.y) * (-(r2.y)) + (c1.y);
    r1.w = dot(r9.xyz, c[22].xyz);
    r0.w = (r0.w) * (r1.z);
    r1.z = saturate((r1.w) * (c[23].x) + (c[23].y));
    r1.w = (r1.z) * (r1.z);
    r1.z = (r1.z) * (c14.x) + (c14.y);
    r0.w = (r1.y) * (r0.w);
    r1.w = (r1.w) * (r1.z);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r0.w) * (r1.w);
    r0.xyz = (r0.xyz) * (r0.www);
    r1.xy = (v1.zw) * (c4.xy);
    r1 = tex2D(s13, r1.xy);
    r2.xy = (v1.zw) * (c4.xy) + (c4.zy);
    r3 = tex2D(s13, r2.xy);
    r1.w = r3.y;
    r4.xyz = (r2.www) * (r0.xyz);
    r7.xy = (r1.yw) * (c2.xx) + (c2.yy);
    r0 = tex2D(s1, v1.xy);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s0, v1.xy);
    r3.w = (r0.w) * (v0.w) + (c1.x);
    r0.w = dot(r7.xy, r7.xy) + (c1.z);
    r5.xy = float2(((r3.w) >= 0.0f ? (r2.x) : (c1.z)), ((r3.w) >= 0.0f ? (r2.y) : (c1.z)));
    r1.w = exp2(-(r0.w));
    r0.w = dot(r5.xy, r5.xy) + (c1.z);
    r1.w = (r1.w) * (c3.x) + (c3.y);
    r1.y = exp2(-(r0.w));
    r0 = (r0.xyzx) * (c1.yyyz) + (c1.zzzy);
    r5.w = (r1.y) * (c3.x) + (c3.y);
    r2 = tex2D(s14, v1.zw);
    r6.xy = (r2.xy) * (c3.ww);
    r2.w = (r1.w) * (r5.w);
    r3.xy = (r3.xz) * (r6.yy);
    r1.w = dot(r7.xy, r5.xy) + (c1.z);
    r1.y = (r2.y) * (c3.w) + (-(r3.x));
    r1.w = saturate((r1.w) * (r2.w) + (r2.w));
    r1.y = (r3.z) * (-(r6.y)) + (r1.y);
    r3.xz = (r3.xy) * (c4.ww);
    r3.y = (r1.y) + (r1.y);
    r3.xyz = (r1.www) * (r3.xyz);
    r1.xy = (r1.xz) * (r6.xx);
    r1.w = (r2.x) * (c3.w) + (-(r1.x));
    r8.xz = (r1.xy) * (c4.ww);
    r2.w = (r1.z) * (-(r6.x)) + (r1.w);
    r1 = v2;
    r1.xyz = (r5.xxx) * (v5.xyz) + (r1.xyz);
    r8.y = (r2.w) + (r2.w);
    r1.xyz = (r5.yyy) * (v4.xyz) + (r1.xyz);
    r7.xyz = normalize(r1.xyz);
    r11.xyz = normalize(v6.xyz);
    r1.xyz = (r8.xyz) * (r5.www) + (r3.xyz);
    r2.w = dot(r11.xyz, r7.xyz);
    r2.y = c1.z;
    r6.xyz = float3(((r3.w) >= 0.0f ? (c[29].x) : (r2.y)), ((r3.w) >= 0.0f ? (c[29].y) : (r2.y)), ((r3.w) >= 0.0f ? (c[29].w) : (r2.y)));
    r2.w = (r2.w) + (r2.w);
    r1.xyz = (r1.xyz) * (r6.yyy);
    r3.xyz = (r7.xyz) * (-(r2.www)) + (r11.xyz);
    r2 = tex2D(s4, v1.xy);
    r2 = float4(((r3.w) >= 0.0f ? (r2.x) : (c1.z)), ((r3.w) >= 0.0f ? (r2.y) : (c1.z)), ((r3.w) >= 0.0f ? (r2.z) : (c1.z)), ((r3.w) >= 0.0f ? (r2.w) : (c1.y)));
    r0 = float4(((r3.w) >= 0.0f ? (r0.x) : (c1.z)), ((r3.w) >= 0.0f ? (r0.y) : (c1.z)), ((r3.w) >= 0.0f ? (r0.z) : (c1.z)), ((r3.w) >= 0.0f ? (r0.w) : (c1.z)));
    r3.w = (r2.w) * (c1.w);
    r3 = texCUBElod(s15, r3);
    r5.w = saturate(dot(r7.xyz, -(r11.xyz)));
    r3.xyz = (r3.xyz) * (r3.xyz);
    r3.w = (-(r5.w)) + (c1.y);
    r5.z = (r3.w) * (r3.w);
    r12.xy = (r2.ww) * (c12.xy) + (c12.zw);
    r3.w = (r3.w) * (r5.z);
    r6.w = 1.0f / (r12.x);
    r5.xyz = (r6.xxx) * (r3.xyz);
    r3.w = (r3.w) * (r6.w);
    r3.xyz = (r2.xyz) * (r2.xyz);
    r2.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.yyy);
    r10.xyz = (r10.xyz) * (r4.www) + (-(r11.xyz));
    r11.xyz = (r2.xyz) * (r3.www) + (r3.xyz);
    r3.w = exp2(r12.y);
    r5.xyz = (r5.xyz) * (r11.xyz);
    r5.xyz = (r8.xyz) * (r5.xyz);
    r6.y = (r2.w) * (c3.z);
    r4.w = (r2.w) * (-(c13.x)) + (c13.y);
    r2.w = saturate(dot(r9.xyz, r7.xyz));
    r5.w = (r5.w) * (r4.w) + (r6.y);
    r4.w = (r2.w) * (r4.w) + (r6.y);
    r5.xyz = (r5.xyz) * (c1.www);
    r4.w = (r4.w) * (r5.w) + (c2.w);
    r4.w = 1.0f / (r4.w);
    r8.xyz = normalize(r10.xyz);
    r5.w = (r2.w) * (r4.w);
    r4.w = saturate(dot(r8.xyz, r9.xyz));
    r7.w = saturate(dot(r7.xyz, r8.xyz));
    r4.w = (-(r4.w)) + (c1.y);
    r6.y = pow(abs(r7.w), r3.w);
    r6.w = (r4.w) * (r4.w);
    r3.w = (r3.w) * (c13.z) + (c13.w);
    r6.w = (r6.w) * (r6.w);
    r3.w = (r6.y) * (r3.w);
    r4.w = (r4.w) * (r6.w);
    r3.w = (r5.w) * (r3.w);
    r2.xyz = (r2.xyz) * (r4.www) + (r3.xyz);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r2.xyz = (r3.www) * (r2.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r3.xyz = (r2.xyz) * (c[7].xyz);
    r2.xyz = (r2.www) * (c[6].xyz);
    r3.xyz = (r6.zzz) * (r3.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r5.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[30].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
