// Mechanically reconstructed from 0xB955048C.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c2 = float4(0.959999979f, 0.0399999991f, -2.0f, 3.0f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v6.ww) * (c[28].xy) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[28].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.y = r0.x;
    r0.xy = (v6.ww) * (c[28].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0.xy = (v6.ww) * (-(c[28].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0.xy = (v1.xy) * (c[29].xy);
    r0 = tex2D(s4, r0.xy);
    r3.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r3.xy = (c[29].zz) * (r3.xy) + (r0.xy);
    r0 = v2;
    r0.xyz = (r3.xxx) * (v5.xyz) + (r0.xyz);
    r0.xyz = (r3.yyy) * (v4.xyz) + (r0.xyz);
    r9.xyz = normalize(r0.xyz);
    r2.w = max(abs(r9.y), abs(r9.z));
    r1.w = r2.x;
    r0.z = max(abs(r9.x), r2.w);
    r2.w = 1.0f / (r0.z);
    r0.xyz = (r9.xyz) * (c[5].xyz);
    r3.w = dot(r1, c3.zzzz);
    r0.xyz = (r0.xyz) * (r2.www) + (v8.xyz);
    r1 = tex3D(s11, r0.xyz);
    r4.xyz = (-(v7.xyz)) + (c[21].xyz);
    r0.y = dot(r4.xyz, r4.xyz);
    r2.w = rsqrt(r0.y);
    r2.xyz = (r4.xyz) * (r2.www);
    r0.z = dot(r2.xyz, c[24].xyz);
    r0.z = saturate((r0.z) * (c[25].x) + (c[25].y));
    r0.z = (r0.z) * (c[25].w);
    r4.w = lerp(r1.w, r3.w, r0.z);
    r0.x = 1.0f / (r2.w);
    r3.xyz = (r1.xyz) * (r1.xyz);
    r1.w = dot(c[26].yz, r0.xy) + (c[26].x);
    r1.xy = saturate((r0.xx) * (c[27].xy) + (c[27].zw));
    r0.xyz = normalize(v7.xyz);
    r5.xy = (r1.xy) * (r1.xy);
    r4.xyz = (r4.xyz) * (r2.www) + (-(r0.xyz));
    r6.xy = (r1.xy) * (c2.zz) + (c2.ww);
    r1.xyz = normalize(r4.xyz);
    r4.xy = (r5.xy) * (r6.xy);
    r2.w = saturate(dot(r1.xyz, r2.xyz));
    r1.w = (r1.w) * (r4.x);
    r2.w = (-(r2.w)) + (c1.z);
    r1.w = (r4.y) * (r1.w);
    r3.w = (r2.w) * (r2.w);
    r1.w = (r4.w) * (r1.w);
    r3.w = (r3.w) * (r3.w);
    r4.z = saturate(dot(r9.xyz, r1.xyz));
    r1.y = (r2.w) * (r3.w);
    r1.z = saturate(dot(r9.xyz, r2.xyz));
    r4.w = (r1.y) * (c2.x) + (c2.y);
    r2 = tex2D(s3, v1.xy);
    r2.x = (r2.w) * (c1.w);
    r1.x = (r2.w) * (-(c1.w)) + (c1.z);
    r1.y = (r1.z) * (r1.x) + (r2.x);
    r2.z = saturate(dot(r9.xyz, -(r0.xyz)));
    r1.x = (r2.z) * (r1.x) + (r2.x);
    r2.x = (-(r2.z)) + (c1.z);
    r1.y = (r1.y) * (r1.x) + (c3.x);
    r4.xy = (r2.ww) * (c4.xy) + (c4.zw);
    r2.z = 1.0f / (r1.y);
    r3.w = exp2(r4.y);
    r1.x = pow(abs(r4.z), r3.w);
    r1.y = (r3.w) * (c3.y) + (c3.z);
    r3.w = (r1.z) * (r2.z);
    r2.z = (r1.x) * (r1.y);
    r1.xyz = (r1.zzz) * (c[22].xyz);
    r3.w = (r3.w) * (r2.z);
    r2.z = 1.0f / (r4.x);
    r3.w = (r4.w) * (r3.w);
    r3.w = (r2.y) * (r3.w);
    r3.xyz = (r3.xyz) * (r2.yyy);
    r4.xyz = (r3.www) * (c[23].xyz);
    r3.xyz = (r3.xyz) * (c1.yyy);
    r5.xyz = (r1.www) * (r4.xyz);
    r4.xyz = (r1.www) * (r1.xyz) + (r3.xyz);
    r1 = tex2D(s0, v1.xy);
    r1.w = dot(r0.xyz, r9.xyz);
    r1.w = (r1.w) + (r1.w);
    r3.xyz = (r9.xyz) * (-(r1.www)) + (r0.xyz);
    r3.w = (r2.w) * (c1.x);
    r3 = texCUBElod(s15, r3);
    r3.xyz = (r3.xyz) * (r3.xyz);
    r0.xyz = (r1.xyz) * (v0.xyz);
    r3.xyz = (r3.xyz) * (c1.xxx);
    r1 = tex3D(s11, v8.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r3.xyz) * (r1.xyz);
    r7.xyz = (r0.xyz) * (r4.xyz) + (r5.xyz);
    r8.xyz = (r1.xyz) * (c1.yyy);
    r2.w = (r2.x) * (r2.x);
    r5 = (-(v7.yyyy)) + (c[7]);
    r1 = (r5) * (r5);
    r4 = (-(v7.xxxx)) + (c[6]);
    r1 = (r4) * (r4) + (r1);
    r3 = (-(v7.zzzz)) + (c[8]);
    r2.w = (r2.x) * (r2.w);
    r1 = (r3) * (r3) + (r1);
    r2.w = (r2.z) * (r2.w);
    r6.x = rsqrt(r1.x);
    r6.y = rsqrt(r1.y);
    r6.z = rsqrt(r1.z);
    r6.w = rsqrt(r1.w);
    r2.w = (r2.w) * (c2.x) + (c2.y);
    r5 = (r5) * (r6);
    r5 = (r9.yyyy) * (r5);
    r4 = (r4) * (r6);
    r3 = (r3) * (r6);
    r4 = (r4) * (r9.xxxx) + (r5);
    r2.x = c1.z;
    r1 = saturate((r1) * (c[9]) + (r2.xxxx));
    r3 = saturate((r3) * (r9.zzzz) + (r4));
    r4.xyz = (r8.xyz) * (r2.www);
    r1 = (r1) * (r3);
    r3.xyz = (r4.xyz) * (r2.yyy) + (r7.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.z = dot(c[20], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[30].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.z;

    return oC0;
}
