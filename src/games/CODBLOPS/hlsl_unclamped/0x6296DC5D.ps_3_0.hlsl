// Mechanically reconstructed from 0x6296DC5D.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c1 = float4(-2.0f, 3.0f, 0.0009765625f, 0.0f);
    const float4 c2 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c3 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r1.xyz = (-(v4.xyz)) + (c[6].xyz);
    r4.y = dot(r1.xyz, r1.xyz);
    r1.w = rsqrt(r4.y);
    r2.xyz = (r1.xyz) * (r1.www);
    r7.xyz = normalize(v2.xyz);
    r5.w = saturate(dot(r7.xyz, r2.xyz));
    r0 = tex2D(s1, v1.xy);
    r3.y = (r0.w) * (c0.w);
    r8.xyz = normalize(v4.xyz);
    r3.z = (r0.w) * (-(c0.w)) + (c0.z);
    r3.w = saturate(dot(r7.xyz, -(r8.xyz)));
    r2.w = (r5.w) * (r3.z) + (r3.y);
    r3.z = (r3.w) * (r3.z) + (r3.y);
    r3.w = (-(r3.w)) + (c0.z);
    r2.w = (r2.w) * (r3.z) + (c1.z);
    r2.w = 1.0f / (r2.w);
    r3.xyz = (r1.xyz) * (r1.www) + (-(r8.xyz));
    r2.w = (r5.w) * (r2.w);
    r1.xyz = normalize(r3.xyz);
    r4.x = 1.0f / (r1.w);
    r3.z = saturate(dot(r7.xyz, r1.xyz));
    r1.z = saturate(dot(r1.xyz, r2.xyz));
    r1.xy = (r0.ww) * (c2.xy) + (c2.zw);
    r1.w = exp2(r1.y);
    r1.z = (-(r1.z)) + (c0.z);
    r2.z = pow(abs(r3.z), r1.w);
    r1.y = (r1.z) * (r1.z);
    r1.w = (r1.w) * (c3.x) + (c3.y);
    r1.y = (r1.y) * (r1.y);
    r1.w = (r2.z) * (r1.w);
    r1.z = (r1.z) * (r1.y);
    r5.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.zzz);
    r1.w = (r2.w) * (r1.w);
    r0.xyz = (r2.xyz) * (r1.zzz) + (r5.xyz);
    r2.w = 1.0f / (r1.x);
    r0.xyz = (r1.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c[20].www);
    r9.xy = c[9].xy;
    r1.xyz = (r9.yyy) * (c[8].xyz);
    r0.w = (r0.w) * (c0.x);
    r3.xyz = (r0.xyz) * (r1.xyz);
    r0.z = dot(c[10].yz, r4.xy) + (c[10].x);
    r1.xy = saturate((r4.xx) * (c[11].xy) + (c[11].zw));
    r0.xy = (r1.xy) * (r1.xy);
    r1.xy = (r1.xy) * (c1.xx) + (c1.yy);
    r1.xy = (r0.xy) * (r1.xy);
    r1.w = max(abs(r7.y), abs(r7.z));
    r1.z = (r0.z) * (r1.x);
    r0.z = max(abs(r7.x), r1.w);
    r1.w = 1.0f / (r0.z);
    r0.xyz = (r7.xyz) * (c[5].xyz);
    r4.w = (r1.y) * (r1.z);
    r0.xyz = (r0.xyz) * (r1.www) + (v5.xyz);
    r1 = tex3D(s11, r0.xyz);
    r4.w = (r4.w) * (r1.w);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r4.xyz = (r3.xyz) * (r4.www);
    r1 = tex2D(s0, v1.xy);
    r1.xyz = (r1.xyz) * (v0.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (c[20].yyy);
    r6.xyz = (r0.xyz) * (c0.yyy);
    r1.w = dot(r8.xyz, r7.xyz);
    r0.xyz = (r9.xxx) * (c[7].xyz);
    r1.w = (r1.w) + (r1.w);
    r3.xyz = (r5.www) * (r0.xyz);
    r0.xyz = (r7.xyz) * (-(r1.www)) + (r8.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r3.xyz = (r4.www) * (r3.xyz) + (r6.xyz);
    r6.xyz = (r0.xyz) * (c0.xxx);
    r0 = tex3D(s11, v5.xyz);
    r0.w = (r3.w) * (r3.w);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r3.w) * (r0.w);
    r0.xyz = (r6.xyz) * (r0.xyz);
    r0.w = (r2.w) * (r0.w);
    r0.xyz = (r0.xyz) * (c0.yyy);
    r2.xyz = (r2.xyz) * (r0.www) + (r5.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz) + (r4.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r0.xyz = (r0.xyz) * (c[20].xxx) + (r1.xyz);
    r0.w = c0.z;
    r1.x = dot(r0, c[22]);
    r1.y = dot(r0, c[23]);
    r1.z = dot(r0, c[24]);
    r0.xyz = (r1.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[21].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.z;

    return oC0;
}
