// Mechanically reconstructed from 0x7F226834.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD5;
    float4 v4 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    const float4 c0 = float4(-0.5f, 1.0f, 8.0f, 0.797884583f);
    const float4 c1 = float4(0.25f, 0.0009765625f, 0.125f, 0.0f);
    const float4 c2 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c3 = float4(4.0f, -2.0f, 0.600000024f, 0.400000006f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v3.yyyy) * (c[25]);
    r0 = (v3.xxxx) * (c[24]) + (r0);
    r0 = (v3.zzzz) * (c[26]) + (r0);
    r0 = (r0) + (c[27]);
    r3.xy = (r0.ww) * (c[28].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s1, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[28].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s1, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[28].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[28].zw) + (r0.xy);
    r2 = tex2Dproj(s1, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s1, r0);
    r0 = (v3.xyzx) * (c2.xxxz) + (c2.zzzx);
    r2.w = dot(r0, c[21]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[10]);
    r2.y = dot(r0, c[11]);
    r3.w = dot(r1, c1.xxxx);
    r1.xy = (r2.ww) * (r2.xy);
    r2.x = dot(r0, c[20]);
    r0.xy = (r1.xy) * (-(c0.xx)) + (-(c0.xx));
    r0 = tex2D(s2, r0.xy);
    r2.y = (r2.x) * (r2.x);
    r0.w = dot(c[8].yz, r2.xy) + (c[8].x);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.z = saturate(1.0f / (r0.w));
    r2.xy = saturate((r2.xx) * (c[9].xy) + (c[9].zw));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c2.z) : (r0.z));
    r0.xy = (r2.xy) * (r2.xy);
    r7.xyz = (-(v3.xyz)) + (c[5].xyz);
    r2.xy = (r2.xy) * (c12.xx) + (c12.yy);
    r1.w = dot(r7.xyz, r7.xyz);
    r0.z = (r0.x) * (r2.x);
    r4.w = rsqrt(r1.w);
    r0.y = (r0.y) * (-(r2.y)) + (c0.y);
    r10.xyz = (r7.xyz) * (r4.www);
    r0.z = (r0.w) * (r0.z);
    r0.w = dot(r10.xyz, c[22].xyz);
    r1.w = (r0.y) * (r0.z);
    r0.z = saturate((r0.w) * (c[23].x) + (c[23].y));
    r0.w = (r0.z) * (r0.z);
    r0.z = (r0.z) * (c12.x) + (c12.y);
    r3.z = (r0.w) * (r0.z);
    r0.xy = (v0.zw) * (c2.xy);
    r0 = tex2D(s13, r0.xy);
    r2.xy = (v0.zw) * (c2.xy) + (c2.zy);
    r2 = tex2D(s13, r2.xy);
    r0.w = r2.y;
    r1.w = (r1.w) * (r3.z);
    r4.xy = (r0.yw) * (c3.xx) + (c3.yy);
    r3.xyz = (r1.xyz) * (r1.www);
    r0.w = dot(r4.xy, r4.xy) + (c2.z);
    r1 = tex2D(s14, v0.zw);
    r4.xy = (r1.xy) * (c2.ww);
    r0.w = exp2(-(r0.w));
    r2.xy = (r2.xz) * (r4.yy);
    r1.w = saturate((r0.w) * (c3.z) + (c3.w));
    r0.w = (r1.y) * (c2.w) + (-(r2.x));
    r5.xz = (r2.xy) * (c3.xx);
    r0.xy = (r0.xz) * (r4.xx);
    r1.z = (r2.z) * (-(r4.y)) + (r0.w);
    r0.w = (r1.x) * (c2.w) + (-(r0.x));
    r5.y = (r1.z) + (r1.z);
    r0.w = (r0.z) * (-(r4.x)) + (r0.w);
    r4.xz = (r0.xy) * (c3.xx);
    r4.y = (r0.w) + (r0.w);
    r0 = tex2D(s3, v4.zw);
    r2.w = (r0.w) * (v4.y) + (c0.x);
    r1.xyz = (r5.xyz) * (r1.www) + (r4.xyz);
    r0.w = c0.y;
    r6.xyz = float3(((r2.w) >= 0.0f ? (r0.w) : (c[29].x)), ((r2.w) >= 0.0f ? (r0.w) : (c[29].y)), ((r2.w) >= 0.0f ? (r0.w) : (c[29].w)));
    r2.xyz = (r3.www) * (r3.xyz);
    r5.xyz = (r1.xyz) * (r6.yyy);
    r1 = tex2D(s0, v0.xy);
    r9.xyz = normalize(v3.xyz);
    r8.xyz = normalize(v1.xyz);
    r0.w = dot(r9.xyz, r8.xyz);
    r0.xyz = float3(((r2.w) >= 0.0f ? (r0.x) : (r1.x)), ((r2.w) >= 0.0f ? (r0.y) : (r1.y)), ((r2.w) >= 0.0f ? (r0.z) : (r1.z)));
    r0.w = (r0.w) + (r0.w);
    r3.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r8.xyz) * (-(r0.www)) + (r9.xyz);
    r0 = tex2D(s4, v0.xy);
    r1.w = (r0.w) * (c0.z);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r6.xxx) * (r1.xyz);
    r3.w = saturate(dot(r8.xyz, -(r9.xyz)));
    r11.xyz = (r7.xyz) * (r4.www) + (-(r9.xyz));
    r1.w = (-(r3.w)) + (c0.y);
    r2.w = (r1.w) * (r1.w);
    r6.xy = (r0.ww) * (c4.xy) + (c4.zw);
    r2.w = (r1.w) * (r2.w);
    r4.w = 1.0f / (r6.x);
    r1.w = exp2(r6.y);
    r2.w = (r2.w) * (r4.w);
    r7.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.yyy);
    r9.xyz = (r0.xyz) * (r2.www) + (r7.xyz);
    r4.w = (r0.w) * (c0.w);
    r2.w = (r0.w) * (-(c0.w)) + (c0.y);
    r0.w = saturate(dot(r10.xyz, r8.xyz));
    r3.w = (r3.w) * (r2.w) + (r4.w);
    r2.w = (r0.w) * (r2.w) + (r4.w);
    r1.xyz = (r1.xyz) * (r9.xyz);
    r2.w = (r2.w) * (r3.w) + (c1.y);
    r2.w = 1.0f / (r2.w);
    r9.xyz = normalize(r11.xyz);
    r3.w = (r0.w) * (r2.w);
    r2.w = saturate(dot(r9.xyz, r10.xyz));
    r4.w = saturate(dot(r8.xyz, r9.xyz));
    r2.w = (-(r2.w)) + (c0.y);
    r5.w = pow(abs(r4.w), r1.w);
    r4.w = (r2.w) * (r2.w);
    r1.w = (r1.w) * (c1.z) + (c1.x);
    r4.w = (r4.w) * (r4.w);
    r1.w = (r5.w) * (r1.w);
    r2.w = (r2.w) * (r4.w);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r0.xyz) * (r2.www) + (r7.xyz);
    r1.xyz = (r4.xyz) * (r1.xyz);
    r0.xyz = (r1.www) * (r0.xyz);
    r1.xyz = (r1.xyz) * (c0.zzz);
    r4.xyz = (r0.xyz) * (c[7].xyz);
    r0.xyz = (r0.www) * (c[6].xyz);
    r4.xyz = (r6.zzz) * (r4.xyz);
    r1.xyz = (r3.xyz) * (r5.xyz) + (r1.xyz);
    r0.xyz = (r3.xyz) * (r0.xyz) + (r4.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.w = v1.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[30].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.y;

    return oC0;
}
