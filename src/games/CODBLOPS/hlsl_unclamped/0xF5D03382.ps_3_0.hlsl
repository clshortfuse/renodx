// Mechanically reconstructed from 0xF5D03382.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler3D s11 : register(s11);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-2.0f, 3.0f, 31.875f, 0.25f);
    const float4 c2 = float4(1.0f, 0.0f, 0.5f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xyz = (-(v7.xyz)) + (c[35].xyz);
    r3.y = dot(r0.xyz, r0.xyz);
    r1.w = rsqrt(r3.y);
    r3.x = 1.0f / (r1.w);
    r2.xy = saturate((r3.xx) * (c[41].xy) + (c[41].zw));
    r1.xy = (r2.xy) * (r2.xy);
    r2.xy = (r2.xy) * (c1.xx) + (c1.yy);
    r0.w = dot(c[40].yz, r3.xy) + (c[40].x);
    r1.xy = (r1.xy) * (r2.xy);
    r0.w = (r0.w) * (r1.x);
    r3.xyz = (r0.xyz) * (r1.www);
    r3.w = (r1.y) * (r0.w);
    r0.xy = (v6.ww) * (c[42].xy) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[42].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.y = r0.x;
    r0.xy = (v6.ww) * (c[42].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0.xy = (v6.ww) * (-(c[42].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0 = tex2D(s1, v1.xy);
    r4.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = v2;
    r0.xyz = (r4.xxx) * (v5.xyz) + (r0.xyz);
    r1.w = r2.x;
    r0.xyz = (r4.yyy) * (v4.xyz) + (r0.xyz);
    r4.z = dot(r1, c1.wwww);
    r9.xyz = normalize(r0.xyz);
    r0.z = dot(r3.xyz, c[38].xyz);
    r1.w = max(abs(r9.y), abs(r9.z));
    r1.z = saturate((r0.z) * (c[39].x) + (c[39].y));
    r0.z = max(abs(r9.x), r1.w);
    r1.w = 1.0f / (r0.z);
    r0.xyz = (r9.xyz) * (c[5].xyz);
    r4.w = (r1.z) * (c[39].w);
    r0.xyz = (r0.xyz) * (r1.www) + (v8.xyz);
    r1 = tex3D(s11, r0.xyz);
    r2.w = lerp(r1.w, r4.z, r4.w);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r2.w = (r3.w) * (r2.w);
    r2.xyz = (r0.xyz) * (c1.zzz);
    r3.w = saturate(dot(r3.xyz, r9.xyz));
    r1 = (v7.yyyy) * (c[32]);
    r0.xyz = c[36].xyz;
    r0.xyz = (r0.xyz) * (c[37].xxx);
    r1 = (v7.xxxx) * (c[31]) + (r1);
    r0.xyz = (r3.www) * (r0.xyz);
    r1 = (v7.zzzz) * (c[33]) + (r1);
    r6.xyz = (r2.www) * (r0.xyz) + (r2.xyz);
    r1 = (r1) + (c[34]);
    r4.xy = (r1.ww) * (c[43].xy) + (r1.xy);
    r4.zw = r1.zw;
    r2 = tex2Dproj(s2, r4);
    r3.zw = r4.zw;
    r1.zw = r3.zw;
    r5.xy = (r4.ww) * (-(c[43].zw)) + (r1.xy);
    r5.zw = r1.zw;
    r5 = tex2Dproj(s2, r5);
    r2.w = r5.x;
    r3.xy = (r4.ww) * (-(c[43].xy)) + (r1.xy);
    r1.xy = (r4.ww) * (c[43].zw) + (r1.xy);
    r3 = tex2Dproj(s2, r3);
    r2.y = r3.x;
    r3 = tex2Dproj(s2, r1);
    r1 = (v7.xyzx) * (c2.xxxy) + (c2.yyyx);
    r0.z = dot(r1, c[28]);
    r2.z = r3.x;
    r0.z = 1.0f / (r0.z);
    r0.x = dot(r1, c[25]);
    r0.y = dot(r1, c[26]);
    r2.w = dot(r2, c1.wwww);
    r2.xy = (r0.zz) * (r0.xy);
    r0.x = dot(r1, c[27]);
    r1.xy = (r2.xy) * (c2.zz) + (c2.zz);
    r1 = tex2D(s3, r1.xy);
    r0.y = (r0.x) * (r0.x);
    r0.z = dot(c[23].yz, r0.xy) + (c[23].x);
    r0.xy = saturate((r0.xx) * (c[24].xy) + (c[24].zw));
    r1.w = saturate(1.0f / (r0.z));
    r3.w = ((-abs(r0.z)) >= 0.0f ? (c2.y) : (r1.w));
    r3.xy = (r0.xy) * (r0.xy);
    r4.xy = (r0.xy) * (c1.xx) + (c1.yy);
    r0.xyz = (-(v7.xyz)) + (c[21].xyz);
    r3.z = (r3.x) * (r4.x);
    r2.xyz = normalize(r0.xyz);
    r1.w = (r3.y) * (-(r4.y)) + (c2.x);
    r0.z = dot(r2.xyz, c[29].xyz);
    r0.x = (r3.w) * (r3.z);
    r0.y = saturate((r0.z) * (c[30].x) + (c[30].y));
    r0.z = (r0.y) * (r0.y);
    r0.y = (r0.y) * (c1.x) + (c1.y);
    r1.w = (r1.w) * (r0.x);
    r3.w = (r0.z) * (r0.y);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.z = (r1.w) * (r3.w);
    r1.w = saturate(dot(r2.xyz, r9.xyz));
    r0.xyz = (r0.xyz) * (r1.zzz);
    r7.xyz = (r2.www) * (r0.xyz);
    r8.xyz = (r1.www) * (c[22].xyz);
    r5 = tex2D(s0, v1.xy);
    r4 = (-(v7.yyyy)) + (c[7]);
    r1 = (r4) * (r4);
    r3 = (-(v7.xxxx)) + (c[6]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v7.zzzz)) + (c[8]);
    r0.xyz = (r5.xyz) * (v0.xyz);
    r1 = (r2) * (r2) + (r1);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r8.xyz = (r8.xyz) * (r0.xyz);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r4.w = c2.x;
    r1 = saturate((r1) * (c[9]) + (r4.wwww));
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.xyz = (r7.xyz) * (r8.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r0.xyz) * (r6.xyz) + (r3.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.z = dot(c[20], r1);
    r1.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r1.w = c2.x;
    r0.x = dot(r1, c[45]);
    r0.y = dot(r1, c[46]);
    r0.z = dot(r1, c[47]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[44].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c2.x;

    return oC0;
}
