// Mechanically reconstructed from 0xA4E050A6.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 1.0f, 0.0f, 8.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v5.yyyy) * (c[32]);
    r0 = (v5.xxxx) * (c[31]) + (r0);
    r0 = (v5.zzzz) * (c[33]) + (r0);
    r1 = (r0) + (c[34]);
    r3.xy = (r1.ww) * (c[35].xy) + (r1.xy);
    r3.zw = r1.zw;
    r0 = tex2Dproj(s1, r3);
    r2.zw = r3.zw;
    r1.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[35].zw)) + (r1.xy);
    r4.zw = r1.zw;
    r4 = tex2Dproj(s1, r4);
    r0.w = r4.x;
    r2.xy = (r3.ww) * (-(c[35].xy)) + (r1.xy);
    r1.xy = (r3.ww) * (c[35].zw) + (r1.xy);
    r2 = tex2Dproj(s1, r2);
    r0.y = r2.x;
    r2 = tex2Dproj(s1, r1);
    r1 = (v5.xyzx) * (c1.yyyz) + (c1.zzzy);
    r0.z = dot(r1, c[28]);
    r2.w = 1.0f / (r0.z);
    r3.x = dot(r1, c[25]);
    r3.y = dot(r1, c[26]);
    r0.z = r2.x;
    r3.xy = (r2.ww) * (r3.xy);
    r2.x = dot(r1, c[27]);
    r1.xy = (r3.xy) * (-(c1.xx)) + (-(c1.xx));
    r1 = tex2D(s2, r1.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[23].yz, r2.xy) + (c[23].x);
    r3.xy = saturate((r2.xx) * (c[24].xy) + (c[24].zw));
    r2.w = saturate(1.0f / (r1.w));
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c1.z) : (r2.w));
    r9.xyz = (-(v5.xyz)) + (c[20].xyz);
    r2.xy = (r3.xy) * (r3.xy);
    r2.w = dot(r9.xyz, r9.xyz);
    r3.xy = (r3.xy) * (c14.xx) + (c14.yy);
    r11.w = rsqrt(r2.w);
    r2.z = (r2.x) * (r3.x);
    r5.xyz = (r9.xyz) * (r11.www);
    r2.y = (r2.y) * (-(r3.y)) + (c1.y);
    r2.w = dot(r5.xyz, c[29].xyz);
    r1.w = (r1.w) * (r2.z);
    r2.z = saturate((r2.w) * (c[30].x) + (c[30].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c14.x) + (c14.y);
    r1.w = (r2.y) * (r1.w);
    r2.w = (r2.w) * (r2.z);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r1.w) * (r2.w);
    r0.w = dot(r0, c3.zzzz);
    r0.xyz = (r1.xyz) * (r1.www);
    r7.xyz = (r0.www) * (r0.xyz);
    r0.xy = (v0.zw) * (c4.xy);
    r1 = tex2D(s13, r0.xy);
    r0.xy = (v0.zw) * (c4.xy) + (c4.zy);
    r3 = tex2D(s13, r0.xy);
    r1.w = r3.y;
    r0 = tex2D(s4, v6.zw);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s3, v6.zw);
    r4.w = (r0.w) * (v6.y) + (c1.x);
    r4.xy = (r1.yw) * (c12.xx) + (c12.yy);
    r6.xy = float2(((r4.w) >= 0.0f ? (r2.x) : (c1.z)), ((r4.w) >= 0.0f ? (r2.y) : (c1.z)));
    r1.w = dot(r4.xy, r4.xy) + (c1.z);
    r0.w = dot(r6.xy, r6.xy) + (c1.z);
    r1.w = exp2(-(r1.w));
    r0.w = exp2(-(r0.w));
    r1.w = (r1.w) * (c2.x) + (c2.y);
    r3.w = (r0.w) * (c2.x) + (c2.y);
    r0 = (r0.xyzx) * (c1.yyyz) + (c1.zzzy);
    r1.y = (r1.w) * (r3.w);
    r2 = tex2D(s14, v0.zw);
    r10.xy = (r2.xy) * (c2.ww);
    r1.w = dot(r4.xy, r6.xy) + (c1.z);
    r3.xy = (r3.xz) * (r10.yy);
    r1.w = saturate((r1.w) * (r1.y) + (r1.y));
    r1.y = (r2.y) * (c2.w) + (-(r3.x));
    r4.xz = (r3.xy) * (c4.ww);
    r1.y = (r3.z) * (-(r10.y)) + (r1.y);
    r4.y = (r1.y) + (r1.y);
    r1.xy = (r1.xz) * (r10.xx);
    r3.xyz = (r1.www) * (r4.xyz);
    r1.w = (r2.x) * (c2.w) + (-(r1.x));
    r8.xz = (r1.xy) * (c4.ww);
    r2.z = (r1.z) * (-(r10.x)) + (r1.w);
    r1 = tex2D(s0, v0.xy);
    r2.w = (r1.w) * (v6.x) + (c1.x);
    r1 = (r1.xyzx) * (c1.yyyz) + (c1.zzzy);
    r8.y = (r2.z) + (r2.z);
    r1 = float4(((r2.w) >= 0.0f ? (r1.x) : (c1.z)), ((r2.w) >= 0.0f ? (r1.y) : (c1.z)), ((r2.w) >= 0.0f ? (r1.z) : (c1.z)), ((r2.w) >= 0.0f ? (r1.w) : (c1.z)));
    r2.xyz = (r8.xyz) * (r3.www) + (r3.xyz);
    r11.xyz = float3(((r4.w) >= 0.0f ? (c[36].x) : (r1.w)), ((r4.w) >= 0.0f ? (c[36].y) : (r1.w)), ((r4.w) >= 0.0f ? (c[36].w) : (r1.w)));
    r0 = float4(((r4.w) >= 0.0f ? (r0.x) : (r1.x)), ((r4.w) >= 0.0f ? (r0.y) : (r1.y)), ((r4.w) >= 0.0f ? (r0.z) : (r1.z)), ((r4.w) >= 0.0f ? (r0.w) : (r1.w)));
    r13.xyz = (r2.xyz) * (r11.yyy);
    r2 = tex2D(s12, v0.zw);
    r1 = v1;
    r1.xyz = (r6.xxx) * (v4.xyz) + (r1.xyz);
    r1.xyz = (r6.yyy) * (v3.xyz) + (r1.xyz);
    r10.xyz = normalize(r1.xyz);
    r2.z = saturate(dot(r10.xyz, c[17].xyz));
    r6.xyz = (r2.zzz) * (c[18].xyz);
    r3 = tex2D(s5, v6.zw);
    r1.z = dot(v5.xyz, v5.xyz);
    r3 = float4(((r4.w) >= 0.0f ? (r3.x) : (c1.z)), ((r4.w) >= 0.0f ? (r3.y) : (c1.z)), ((r4.w) >= 0.0f ? (r3.z) : (c1.z)), ((r4.w) >= 0.0f ? (r3.w) : (c1.y)));
    r4.w = rsqrt(r1.z);
    r8.w = (r3.w) * (c2.z);
    r1.xyz = (r4.www) * (v5.xyz);
    r7.w = (r3.w) * (-(c12.z)) + (c12.w);
    r2.w = saturate(dot(r10.xyz, -(r1.xyz)));
    r2.x = (r2.z) * (r7.w) + (r8.w);
    r5.w = (r2.w) * (r7.w) + (r8.w);
    r12.xyz = (v5.xyz) * (-(r4.www)) + (c[17].xyz);
    r2.x = (r2.x) * (r5.w) + (c3.x);
    r2.x = 1.0f / (r2.x);
    r4.xyz = normalize(r12.xyz);
    r9.w = (r2.z) * (r2.x);
    r4.w = saturate(dot(r10.xyz, r4.xyz));
    r2.z = saturate(dot(r4.xyz, c[17].xyz));
    r14.xy = (r3.ww) * (c13.xy) + (c13.zw);
    r6.w = exp2(r14.y);
    r2.x = (-(r2.z)) + (c1.y);
    r2.z = pow(abs(r4.w), r6.w);
    r4.z = (r2.x) * (r2.x);
    r4.w = (r6.w) * (c3.y) + (c3.z);
    r4.z = (r4.z) * (r4.z);
    r2.z = (r2.z) * (r4.w);
    r2.x = (r2.x) * (r4.z);
    r4.xyz = (r3.xyz) * (r3.xyz);
    r3.xyz = (r3.xyz) * (-(r3.xyz)) + (c1.yyy);
    r2.z = (r9.w) * (r2.z);
    r12.xyz = (r3.xyz) * (r2.xxx) + (r4.xyz);
    r6.xyz = (r2.yyy) * (r6.xyz) + (r13.xyz);
    r12.xyz = (r2.zzz) * (r12.xyz);
    r10.w = (-(r2.w)) + (c1.y);
    r12.xyz = (r11.zzz) * (r12.xyz);
    r9.w = 1.0f / (r14.x);
    r12.xyz = (r12.xyz) * (c[19].xyz);
    r2.w = (r3.w) * (c1.w);
    r2.xyz = (r2.yyy) * (r12.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r3.w = dot(r1.xyz, r10.xyz);
    r6.xyz = (r0.xyz) * (r6.xyz) + (r2.xyz);
    r2.z = (r3.w) + (r3.w);
    r2.xyz = (r10.xyz) * (-(r2.zzz)) + (r1.xyz);
    r1.xyz = (r9.xyz) * (r11.www) + (-(r1.xyz));
    r2 = texCUBElod(s15, r2);
    r2.w = (r10.w) * (r10.w);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r10.w) * (r2.w);
    r2.xyz = (r11.xxx) * (r2.xyz);
    r2.w = (r9.w) * (r2.w);
    r9.xyz = (r3.xyz) * (r2.www) + (r4.xyz);
    r2.w = saturate(dot(r5.xyz, r10.xyz));
    r2.xyz = (r2.xyz) * (r9.xyz);
    r3.w = (r2.w) * (r7.w) + (r8.w);
    r2.xyz = (r8.xyz) * (r2.xyz);
    r3.w = (r3.w) * (r5.w) + (c3.x);
    r8.xyz = (r2.xyz) * (c1.www) + (r6.xyz);
    r2.z = 1.0f / (r3.w);
    r3.w = (r2.w) * (r2.z);
    r2.xyz = normalize(r1.xyz);
    r1.xyz = (r2.www) * (c[21].xyz);
    r2.w = saturate(dot(r2.xyz, r5.xyz));
    r5.w = saturate(dot(r10.xyz, r2.xyz));
    r2.w = (-(r2.w)) + (c1.y);
    r2.y = pow(abs(r5.w), r6.w);
    r2.z = (r2.w) * (r2.w);
    r2.y = (r4.w) * (r2.y);
    r2.z = (r2.z) * (r2.z);
    r6.w = (r3.w) * (r2.y);
    r2.w = (r2.w) * (r2.z);
    r6.xyz = (r3.xyz) * (r2.www) + (r4.xyz);
    r5 = (-(v5.yyyy)) + (c[6]);
    r2 = (r5) * (r5);
    r4 = (-(v5.xxxx)) + (c[5]);
    r2 = (r4) * (r4) + (r2);
    r3 = (-(v5.zzzz)) + (c[7]);
    r6.xyz = (r6.www) * (r6.xyz);
    r2 = (r3) * (r3) + (r2);
    r9.xyz = (r6.xyz) * (c[22].xyz);
    r6.x = rsqrt(r2.x);
    r6.y = rsqrt(r2.y);
    r6.z = rsqrt(r2.z);
    r6.w = rsqrt(r2.w);
    r9.xyz = (r11.zzz) * (r9.xyz);
    r5 = (r5) * (r6);
    r5 = (r10.yyyy) * (r5);
    r4 = (r4) * (r6);
    r3 = (r3) * (r6);
    r4 = (r4) * (r10.xxxx) + (r5);
    r5.z = c1.y;
    r2 = saturate((r2) * (c[8]) + (r5.zzzz));
    r3 = saturate((r3) * (r10.zzzz) + (r4));
    r1.xyz = (r0.xyz) * (r1.xyz) + (r9.xyz);
    r2 = (r2) * (r3);
    r3.xyz = (r7.xyz) * (r1.xyz) + (r8.xyz);
    r1.x = dot(c[9], r2);
    r1.y = dot(c[10], r2);
    r1.z = dot(c[11], r2);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[37].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
