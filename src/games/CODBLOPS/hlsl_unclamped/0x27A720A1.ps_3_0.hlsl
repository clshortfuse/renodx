// Mechanically reconstructed from 0x27A720A1.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);

struct PS_INPUT
{
    float4 v0 : TEXCOORD5;
    float4 v1 : TEXCOORD6;
    float4 v2 : TEXCOORD7;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD9;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    const float4 c1 = float4(6.28318548f, -3.14159274f, 0.0f, 1.0f);
    const float4 c2 = float4(-0.5f, 0.5f, 1.0f, 1.52587891e-05f);
    const float4 c3 = float4(0.707106769f, 0.5f, -2.0f, 3.0f);
    const float4 c4 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c12 = float4(0.25f, 0.75f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v4.xy) + (c2.xx);
    r0.z = c2.y;
    r0.xy = (r0.xy) * (c[21].xy) + (r0.zz);
    r0 = tex2D(s0, r0.xy);
    r3.xy = (r0.xy) * (c[22].xx);
    r0.w = (r3.y) * (c2.z) + (c2.y);
    r0.xyz = ddy(v4.xyx);
    r0.w = frac(r0.w);
    r2.xyz = (r0.xyy) * (r0.xyz);
    r1.w = (r0.w) * (c1.x) + (c1.y);
    r1.xyz = ddx(v4.xyx);
    r0.xy = float2(cos(r1.w), sin(r1.w));
    r0.w = (r3.x) * (r3.x);
    r0.xyz = (r0.xyx) * (r0.xyy);
    r1.xyz = (r1.xyz) * (r1.xyy) + (r2.xyz);
    r0.z = (r0.w) * (r0.z);
    r0.xy = (r0.ww) * (r0.xy) + (r0.ww);
    r1.xyz = (r0.xyz) * (c2.www) + (r1.xyz);
    r0.x = rsqrt(r1.x);
    r0.y = rsqrt(r1.y);
    r2.x = 1.0f / (r0.x);
    r2.y = 1.0f / (r0.y);
    r0.w = (r2.y) * (r2.x);
    r0.w = 1.0f / (r0.w);
    r0.w = (r1.z) * (r0.w);
    r3.xy = frac(v4.xy);
    r0.x = ((-abs(r1.z)) >= 0.0f ? (c1.z) : (r0.w));
    r3.xy = (r3.xy) + (c2.xx);
    r0.y = dot(v1.xyz, v1.xyz);
    r0.w = dot(r3.xy, r3.xy) + (c2.x);
    r0.z = (r0.y) * (r0.w);
    r0.w = dot(v1.xy, r3.xy) + (c1.z);
    r1.z = (-(r1.x)) + (r1.y);
    r0.w = (r0.w) * (r0.w) + (-(r0.z));
    r0.z = (r0.x) * (-(r0.x)) + (c1.w);
    r0.w = rsqrt(r0.w);
    r0.z = rsqrt(r0.z);
    r0.w = 1.0f / (r0.w);
    r0.y = 1.0f / (r0.y);
    r0.w = dot(v1.xy, -(r3.xy)) + (r0.w);
    r0.z = 1.0f / (r0.z);
    r1.w = (r0.y) * (r0.w);
    r0.yw = c1.wz;
    r4.xy = (r1.ww) * (v1.xy);
    r0 = float4(((r1.z) >= 0.0f ? (r0.x) : (r0.y)), ((r1.z) >= 0.0f ? (r0.y) : (r0.x)), ((r1.z) >= 0.0f ? (r0.z) : (r0.w)), ((r1.z) >= 0.0f ? (r0.w) : (r0.z)));
    r1.xy = ddx(r4.xy);
    r3.xy = (v1.xy) * (r1.ww) + (r3.xy);
    r1.xy = (r2.xy) * (r0.xy) + (r1.xy);
    r0.xy = ddy(r4.xy);
    r1.xy = (r1.xy) * (c[23].xy);
    r0.xy = (r2.xy) * (r0.zw) + (r0.xy);
    r1.xy = (r1.xy) * (c3.xx);
    r2.xy = (r0.xy) * (c[23].xy);
    r0.xy = (r3.xy) * (c[23].xy);
    r2.xy = (r2.xy) * (c3.xx);
    r0.xy = (r0.xy) * (c3.xx) + (c3.yy);
    r1 = tex2Dgrad(s1, r0.xy, r1.xy, r2.xy);
    r0.xyz = v0.xyz;
    r2.xyz = (r0.yzx) * (v3.zxy);
    r0.xyz = (v3.yzx) * (r0.zxy) + (-(r2.xyz));
    r2.xyz = (r0.xyz) * (v0.www);
    r0 = tex2D(s2, v4.xy);
    r3.xy = (r0.wy) * (c4.xy) + (c4.zw);
    r0.xyz = (r2.xyz) * (r3.yyy);
    r0.xyz = (r3.xxx) * (v0.xyz) + (r0.xyz);
    r0.xyz = (r0.xyz) + (v3.xyz);
    r2.xyz = normalize(r0.xyz);
    r4.xyz = normalize(-(v2.xyz));
    r0.xyz = (-(v2.xyz)) + (c[5].xyz);
    r0.w = dot(r4.xyz, r2.xyz);
    r6.y = dot(r0.xyz, r0.xyz);
    r0.w = (r0.w) * (-(r0.w)) + (c1.w);
    r3.w = rsqrt(r6.y);
    r0.w = rsqrt(r0.w);
    r6.x = 1.0f / (r3.w);
    r0.w = 1.0f / (r0.w);
    r5.xy = saturate((r6.xx) * (c[8].xy) + (c[8].zw));
    r3.xy = (r5.xy) * (r5.xy);
    r5.xy = (r5.xy) * (c3.zz) + (c3.ww);
    r2.w = dot(c[7].yz, r6.xy) + (c[7].x);
    r5.xy = (r3.xy) * (r5.xy);
    r0.w = saturate((r0.w) * (c12.x));
    r2.w = (r2.w) * (r5.x);
    r3.xyz = (r0.xyz) * (r3.www);
    r0.z = (r5.y) * (r2.w);
    r0.xyz = (r0.zzz) * (c[6].xyz);
    r3.w = saturate(dot(r4.xyz, r3.xyz));
    r2.w = saturate(dot(r2.xyz, r3.xyz));
    r2.xyz = (r0.xyz) * (r3.www);
    r0.xyz = (r0.xyz) * (r2.www);
    r2.xyz = (r0.www) * (r2.xyz);
    r2.xyz = (r0.xyz) * (c12.yyy) + (r2.xyz);
    r0 = tex2D(s3, v4.xy);
    r0.xyz = saturate((r2.xyz) * (r0.xyz));
    r2.xyz = (r1.xyz) * (c[24].xyz);
    r0.xyz = (r1.xyz) * (-(c[24].xyz)) + (r0.xyz);
    r1.x = c1.w;
    r1.w = (r1.w) * (-(c[24].w)) + (r1.x);
    r1.xyz = normalize(v2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (r2.xyz);
    r1.z = dot(c[9].xyz, r1.xyz);
    r2.w = saturate((c[11].y) * (r1.z) + (c[11].x));
    r1.xyz = c[0].xyz;
    r1.xyz = (-(r1.xyz)) + (c[10].xyz);
    r0.w = (-(r0.w)) + (c1.w);
    r1.xyz = (r2.www) * (r1.xyz) + (c[0].xyz);
    r0.w = (r1.w) * (-(r0.w)) + (c1.w);
    r1.xyz = (r1.xyz) * (c[0].www);
    r0.xyz = (r0.xyz) * (r0.www) + (-(r1.xyz));
    r0.xyz = (v1.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[20].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
