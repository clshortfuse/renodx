// Mechanically reconstructed from 0x015C8C3C.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(1.0f, -0.0f, -1.0f, 8.0f);
    const float4 c2 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c3 = float4(31.875f, 3.5f, 1.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s0, v1.xy);
    r4.xyz = (r0.xyz) * (v0.xyz);
    r0.xy = (v1.xy) + (c2.xx);
    r0.z = c2.y;
    r1.xy = (r0.xy) * (c[20].xx) + (r0.zz);
    r0.xy = (r0.xy) * (c[11].xx) + (r0.zz);
    r2 = tex2D(s4, r1.xy);
    r0 = tex2D(s3, r0.xy);
    r4.w = (v7.w) * (c2.z) + (-(r0.x));
    r3.w = saturate((r4.w) * (c2.w));
    r1 = tex2D(s1, v1.xy);
    r6.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s5, v1.xy);
    r1.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r3.xyz = (r4.xyz) * (-(r4.xyz)) + (r2.xyz);
    r5.xy = lerp(r6.xy, r1.xy, r3.ww);
    r2.xyz = (r4.xyz) * (r4.xyz);
    r1 = v2;
    r1.xyz = (r5.xxx) * (v5.xyz) + (r1.xyz);
    r0.w = (r2.w) + (c1.z);
    r1.xyz = (r5.yyy) * (v4.xyz) + (r1.xyz);
    r0.xyz = (r0.yyy) * (r3.xyz);
    r4.xyz = normalize(r1.xyz);
    r2.w = c1.x;
    r3.z = max(abs(r4.y), abs(r4.z));
    r0 = (r3.wwww) * (r0) + (r2);
    r1.z = max(abs(r4.x), r3.z);
    r2.w = 1.0f / (r1.z);
    r1.xyz = (r4.xyz) * (c[5].xyz);
    r5.w = (-(r3.w)) + (c1.x);
    r1.xyz = (r1.xyz) * (r2.www) + (v7.xyz);
    r2 = tex3D(s11, r1.xyz);
    r5.xyz = normalize(v6.xyz);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r2.w = dot(r5.xyz, r4.xyz);
    r1.xyz = (r1.xyz) * (c[6].yyy);
    r2.w = (r2.w) + (r2.w);
    r0.xyz = (r0.xyz) * (r1.xyz);
    r3.xyz = (r4.xyz) * (-(r2.www)) + (r5.xyz);
    r2 = tex2D(s2, v1.xy);
    r3.w = (r2.w) * (c1.w);
    r3 = texCUBElod(s15, r3);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r5.z = saturate(dot(r4.xyz, -(r5.xyz)));
    r4.xyz = (r1.xyz) * (c1.www);
    r3 = tex3D(s11, v7.xyz);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r3.z = (-(r5.z)) + (c1.x);
    r3.y = (r3.z) * (r3.z);
    r3.w = (r2.w) * (c3.y) + (c3.z);
    r2.w = (r3.z) * (r3.y);
    r3.w = 1.0f / (r3.w);
    r2.w = (r2.w) * (r3.w);
    r3.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.xxx);
    r1.xyz = (r4.xyz) * (r1.xyz);
    r3.xyz = (r2.www) * (r3.xyz);
    r1.xyz = (r1.xyz) * (c3.xxx);
    r2.xyz = (r2.xyz) * (r2.xyz) + (r3.xyz);
    r1.xyz = (r1.xyz) * (r2.xyz);
    r2.w = ((r4.w) >= 0.0f ? (r5.w) : (c1.y));
    r1.xyz = (r1.xyz) * (c[6].xxx);
    r2.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c3.xxx) + (r1.xyz);
    r1.z = (r2.w) * (c[21].w);
    r2.xyz = (c[21].xyz) * (r1.zzz) + (r0.xyz);
    r2.w = c1.x;
    r0.x = dot(r2, c[8]);
    r0.y = dot(r2, c[9]);
    r0.z = dot(r2, c[10]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[7].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
