// Mechanically reconstructed from 0xA26C8930.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
    float4 v8 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-1.0f, 1.0f, -0.0f, 8.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.5f, -0.0f, 4.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s2, v7.xy);
    r8.w = (r1.w) * (v0.y);
    r6.xyz = (-(v6.xyz)) + (c[5].xyz);
    r14.xy = (r8.ww) * (-(r0.xy)) + (r0.xy);
    r13.y = dot(r6.xyz, r6.xyz);
    r0 = v2;
    r0.xyz = (r14.xxx) * (v5.xyz) + (r0.xyz);
    r1.w = rsqrt(r13.y);
    r2.xyz = (r14.yyy) * (v4.xyz) + (r0.xyz);
    r5.xyz = (r6.xyz) * (r1.www);
    r0.xyz = normalize(r2.xyz);
    r5.w = saturate(dot(r0.xyz, r5.xyz));
    r4 = tex2D(s5, v1.xy);
    r3 = tex2D(s6, v7.xy);
    r2 = lerp(r4, r3, r8.wwww);
    r3.x = (r2.w) * (c2.z);
    r10.xyz = normalize(v6.xyz);
    r3.y = (r2.w) * (-(c12.z)) + (c12.w);
    r3.z = saturate(dot(r0.xyz, -(r10.xyz)));
    r3.w = (r5.w) * (r3.y) + (r3.x);
    r3.y = (r3.z) * (r3.y) + (r3.x);
    r3.w = (r3.w) * (r3.y) + (c3.x);
    r7.w = (-(r3.z)) + (c1.y);
    r3.w = 1.0f / (r3.w);
    r4.w = (r5.w) * (r3.w);
    r4.xyz = (r6.xyz) * (r1.www) + (-(r10.xyz));
    r11.xyz = (r5.www) * (c[6].xyz);
    r3.xyz = normalize(r4.xyz);
    r13.x = 1.0f / (r1.w);
    r4.y = saturate(dot(r0.xyz, r3.xyz));
    r1.w = saturate(dot(r3.xyz, r5.xyz));
    r3.xy = (r2.ww) * (c13.xy) + (c13.zw);
    r4.z = exp2(r3.y);
    r6.w = 1.0f / (r3.x);
    r3.z = pow(abs(r4.y), r4.z);
    r3.w = (r4.z) * (c3.y) + (c3.z);
    r3.w = (r3.z) * (r3.w);
    r1.w = (-(r1.w)) + (c1.y);
    r4.w = (r4.w) * (r3.w);
    r4.z = (r1.w) * (r1.w);
    r3 = tex2D(s3, v7.zw);
    r3.xyz = (r3.xyz) + (c1.xxx);
    r4.z = (r4.z) * (r4.z);
    r12.xyz = (v0.zzz) * (r3.xyz) + (c1.yyy);
    r3 = tex2D(s4, v8.xy);
    r3.xyz = (r3.xyz) + (c1.xxx);
    r2.xyz = (r2.xyz) * (r12.xyz);
    r6.xyz = (v0.www) * (r3.xyz) + (c1.yyy);
    r3.w = (r1.w) * (r4.z);
    r2.xyz = (r2.xyz) * (r6.xyz);
    r8.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.yyy);
    r1.w = (r2.w) * (c1.w);
    r2.xyz = (r7.xyz) * (r3.www) + (r8.xyz);
    r2.xyz = (r4.www) * (r2.xyz);
    r3.xyw = c[10].xyw;
    r3.xyz = (-(r3.xyw)) + (c[11].xyw);
    r9.xyz = (r8.www) * (r3.xyz) + (c[10].xyw);
    r4.xy = saturate((r13.xx) * (c[9].xy) + (c[9].zw));
    r3.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c14.xx) + (c14.yy);
    r2.w = dot(c[8].yz, r13.xy) + (c[8].x);
    r3.xy = (r3.xy) * (r4.xy);
    r2.xyz = (r2.xyz) * (r9.zzz);
    r2.w = (r2.w) * (r3.x);
    r13.xyz = (r2.xyz) * (c[7].xyz);
    r9.w = (r3.y) * (r2.w);
    r2 = tex2D(s12, v1.zw);
    r3.xy = (v1.zw) * (c4.xy);
    r3 = tex2D(s13, r3.xy);
    r4.xy = (v1.zw) * (c4.xy) + (c4.zy);
    r5 = tex2D(s13, r4.xy);
    r3.w = r5.y;
    r4.xy = (r3.yw) * (c12.xx) + (c12.yy);
    r2.w = dot(r14.xy, r14.xy) + (c1.z);
    r2.z = dot(r4.xy, r4.xy) + (c1.z);
    r2.w = exp2(-(r2.w));
    r2.z = exp2(-(r2.z));
    r2.w = (r2.w) * (c2.x) + (c2.y);
    r2.x = (r2.z) * (c2.x) + (c2.y);
    r2.z = dot(r4.xy, r14.xy) + (c1.z);
    r2.x = (r2.w) * (r2.x);
    r4 = tex2D(s14, v1.zw);
    r15.xy = (r4.xy) * (c2.ww);
    r2.x = saturate((r2.z) * (r2.x) + (r2.x));
    r5.xy = (r5.xz) * (r15.yy);
    r2.z = (r4.y) * (c2.w) + (-(r5.x));
    r14.xz = (r5.xy) * (c4.ww);
    r2.z = (r5.z) * (-(r15.y)) + (r2.z);
    r3.xy = (r3.xz) * (r15.xx);
    r14.y = (r2.z) + (r2.z);
    r2.z = (r4.x) * (c2.w) + (-(r3.x));
    r4.xyz = (r2.xxx) * (r14.xyz);
    r2.z = (r3.z) * (-(r15.x)) + (r2.z);
    r3.xz = (r3.xy) * (c4.ww);
    r3.y = (r2.z) + (r2.z);
    r4.xyz = (r3.xyz) * (r2.www) + (r4.xyz);
    r2.w = (r9.w) * (r2.y);
    r2.xyz = (r9.yyy) * (r4.xyz);
    r5.xyz = (r13.xyz) * (r2.www);
    r4.xyz = (r2.www) * (r11.xyz) + (r2.xyz);
    r2 = tex2D(s0, v1.xy);
    r11.xyz = lerp(r2.xyz, r1.xyz, r8.www);
    r2.w = dot(r10.xyz, r0.xyz);
    r1.xyz = (r12.xyz) * (r11.xyz);
    r2.w = (r2.w) + (r2.w);
    r6.xyz = (r6.xyz) * (r1.xyz);
    r1.xyz = (r0.xyz) * (-(r2.www)) + (r10.xyz);
    r1 = texCUBElod(s15, r1);
    r0.z = (r7.w) * (r7.w);
    r1.w = (r7.w) * (r0.z);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r6.w) * (r1.w);
    r0.xyz = (r9.xxx) * (r0.xyz);
    r2.xyz = (r7.xyz) * (r1.www) + (r8.xyz);
    r1.xyz = (r6.xyz) * (r6.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r1.xyz) * (r4.xyz) + (r5.xyz);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c1.www) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[20].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.y;

    return oC0;
}
