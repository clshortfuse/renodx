// Mechanically reconstructed from 0xDFF60565.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD4;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(-0.5f, 1.0f, 0.0f, 8.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    const float4 c14 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s0, v0.xy);
    r1.w = (r0.w) * (v7.x) + (c0.x);
    r0 = (r0.xyzx) * (c0.yyyz) + (c0.zzzy);
    r1 = float4(((r1.w) >= 0.0f ? (r0.x) : (c0.z)), ((r1.w) >= 0.0f ? (r0.y) : (c0.z)), ((r1.w) >= 0.0f ? (r0.z) : (c0.z)), ((r1.w) >= 0.0f ? (r0.w) : (c0.z)));
    r2 = tex2D(s2, v7.zw);
    r0 = (r2.xyzx) * (c0.yyyz) + (c0.zzzy);
    r3.w = (r2.w) * (v7.y) + (c0.x);
    r0 = float4(((r3.w) >= 0.0f ? (r0.x) : (r1.x)), ((r3.w) >= 0.0f ? (r0.y) : (r1.y)), ((r3.w) >= 0.0f ? (r0.z) : (r1.z)), ((r3.w) >= 0.0f ? (r0.w) : (r1.w)));
    r10.xyz = float3(((r3.w) >= 0.0f ? (c[21].x) : (r1.w)), ((r3.w) >= 0.0f ? (c[21].y) : (r1.w)), ((r3.w) >= 0.0f ? (c[21].w) : (r1.w)));
    r1.xy = (v5.ww) * (c[20].xy) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r2.xy = (v5.ww) * (-(c[20].xy)) + (v5.xy);
    r2.zw = v5.zw;
    r2 = tex2Dproj(s1, r2);
    r1.y = r2.x;
    r2.xy = (v5.ww) * (c[20].zw) + (v5.xy);
    r2.zw = v5.zw;
    r2 = tex2Dproj(s1, r2);
    r1.z = r2.x;
    r2.xy = (v5.ww) * (-(c[20].zw)) + (v5.xy);
    r2.zw = v5.zw;
    r2 = tex2Dproj(s1, r2);
    r1.w = r2.x;
    r6.xyz = (r0.xyz) * (r0.xyz);
    r4.w = dot(r1, c3.zzzz);
    r1 = tex2D(s12, v0.zw);
    r0.xyz = (-(v6.xyz)) + (c[5].xyz);
    r5.y = dot(r0.xyz, r0.xyz);
    r2.z = rsqrt(r5.y);
    r3.xyz = (r0.xyz) * (r2.zzz);
    r1.w = dot(r3.xyz, c[8].xyz);
    r1.w = saturate((r1.w) * (c[9].x) + (c[9].y));
    r5.x = 1.0f / (r2.z);
    r1.z = (r1.w) * (c[9].w);
    r4.xy = saturate((r5.xx) * (c[11].xy) + (c[11].zw));
    r2.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c13.xx) + (c13.yy);
    r1.w = dot(c[10].yz, r5.xy) + (c[10].x);
    r2.xy = (r2.xy) * (r4.xy);
    r2.w = lerp(r1.y, r4.w, r1.z);
    r2.x = (r1.w) * (r2.x);
    r1 = tex2D(s3, v7.zw);
    r1.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r4.w = (r2.y) * (r2.x);
    r4.xy = float2(((r3.w) >= 0.0f ? (r1.x) : (c0.z)), ((r3.w) >= 0.0f ? (r1.y) : (c0.z)));
    r11.xyz = normalize(v6.xyz);
    r1 = v1;
    r1.xyz = (r4.xxx) * (v4.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) * (r2.zzz) + (-(r11.xyz));
    r2.xyz = (r4.yyy) * (v3.xyz) + (r1.xyz);
    r1.xyz = normalize(r0.xyz);
    r0.xyz = normalize(r2.xyz);
    r6.w = (r2.w) * (r4.w);
    r4.w = saturate(dot(r0.xyz, r1.xyz));
    r2 = tex2D(s4, v7.zw);
    r2 = float4(((r3.w) >= 0.0f ? (r2.x) : (c0.z)), ((r3.w) >= 0.0f ? (r2.y) : (c0.z)), ((r3.w) >= 0.0f ? (r2.z) : (c0.z)), ((r3.w) >= 0.0f ? (r2.w) : (c0.y)));
    r1.z = saturate(dot(r1.xyz, r3.xyz));
    r1.xy = (r2.ww) * (c12.xy) + (c12.zw);
    r3.w = saturate(dot(r0.xyz, r3.xyz));
    r1.y = exp2(r1.y);
    r3.z = pow(abs(r4.w), r1.y);
    r1.y = (r1.y) * (c3.y) + (c3.z);
    r7.w = 1.0f / (r1.x);
    r1.y = (r3.z) * (r1.y);
    r3.x = (r2.w) * (c2.z);
    r3.y = (r2.w) * (-(c14.z)) + (c14.w);
    r1.x = (r3.w) * (r3.y) + (r3.x);
    r3.z = saturate(dot(r0.xyz, -(r11.xyz)));
    r3.y = (r3.z) * (r3.y) + (r3.x);
    r1.z = (-(r1.z)) + (c0.y);
    r1.x = (r1.x) * (r3.y) + (c3.x);
    r3.y = (r1.z) * (r1.z);
    r1.x = 1.0f / (r1.x);
    r3.y = (r3.y) * (r3.y);
    r1.x = (r3.w) * (r1.x);
    r1.z = (r1.z) * (r3.y);
    r9.xyz = (r2.xyz) * (r2.xyz);
    r8.xyz = (r2.xyz) * (-(r2.xyz)) + (c0.yyy);
    r2.z = (r1.y) * (r1.x);
    r1.xyz = (r8.xyz) * (r1.zzz) + (r9.xyz);
    r8.w = (-(r3.z)) + (c0.y);
    r2.xyz = (r2.zzz) * (r1.xyz);
    r1.xyz = (r3.www) * (c[6].xyz);
    r2.xyz = (r10.zzz) * (r2.xyz);
    r2.w = (r2.w) * (c0.w);
    r2.xyz = (r2.xyz) * (c[7].xyz);
    r7.xyz = (r6.www) * (r2.xyz);
    r2.xy = (v0.zw) * (c4.xy);
    r3 = tex2D(s13, r2.xy);
    r2.xy = (v0.zw) * (c4.xy) + (c4.zy);
    r5 = tex2D(s13, r2.xy);
    r3.w = r5.y;
    r2.z = dot(r4.xy, r4.xy) + (c0.z);
    r2.xy = (r3.yw) * (c14.xx) + (c14.yy);
    r3.w = exp2(-(r2.z));
    r2.z = dot(r2.xy, r2.xy) + (c0.z);
    r5.w = (r3.w) * (c2.x) + (c2.y);
    r3.w = exp2(-(r2.z));
    r2.z = dot(r2.xy, r4.xy) + (c0.z);
    r2.y = (r3.w) * (c2.x) + (c2.y);
    r4 = tex2D(s14, v0.zw);
    r12.xy = (r4.xy) * (c2.ww);
    r3.w = (r5.w) * (r2.y);
    r2.xy = (r5.xz) * (r12.yy);
    r3.w = saturate((r2.z) * (r3.w) + (r3.w));
    r3.y = (r4.y) * (c2.w) + (-(r2.x));
    r2.xz = (r2.xy) * (c4.ww);
    r2.y = (r5.z) * (-(r12.y)) + (r3.y);
    r2.y = (r2.y) + (r2.y);
    r3.xy = (r3.xz) * (r12.xx);
    r2.xyz = (r3.www) * (r2.xyz);
    r3.w = (r4.x) * (c2.w) + (-(r3.x));
    r4.xz = (r3.xy) * (c4.ww);
    r3.w = (r3.z) * (-(r12.x)) + (r3.w);
    r4.y = (r3.w) + (r3.w);
    r3.w = dot(r11.xyz, r0.xyz);
    r2.xyz = (r4.xyz) * (r5.www) + (r2.xyz);
    r3.w = (r3.w) + (r3.w);
    r3.xyz = (r10.yyy) * (r2.xyz);
    r2.xyz = (r0.xyz) * (-(r3.www)) + (r11.xyz);
    r2 = texCUBElod(s15, r2);
    r0.z = (r8.w) * (r8.w);
    r2.w = (r8.w) * (r0.z);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r7.w) * (r2.w);
    r0.xyz = (r10.xxx) * (r0.xyz);
    r2.xyz = (r8.xyz) * (r2.www) + (r9.xyz);
    r1.xyz = (r6.www) * (r1.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r6.xyz) * (r1.xyz) + (r7.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c0.www) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[23].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = (r0.w) * (c[22].w);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
