// Mechanically reconstructed from 0x5008DA0A.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD4;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(-0.5f, 0.200000003f, 8.0f, 31.875f);
    const float4 c1 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.75f);
    const float4 c3 = float4(0.125f, 0.25f, 0.000244140625f, 0.0f);
    const float4 c4 = float4(4.0f, -3.0f, -4.0f, 2.0f);
    const float4 c12 = float4(1.0f, 0.797884583f, 0.0009765625f, 0.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v5.xyz, v5.xyz);
    r1.xyz = (-(v5.xyz)) + (c[21].xyz);
    r4.w = rsqrt(r0.w);
    r0.w = dot(r1.xyz, r1.xyz);
    r0.w = rsqrt(r0.w);
    r0.xyz = (r4.www) * (v5.xyz);
    r3.xyz = (r1.xyz) * (r0.www) + (-(r0.xyz));
    r2.xyz = normalize(r3.xyz);
    r3.xyz = (r1.xyz) * (r0.www);
    r0.w = saturate(dot(r2.xyz, r3.xyz));
    r4.z = (-(r0.w)) + (c12.x);
    r1.w = (r4.z) * (r4.z);
    r0.w = dot(r3.xyz, c[30].xyz);
    r4.y = (r1.w) * (r1.w);
    r1 = tex2D(s4, v1.xy);
    r12.z = (r1.w) * (-(c12.y)) + (c12.x);
    r12.y = (r1.w) * (c12.y);
    r9.xyz = normalize(v2.xyz);
    r1.z = saturate(dot(r3.xyz, r9.xyz));
    r2.w = saturate(dot(r9.xyz, -(r0.xyz)));
    r3.w = (r1.z) * (r12.z) + (r12.y);
    r12.w = (r2.w) * (r12.z) + (r12.y);
    r9.w = (r4.z) * (r4.y);
    r3.w = (r3.w) * (r12.w) + (c12.z);
    r3.w = 1.0f / (r3.w);
    r2.w = (-(r2.w)) + (c12.x);
    r3.z = (r1.z) * (r3.w);
    r3.w = (r2.w) * (r2.w);
    r2.w = (r2.w) * (r3.w);
    r3.w = dot(r0.xyz, r9.xyz);
    r3.w = (r3.w) + (r3.w);
    r3.xy = (r1.ww) * (c13.xy) + (c13.zw);
    r4.z = saturate(dot(r9.xyz, r2.xyz));
    r2.z = 1.0f / (r3.x);
    r6.w = (r2.w) * (r2.z);
    r11.w = exp2(r3.y);
    r2.w = pow(abs(r4.z), r11.w);
    r10.w = (r11.w) * (c3.x) + (c3.y);
    r2.w = (r2.w) * (r10.w);
    r2.xyz = (v5.xyz) * (-(r4.www)) + (c[17].xyz);
    r8.w = (r3.z) * (r2.w);
    r6.xyz = normalize(r2.xyz);
    r12.x = saturate(dot(r6.xyz, c[17].xyz));
    r0.w = saturate((r0.w) * (c[31].x) + (c[31].y));
    r2.z = (r0.w) * (-(c4.w)) + (-(c4.y));
    r2.w = (r0.w) * (r0.w);
    r0.w = (r1.w) * (c0.z);
    r0.xyz = (r9.xyz) * (-(r3.www)) + (r0.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r7.w = (r2.z) * (r2.w);
    r3.xyz = (r0.xyz) * (c0.zzz);
    r0 = tex3D(s11, v6.xyz);
    r1.w = max(abs(r9.y), abs(r9.z));
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.w = max(abs(r9.x), r1.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r9.xyz) * (c[5].xyz);
    r2.xyz = (r3.xyz) * (r2.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (v6.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r7.xyz = (r2.xyz) * (c0.www);
    r2.xyz = (r1.yyy) * (r0.xyz);
    r11.z = saturate(dot(r9.xyz, c[17].xyz));
    r11.xy = c[36].xy;
    r0.xyz = (r11.xxx) * (c[18].xyz);
    r10.xyz = (r2.xyz) * (c0.www);
    r8.xyz = (r11.zzz) * (r0.xyz);
    if ((c12.x) >= (v4.w))
    {
        r2 = (v4.xyzx) * (c12.xxxw);
        r0 = (r2) + (-(c1.xyzz));
        r0 = tex2Dlod(s1, r0);
        r0.w = r0.x;
        r3 = (r2) + (c3.zzww);
        r3 = tex2Dlod(s1, r3);
        r0.x = r3.x;
        r3 = (r2) + (-(c3.zzww));
        r3 = tex2Dlod(s1, r3);
        r0.y = r3.x;
        r2 = (r2) + (c1.xyzz);
        r2 = tex2Dlod(s1, r2);
        r0.z = r2.x;
        r1.w = dot(r0, c3.yyyy);
        if ((c1.w) < (v4.w))
        {
            r5.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
            r0.xy = (r5.xy) + (c3.zz);
            r0.zw = (v4.zx) * (c12.xw);
            r0 = tex2Dlod(s1, r0);
            r2.xy = (r5.xy) + (-(c3.zz));
            r2.zw = (v4.zx) * (c12.xw);
            r4 = tex2Dlod(s1, r2);
            r2.xy = (r5.xy) + (c1.xy);
            r2.zw = (v4.zx) * (c12.xw);
            r3 = tex2Dlod(s1, r2);
            r2.xy = (r5.xy) + (-(c1.xy));
            r2.zw = (v4.zx) * (c12.xw);
            r2 = tex2Dlod(s1, r2);
            r0.y = r4.x;
            r0.z = r3.x;
            r0.w = r2.x;
            r0.w = dot(r0, c3.yyyy);
            r0.z = (-(r1.w)) + (r0.w);
            r0.w = (v4.w) * (c4.x) + (c4.y);
            r1.w = (r0.w) * (r0.z) + (r1.w);
        }
    }
    else
    {
        r0.z = (v4.w) + (c4.z);
        r0.z = ((r0.z) >= 0.0f ? (c12.w) : (c12.x));
        if ((r0.z) != (-(r0.z)))
        {
            r0.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
            r2.xy = (r0.xy) + (c3.zz);
            r2.zw = (v4.zz) * (c12.xw);
            r2 = tex2Dlod(s1, r2);
            r3.xy = (r0.xy) + (-(c3.zz));
            r3.zw = (v4.zz) * (c12.xw);
            r5 = tex2Dlod(s1, r3);
            r3.xy = (r0.xy) + (c1.xy);
            r3.zw = (v4.zz) * (c12.xw);
            r4 = tex2Dlod(s1, r3);
            r3.xy = (r0.xy) + (-(c1.xy));
            r3.zw = (v4.zz) * (c12.xw);
            r3 = tex2Dlod(s1, r3);
            r2.y = r5.x;
            r2.z = r4.x;
            r2.w = r3.x;
            r0.y = dot(r2, c3.yyyy);
            r0.z = saturate((v4.w) + (c4.y));
            r0.w = (r0.w) + (-(r0.y));
            r0.w = (r0.z) * (r0.w) + (r0.y);
        }
        r1.w = r0.w;
    }
    r0.z = (-(r12.x)) + (c12.x);
    r5.xyz = (r1.www) * (r8.xyz) + (r10.xyz);
    r0.w = (r0.z) * (r0.z);
    r0.y = (r0.w) * (r0.w);
    r0.w = (r11.z) * (r12.z) + (r12.y);
    r3.w = (r0.z) * (r0.y);
    r0.w = (r0.w) * (r12.w) + (c12.z);
    r0.w = 1.0f / (r0.w);
    r2.w = saturate(dot(r9.xyz, r6.xyz));
    r4.w = (r11.z) * (r0.w);
    r0.w = pow(abs(r2.w), r11.w);
    r2.w = (r10.w) * (r0.w);
    r0.xy = (v1.xy) * (c[38].xy);
    r0 = tex2D(s5, r0.xy);
    r2.xyz = (r0.xyz) + (c0.xxx);
    r0 = tex2D(s0, v1.xy);
    r3.xyz = saturate((r2.xyz) * (r0.www) + (r0.xyz));
    r2.w = (r4.w) * (r2.w);
    r0.xyz = (r1.xxx) * (r3.xyz);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r2.xyz = lerp(r3.xyz, c0.yyy, r1.xxx);
    r4.xyz = (r2.xyz) * (-(r2.xyz)) + (c12.xxx);
    r6.xyz = (r2.xyz) * (r2.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r4.xyz) * (r9.www) + (r6.xyz);
    r3.xyz = (r8.www) * (r2.xyz);
    r2.xyz = (r4.xyz) * (r3.www) + (r6.xyz);
    r4.xyz = (r4.xyz) * (r6.www) + (r6.xyz);
    r2.xyz = (r2.www) * (r2.xyz);
    r2.xyz = (r1.yyy) * (r2.xyz);
    r6.xyz = (r11.yyy) * (c[19].xyz);
    r4.xyz = (r7.xyz) * (r4.xyz);
    r2.xyz = (r2.xyz) * (r6.xyz);
    r6.xyz = (r1.www) * (r2.xyz);
    r2 = (v5.yyyy) * (c[33]);
    r5.xyz = (r0.xyz) * (r5.xyz) + (r6.xyz);
    r2 = (v5.xxxx) * (c[32]) + (r2);
    r7.xyz = (r4.xyz) * (r1.yyy) + (r5.xyz);
    r2 = (v5.zzzz) * (c[34]) + (r2);
    r3.xyz = (r3.xyz) * (c[23].xyz);
    r4 = (r2) + (c[35]);
    r2.xyz = (r1.yyy) * (r3.xyz);
    r3.zw = r4.zw;
    r1.xyz = (r1.zzz) * (c[22].xyz);
    r5.zw = r3.zw;
    r6.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r1.zw = r5.zw;
    r2.xy = (r4.ww) * (-(c[37].zw)) + (r4.xy);
    r2.zw = r1.zw;
    r2 = tex2Dproj(s2, r2);
    r2.w = r2.x;
    r5.xy = (r3.ww) * (-(c[37].xy)) + (r4.xy);
    r5 = tex2Dproj(s2, r5);
    r2.y = r5.x;
    r3.xy = (r3.ww) * (c[37].xy) + (r4.xy);
    r5 = tex2Dproj(s2, r3);
    r2.x = r5.x;
    r1.xy = (r3.ww) * (c[37].zw) + (r4.xy);
    r1 = tex2Dproj(s2, r1);
    r2.z = r1.x;
    r1 = (v5.xyzx) * (c12.xxxw) + (c12.wwwx);
    r6.w = dot(r2, c3.yyyy);
    r2.w = dot(r1, c[29]);
    r2.w = 1.0f / (r2.w);
    r3.x = dot(r1, c[28]);
    r2.x = dot(r1, c[26]);
    r3.y = (r3.x) * (r3.x);
    r2.y = dot(r1, c[27]);
    r1.w = dot(c[24].yz, r3.xy) + (c[24].x);
    r1.z = saturate(1.0f / (r1.w));
    r1.xy = saturate((r3.xx) * (c[25].xy) + (c[25].zw));
    r3.xy = (r1.xy) * (-(c4.ww)) + (-(c4.yy));
    r1.xy = (r1.xy) * (r1.xy);
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c12.w) : (r1.z));
    r1.z = (r3.x) * (r1.x);
    r1.w = (r1.w) * (r1.z);
    r1.z = (r1.y) * (-(r3.y)) + (c12.x);
    r1.xy = (r2.ww) * (r2.xy);
    r1.w = (r1.w) * (r1.z);
    r7.w = (r7.w) * (r1.w);
    r1.xy = (r1.xy) * (-(c0.xx)) + (-(c0.xx));
    r5 = tex2D(s3, r1.xy);
    r4 = (-(v5.yyyy)) + (c[7]);
    r3 = (-(v5.xxxx)) + (c[6]);
    r1 = (r4) * (r4);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v5.zzzz)) + (c[8]);
    r1 = (r2) * (r2) + (r1);
    r8.xyz = (r5.xyz) * (r5.xyz);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r8.xyz = (r7.www) * (r8.xyz);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.w = c12.x;
    r1 = saturate((r1) * (c[9]) + (r3.wwww));
    r3.xyz = (r6.www) * (r8.xyz);
    r1 = (r2) * (r1);
    r3.xyz = (r3.xyz) * (r6.xyz) + (r7.xyz);
    r2.z = dot(c[20], r1);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r1.w = (r0.w) * (v0.w);
    r0.xyz = (r0.xyz) * (r1.www);
    r0.w = c12.x;
    r1.z = dot(r0, c[42]);
    r1.x = dot(r0, c[40]);
    r1.y = dot(r0, c[41]);
    r0.xyz = (v3.xyz) * (-(r1.www)) + (r1.xyz);
    r0.xyz = (r0.xyz) * (v2.www);
    r0.xyz = (v3.xyz) * (r1.www) + (r0.xyz);
    r0.xyz = max(((r0.xyz) * (c[39].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    r0.w = rsqrt(r1.w);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
