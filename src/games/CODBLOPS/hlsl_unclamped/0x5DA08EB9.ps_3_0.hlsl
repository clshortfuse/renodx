// Mechanically reconstructed from 0x5DA08EB9.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s8 : register(s8);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : TEXCOORD7;
    float4 v8 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(2.0f, -1.0f, 0.0f, -0.5f);
    const float4 c3 = float4(0.0f, 1.0f, 0.200000003f, 8.0f);
    const float4 c4 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c12 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c13 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c14 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c15 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 r16 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v5.xyzx) * (-(c2.yyyz)) + (-(c2.zzzy));
    r1.w = dot(r0, c[21]);
    r1.w = 1.0f / (r1.w);
    r1.x = dot(r0, c[10]);
    r1.y = dot(r0, c[11]);
    r2.xy = (r1.ww) * (r1.xy);
    r1.x = dot(r0, c[20]);
    r0.xy = (r2.xy) * (-(c2.ww)) + (-(c2.ww));
    r0 = tex2D(s2, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[8].yz, r1.xy) + (c[8].x);
    r2.xy = saturate((r1.xx) * (c[9].xy) + (c[9].zw));
    r1.w = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c2.z) : (r1.w));
    r14.xyz = (-(v5.xyz)) + (c[5].xyz);
    r1.xy = (r2.xy) * (r2.xy);
    r1.w = dot(r14.xyz, r14.xyz);
    r2.xy = (r2.xy) * (c15.xx) + (c15.yy);
    r5.w = rsqrt(r1.w);
    r1.z = (r1.x) * (r2.x);
    r10.xyz = (r14.xyz) * (r5.www);
    r1.y = (r1.y) * (-(r2.y)) + (-(c2.y));
    r1.w = dot(r10.xyz, c[22].xyz);
    r0.w = (r0.w) * (r1.z);
    r1.z = saturate((r1.w) * (c[23].x) + (c[23].y));
    r1.w = (r1.z) * (r1.z);
    r1.z = (r1.z) * (c15.x) + (c15.y);
    r0.w = (r1.y) * (r0.w);
    r1.w = (r1.w) * (r1.z);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r0.w) * (r1.w);
    r4.xyz = (r0.xyz) * (r0.www);
    r0.xy = (v1.zw) * (-(c2.yw));
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (-(c2.yw)) + (-(c2.zw));
    r3 = tex2D(s13, r1.xy);
    r0.w = r3.y;
    r1 = tex2D(s1, v1.xy);
    r6.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = tex2D(s6, v6.xy);
    r2.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = (c2.xxxx) * (v8) + (c2.yyyy);
    r5.x = dot(r2.xy, r1.xy) + (c2.z);
    r5.y = dot(r2.xy, r1.zw) + (c2.z);
    r1 = tex2D(s3, v6.xy);
    r1.w = (r1.w) * (v0.y) + (c2.w);
    r2.xy = (r0.yw) * (c13.xx) + (c13.yy);
    r6.xy = float2(((r1.w) >= 0.0f ? (r5.x) : (r6.x)), ((r1.w) >= 0.0f ? (r5.y) : (r6.y)));
    r0.w = dot(r2.xy, r2.xy) + (c2.z);
    r0.y = dot(r6.xy, r6.xy) + (c2.z);
    r0.w = exp2(-(r0.w));
    r0.y = exp2(-(r0.y));
    r0.w = (r0.w) * (c4.x) + (c4.y);
    r4.w = (r0.y) * (c4.x) + (c4.y);
    r0.y = (r0.w) * (r4.w);
    r0.w = dot(r2.xy, r6.xy) + (c2.z);
    r2 = tex2D(s14, v1.zw);
    r7.xy = (r2.xy) * (c4.ww);
    r2.w = saturate((r0.w) * (r0.y) + (r0.y));
    r3.xy = (r3.xz) * (r7.yy);
    r0.w = (r2.y) * (c4.w) + (-(r3.x));
    r5.xz = (r3.xy) * (c13.xx);
    r0.w = (r3.z) * (-(r7.y)) + (r0.w);
    r0.xy = (r0.xz) * (r7.xx);
    r5.y = (r0.w) + (r0.w);
    r0.w = (r2.x) * (c4.w) + (-(r0.x));
    r15.xyz = (r2.www) * (r5.xyz);
    r0.w = (r0.z) * (-(r7.x)) + (r0.w);
    r7.xz = (r0.xy) * (c13.xx);
    r7.y = (r0.w) + (r0.w);
    r0 = tex2D(s8, v6.xy);
    r2.xyz = v2.xyz;
    r2.xyz = (r6.xxx) * (v4.xyz) + (r2.xyz);
    r2.xyz = (r6.yyy) * (v3.xyz) + (r2.xyz);
    r11.xyz = normalize(r2.xyz);
    r5.xyz = normalize(v5.xyz);
    r6.xyz = float3(((r1.w) >= 0.0f ? (r0.y) : (c[27].x)), ((r1.w) >= 0.0f ? (r0.y) : (c[27].y)), ((r1.w) >= 0.0f ? (r0.y) : (c[27].w)));
    r2.w = dot(r5.xyz, r11.xyz);
    r0 = (r0.wwww) * (c3.xxxy) + (c3.zzzx);
    r3.w = (r2.w) + (r2.w);
    r2 = tex2D(s7, v1.xy);
    r0 = float4(((r1.w) >= 0.0f ? (r0.x) : (r2.x)), ((r1.w) >= 0.0f ? (r0.y) : (r2.y)), ((r1.w) >= 0.0f ? (r0.z) : (r2.z)), ((r1.w) >= 0.0f ? (r0.w) : (r2.w)));
    r2.xyz = (r11.xyz) * (-(r3.www)) + (r5.xyz);
    r2.w = (r0.w) * (c3.w);
    r2 = texCUBElod(s15, r2);
    r3.w = saturate(dot(r11.xyz, -(r5.xyz)));
    r2.w = (-(r3.w)) + (-(c2.y));
    r3.xyz = (r2.xyz) * (r2.xyz);
    r2.z = (r2.w) * (r2.w);
    r6.w = (r2.w) * (r2.z);
    r16.xy = (r0.ww) * (c14.xy) + (c14.zw);
    r2 = tex2D(s4, v6.zw);
    r2.xyz = (r2.xyz) + (c2.yyy);
    r7.w = 1.0f / (r16.x);
    r13.xyz = (v0.zzz) * (r2.xyz) + (-(c2.yyy));
    r2 = tex2D(s5, v7.xy);
    r2.xyz = (r2.xyz) + (c2.yyy);
    r0.xyz = (r0.xyz) * (r13.xyz);
    r12.xyz = (v0.www) * (r2.xyz) + (-(c2.yyy));
    r2.w = (r6.w) * (r7.w);
    r0.xyz = (r0.xyz) * (r12.xyz);
    r9.xyz = (r0.xyz) * (r0.xyz);
    r8.xyz = (r0.xyz) * (-(r0.xyz)) + (-(c2.yyy));
    r0.xyz = (r6.xxx) * (r3.xyz);
    r3.xyz = (r8.xyz) * (r2.www) + (r9.xyz);
    r2.xyz = (r7.xyz) * (r4.www) + (r15.xyz);
    r0.xyz = (r0.xyz) * (r3.xyz);
    r3.xyz = (r6.yyy) * (r2.xyz);
    r0.xyz = (r7.xyz) * (r0.xyz);
    r4.w = exp2(r16.y);
    r7.xyz = (r0.xyz) * (c3.www);
    r2 = tex2D(s0, v1.xy);
    r0.xyz = float3(((r1.w) >= 0.0f ? (r1.x) : (r2.x)), ((r1.w) >= 0.0f ? (r1.y) : (r2.y)), ((r1.w) >= 0.0f ? (r1.z) : (r2.z)));
    r2.xyz = (r14.xyz) * (r5.www) + (-(r5.xyz));
    r0.xyz = (r13.xyz) * (r0.xyz);
    r1.xyz = normalize(r2.xyz);
    r0.xyz = (r12.xyz) * (r0.xyz);
    r5.w = saturate(dot(r11.xyz, r1.xyz));
    r1.z = saturate(dot(r1.xyz, r10.xyz));
    r1.w = saturate(dot(r10.xyz, r11.xyz));
    r2.w = pow(abs(r5.w), r4.w);
    r1.x = (r4.w) * (c12.y) + (c12.z);
    r2.z = (r0.w) * (c4.z);
    r1.y = (r0.w) * (-(c13.z)) + (c13.w);
    r0.w = (r2.w) * (r1.x);
    r1.x = (r3.w) * (r1.y) + (r2.z);
    r1.y = (r1.w) * (r1.y) + (r2.z);
    r1.z = (-(r1.z)) + (-(c2.y));
    r1.y = (r1.y) * (r1.x) + (c12.x);
    r1.x = (r1.z) * (r1.z);
    r1.y = 1.0f / (r1.y);
    r1.x = (r1.x) * (r1.x);
    r1.y = (r1.w) * (r1.y);
    r1.z = (r1.z) * (r1.x);
    r0.w = (r0.w) * (r1.y);
    r1.xyz = (r8.xyz) * (r1.zzz) + (r9.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r0.www) * (r1.xyz);
    r2.xyz = (r0.xyz) * (r3.xyz) + (r7.xyz);
    r3.xyz = (r1.xyz) * (c[7].xyz);
    r1.xyz = (r1.www) * (c[6].xyz);
    r3.xyz = (r6.zzz) * (r3.xyz);
    r0.w = dot(c[24].xyz, r5.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r3.xyz);
    r0.w = saturate((c[26].y) * (r0.w) + (c[26].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[25].xyz);
    r1.xyz = (r4.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[28].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = -(c2.y);

    return oC0;
}
