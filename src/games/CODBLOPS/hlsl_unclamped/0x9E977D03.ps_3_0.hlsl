// Mechanically reconstructed from 0x9E977D03.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(-0.5f, 0.200000003f, 8.0f, 31.875f);
    const float4 c1 = float4(1.0f, 0.797884583f, -2.0f, 3.0f);
    const float4 c2 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c3 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r1.xyz = (-(v4.xyz)) + (c[6].xyz);
    r6.y = dot(r1.xyz, r1.xyz);
    r2.w = rsqrt(r6.y);
    r2.xyz = (r1.xyz) * (r2.www);
    r8.xyz = normalize(v2.xyz);
    r4.w = saturate(dot(r8.xyz, r2.xyz));
    r0 = tex2D(s1, v1.xy);
    r3.z = (r0.w) * (c1.y);
    r9.xyz = normalize(v4.xyz);
    r3.w = (r0.w) * (-(c1.y)) + (c1.x);
    r1.w = saturate(dot(r8.xyz, -(r9.xyz)));
    r0.z = (r4.w) * (r3.w) + (r3.z);
    r3.w = (r1.w) * (r3.w) + (r3.z);
    r10.xy = c[9].xy;
    r4.xyz = (r10.yyy) * (c[8].xyz);
    r0.z = (r0.z) * (r3.w) + (c2.x);
    r3.w = (-(r1.w)) + (c1.x);
    r0.z = 1.0f / (r0.z);
    r5.z = (r4.w) * (r0.z);
    r3.xyz = (r1.xyz) * (r2.www) + (-(r9.xyz));
    r1.xyz = normalize(r3.xyz);
    r5.xy = (r0.ww) * (c3.xy) + (c3.zw);
    r3.z = saturate(dot(r8.xyz, r1.xyz));
    r0.z = exp2(r5.y);
    r1.z = saturate(dot(r1.xyz, r2.xyz));
    r1.w = pow(abs(r3.z), r0.z);
    r0.z = (r0.z) * (c2.y) + (c2.z);
    r3.z = (-(r1.z)) + (c1.x);
    r0.z = (r1.w) * (r0.z);
    r1.w = (r3.z) * (r3.z);
    r3.y = (r1.w) * (r1.w);
    r1.xy = (v1.xy) * (c[20].xy);
    r1 = tex2D(s2, r1.xy);
    r2.xyz = (r1.xyz) + (c0.xxx);
    r1 = tex2D(s0, v1.xy);
    r1.xyz = saturate((r2.xyz) * (r1.www) + (r1.xyz));
    r5.w = (r3.z) * (r3.y);
    r2.xyz = lerp(r1.xyz, c0.yyy, r0.xxx);
    r7.xyz = (r2.xyz) * (r2.xyz);
    r3.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.xxx);
    r0.z = (r5.z) * (r0.z);
    r2.xyz = (r3.xyz) * (r5.www) + (r7.xyz);
    r6.x = 1.0f / (r2.w);
    r2.xyz = (r0.zzz) * (r2.xyz);
    r0.z = 1.0f / (r5.x);
    r2.xyz = (r0.yyy) * (r2.xyz);
    r1.xyz = (r0.xxx) * (r1.xyz);
    r5.xyz = (r4.xyz) * (r2.xyz);
    r0.x = dot(c[10].yz, r6.xy) + (c[10].x);
    r4.xy = saturate((r6.xx) * (c[11].xy) + (c[11].zw));
    r2.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c1.zz) + (c1.ww);
    r4.xy = (r2.xy) * (r4.xy);
    r4.z = max(abs(r8.y), abs(r8.z));
    r0.x = (r0.x) * (r4.x);
    r2.w = max(abs(r8.x), r4.z);
    r2.w = 1.0f / (r2.w);
    r2.xyz = (r8.xyz) * (c[5].xyz);
    r0.x = (r4.y) * (r0.x);
    r2.xyz = (r2.xyz) * (r2.www) + (v5.xyz);
    r2 = tex3D(s11, r2.xyz);
    r0.x = (r0.x) * (r2.w);
    r4.xyz = (r2.xyz) * (r2.xyz);
    r6.xyz = (r5.xyz) * (r0.xxx);
    r2.xyz = (r10.xxx) * (c[7].xyz);
    r4.xyz = (r0.yyy) * (r4.xyz);
    r2.xyz = (r4.www) * (r2.xyz);
    r4.xyz = (r4.xyz) * (c0.www);
    r5.xyz = (r0.xxx) * (r2.xyz) + (r4.xyz);
    r0.x = dot(r9.xyz, r8.xyz);
    r4.xyz = (r1.xyz) * (v0.xyz);
    r0.x = (r0.x) + (r0.x);
    r2.xyz = (r8.xyz) * (-(r0.xxx)) + (r9.xyz);
    r2.w = (r0.w) * (c0.z);
    r2 = texCUBElod(s15, r2);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r4.xyz = (r4.xyz) * (r4.xyz);
    r8.xyz = (r1.xyz) * (c0.zzz);
    r2 = tex3D(s11, v5.xyz);
    r0.w = (r3.w) * (r3.w);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r0.w = (r3.w) * (r0.w);
    r1.xyz = (r8.xyz) * (r1.xyz);
    r0.w = (r0.z) * (r0.w);
    r1.xyz = (r1.xyz) * (c0.www);
    r3.xyz = (r3.xyz) * (r0.www) + (r7.xyz);
    r2.xyz = (r4.xyz) * (r5.xyz) + (r6.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz);
    r1.w = (r1.w) * (v0.w);
    r0.xyz = (r1.xyz) * (r0.yyy) + (r2.xyz);
    r0.xyz = (r1.www) * (r0.xyz);
    r0.w = c1.x;
    r1.x = dot(r0, c[22]);
    r1.y = dot(r0, c[23]);
    r1.z = dot(r0, c[24]);
    r0.xyz = (v3.xyz) * (-(r1.www)) + (r1.xyz);
    r0.xyz = (r0.xyz) * (v2.www);
    r0.xyz = (v3.xyz) * (r1.www) + (r0.xyz);
    r0.xyz = max(((r0.xyz) * (c[21].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r1.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
