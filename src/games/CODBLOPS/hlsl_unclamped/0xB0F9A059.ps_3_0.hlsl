// Mechanically reconstructed from 0xB0F9A059.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(-0.5f, 0.200000003f, 1.0f, 0.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(8.0f, 31.875f, 0.797884583f, 1.0f);
    const float4 c3 = float4(-2.0f, 3.0f, 0.0009765625f, 0.25f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 oC0 = 0.0f;

    r5.xyz = (-(v7.xyz)) + (c[6].xyz);
    r2.y = dot(r5.xyz, r5.xyz);
    r4.w = rsqrt(r2.y);
    r2.x = 1.0f / (r4.w);
    r1.xy = saturate((r2.xx) * (c[21].xy) + (c[21].zw));
    r0.xy = (r1.xy) * (r1.xy);
    r1.xy = (r1.xy) * (c3.xx) + (c3.yy);
    r0.w = dot(c[20].yz, r2.xy) + (c[20].x);
    r0.xy = (r0.xy) * (r1.xy);
    r0.w = (r0.w) * (r0.x);
    r3.z = (r0.y) * (r0.w);
    r0.xy = (v6.ww) * (c[22].xy) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[22].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r2.y = r0.x;
    r0.xy = (v6.ww) * (c[22].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r2.z = r0.x;
    r0.xy = (v6.ww) * (-(c[22].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0 = tex2D(s1, v1.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s0, v1.xy);
    r6.w = (r0.w) * (v0.w) + (c0.x);
    r3.xy = float2(((r6.w) >= 0.0f ? (r3.x) : (c0.w)), ((r6.w) >= 0.0f ? (r3.y) : (c0.w)));
    r2.w = r1.x;
    r1 = v2;
    r1.xyz = (r3.xxx) * (v5.xyz) + (r1.xyz);
    r6.z = dot(r2, c3.wwww);
    r1.xyz = (r3.yyy) * (v4.xyz) + (r1.xyz);
    r4.xyz = (r5.xyz) * (r4.www);
    r7.xyz = normalize(r1.xyz);
    r1.z = dot(r4.xyz, c[10].xyz);
    r2.w = max(abs(r7.y), abs(r7.z));
    r2.z = saturate((r1.z) * (c[11].x) + (c[11].y));
    r1.z = max(abs(r7.x), r2.w);
    r2.w = 1.0f / (r1.z);
    r1.xyz = (r7.xyz) * (c[5].xyz);
    r5.w = (r2.z) * (c[11].w);
    r1.xyz = (r1.xyz) * (r2.www) + (v8.xyz);
    r2 = tex3D(s11, r1.xyz);
    r3.w = lerp(r2.w, r6.z, r5.w);
    r1.xyz = (r2.xyz) * (r2.xyz);
    r2.xy = (v1.xy) * (c[23].xy);
    r2 = tex2D(s4, r2.xy);
    r2.xyz = (r2.xyz) + (c0.xxx);
    r7.w = (r3.z) * (r3.w);
    r0.xyz = saturate((r2.xyz) * (r0.www) + (r0.xyz));
    r3 = tex2D(s3, v1.xy);
    r2.xyz = lerp(r0.xyz, c0.yyy, r3.xxx);
    r0.xyz = (r0.xyz) * (r3.xxx);
    r2.w = r3.w;
    r3.w = ((r6.w) >= 0.0f ? (r3.y) : (c0.w));
    r2 = float4(((r6.w) >= 0.0f ? (r2.x) : (c0.w)), ((r6.w) >= 0.0f ? (r2.y) : (c0.w)), ((r6.w) >= 0.0f ? (r2.z) : (c0.w)), ((r6.w) >= 0.0f ? (r2.w) : (c0.z)));
    r5.w = (r2.w) * (c2.z);
    r3.y = (r2.w) * (-(c2.z)) + (c2.w);
    r0.w = saturate(dot(r7.xyz, r4.xyz));
    r8.xyz = normalize(v7.xyz);
    r3.z = (r0.w) * (r3.y) + (r5.w);
    r3.x = saturate(dot(r7.xyz, -(r8.xyz)));
    r3.y = (r3.x) * (r3.y) + (r5.w);
    r5.w = (-(r3.x)) + (c0.z);
    r3.z = (r3.z) * (r3.y) + (c3.z);
    r5.xyz = (r5.xyz) * (r4.www) + (-(r8.xyz));
    r4.w = 1.0f / (r3.z);
    r3.xyz = normalize(r5.xyz);
    r4.w = (r0.w) * (r4.w);
    r5.z = saturate(dot(r7.xyz, r3.xyz));
    r4.z = saturate(dot(r3.xyz, r4.xyz));
    r3.xy = (r2.ww) * (c4.xy) + (c4.zw);
    r3.z = exp2(r3.y);
    r3.y = (-(r4.z)) + (c0.z);
    r4.y = pow(abs(r5.z), r3.z);
    r4.z = (r3.y) * (r3.y);
    r3.z = (r3.z) * (c12.x) + (c12.y);
    r4.z = (r4.z) * (r4.z);
    r3.z = (r4.y) * (r3.z);
    r3.y = (r3.y) * (r4.z);
    r6.xyz = (r2.xyz) * (r2.xyz);
    r5.xyz = (r2.xyz) * (-(r2.xyz)) + (c0.zzz);
    r3.z = (r4.w) * (r3.z);
    r2.xyz = (r5.xyz) * (r3.yyy) + (r6.xyz);
    r4.w = 1.0f / (r3.x);
    r2.xyz = (r3.zzz) * (r2.xyz);
    r2.xyz = (r3.www) * (r2.xyz);
    r4.xy = c[9].xy;
    r3.xyz = (r4.yyy) * (c[8].xyz);
    r2.w = (r2.w) * (c2.x);
    r3.xyz = (r2.xyz) * (r3.xyz);
    r2.xyz = (r1.xyz) * (r3.www);
    r1.xyz = (r4.xxx) * (c[7].xyz);
    r2.xyz = (r2.xyz) * (c2.yyy);
    r1.xyz = (r0.www) * (r1.xyz);
    r4.xyz = (r7.www) * (r3.xyz);
    r3.xyz = (r7.www) * (r1.xyz) + (r2.xyz);
    r0.w = c0.z;
    r1.z = dot(r8.xyz, r7.xyz);
    r0 = float4(((r6.w) >= 0.0f ? (r0.x) : (c0.w)), ((r6.w) >= 0.0f ? (r0.y) : (c0.w)), ((r6.w) >= 0.0f ? (r0.z) : (c0.w)), ((r6.w) >= 0.0f ? (r0.w) : (c0.w)));
    r2.z = (r1.z) + (r1.z);
    r1.xyz = (r0.xyz) * (v0.xyz);
    r2.xyz = (r7.xyz) * (-(r2.zzz)) + (r8.xyz);
    r2 = texCUBElod(s15, r2);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r0.xyz) * (c2.xxx);
    r2 = tex3D(s11, v8.xyz);
    r2.w = (r5.w) * (r5.w);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r5.w) * (r2.w);
    r0.xyz = (r7.xyz) * (r0.xyz);
    r2.w = (r4.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c2.yyy);
    r2.xyz = (r5.xyz) * (r2.www) + (r6.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz) + (r4.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r2.xyz = (r0.xyz) * (r3.www) + (r1.xyz);
    r2.w = c0.z;
    r0.x = dot(r2, c[25]);
    r0.y = dot(r2, c[26]);
    r0.z = dot(r2, c[27]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[24].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
