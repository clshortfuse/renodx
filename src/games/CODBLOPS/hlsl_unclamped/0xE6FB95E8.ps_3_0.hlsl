// Mechanically reconstructed from 0xE6FB95E8.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD4;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(1.0f, 0.0f, -1.0f, 8.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    const float4 c14 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v5.ww) * (c[27].xy) + (v5.xy);
    r0.zw = v5.zw;
    r0 = tex2Dproj(s1, r0);
    r1.xy = (v5.ww) * (-(c[27].xy)) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.y = r1.x;
    r1.xy = (v5.ww) * (c[27].zw) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.z = r1.x;
    r1.xy = (v5.ww) * (-(c[27].zw)) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.w = r1.x;
    r1.w = dot(r0, c3.zzzz);
    r0 = tex2D(s12, v0.zw);
    r3.xyz = (-(v6.xyz)) + (c[20].xyz);
    r5.y = dot(r3.xyz, r3.xyz);
    r3.w = rsqrt(r5.y);
    r2.xyz = (r3.xyz) * (r3.www);
    r0.w = dot(r2.xyz, c[23].xyz);
    r0.w = saturate((r0.w) * (c[24].x) + (c[24].y));
    r5.x = 1.0f / (r3.w);
    r0.z = (r0.w) * (c[24].w);
    r4.xy = saturate((r5.xx) * (c[26].xy) + (c[26].zw));
    r1.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c13.xx) + (c13.yy);
    r0.w = dot(c[25].yz, r5.xy) + (c[25].x);
    r4.xy = (r1.xy) * (r4.xy);
    r2.w = lerp(r0.y, r1.w, r0.z);
    r4.w = (r0.w) * (r4.x);
    r0 = tex2D(s3, v7.zw);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s2, v7.zw);
    r5.w = (r1.w) * (v7.y);
    r0.w = (r4.y) * (r4.w);
    r7.xy = (r0.xy) * (r5.ww);
    r12.xyz = normalize(v6.xyz);
    r0.xyz = v1.xyz;
    r0.xyz = (r7.xxx) * (v4.xyz) + (r0.xyz);
    r4.xyz = (r3.xyz) * (r3.www) + (-(r12.xyz));
    r0.xyz = (r7.yyy) * (v3.xyz) + (r0.xyz);
    r3.xyz = normalize(r4.xyz);
    r8.xyz = normalize(r0.xyz);
    r6.w = (r2.w) * (r0.w);
    r4.w = saturate(dot(r8.xyz, r3.xyz));
    r0 = tex2D(s4, v7.zw);
    r0 = (r0) + (c1.yyyz);
    r3.z = saturate(dot(r3.xyz, r2.xyz));
    r0.w = (r5.w) * (r0.w) + (c1.x);
    r3.w = saturate(dot(r8.xyz, r2.xyz));
    r2.xy = (r0.ww) * (c12.xy) + (c12.zw);
    r0.xyz = (r5.www) * (r0.xyz);
    r3.y = exp2(r2.y);
    r2.z = pow(abs(r4.w), r3.y);
    r2.w = (r3.y) * (c3.y) + (c3.z);
    r7.w = 1.0f / (r2.x);
    r2.w = (r2.z) * (r2.w);
    r3.y = (r0.w) * (c2.z);
    r2.x = (r0.w) * (-(c14.z)) + (c14.w);
    r0.w = (r0.w) * (c1.w);
    r2.z = saturate(dot(r8.xyz, -(r12.xyz)));
    r2.y = (r3.w) * (r2.x) + (r3.y);
    r2.x = (r2.z) * (r2.x) + (r3.y);
    r8.w = (-(r2.z)) + (c1.x);
    r2.z = (-(r3.z)) + (c1.x);
    r2.y = (r2.y) * (r2.x) + (c3.x);
    r2.x = (r2.z) * (r2.z);
    r2.y = 1.0f / (r2.y);
    r2.x = (r2.x) * (r2.x);
    r2.y = (r3.w) * (r2.y);
    r2.z = (r2.z) * (r2.x);
    r10.xyz = (r0.xyz) * (r0.xyz);
    r6.xyz = (r0.xyz) * (-(r0.xyz)) + (c1.xxx);
    r2.w = (r2.w) * (r2.y);
    r2.xyz = (r6.xyz) * (r2.zzz) + (r10.xyz);
    r0.xyz = (r3.www) * (c[21].xyz);
    r5.xyz = (r2.www) * (r2.xyz);
    r13.xz = c1.xz;
    r3.xyz = (r13.zzz) + (c[28].xyw);
    r2.xy = (v0.zw) * (c4.xy);
    r2 = tex2D(s13, r2.xy);
    r4.xy = (v0.zw) * (c4.xy) + (c4.zy);
    r4 = tex2D(s13, r4.xy);
    r2.w = r4.y;
    r11.xyz = (r5.www) * (r3.xyz) + (c1.xxx);
    r3.xy = (r2.yw) * (c14.xx) + (c14.yy);
    r2.y = dot(r3.xy, r3.xy) + (c1.y);
    r2.w = dot(r7.xy, r7.xy) + (c1.y);
    r2.y = exp2(-(r2.y));
    r2.w = exp2(-(r2.w));
    r3.w = (r2.y) * (c2.x) + (c2.y);
    r2.w = (r2.w) * (c2.x) + (c2.y);
    r2.y = dot(r3.xy, r7.xy) + (c1.y);
    r4.w = (r3.w) * (r2.w);
    r3 = tex2D(s14, v0.zw);
    r9.xy = (r3.xy) * (c2.ww);
    r3.z = saturate((r2.y) * (r4.w) + (r4.w));
    r4.xy = (r4.xz) * (r9.yy);
    r2.y = (r3.y) * (c2.w) + (-(r4.x));
    r7.xz = (r4.xy) * (c4.ww);
    r3.w = (r4.z) * (-(r9.y)) + (r2.y);
    r2.xy = (r2.xz) * (r9.xx);
    r7.y = (r3.w) + (r3.w);
    r3.w = (r3.x) * (c2.w) + (-(r2.x));
    r4.xyz = (r3.zzz) * (r7.xyz);
    r2.z = (r2.z) * (-(r9.x)) + (r3.w);
    r9.xz = (r2.xy) * (c4.ww);
    r9.y = (r2.z) + (r2.z);
    r3.xyz = (r5.xyz) * (r11.zzz);
    r2.xyz = (r9.xyz) * (r2.www) + (r4.xyz);
    r3.xyz = (r3.xyz) * (c[22].xyz);
    r2.xyz = (r11.yyy) * (r2.xyz);
    r3.xyz = (r6.www) * (r3.xyz);
    r0.xyz = (r6.www) * (r0.xyz) + (r2.xyz);
    r2 = tex2D(s0, v0.xy);
    r4.xyz = lerp(r2.xyz, r1.xyz, r5.www);
    r5.w = (r2.w) * (-(v7.x)) + (c1.x);
    r6.w = (r1.w) * (-(v7.y)) + (c1.x);
    r1.w = dot(r12.xyz, r8.xyz);
    r5.xyz = (r4.xyz) * (r4.xyz);
    r1.w = (r1.w) + (r1.w);
    r7.xyz = (r5.xyz) * (r0.xyz) + (r3.xyz);
    r0.xyz = (r8.xyz) * (-(r1.www)) + (r12.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r8.w) * (r8.w);
    r4.xyz = (r11.xxx) * (r0.xyz);
    r0.w = (r8.w) * (r0.w);
    r4.w = (r7.w) * (r0.w);
    r3 = (-(v6.yyyy)) + (c[6]);
    r0 = (r3) * (r3);
    r2 = (-(v6.xxxx)) + (c[5]);
    r0 = (r2) * (r2) + (r0);
    r1 = (-(v6.zzzz)) + (c[7]);
    r6.xyz = (r6.xyz) * (r4.www) + (r10.xyz);
    r0 = (r1) * (r1) + (r0);
    r6.xyz = (r4.xyz) * (r6.xyz);
    r4.x = rsqrt(r0.x);
    r4.y = rsqrt(r0.y);
    r4.z = rsqrt(r0.z);
    r4.w = rsqrt(r0.w);
    r6.xyz = (r9.xyz) * (r6.xyz);
    r3 = (r3) * (r4);
    r3 = (r8.yyyy) * (r3);
    r2 = (r2) * (r4);
    r1 = (r1) * (r4);
    r2 = (r2) * (r8.xxxx) + (r3);
    r0 = saturate((r0) * (c[8]) + (r13.xxxx));
    r1 = saturate((r1) * (r8.zzzz) + (r2));
    r2.xyz = (r6.xyz) * (c1.www) + (r7.xyz);
    r0 = (r0) * (r1);
    r1.x = dot(c[9], r0);
    r1.y = dot(c[10], r0);
    r1.z = dot(c[11], r0);
    r0.w = (r5.w) * (-(r6.w)) + (c1.x);
    r0.xyz = (r5.xyz) * (r1.xyz) + (r2.xyz);
    r1.xyz = (r0.www) * (v2.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (-(r1.xyz));
    r0.xyz = (v1.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
