// Mechanically reconstructed from 0xF5A528A5.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(-0.5f, 8.0f, 31.875f, 1.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(0.797884583f, -2.0f, 3.0f, 1.0f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.xy) * (c[24].xy);
    r0 = tex2D(s5, r0.xy);
    r1.xyz = (r0.xyz) + (c0.xxx);
    r0 = tex2D(s0, v1.xy);
    r0.xyz = saturate((r1.xyz) * (r0.www) + (r0.xyz));
    r0.xyz = (r0.xyz) * (v0.xyz);
    r3.xyz = (r0.xyz) * (r0.xyz);
    r0.xy = (v6.ww) * (c[21].xy) + (v6.xy);
    r0.zw = v6.zw;
    r1 = tex2Dproj(s2, r0);
    r0.xy = (v6.ww) * (-(c[21].xy)) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.y = r0.x;
    r0.xy = (v6.ww) * (c[21].zw) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.z = r0.x;
    r0.xy = (v6.ww) * (-(c[21].zw)) + (v6.xy);
    r0.zw = v6.zw;
    r2 = tex2Dproj(s2, r0);
    r0.xy = (v1.xy) * (c[23].xy);
    r0 = tex2D(s4, r0.xy);
    r4.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r4.xy = (c[23].zz) * (r4.xy) + (r0.xy);
    r0 = v2;
    r0.xyz = (r4.xxx) * (v5.xyz) + (r0.xyz);
    r4.xyz = (r4.yyy) * (v4.xyz) + (r0.xyz);
    r0.xyz = normalize(r4.xyz);
    r3.w = max(abs(r0.y), abs(r0.z));
    r1.w = r2.x;
    r2.w = max(abs(r0.x), r3.w);
    r2.w = 1.0f / (r2.w);
    r2.xyz = (r0.xyz) * (c[5].xyz);
    r4.w = dot(r1, c3.zzzz);
    r1.xyz = (r2.xyz) * (r2.www) + (v8.xyz);
    r1 = tex3D(s11, r1.xyz);
    r5.xyz = (-(v7.xyz)) + (c[6].xyz);
    r7.y = dot(r5.xyz, r5.xyz);
    r6.w = rsqrt(r7.y);
    r4.xyz = (r5.xyz) * (r6.www);
    r2.w = dot(r4.xyz, c[9].xyz);
    r2.w = saturate((r2.w) * (c[10].x) + (c[10].y));
    r7.x = 1.0f / (r6.w);
    r3.w = (r2.w) * (c[10].w);
    r6.xy = saturate((r7.xx) * (c[20].xy) + (c[20].zw));
    r2.xy = (r6.xy) * (r6.xy);
    r6.xy = (r6.xy) * (c2.yy) + (c2.zz);
    r2.z = dot(c[11].yz, r7.xy) + (c[11].x);
    r2.xy = (r2.xy) * (r6.xy);
    r2.w = lerp(r1.w, r4.w, r3.w);
    r1.w = (r2.z) * (r2.x);
    r7.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r2.y) * (r1.w);
    r3.w = (r2.w) * (r1.w);
    r4.w = saturate(dot(r0.xyz, r4.xyz));
    r2 = tex2D(s3, v1.xy);
    r1.x = (r2.w) * (c2.x);
    r1.z = (r2.w) * (-(c2.x)) + (c2.w);
    r9.xyz = normalize(v7.xyz);
    r1.w = (r4.w) * (r1.z) + (r1.x);
    r1.y = saturate(dot(r0.xyz, -(r9.xyz)));
    r1.z = (r1.y) * (r1.z) + (r1.x);
    r5.w = (-(r1.y)) + (c0.w);
    r1.w = (r1.w) * (r1.z) + (c3.x);
    r5.xyz = (r5.xyz) * (r6.www) + (-(r9.xyz));
    r1.w = 1.0f / (r1.w);
    r1.xyz = normalize(r5.xyz);
    r6.w = (r4.w) * (r1.w);
    r5.z = saturate(dot(r0.xyz, r1.xyz));
    r1.w = saturate(dot(r1.xyz, r4.xyz));
    r4.xy = (r2.ww) * (c4.xy) + (c4.zw);
    r4.z = exp2(r4.y);
    r1.z = (-(r1.w)) + (c0.w);
    r1.x = pow(abs(r5.z), r4.z);
    r1.y = (r1.z) * (r1.z);
    r1.w = (r4.z) * (c3.y) + (c3.z);
    r1.y = (r1.y) * (r1.y);
    r1.w = (r1.x) * (r1.w);
    r1.z = (r1.z) * (r1.y);
    r8.xyz = (r2.xyz) * (r2.xyz);
    r5.xyz = (r2.xyz) * (-(r2.xyz)) + (c0.www);
    r1.w = (r6.w) * (r1.w);
    r1.xyz = (r5.xyz) * (r1.zzz) + (r8.xyz);
    r6.xyz = (r4.www) * (c[7].xyz);
    r1.xyz = (r1.www) * (r1.xyz);
    r4.w = 1.0f / (r4.x);
    r2.xyz = (r1.xyz) * (c[22].www);
    r1 = tex2D(s6, v1.xy);
    r2.xyz = (r2.xyz) * (r1.www);
    r2.w = (r2.w) * (c0.y);
    r2.xyz = (r2.xyz) * (c[8].xyz);
    r4.xyz = (r3.www) * (r2.xyz);
    r6.w = dot(r9.xyz, r0.xyz);
    r2.xyz = (r7.xyz) * (c[22].yyy);
    r6.w = (r6.w) + (r6.w);
    r7.xyz = (r1.www) * (r2.xyz);
    r2.xyz = (r0.xyz) * (-(r6.www)) + (r9.xyz);
    r2 = texCUBElod(s15, r2);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r7.xyz) * (c0.zzz);
    r9.xyz = (r0.xyz) * (c0.yyy);
    r2 = tex3D(s11, v8.xyz);
    r1.w = (r5.w) * (r5.w);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r1.w = (r5.w) * (r1.w);
    r0.xyz = (r9.xyz) * (r0.xyz);
    r1.w = (r4.w) * (r1.w);
    r0.xyz = (r0.xyz) * (c0.zzz);
    r5.xyz = (r5.xyz) * (r1.www) + (r8.xyz);
    r2.xyz = (r3.www) * (r6.xyz) + (r7.xyz);
    r0.xyz = (r0.xyz) * (r5.xyz);
    r2.xyz = (r3.xyz) * (r2.xyz) + (r4.xyz);
    r0.xyz = (r0.xyz) * (c[22].xxx);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[25].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.w;

    return oC0;
}
