// Mechanically reconstructed from 0x5B3F1F53.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);

struct PS_INPUT
{
    float4 v0 : TEXCOORD5;
    float4 v1 : TEXCOORD6;
    float4 v2 : TEXCOORD7;
    float4 v3 : TEXCOORD8;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    const float4 c0 = float4(-0.5f, 0.5f, 4.06451607f, -2.06451607f);
    const float4 c1 = float4(4.0f, -0.5f, -2.07999992f, 200.0f);
    const float4 c2 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 oC0 = 0.0f;

    r2.xy = c0.xy;
    r0.xy = (v3.xy) * (c[10].xx) + (r2.xx);
    r0.xy = (r0.xy) * (c[11].xx) + (r2.yy);
    r0 = tex2D(s0, r0.xy);
    r4.xy = (v3.xy) * (c[10].xx);
    r0.w = (r0.y) * (c0.z) + (c0.w);
    r1.xy = (r4.xy) * (c1.xx) + (c1.yy);
    r0.y = (r0.w) * (c0.z) + (c0.w);
    r1.xy = (r1.xy) * (c[20].xx) + (r2.yy);
    r1 = tex2D(s1, r1.xy);
    r0.w = (r1.y) * (c0.z) + (c0.w);
    r0.w = (r0.w) * (c0.z) + (c0.w);
    r0.xz = c1.zz;
    r3.xy = (r0.zw) * (c[22].xx);
    r1.xyz = v2.xyz;
    r2.xyz = (r1.zxy) * (v0.yzx);
    r3.xy = (c[21].xx) * (r0.xy) + (r3.xy);
    r0.xyz = (r1.yzx) * (v0.zxy) + (-(r2.xyz));
    r0.xyz = (r3.yyy) * (-(r0.xyz));
    r0.xyz = (r3.xxx) * (v0.xyz) + (r0.xyz);
    r2.xyz = (r0.xyz) + (v2.xyz);
    r1.xyz = (-(v1.xyz)) + (c[5].xyz);
    r0.xyz = normalize(r2.xyz);
    r5.y = dot(r1.xyz, r1.xyz);
    r0.w = dot(-(v1.xyz), -(v1.xyz));
    r1.w = rsqrt(r5.y);
    r0.w = rsqrt(r0.w);
    r1.xyz = (r1.xyz) * (r1.www);
    r5.x = 1.0f / (r1.w);
    r2.xyz = (-(v1.xyz)) * (r0.www) + (r1.xyz);
    r0.w = saturate(dot(r0.xyz, r1.xyz));
    r3.xy = saturate((r5.xx) * (c[8].xy) + (c[8].zw));
    r1.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c2.xx) + (c2.yy);
    r1.w = dot(c[7].yz, r5.xy) + (c[7].x);
    r3.xy = (r1.xy) * (r3.xy);
    r1.xyz = normalize(r2.xyz);
    r1.w = (r1.w) * (r3.x);
    r0.y = saturate(dot(r0.xyz, r1.xyz));
    r0.z = (r3.y) * (r1.w);
    r1.w = pow(abs(r0.y), c1.w);
    r0.xyz = (r0.zzz) * (c[6].xyz);
    r3.xyz = (r1.www) * (r0.xyz);
    r2.xyz = (r0.www) * (r0.xyz);
    r1 = tex2D(s3, r4.xy);
    r0 = tex2D(s2, r4.xy);
    r1.xyz = (r3.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (v1.www);
    r0.xyz = max(((r0.xyz) * (c[9].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
