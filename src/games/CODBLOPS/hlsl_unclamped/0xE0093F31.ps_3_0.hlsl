// Mechanically reconstructed from 0xE0093F31.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD2;
    float4 v2 : TEXCOORD3;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
    float4 v6 : TEXCOORD7;
    float4 v7 : TEXCOORD8;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c1 = float4(-0.5f, 0.5f, 4.06451607f, -2.06451607f);
    const float4 c2 = float4(4.0f, -0.5f, -2.07999992f, 0.25f);
    const float4 c3 = float4(1.0f, 0.0f, -2.0f, 3.0f);
    const float4 c4 = float4(200.0f, 31.875f, 8.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v5.yyyy) * (c[25]);
    r0 = (v5.xxxx) * (c[24]) + (r0);
    r0 = (v5.zzzz) * (c[26]) + (r0);
    r0 = (r0) + (c[27]);
    r3.xy = (r0.ww) * (c[32].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s0, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[32].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s0, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[32].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[32].zw) + (r0.xy);
    r2 = tex2Dproj(s0, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s0, r0);
    r0 = (v5.xyzx) * (c3.xxxy) + (c3.yyyx);
    r2.w = dot(r0, c[21]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[10]);
    r2.y = dot(r0, c[11]);
    r1.w = dot(r1, c2.wwww);
    r2.xy = (r2.ww) * (r2.xy);
    r1.x = dot(r0, c[20]);
    r0.xy = (r2.xy) * (c1.yy) + (c1.yy);
    r0 = tex2D(s1, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[8].yz, r1.xy) + (c[8].x);
    r1.xy = saturate((r1.xx) * (c[9].xy) + (c[9].zw));
    r1.z = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c3.y) : (r1.z));
    r2.xy = (r1.xy) * (r1.xy);
    r3.xy = (r1.xy) * (c3.zz) + (c3.ww);
    r1.xyz = (-(v5.xyz)) + (c[6].xyz);
    r2.w = (r2.x) * (r3.x);
    r7.xyz = normalize(r1.xyz);
    r1.x = (r2.y) * (-(r3.y)) + (c3.x);
    r1.z = dot(r7.xyz, c[22].xyz);
    r0.w = (r0.w) * (r2.w);
    r1.y = saturate((r1.z) * (c[23].x) + (c[23].y));
    r1.z = (r1.y) * (r1.y);
    r1.y = (r1.y) * (c3.z) + (c3.w);
    r0.w = (r1.x) * (r0.w);
    r1.z = (r1.z) * (r1.y);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r0.w) * (r1.z);
    r1.xyz = (r0.xyz) * (r0.www);
    r0.xyz = (-(v5.xyz)) + (c[28].xyz);
    r1.xyz = (r1.www) * (r1.xyz);
    r6.y = dot(r0.xyz, r0.xyz);
    r3.xyz = (r1.xyz) * (c[7].xyz);
    r0.w = rsqrt(r6.y);
    r6.x = 1.0f / (r0.w);
    r2.xy = c1.xy;
    r1.xy = (v7.xy) * (c[39].xx) + (r2.xx);
    r5.xyz = (r0.xyz) * (r0.www);
    r0.xy = (r1.xy) * (c[40].xx) + (r2.yy);
    r0 = tex2D(s3, r0.xy);
    r9.xy = (v7.xy) * (c[39].xx);
    r0.w = (r0.y) * (c1.z) + (c1.w);
    r1.xy = (r9.xy) * (c2.xx) + (c2.yy);
    r0.y = (r0.w) * (c1.z) + (c1.w);
    r1.xy = (r1.xy) * (c[41].xx) + (r2.yy);
    r1 = tex2D(s4, r1.xy);
    r0.w = (r1.y) * (c1.z) + (c1.w);
    r0.w = (r0.w) * (c1.z) + (c1.w);
    r0.xz = c2.zz;
    r4.xy = (r0.zw) * (c[43].xx);
    r1.xyz = v6.xyz;
    r2.xyz = (r1.zxy) * (v4.yzx);
    r4.xy = (c[42].xx) * (r0.xy) + (r4.xy);
    r0.xyz = (r1.yzx) * (v4.zxy) + (-(r2.xyz));
    r0.w = dot(c[30].yz, r6.xy) + (c[30].x);
    r0.xyz = (r4.yyy) * (-(r0.xyz));
    r1.xy = saturate((r6.xx) * (c[31].xy) + (c[31].zw));
    r0.xyz = (r4.xxx) * (v4.xyz) + (r0.xyz);
    r2.xy = (r1.xy) * (r1.xy);
    r0.xyz = (r0.xyz) + (v6.xyz);
    r4.xy = (r1.xy) * (c3.zz) + (c3.ww);
    r1.xyz = normalize(r0.xyz);
    r2.xy = (r2.xy) * (r4.xy);
    r2.w = max(abs(r1.y), abs(r1.z));
    r1.w = (r0.w) * (r2.x);
    r0.w = max(abs(r1.x), r2.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r1.xyz) * (c[5].xyz);
    r1.w = (r2.y) * (r1.w);
    r0.xyz = (r0.xyz) * (r0.www) + (v0.xyz);
    r0 = tex3D(s11, r0.xyz);
    r1.w = (r1.w) * (r0.w);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.w = dot(-(v5.xyz), -(v5.xyz));
    r4.xyz = (r1.www) * (c[29].xyz);
    r0.w = rsqrt(r0.w);
    r0.xyz = (-(v5.xyz)) * (r0.www) + (r5.xyz);
    r1.w = saturate(dot(r1.xyz, r5.xyz));
    r5.xyz = normalize(r0.xyz);
    r0.xyz = (r0.www) * (-(v5.xyz));
    r4.w = saturate(dot(r1.xyz, r5.xyz));
    r2.w = dot(-(r0.xyz), r1.xyz);
    r3.w = pow(abs(r4.w), c4.x);
    r2.w = (r2.w) + (r2.w);
    r6.xyz = (r4.xyz) * (r3.www);
    r0.xyz = (r1.xyz) * (-(r2.www)) + (-(r0.xyz));
    r4.xyz = (r4.xyz) * (r1.www);
    r5.xyz = (r0.yyy) * (v2.xyw);
    r8.xyz = (-(v5.xyz)) * (r0.www) + (r7.xyz);
    r5.xyz = (r0.xxx) * (v1.xyw) + (r5.xyz);
    r2.w = saturate(dot(r1.xyz, r7.xyz));
    r7.xyz = (r0.zzz) * (v3.xyw) + (r5.xyz);
    r5.xyz = normalize(r8.xyz);
    r1.w = 1.0f / (r7.z);
    r0.w = saturate(dot(r1.xyz, r5.xyz));
    r1.xy = (r7.xy) * (r1.ww);
    r3.w = pow(abs(r0.w), c4.x);
    r0.w = max(abs(r1.x), abs(r1.y));
    r1.xy = (r1.xy) * (c1.yx) + (c1.yy);
    r4.w = saturate((r0.w) * (-(r0.w)) + (c3.x));
    r1 = tex2D(s2, r1.xy);
    r5.xyz = (r1.xyz) * (r1.xyz);
    r0.w = c3.y;
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = 1.0f / (c[36].x);
    r1.xyz = (r0.xyz) * (c[37].xxx);
    r0.xyz = (r5.xyz) * (r0.www);
    r5.xyz = (r1.xyz) * (c4.zzz);
    r0.w = ((-(r7.z)) >= 0.0f ? (c3.y) : (r4.w));
    r1.xyz = (r0.xyz) * (c[38].xxx) + (-(r5.xyz));
    r0.xyz = (r3.www) * (r3.xyz) + (r6.xyz);
    r1.xyz = (r0.www) * (r1.xyz) + (r5.xyz);
    r4.xyz = (r2.www) * (r3.xyz) + (r4.xyz);
    r3.xyz = (r0.xyz) + (r1.xyz);
    r1 = tex2D(s6, r9.xy);
    r0 = tex2D(s5, r9.xy);
    r3.xyz = (r3.xyz) * (r1.xyz);
    r1.xyz = (r2.xyz) * (c4.yyy) + (r4.xyz);
    r4.xyz = normalize(v5.xyz);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.z = dot(c[33].xyz, r4.xyz);
    r1.w = saturate((c[35].y) * (r0.z) + (c[35].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[34].xyz);
    r1.xyz = (r1.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r1.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v5.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[36].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r0.w;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
