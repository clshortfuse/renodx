// Mechanically reconstructed from 0x9B6C68F6.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(2.0f, -1.0f, 0.200000003f, 0.0f);
    const float4 c2 = float4(-0.5f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c3 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    const float4 c4 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c12 = float4(0.797884583f, 1.0f, 0.5f, 0.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s1, v0.xy);
    r1.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = tex2D(s3, v6.zw);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0 = (c1.xxxx) * (v7) + (c1.yyyy);
    r0.x = dot(r2.xy, r0.xy) + (c1.w);
    r0.y = dot(r2.xy, r0.zw) + (c1.w);
    r2 = tex2D(s2, v6.zw);
    r2.w = (r2.w) * (v6.y) + (c2.x);
    r12.xy = float2(((r2.w) >= 0.0f ? (r0.x) : (r1.x)), ((r2.w) >= 0.0f ? (r0.y) : (r1.y)));
    r0 = v1;
    r0.xyz = (r12.xxx) * (v4.xyz) + (r0.xyz);
    r1.xyz = (r12.yyy) * (v3.xyz) + (r0.xyz);
    r0.xyz = normalize(r1.xyz);
    r10.xyz = normalize(v5.xyz);
    r6.w = saturate(dot(r0.xyz, -(r10.xyz)));
    r1 = tex2D(s5, v6.zw);
    r5 = tex2D(s0, v0.xy);
    r4 = tex2D(s4, v0.xy);
    r3.xyz = lerp(r5.xyz, c1.zzz, r4.xxx);
    r5.xyz = (r5.xyz) * (r4.xxx);
    r3.w = r4.w;
    r9.xyz = float3(((r2.w) >= 0.0f ? (c[10].x) : (r4.y)), ((r2.w) >= 0.0f ? (c[10].y) : (r4.y)), ((r2.w) >= 0.0f ? (c[10].w) : (r4.y)));
    r1 = float4(((r2.w) >= 0.0f ? (r1.x) : (r3.x)), ((r2.w) >= 0.0f ? (r1.y) : (r3.y)), ((r2.w) >= 0.0f ? (r1.z) : (r3.z)), ((r2.w) >= 0.0f ? (r1.w) : (r3.w)));
    r4.xyz = (-(v5.xyz)) + (c[5].xyz);
    r6.xyz = float3(((r2.w) >= 0.0f ? (r2.x) : (r5.x)), ((r2.w) >= 0.0f ? (r2.y) : (r5.y)), ((r2.w) >= 0.0f ? (r2.z) : (r5.z)));
    r5.y = dot(r4.xyz, r4.xyz);
    r3.w = (r1.w) * (c12.x);
    r2.y = rsqrt(r5.y);
    r2.z = (r1.w) * (-(c12.x)) + (c12.y);
    r3.xyz = (r4.xyz) * (r2.yyy);
    r2.x = (r6.w) * (r2.z) + (r3.w);
    r2.w = saturate(dot(r0.xyz, r3.xyz));
    r6.w = (-(r6.w)) + (-(c1.y));
    r2.z = (r2.w) * (r2.z) + (r3.w);
    r4.xyz = (r4.xyz) * (r2.yyy) + (-(r10.xyz));
    r2.z = (r2.z) * (r2.x) + (c4.w);
    r5.x = 1.0f / (r2.y);
    r2.z = 1.0f / (r2.z);
    r3.w = (r2.w) * (r2.z);
    r2.xyz = normalize(r4.xyz);
    r11.xyz = (r2.www) * (c[6].xyz);
    r4.w = saturate(dot(r0.xyz, r2.xyz));
    r2.w = saturate(dot(r2.xyz, r3.xyz));
    r2.xy = (r1.ww) * (c13.xy) + (c13.zw);
    r3.z = exp2(r2.y);
    r5.w = 1.0f / (r2.x);
    r2.x = pow(abs(r4.w), r3.z);
    r2.z = (-(r2.w)) + (-(c1.y));
    r2.w = (r3.z) * (c3.x) + (c3.y);
    r2.y = (r2.z) * (r2.z);
    r2.w = (r2.x) * (r2.w);
    r2.y = (r2.y) * (r2.y);
    r2.w = (r3.w) * (r2.w);
    r2.z = (r2.z) * (r2.y);
    r8.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r1.xyz) * (-(r1.xyz)) + (-(c1.yyy));
    r1.w = (r1.w) * (c2.w);
    r1.xyz = (r7.xyz) * (r2.zzz) + (r8.xyz);
    r1.xyz = (r2.www) * (r1.xyz);
    r3.xy = saturate((r5.xx) * (c[9].xy) + (c[9].zw));
    r2.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c3.zz) + (c3.ww);
    r2.w = dot(c[8].yz, r5.xy) + (c[8].x);
    r2.xy = (r2.xy) * (r3.xy);
    r1.xyz = (r9.zzz) * (r1.xyz);
    r2.w = (r2.w) * (r2.x);
    r1.xyz = (r1.xyz) * (c[7].xyz);
    r3.w = (r2.y) * (r2.w);
    r2 = tex2D(s12, v0.zw);
    r7.w = (r3.w) * (r2.y);
    r2.xy = (v0.zw) * (c12.yz);
    r2 = tex2D(s13, r2.xy);
    r3.xy = (v0.zw) * (c12.yz) + (c12.wz);
    r4 = tex2D(s13, r3.xy);
    r2.w = r4.y;
    r5.xyz = (r1.xyz) * (r7.www);
    r1.xy = (r2.yw) * (c4.yy) + (c4.zz);
    r2.w = dot(r12.xy, r12.xy) + (c1.w);
    r1.z = dot(r1.xy, r1.xy) + (c1.w);
    r2.w = exp2(-(r2.w));
    r1.z = exp2(-(r1.z));
    r2.w = (r2.w) * (c2.y) + (c2.z);
    r2.y = (r1.z) * (c2.y) + (c2.z);
    r1.z = dot(r1.xy, r12.xy) + (c1.w);
    r1.y = (r2.w) * (r2.y);
    r3 = tex2D(s14, v0.zw);
    r12.xy = (r3.xy) * (c4.xx);
    r3.z = saturate((r1.z) * (r1.y) + (r1.y));
    r1.xy = (r4.xz) * (r12.yy);
    r2.y = (r3.y) * (c4.x) + (-(r1.x));
    r1.xz = (r1.xy) * (c4.yy);
    r1.y = (r4.z) * (-(r12.y)) + (r2.y);
    r2.xy = (r2.xz) * (r12.xx);
    r1.y = (r1.y) + (r1.y);
    r3.w = (r3.x) * (c4.x) + (-(r2.x));
    r1.xyz = (r3.zzz) * (r1.xyz);
    r3.w = (r2.z) * (-(r12.x)) + (r3.w);
    r2.xz = (r2.xy) * (c4.yy);
    r2.y = (r3.w) + (r3.w);
    r1.xyz = (r2.xyz) * (r2.www) + (r1.xyz);
    r2.w = dot(r10.xyz, r0.xyz);
    r1.xyz = (r9.yyy) * (r1.xyz);
    r2.w = (r2.w) + (r2.w);
    r3.xyz = (r7.www) * (r11.xyz) + (r1.xyz);
    r1.xyz = (r0.xyz) * (-(r2.www)) + (r10.xyz);
    r1 = texCUBElod(s15, r1);
    r0.z = (r6.w) * (r6.w);
    r1.w = (r6.w) * (r0.z);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r5.w) * (r1.w);
    r0.xyz = (r9.xxx) * (r0.xyz);
    r4.xyz = (r7.xyz) * (r1.www) + (r8.xyz);
    r1.xyz = (r6.xyz) * (r6.xyz);
    r0.xyz = (r0.xyz) * (r4.xyz);
    r1.xyz = (r1.xyz) * (r3.xyz) + (r5.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c2.www) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[11].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = -(c1.y);

    return oC0;
}
