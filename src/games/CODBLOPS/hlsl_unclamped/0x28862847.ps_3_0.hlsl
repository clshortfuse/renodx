// Mechanically reconstructed from 0x28862847.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : TEXCOORD7;
    float4 v8 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(2.0f, -1.0f, 0.0f, 1.0f);
    const float4 c3 = float4(0.200000003f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c4 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c10 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    const float4 c11 = float4(0.797884583f, 1.0f, 0.5f, 0.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c11.yz);
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (c11.yz) + (c11.wz);
    r3 = tex2D(s13, r1.xy);
    r0.w = r3.y;
    r1 = tex2D(s1, v1.xy);
    r5.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = tex2D(s5, v6.xy);
    r2.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1 = (c2.xxxx) * (v8) + (c2.yyyy);
    r4.x = dot(r2.xy, r1.xy) + (c2.z);
    r4.y = dot(r2.xy, r1.zw) + (c2.z);
    r1 = tex2D(s2, v6.xy);
    r4.w = (r1.w) * (v0.y);
    r2.xy = (r0.yw) * (c4.yy) + (c4.zz);
    r6.xy = lerp(r5.xy, r4.xy, r4.ww);
    r0.y = dot(r2.xy, r2.xy) + (c2.z);
    r0.w = dot(r6.xy, r6.xy) + (c2.z);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c3.y) + (c3.z);
    r0.w = (r0.w) * (c3.y) + (c3.z);
    r1.w = (r0.y) * (r0.w);
    r0.y = dot(r2.xy, r6.xy) + (c2.z);
    r2 = tex2D(s14, v1.zw);
    r5.xy = (r2.xy) * (c4.xx);
    r2.w = saturate((r0.y) * (r1.w) + (r1.w));
    r3.xy = (r3.xz) * (r5.yy);
    r0.y = (r2.y) * (c4.x) + (-(r3.x));
    r4.xz = (r3.xy) * (c4.yy);
    r1.w = (r3.z) * (-(r5.y)) + (r0.y);
    r0.xy = (r0.xz) * (r5.xx);
    r4.y = (r1.w) + (r1.w);
    r1.w = (r2.x) * (c4.x) + (-(r0.x));
    r2.xyz = (r2.www) * (r4.xyz);
    r0.z = (r0.z) * (-(r5.x)) + (r1.w);
    r3.xz = (r0.xy) * (c4.yy);
    r3.y = (r0.z) + (r0.z);
    r4.xyz = (r3.xyz) * (r0.www) + (r2.xyz);
    r2 = tex2D(s6, v1.xy);
    r0 = tex2D(s7, v6.xy);
    r5.xyz = v2.xyz;
    r5.xyz = (r6.xxx) * (v4.xyz) + (r5.xyz);
    r5.xyz = (r6.yyy) * (v3.xyz) + (r5.xyz);
    r9.xyz = normalize(r5.xyz);
    r13.xy = lerp(r2.wy, r0.wy, r4.ww);
    r1.w = saturate(dot(r9.xyz, c[17].xyz));
    r6.xyz = (r4.xyz) * (r13.yyy);
    r5.xyz = (r1.www) * (c[18].xyz);
    r0 = tex2D(s12, v1.zw);
    r0.w = dot(v5.xyz, v5.xyz);
    r0.x = rsqrt(r0.w);
    r2.z = (r13.x) * (c11.x);
    r4.xyz = (r0.xxx) * (v5.xyz);
    r2.w = (r13.x) * (-(c11.x)) + (c11.y);
    r0.w = saturate(dot(r9.xyz, -(r4.xyz)));
    r0.z = (r1.w) * (r2.w) + (r2.z);
    r2.w = (r0.w) * (r2.w) + (r2.z);
    r0.z = (r0.z) * (r2.w) + (c4.w);
    r0.z = 1.0f / (r0.z);
    r7.xyz = (v5.xyz) * (-(r0.xxx)) + (c[17].xyz);
    r1.w = (r1.w) * (r0.z);
    r2.xyz = normalize(r7.xyz);
    r11.xy = (r13.xx) * (c12.xy) + (c12.zw);
    r3.w = saturate(dot(r9.xyz, r2.xyz));
    r0.z = exp2(r11.y);
    r0.x = saturate(dot(r2.xyz, c[17].xyz));
    r2.w = pow(abs(r3.w), r0.z);
    r0.z = (r0.z) * (c10.x) + (c10.y);
    r0.x = (-(r0.x)) + (c2.w);
    r0.z = (r2.w) * (r0.z);
    r3.w = (r0.x) * (r0.x);
    r2 = tex2D(s3, v6.zw);
    r2.xyz = (r2.xyz) + (c2.yyy);
    r3.w = (r3.w) * (r3.w);
    r12.xyz = (v0.zzz) * (r2.xyz) + (c2.www);
    r2 = tex2D(s4, v7.xy);
    r7.xyz = (r2.xyz) + (c2.yyy);
    r2.xyz = (r12.xyz) * (c3.xxx);
    r10.xyz = (v0.www) * (r7.xyz) + (c2.www);
    r0.x = (r0.x) * (r3.w);
    r2.xyz = (r2.xyz) * (r10.xyz);
    r8.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r2.xyz) * (-(r2.xyz)) + (c2.www);
    r0.z = (r1.w) * (r0.z);
    r2.xyz = (r7.xyz) * (r0.xxx) + (r8.xyz);
    r5.xyz = (r0.yyy) * (r5.xyz) + (r6.xyz);
    r2.xyz = (r0.zzz) * (r2.xyz);
    r3.w = (-(r0.w)) + (c2.w);
    r2.xyz = (r13.yyy) * (r2.xyz);
    r1.w = 1.0f / (r11.x);
    r6.xyz = (r2.xyz) * (c[19].xyz);
    r2 = tex2D(s0, v1.xy);
    r11.xyz = lerp(r2.xyz, r1.xyz, r4.www);
    r6.xyz = (r0.yyy) * (r6.xyz);
    r0.xyz = (r12.xyz) * (r11.xyz);
    r0.xyz = (r10.xyz) * (r0.xyz);
    r0.w = dot(r4.xyz, r9.xyz);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r0.w) + (r0.w);
    r0.xyz = (r9.xyz) * (-(r0.www)) + (r4.xyz);
    r0.w = (r13.x) * (c3.w);
    r0 = texCUBElod(s15, r0);
    r0.w = (r3.w) * (r3.w);
    r0.w = (r3.w) * (r0.w);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r1.w) * (r0.w);
    r0.xyz = (r13.yyy) * (r0.xyz);
    r1.xyz = (r7.xyz) * (r0.www) + (r8.xyz);
    r2.xyz = (r2.xyz) * (r5.xyz) + (r6.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz);
    r0.w = dot(c[5].xyz, r4.xyz);
    r1.xyz = (r3.xyz) * (r0.xyz);
    r0.w = saturate((c[7].y) * (r0.w) + (c[7].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[6].xyz);
    r1.xyz = (r1.xyz) * (c3.www) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[9].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[8].w;

    return oC0;
}
