// Mechanically reconstructed from 0x5928DB40.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD4;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-1.0f, 0.0f, 0.200000003f, 1.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c14 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v5.ww) * (c[27].xy) + (v5.xy);
    r0.zw = v5.zw;
    r0 = tex2Dproj(s1, r0);
    r1.xy = (v5.ww) * (-(c[27].xy)) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.y = r1.x;
    r1.xy = (v5.ww) * (c[27].zw) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.z = r1.x;
    r1.xy = (v5.ww) * (-(c[27].zw)) + (v5.xy);
    r1.zw = v5.zw;
    r1 = tex2Dproj(s1, r1);
    r0.w = r1.x;
    r1.w = dot(r0, c4.zzzz);
    r0 = tex2D(s12, v0.zw);
    r2.xyz = (-(v6.xyz)) + (c[20].xyz);
    r5.y = dot(r2.xyz, r2.xyz);
    r2.w = rsqrt(r5.y);
    r3.xyz = (r2.xyz) * (r2.www);
    r0.w = dot(r3.xyz, c[23].xyz);
    r5.x = 1.0f / (r2.w);
    r0.z = saturate((r0.w) * (c[24].x) + (c[24].y));
    r4.xy = saturate((r5.xx) * (c[26].xy) + (c[26].zw));
    r1.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c14.xx) + (c14.yy);
    r0.w = dot(c[25].yz, r5.xy) + (c[25].x);
    r1.xy = (r1.xy) * (r4.xy);
    r0.z = (r0.z) * (c[24].w);
    r0.w = (r0.w) * (r1.x);
    r3.w = lerp(r0.y, r1.w, r0.z);
    r4.w = (r1.y) * (r0.w);
    r0 = tex2D(s3, v7.zw);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s2, v7.zw);
    r9.w = (r1.w) * (v7.y);
    r10.w = (r3.w) * (r4.w);
    r6.xy = (r0.xy) * (r9.ww);
    r5.xyz = normalize(v6.xyz);
    r0 = v1;
    r0.xyz = (r6.xxx) * (v4.xyz) + (r0.xyz);
    r4.xyz = (r2.xyz) * (r2.www) + (-(r5.xyz));
    r2.xyz = (r6.yyy) * (v3.xyz) + (r0.xyz);
    r0.xyz = normalize(r4.xyz);
    r9.xyz = normalize(r2.xyz);
    r2 = tex2D(s4, v7.zw);
    r2.yzw = (r2.wyy) + (c1.xyx);
    r1.w = saturate(dot(r9.xyz, r0.xyz));
    r10.xy = (r9.ww) * (r2.yw) + (c1.ww);
    r2.w = saturate(dot(r0.xyz, r3.xyz));
    r0.xy = (r10.xx) * (c13.xy) + (c13.zw);
    r0.z = saturate(dot(r9.xyz, r3.xyz));
    r0.y = exp2(r0.y);
    r7.w = 1.0f / (r0.x);
    r2.x = pow(abs(r1.w), r0.y);
    r1.w = (r0.y) * (c4.y) + (c4.z);
    r3.z = (r10.x) * (c2.w);
    r2.y = (r10.x) * (-(c12.z)) + (c12.w);
    r0.x = saturate(dot(r9.xyz, -(r5.xyz)));
    r0.y = (r0.z) * (r2.y) + (r3.z);
    r2.y = (r0.x) * (r2.y) + (r3.z);
    r1.w = (r2.x) * (r1.w);
    r0.y = (r0.y) * (r2.y) + (c4.x);
    r8.w = (-(r0.x)) + (c1.w);
    r0.y = 1.0f / (r0.y);
    r2.y = (r0.z) * (r0.y);
    r2.w = (-(r2.w)) + (c1.w);
    r0.xyz = (r0.zzz) * (c[21].xyz);
    r2.x = (r2.w) * (r2.w);
    r3.w = (r2.x) * (r2.x);
    r2.x = c1.z;
    r2.w = (r2.w) * (r3.w);
    r7.xz = (r9.ww) * (r2.xz);
    r6.w = (r7.x) * (r7.x);
    r5.w = (r7.x) * (-(r7.x)) + (c1.w);
    r1.w = (r1.w) * (r2.y);
    r2.w = (r5.w) * (r2.w) + (r6.w);
    r1.w = (r1.w) * (r2.w);
    r2.xy = (v0.zw) * (c3.xy);
    r2 = tex2D(s13, r2.xy);
    r3.xy = (v0.zw) * (c3.xy) + (c3.zy);
    r4 = tex2D(s13, r3.xy);
    r2.w = r4.y;
    r4.w = (r7.z) * (r1.w);
    r3.xy = (r2.yw) * (c12.xx) + (c12.yy);
    r1.w = dot(r6.xy, r6.xy) + (c1.y);
    r2.w = dot(r3.xy, r3.xy) + (c1.y);
    r1.w = exp2(-(r1.w));
    r2.w = exp2(-(r2.w));
    r1.w = (r1.w) * (c2.x) + (c2.y);
    r2.y = (r2.w) * (c2.x) + (c2.y);
    r2.w = dot(r3.xy, r6.xy) + (c1.y);
    r2.y = (r1.w) * (r2.y);
    r3 = tex2D(s14, v0.zw);
    r7.xy = (r3.xy) * (c3.ww);
    r3.w = saturate((r2.w) * (r2.y) + (r2.y));
    r4.xy = (r4.xz) * (r7.yy);
    r2.w = (r3.y) * (c3.w) + (-(r4.x));
    r6.xz = (r4.xy) * (c12.xx);
    r2.w = (r4.z) * (-(r7.y)) + (r2.w);
    r2.xy = (r2.xz) * (r7.xx);
    r6.y = (r2.w) + (r2.w);
    r2.w = (r3.x) * (c3.w) + (-(r2.x));
    r4.xyz = (r3.www) * (r6.xyz);
    r2.w = (r2.z) * (-(r7.x)) + (r2.w);
    r8.xz = (r2.xy) * (c12.xx);
    r8.y = (r2.w) + (r2.w);
    r3.xyz = (r4.www) * (c[22].xyz);
    r2.xyz = (r8.xyz) * (r1.www) + (r4.xyz);
    r4.xyz = (r10.www) * (r3.xyz);
    r2.xyz = (r10.yyy) * (r2.xyz);
    r1.w = (r10.x) * (c2.z);
    r3.xyz = (r10.www) * (r0.xyz) + (r2.xyz);
    r2 = tex2D(s0, v0.xy);
    r0.xyz = lerp(r2.xyz, r1.xyz, r9.www);
    r1.z = dot(r5.xyz, r9.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.z = (r1.z) + (r1.z);
    r6.xyz = (r0.xyz) * (r3.xyz) + (r4.xyz);
    r1.xyz = (r9.xyz) * (-(r1.zzz)) + (r5.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r7.xyz = (r7.zzz) * (r1.xyz);
    r1.w = (r8.w) * (r8.w);
    r5.z = (r8.w) * (r1.w);
    r4 = (-(v6.yyyy)) + (c[6]);
    r1 = (r4) * (r4);
    r3 = (-(v6.xxxx)) + (c[5]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v6.zzzz)) + (c[7]);
    r5.z = (r7.w) * (r5.z);
    r1 = (r2) * (r2) + (r1);
    r6.w = (r5.w) * (r5.z) + (r6.w);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r7.xyz = (r7.xyz) * (r6.www);
    r4 = (r4) * (r5);
    r4 = (r9.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r9.xxxx) + (r4);
    r4.x = c1.w;
    r1 = saturate((r1) * (c[8]) + (r4.xxxx));
    r2 = saturate((r2) * (r9.zzzz) + (r3));
    r3.xyz = (r8.xyz) * (r7.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c2.zzz) + (r6.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r0.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v2.xyz);
    r0.xyz = max(((r0.xyz) * (c[28].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.w;

    return oC0;
}
