// Mechanically reconstructed from 0xF86FF573.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c2 = float4(-2.0f, 3.0f, 0.0009765625f, 0.25f);
    const float4 c3 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c4 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v6.ww) * (c[29].xy) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.xy = (v6.ww) * (-(c[29].xy)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.y = r1.x;
    r1.xy = (v6.ww) * (c[29].zw) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.z = r1.x;
    r1.xy = (v6.ww) * (-(c[29].zw)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r2.xy = (v1.xy) * (c[31].xy);
    r2 = tex2D(s4, r2.xy);
    r3.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r2 = tex2D(s1, v1.xy);
    r2.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r3.xy = (c[31].zz) * (r3.xy) + (r2.xy);
    r2.xyz = v2.xyz;
    r2.xyz = (r3.xxx) * (v5.xyz) + (r2.xyz);
    r2.xyz = (r3.yyy) * (v4.xyz) + (r2.xyz);
    r10.xyz = normalize(r2.xyz);
    r2.w = max(abs(r10.y), abs(r10.z));
    r0.w = r1.x;
    r1.w = max(abs(r10.x), r2.w);
    r1.w = 1.0f / (r1.w);
    r1.xyz = (r10.xyz) * (c[5].xyz);
    r2.z = dot(r0, c2.wwww);
    r0.xyz = (r1.xyz) * (r1.www) + (v8.xyz);
    r0 = tex3D(s11, r0.xyz);
    r3.xyz = (-(v7.xyz)) + (c[21].xyz);
    r5.y = dot(r3.xyz, r3.xyz);
    r4.w = rsqrt(r5.y);
    r4.xyz = (r3.xyz) * (r4.www);
    r1.w = dot(r4.xyz, c[25].xyz);
    r1.w = saturate((r1.w) * (c[26].x) + (c[26].y));
    r5.x = 1.0f / (r4.w);
    r2.w = (r1.w) * (c[26].w);
    r2.xy = saturate((r5.xx) * (c[28].xy) + (c[28].zw));
    r1.xy = (r2.xy) * (r2.xy);
    r2.xy = (r2.xy) * (c2.xx) + (c2.yy);
    r1.z = dot(c[27].yz, r5.xy) + (c[27].x);
    r1.xy = (r1.xy) * (r2.xy);
    r1.w = lerp(r0.w, r2.z, r2.w);
    r0.w = (r1.z) * (r1.x);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r1.y) * (r0.w);
    r2.w = (r1.w) * (r0.w);
    r3.w = saturate(dot(r10.xyz, r4.xyz));
    r0 = tex2D(s3, v1.xy);
    r5.y = (r0.w) * (c1.w);
    r5.w = (r0.w) * (-(c1.w)) + (c1.z);
    r1.xyz = normalize(v7.xyz);
    r1.w = (r3.w) * (r5.w) + (r5.y);
    r5.z = saturate(dot(r10.xyz, -(r1.xyz)));
    r5.w = (r5.z) * (r5.w) + (r5.y);
    r6.w = (-(r5.z)) + (c1.z);
    r1.w = (r1.w) * (r5.w) + (c2.z);
    r5.xyz = (r3.xyz) * (r4.www) + (-(r1.xyz));
    r1.w = 1.0f / (r1.w);
    r3.xyz = normalize(r5.xyz);
    r4.w = (r3.w) * (r1.w);
    r5.w = saturate(dot(r10.xyz, r3.xyz));
    r3.z = saturate(dot(r3.xyz, r4.xyz));
    r3.xy = (r0.ww) * (c3.xy) + (c3.zw);
    r1.w = exp2(r3.y);
    r3.z = (-(r3.z)) + (c1.z);
    r4.z = pow(abs(r5.w), r1.w);
    r3.y = (r3.z) * (r3.z);
    r1.w = (r1.w) * (c4.x) + (c4.y);
    r3.y = (r3.y) * (r3.y);
    r1.w = (r4.z) * (r1.w);
    r3.z = (r3.z) * (r3.y);
    r11.xyz = (r0.xyz) * (r0.xyz);
    r9.xyz = (r0.xyz) * (-(r0.xyz)) + (c1.zzz);
    r1.w = (r4.w) * (r1.w);
    r0.xyz = (r9.xyz) * (r3.zzz) + (r11.xyz);
    r5.w = 1.0f / (r3.x);
    r0.xyz = (r1.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c[30].www);
    r4.xy = c[24].xy;
    r3.xyz = (r4.yyy) * (c[23].xyz);
    r1.w = (r0.w) * (c1.x);
    r3.xyz = (r0.xyz) * (r3.xyz);
    r2.xyz = (r2.xyz) * (c[30].yyy);
    r0.xyz = (r4.xxx) * (c[22].xyz);
    r2.xyz = (r2.xyz) * (c1.yyy);
    r0.xyz = (r3.www) * (r0.xyz);
    r3.xyz = (r2.www) * (r3.xyz);
    r2.xyz = (r2.www) * (r0.xyz) + (r2.xyz);
    r0 = tex2D(s0, v1.xy);
    r2.w = dot(r1.xyz, r10.xyz);
    r2.w = (r2.w) + (r2.w);
    r1.xyz = (r10.xyz) * (-(r2.www)) + (r1.xyz);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0 = (r0.wxyz) * (v0.wxyz);
    r4.xyz = (r1.xyz) * (c1.xxx);
    r1 = tex3D(s11, v8.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r6.xyz = (r0.yzw) * (r0.yzw);
    r1.xyz = (r4.xyz) * (r1.xyz);
    r7.xyz = (r6.xyz) * (r2.xyz) + (r3.xyz);
    r8.xyz = (r1.xyz) * (c1.yyy);
    r0.w = (r6.w) * (r6.w);
    r4 = (-(v7.yyyy)) + (c[7]);
    r1 = (r4) * (r4);
    r3 = (-(v7.xxxx)) + (c[6]);
    r1 = (r3) * (r3) + (r1);
    r2 = (-(v7.zzzz)) + (c[8]);
    r0.w = (r6.w) * (r0.w);
    r1 = (r2) * (r2) + (r1);
    r0.w = (r5.w) * (r0.w);
    r5.x = rsqrt(r1.x);
    r5.y = rsqrt(r1.y);
    r5.z = rsqrt(r1.z);
    r5.w = rsqrt(r1.w);
    r9.xyz = (r9.xyz) * (r0.www) + (r11.xyz);
    r4 = (r4) * (r5);
    r4 = (r10.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r10.xxxx) + (r4);
    r0.y = c1.z;
    r1 = saturate((r1) * (c[9]) + (r0.yyyy));
    r2 = saturate((r2) * (r10.zzzz) + (r3));
    r3.xyz = (r8.xyz) * (r9.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r3.xyz) * (c[30].xxx) + (r7.xyz);
    r2.x = dot(c[10], r1);
    r2.y = dot(c[11], r1);
    r2.z = dot(c[20], r1);
    r1.xyz = (r6.xyz) * (r2.xyz) + (r3.xyz);
    r1.xyz = (r0.xxx) * (r1.xyz);
    r1.w = c1.z;
    r2.x = dot(r1, c[33]);
    r2.y = dot(r1, c[34]);
    r2.z = dot(r1, c[35]);
    r1.xyz = (v3.xyz) * (-(r0.xxx)) + (r2.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r1.xyz = (v3.xyz) * (r0.xxx) + (r1.xyz);
    r1.xyz = max(((r1.xyz) * (c[32].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.x);
    r0.x = rsqrt(r1.x);
    r0.y = rsqrt(r1.y);
    r0.z = rsqrt(r1.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
