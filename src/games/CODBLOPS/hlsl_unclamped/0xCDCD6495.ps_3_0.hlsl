// Mechanically reconstructed from 0xCDCD6495.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.200000003f, 0.0f, 1.0f, 0.200000003f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c9 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c10 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s6, v7.zw);
    r1 = tex2D(s5, v7.xy);
    r2.xyz = c1.xxx;
    r4 = tex2D(s4, v1.xy);
    r2.w = -(r4.w);
    r1 = (r1) + (r2);
    r3 = (r4.wwww) * (c1.yyyz) + (c1.wwwy);
    r2 = tex2D(s2, v7.xy);
    r9.w = (r2.w) * (v0.y);
    r5 = (r9.wwww) * (r1) + (r3);
    r1 = tex2D(s3, v7.zw);
    r8.w = (r1.w) * (v0.z);
    r3 = lerp(r5, r0, r8.wwww);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r5.w = (r3.w) * (c2.w);
    r0.xy = (r9.ww) * (-(r0.xy)) + (r0.xy);
    r2.w = (r3.w) * (-(c9.z)) + (c9.w);
    r13.xy = (r8.ww) * (-(r0.xy)) + (r0.xy);
    r0 = v2;
    r0.xyz = (r13.xxx) * (v5.xyz) + (r0.xyz);
    r1.w = dot(v6.xyz, v6.xyz);
    r5.xyz = (r13.yyy) * (v4.xyz) + (r0.xyz);
    r4.z = rsqrt(r1.w);
    r0.xyz = normalize(r5.xyz);
    r10.xyz = (r4.zzz) * (v6.xyz);
    r4.w = saturate(dot(r0.xyz, -(r10.xyz)));
    r1.w = saturate(dot(r0.xyz, c[17].xyz));
    r4.x = (r4.w) * (r2.w) + (r5.w);
    r2.w = (r1.w) * (r2.w) + (r5.w);
    r2.w = (r2.w) * (r4.x) + (c4.x);
    r6.xyz = (v6.xyz) * (-(r4.zzz)) + (c[17].xyz);
    r2.w = 1.0f / (r2.w);
    r7.w = (-(r4.w)) + (c1.z);
    r4.z = (r1.w) * (r2.w);
    r11.xyz = (r1.www) * (c[18].xyz);
    r5.xyz = normalize(r6.xyz);
    r5.w = saturate(dot(r0.xyz, r5.xyz));
    r6.xy = (r3.ww) * (c10.xy) + (c10.zw);
    r2.w = saturate(dot(r5.xyz, c[17].xyz));
    r1.w = exp2(r6.y);
    r4.x = pow(abs(r5.w), r1.w);
    r2.w = (-(r2.w)) + (c1.z);
    r1.w = (r1.w) * (c4.y) + (c4.z);
    r4.w = (r2.w) * (r2.w);
    r1.w = (r4.x) * (r1.w);
    r4.w = (r4.w) * (r4.w);
    r1.w = (r4.z) * (r1.w);
    r2.w = (r2.w) * (r4.w);
    r8.xyz = (r3.xyz) * (r3.xyz);
    r7.xyz = (r3.xyz) * (-(r3.xyz)) + (c1.zzz);
    r3.xyz = (r7.xyz) * (r2.www) + (r8.xyz);
    r5.xyz = lerp(r4.yyy, c[5].xyw, r9.www);
    r3.xyz = (r1.www) * (r3.xyz);
    r9.xyz = lerp(r5.xyz, c[6].xyw, r8.www);
    r2.w = 1.0f / (r6.x);
    r3.xyz = (r3.xyz) * (r9.zzz);
    r1.w = (r3.w) * (c2.z);
    r12.xyz = (r3.xyz) * (c[19].xyz);
    r3 = tex2D(s12, v1.zw);
    r4.xy = (v1.zw) * (c3.xy);
    r4 = tex2D(s13, r4.xy);
    r5.xy = (v1.zw) * (c3.xy) + (c3.zy);
    r6 = tex2D(s13, r5.xy);
    r4.w = r6.y;
    r5.xy = (r4.yw) * (c9.xx) + (c9.yy);
    r3.w = dot(r13.xy, r13.xy) + (c1.y);
    r3.z = dot(r5.xy, r5.xy) + (c1.y);
    r3.w = exp2(-(r3.w));
    r3.z = exp2(-(r3.z));
    r3.w = (r3.w) * (c2.x) + (c2.y);
    r3.x = (r3.z) * (c2.x) + (c2.y);
    r3.z = dot(r5.xy, r13.xy) + (c1.y);
    r3.x = (r3.w) * (r3.x);
    r5 = tex2D(s14, v1.zw);
    r14.xy = (r5.xy) * (c3.ww);
    r3.x = saturate((r3.z) * (r3.x) + (r3.x));
    r6.xy = (r6.xz) * (r14.yy);
    r3.z = (r5.y) * (c3.w) + (-(r6.x));
    r13.xz = (r6.xy) * (c9.xx);
    r3.z = (r6.z) * (-(r14.y)) + (r3.z);
    r4.xy = (r4.xz) * (r14.xx);
    r13.y = (r3.z) + (r3.z);
    r3.z = (r5.x) * (c3.w) + (-(r4.x));
    r5.xyz = (r3.xxx) * (r13.xyz);
    r3.z = (r4.z) * (-(r14.x)) + (r3.z);
    r4.xz = (r4.xy) * (c9.xx);
    r4.y = (r3.z) + (r3.z);
    r5.xyz = (r4.xyz) * (r3.www) + (r5.xyz);
    r5.xyz = (r9.yyy) * (r5.xyz);
    r6.xyz = (r12.xyz) * (r3.yyy);
    r5.xyz = (r3.yyy) * (r11.xyz) + (r5.xyz);
    r3 = tex2D(s0, v1.xy);
    r3.w = dot(r10.xyz, r0.xyz);
    r11.xyz = lerp(r3.xyz, r2.xyz, r9.www);
    r2.z = (r3.w) + (r3.w);
    r3.xyz = lerp(r11.xyz, r1.xyz, r8.www);
    r1.xyz = (r0.xyz) * (-(r2.zzz)) + (r10.xyz);
    r1 = texCUBElod(s15, r1);
    r0.z = (r7.w) * (r7.w);
    r1.w = (r7.w) * (r0.z);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r2.w) * (r1.w);
    r0.xyz = (r9.xxx) * (r0.xyz);
    r2.xyz = (r7.xyz) * (r1.www) + (r8.xyz);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r1.xyz) * (r5.xyz) + (r6.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c2.zzz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[8].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c[7].w;

    return oC0;
}
