// Mechanically reconstructed from 0x58C98369.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(0.600000024f, 0.400000006f, 8.0f, 31.875f);
    const float4 c2 = float4(2.0f, -1.0f, 0.0f, 1.0f);
    const float4 c3 = float4(4.0f, -2.0f, 0.0f, 0.0f);
    const float4 c4 = float4(3.5f, 1.0f, 0.5f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v0.zw) * (c4.yz);
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v0.zw) * (c4.yz) + (c4.wz);
    r2 = tex2D(s13, r1.xy);
    r0.w = r2.y;
    r1 = tex2D(s1, v0.xy);
    r3.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s3, v6.zw);
    r4.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = (c2.xxxx) * (v7) + (c2.yyyy);
    r1.x = dot(r4.xy, r1.xy) + (c2.z);
    r1.y = dot(r4.xy, r1.zw) + (c2.z);
    r4 = tex2D(s2, v6.zw);
    r6.w = (r4.w) * (v6.y);
    r6.xy = (r0.yw) * (c3.xx) + (c3.yy);
    r5.xy = lerp(r3.xy, r1.xy, r6.ww);
    r0.w = dot(r6.xy, r6.xy) + (c2.z);
    r0.y = dot(r5.xy, r5.xy) + (c2.z);
    r0.w = exp2(-(r0.w));
    r0.y = exp2(-(r0.y));
    r0.w = (r0.w) * (c1.x) + (c1.y);
    r5.w = (r0.y) * (c1.x) + (c1.y);
    r1 = tex2D(s14, v0.zw);
    r3.xy = (r1.xy) * (c1.ww);
    r1.w = (r0.w) * (r5.w);
    r2.xy = (r2.xz) * (r3.yy);
    r0.w = dot(r6.xy, r5.xy) + (c2.z);
    r0.y = (r1.y) * (c1.w) + (-(r2.x));
    r0.w = saturate((r0.w) * (r1.w) + (r1.w));
    r0.y = (r2.z) * (-(r3.y)) + (r0.y);
    r2.xz = (r2.xy) * (c3.xx);
    r2.y = (r0.y) + (r0.y);
    r6.xyz = (r0.www) * (r2.xyz);
    r0.xy = (r0.xz) * (r3.xx);
    r0.w = (r1.x) * (c1.w) + (-(r0.x));
    r1.xyz = v1.xyz;
    r1.xyz = (r5.xxx) * (v4.xyz) + (r1.xyz);
    r5.xz = (r0.xy) * (c3.xx);
    r1.xyz = (r5.yyy) * (v3.xyz) + (r1.xyz);
    r8.xyz = normalize(r1.xyz);
    r7.xyz = normalize(v5.xyz);
    r5.y = (r0.z) * (-(r3.x)) + (r0.w);
    r0.w = dot(r7.xyz, r8.xyz);
    r0.xy = c[20].xy;
    r0.xy = (-(r0.xy)) + (c[21].xy);
    r0.w = (r0.w) + (r0.w);
    r9.xy = (r6.ww) * (r0.xy) + (c[20].xy);
    r1.xyz = (r8.xyz) * (-(r0.www)) + (r7.xyz);
    r3 = tex2D(s4, v0.xy);
    r2 = tex2D(s5, v6.zw);
    r0 = lerp(r3, r2, r6.wwww);
    r2.w = saturate(dot(r8.xyz, -(r7.xyz)));
    r1.w = (r0.w) * (c1.z);
    r1 = texCUBElod(s15, r1);
    r2.w = (-(r2.w)) + (c2.w);
    r2.z = (r2.w) * (r2.w);
    r1.w = (r0.w) * (c4.x) + (c4.y);
    r0.w = (r2.w) * (r2.z);
    r1.w = 1.0f / (r1.w);
    r0.w = (r0.w) * (r1.w);
    r2.xyz = (r0.xyz) * (-(r0.xyz)) + (c2.www);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r2.xyz = (r0.www) * (r2.xyz);
    r1.xyz = (r9.xxx) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz) + (r2.xyz);
    r5.y = (r5.y) + (r5.y);
    r0.xyz = (r1.xyz) * (r0.xyz);
    r1.xyz = (r5.xyz) * (r5.www) + (r6.xyz);
    r0.xyz = (r5.xyz) * (r0.xyz);
    r6.xyz = (r9.yyy) * (r1.xyz);
    r7.xyz = (r0.xyz) * (c1.zzz);
    r5 = tex2D(s0, v0.xy);
    r9.xyz = lerp(r5.xyz, r4.xyz, r6.www);
    r3 = (-(v5.yyyy)) + (c[6]);
    r0 = (r3) * (r3);
    r2 = (-(v5.xxxx)) + (c[5]);
    r0 = (r2) * (r2) + (r0);
    r1 = (-(v5.zzzz)) + (c[7]);
    r5.w = (r5.w) * (-(v6.x)) + (c2.w);
    r0 = (r1) * (r1) + (r0);
    r6.w = (r4.w) * (-(v6.y)) + (c2.w);
    r4.x = rsqrt(r0.x);
    r4.y = rsqrt(r0.y);
    r4.z = rsqrt(r0.z);
    r4.w = rsqrt(r0.w);
    r5.xyz = (r9.xyz) * (r9.xyz);
    r3 = (r3) * (r4);
    r3 = (r8.yyyy) * (r3);
    r2 = (r2) * (r4);
    r1 = (r1) * (r4);
    r2 = (r2) * (r8.xxxx) + (r3);
    r3.x = c2.w;
    r0 = saturate((r0) * (c[8]) + (r3.xxxx));
    r1 = saturate((r1) * (r8.zzzz) + (r2));
    r2.xyz = (r5.xyz) * (r6.xyz) + (r7.xyz);
    r0 = (r0) * (r1);
    r1.x = dot(c[9], r0);
    r1.y = dot(c[10], r0);
    r1.z = dot(c[11], r0);
    r0.w = (r5.w) * (-(r6.w)) + (c2.w);
    r0.xyz = (r5.xyz) * (r1.xyz) + (r2.xyz);
    r1.xyz = (r0.www) * (v2.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (-(r1.xyz));
    r0.xyz = (v1.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[22].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
