// Mechanically reconstructed from 0x8A8E3761.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s8 : register(s8);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(-0.5f, 0.200000003f, 8.0f, 31.875f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c3 = float4(1.0f, 0.797884583f, 0.0009765625f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, 0.000244140625f, 0.0f);
    const float4 c13 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.75f);
    const float4 c14 = float4(4.0f, -3.0f, -4.0f, 2.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 r15 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v7.xyz, v7.xyz);
    r0.w = rsqrt(r0.w);
    r0.xyz = (v7.xyz) * (-(r0.www)) + (c[17].xyz);
    r2.xyz = (-(v7.xyz)) + (c[6].xyz);
    r4.xyz = normalize(r0.xyz);
    r0.z = dot(r2.xyz, r2.xyz);
    r1.w = rsqrt(r0.z);
    r5.xyz = (r0.www) * (v7.xyz);
    r0.w = saturate(dot(r4.xyz, c[17].xyz));
    r0.xyz = (r2.xyz) * (r1.www) + (-(r5.xyz));
    r4.w = (-(r0.w)) + (c3.x);
    r1.xyz = normalize(r0.xyz);
    r0 = tex2D(s1, v1.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = v2;
    r0.xyz = (r3.xxx) * (v5.xyz) + (r0.xyz);
    r10.xyz = (r2.xyz) * (r1.www);
    r2.xyz = (r3.yyy) * (v4.xyz) + (r0.xyz);
    r1.w = saturate(dot(r1.xyz, r10.xyz));
    r0.xyz = normalize(r2.xyz);
    r2.w = (-(r1.w)) + (c3.x);
    r3.w = saturate(dot(r0.xyz, r1.xyz));
    r1.w = (r2.w) * (r2.w);
    r2.z = (r1.w) * (r1.w);
    r6.w = saturate(dot(r10.xyz, r0.xyz));
    r1 = tex2D(s5, v1.xy);
    r11.w = (r1.w) * (-(c3.y)) + (c3.x);
    r12.w = (r1.w) * (c3.y);
    r5.w = saturate(dot(r0.xyz, -(r5.xyz)));
    r1.z = (r6.w) * (r11.w) + (r12.w);
    r10.w = (r5.w) * (r11.w) + (r12.w);
    r6.z = (r2.w) * (r2.z);
    r1.z = (r1.z) * (r10.w) + (c3.z);
    r1.z = 1.0f / (r1.z);
    r6.xy = (r1.ww) * (c4.xy) + (c4.zw);
    r7.w = (r6.w) * (r1.z);
    r1.z = exp2(r6.y);
    r2.w = pow(abs(r3.w), r1.z);
    r8.w = (r1.z) * (c12.x) + (c12.y);
    r6.y = (r2.w) * (r8.w);
    r2.xy = (v1.xy) * (c[32].xy);
    r2 = tex2D(s8, r2.xy);
    r2.xyz = (r2.xyz) + (c0.xxx);
    r3 = tex2D(s0, v1.xy);
    r2.xyz = saturate((r2.xyz) * (r3.www) + (r3.xyz));
    r2.w = (r7.w) * (r6.y);
    r3.xyz = lerp(r2.xyz, c0.yyy, r1.xxx);
    r13.xyz = (r3.xyz) * (-(r3.xyz)) + (c3.xxx);
    r14.xyz = (r3.xyz) * (r3.xyz);
    r3.xyz = (r1.xxx) * (r2.xyz);
    r2.xyz = (r13.xyz) * (r6.zzz) + (r14.xyz);
    r3.xyz = (r3.xyz) * (v0.xyz);
    r2.xyz = (r2.www) * (r2.xyz);
    r9.xyz = (r3.xyz) * (r3.xyz);
    r2.xyz = (r2.xyz) * (c[8].xyz);
    r3.xyz = (r1.yyy) * (r2.xyz);
    r2.xyz = (r6.www) * (c[7].xyz);
    r3.xyz = (r9.xyz) * (r2.xyz) + (r3.xyz);
    r1.x = 1.0f / (r6.x);
    r5.w = (-(r5.w)) + (c3.x);
    r2.w = dot(r5.xyz, r0.xyz);
    r6.w = (r5.w) * (r5.w);
    r2.z = (r2.w) + (r2.w);
    r2.w = (r1.w) * (c0.z);
    r2.xyz = (r0.xyz) * (-(r2.zzz)) + (r5.xyz);
    r2 = texCUBElod(s15, r2);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r1.w = (r5.w) * (r6.w);
    r5.xyz = (r2.xyz) * (c0.zzz);
    r2 = tex3D(s11, v8.xyz);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r1.w = (r1.x) * (r1.w);
    r2.xyz = (r5.xyz) * (r2.xyz);
    r2.xyz = (r2.xyz) * (c0.www);
    r5.xyz = (r13.xyz) * (r1.www) + (r14.xyz);
    r8.xyz = (r2.xyz) * (r5.xyz);
    r1.w = (r4.w) * (r4.w);
    r1.w = (r1.w) * (r1.w);
    r2.w = max(abs(r0.y), abs(r0.z));
    r1.x = (r4.w) * (r1.w);
    r1.w = max(abs(r0.x), r2.w);
    r2.w = 1.0f / (r1.w);
    r2.xyz = (r0.xyz) * (c[5].xyz);
    r1.w = saturate(dot(r0.xyz, r4.xyz));
    r2.xyz = (r2.xyz) * (r2.www) + (v8.xyz);
    r2 = tex3D(s11, r2.xyz);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r9.w = pow(abs(r1.w), r1.z);
    r2.xyz = (r1.yyy) * (r2.xyz);
    r1.z = saturate(dot(r0.xyz, c[17].xyz));
    r15.xy = c[29].xy;
    r0.xyz = (r15.xxx) * (c[18].xyz);
    r12.xyz = (r2.xyz) * (c0.www);
    r11.xyz = (r1.zzz) * (r0.xyz);
    if ((c3.x) >= (v6.w))
    {
        r4 = (v6.xyzx) * (c3.xxxw);
        r2 = (r4) + (-(c13.xyzz));
        r2 = tex2Dlod(s2, r2);
        r2.w = r2.x;
        r5 = (r4) + (c12.zzww);
        r5 = tex2Dlod(s2, r5);
        r2.x = r5.x;
        r5 = (r4) + (-(c12.zzww));
        r5 = tex2Dlod(s2, r5);
        r2.y = r5.x;
        r4 = (r4) + (c13.xyzz);
        r4 = tex2Dlod(s2, r4);
        r2.z = r4.x;
        r1.w = dot(r2, c12.yyyy);
        if ((c13.w) < (v6.w))
        {
            r0.xy = (v6.xy) * (c[2].ww) + (c[2].xy);
            r2.xy = (r0.xy) + (c12.zz);
            r2.zw = (v6.zx) * (c3.xw);
            r2 = tex2Dlod(s2, r2);
            r4.xy = (r0.xy) + (-(c12.zz));
            r4.zw = (v6.zx) * (c3.xw);
            r6 = tex2Dlod(s2, r4);
            r4.xy = (r0.xy) + (c13.xy);
            r4.zw = (v6.zx) * (c3.xw);
            r5 = tex2Dlod(s2, r4);
            r4.xy = (r0.xy) + (-(c13.xy));
            r4.zw = (v6.zx) * (c3.xw);
            r4 = tex2Dlod(s2, r4);
            r2.y = r6.x;
            r2.z = r5.x;
            r2.w = r4.x;
            r0.z = dot(r2, c12.yyyy);
            r0.y = (-(r1.w)) + (r0.z);
            r0.z = (v6.w) * (c14.x) + (c14.y);
            r1.w = (r0.z) * (r0.y) + (r1.w);
        }
    }
    else
    {
        r0.z = (v6.w) + (c14.z);
        r0.z = ((r0.z) >= 0.0f ? (c3.w) : (c3.x));
        if ((r0.z) != (-(r0.z)))
        {
            r0.xy = (v6.xy) * (c[2].ww) + (c[2].xy);
            r4.xy = (r0.xy) + (c12.zz);
            r4.zw = (v6.zz) * (c3.xw);
            r4 = tex2Dlod(s2, r4);
            r5.xy = (r0.xy) + (-(c12.zz));
            r5.zw = (v6.zz) * (c3.xw);
            r7 = tex2Dlod(s2, r5);
            r5.xy = (r0.xy) + (c13.xy);
            r5.zw = (v6.zz) * (c3.xw);
            r6 = tex2Dlod(s2, r5);
            r5.xy = (r0.xy) + (-(c13.xy));
            r5.zw = (v6.zz) * (c3.xw);
            r5 = tex2Dlod(s2, r5);
            r4.y = r7.x;
            r4.z = r6.x;
            r4.w = r5.x;
            r0.x = dot(r4, c12.yyyy);
            r0.z = saturate((v6.w) + (c14.y));
            r0.y = (r2.w) + (-(r0.x));
            r0.z = (r0.z) * (r0.y) + (r0.x);
        }
        else
        {
            r0.z = r2.w;
        }
        r1.w = r0.z;
    }
    r0.z = (r1.z) * (r11.w) + (r12.w);
    r0.z = (r0.z) * (r10.w) + (c3.z);
    r0.y = 1.0f / (r0.z);
    r0.z = (r8.w) * (r9.w);
    r0.y = (r1.z) * (r0.y);
    r1.z = (r0.z) * (r0.y);
    r0.xyz = (r13.xyz) * (r1.xxx) + (r14.xyz);
    r0.xyz = (r1.zzz) * (r0.xyz);
    r2.xyz = (r1.yyy) * (r0.xyz);
    r4.xyz = (r15.yyy) * (c[19].xyz);
    r0.xyz = (r1.www) * (r11.xyz) + (r12.xyz);
    r2.xyz = (r2.xyz) * (r4.xyz);
    r4.xyz = (r1.www) * (r2.xyz);
    r2 = (v7.yyyy) * (c[26]);
    r9.xyz = (r9.xyz) * (r0.xyz) + (r4.xyz);
    r2 = (v7.xxxx) * (c[25]) + (r2);
    r0.z = dot(r10.xyz, c[23].xyz);
    r2 = (v7.zzzz) * (c[27]) + (r2);
    r0.z = saturate((r0.z) * (c[24].x) + (c[24].y));
    r6 = (r2) + (c[28]);
    r0.y = (r0.z) * (-(c14.w)) + (-(c14.y));
    r5.zw = r6.zw;
    r0.z = (r0.z) * (r0.z);
    r7.zw = r5.zw;
    r1.w = (r0.y) * (r0.z);
    r4.zw = r7.zw;
    r2.xy = (r6.ww) * (-(c[30].zw)) + (r6.xy);
    r2.zw = r4.zw;
    r2 = tex2Dproj(s3, r2);
    r2.w = r2.x;
    r7.xy = (r5.ww) * (-(c[30].xy)) + (r6.xy);
    r7 = tex2Dproj(s3, r7);
    r2.y = r7.x;
    r5.xy = (r5.ww) * (c[30].xy) + (r6.xy);
    r7 = tex2Dproj(s3, r5);
    r2.x = r7.x;
    r4.xy = (r5.ww) * (c[30].zw) + (r6.xy);
    r5 = tex2Dproj(s3, r4);
    r4 = (v7.xyzx) * (c3.xxxw) + (c3.wwwx);
    r2.z = r5.x;
    r0.z = dot(r4, c[22]);
    r1.x = 1.0f / (r0.z);
    r5.x = dot(r4, c[21]);
    r0.x = dot(r4, c[11]);
    r5.y = (r5.x) * (r5.x);
    r0.y = dot(r4, c[20]);
    r0.z = dot(c[9].yz, r5.xy) + (c[9].x);
    r1.z = saturate(1.0f / (r0.z));
    r4.xy = saturate((r5.xx) * (c[10].xy) + (c[10].zw));
    r5.xy = (r4.xy) * (-(c14.ww)) + (-(c14.yy));
    r4.xy = (r4.xy) * (r4.xy);
    r0.z = ((-abs(r0.z)) >= 0.0f ? (c3.w) : (r1.z));
    r1.z = (r5.x) * (r4.x);
    r0.z = (r0.z) * (r1.z);
    r1.z = (r4.y) * (-(r5.y)) + (c3.x);
    r0.xy = (r1.xx) * (r0.xy);
    r0.z = (r0.z) * (r1.z);
    r1.z = (r1.w) * (r0.z);
    r0.xy = (r0.xy) * (-(c0.xx)) + (-(c0.xx));
    r4 = tex2D(s4, r0.xy);
    r0.xyz = (r4.xyz) * (r4.xyz);
    r1.w = dot(r2, c12.yyyy);
    r0.xyz = (r1.zzz) * (r0.xyz);
    r1.xyz = (r8.xyz) * (r1.yyy) + (r9.xyz);
    r0.xyz = (r1.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (r3.xyz) + (r1.xyz);
    r2.x = c[31].w;
    r1.xy = (c[33].xy) * (-(r2.xx)) + (v1.xy);
    r1 = tex2D(s7, r1.xy);
    r2.xy = (c[34].xy) * (-(r2.xx)) + (v1.xy);
    r2 = tex2D(s6, r2.xy);
    r0.xyz = (r1.xyz) * (-(r2.xyz)) + (r0.xyz);
    r0.xyz = (r3.www) * (r0.xyz);
    r1.xyz = (r1.xyz) * (r2.xyz) + (r0.xyz);
    r1.w = c3.x;
    r0.z = dot(r1, c[38]);
    r0.x = dot(r1, c[36]);
    r0.y = dot(r1, c[37]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[35].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c3.x;

    return oC0;
}
