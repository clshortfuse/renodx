// Mechanically reconstructed from 0x10C59BA5.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(2.0f, -1.0f, -0.5f, -0.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c4 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = tex2D(s0, v0.xy);
    r0 = (r1.xyzx) * (-(c1.yyyw)) + (-(c1.wwwy));
    r5.w = (r1.w) * (v6.x) + (c1.z);
    r1 = float4(((r5.w) >= 0.0f ? (r0.x) : (c1.w)), ((r5.w) >= 0.0f ? (r0.y) : (c1.w)), ((r5.w) >= 0.0f ? (r0.z) : (c1.w)), ((r5.w) >= 0.0f ? (r0.w) : (c1.w)));
    r0 = tex2D(s2, v6.zw);
    r2 = tex2D(s1, v0.xy);
    r2.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r3.xy = float2(((r5.w) >= 0.0f ? (r2.x) : (c1.w)), ((r5.w) >= 0.0f ? (r2.y) : (c1.w)));
    r2 = tex2D(s3, v6.zw);
    r4.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r2 = (c1.xxxx) * (v7) + (c1.yyyy);
    r2.x = dot(r4.xy, r2.xy) + (c1.w);
    r7.z = (r0.w) * (v6.y);
    r2.y = dot(r4.xy, r2.zw) + (c1.w);
    r12.xyz = lerp(r1.xyz, r0.xyz, r7.zzz);
    r5.xy = lerp(r3.xy, r2.xy, r7.zz);
    r0.xy = (v0.zw) * (-(c1.yz));
    r2 = tex2D(s13, r0.xy);
    r0.xy = (v0.zw) * (-(c1.yz)) + (-(c1.wz));
    r4 = tex2D(s13, r0.xy);
    r2.w = r4.y;
    r0.z = dot(r5.xy, r5.xy) + (c1.w);
    r0.xy = (r2.yw) * (c3.yy) + (c3.zz);
    r1.z = exp2(-(r0.z));
    r0.z = dot(r0.xy, r0.xy) + (c1.w);
    r4.w = (r1.z) * (c2.x) + (c2.y);
    r1.z = exp2(-(r0.z));
    r0.z = dot(r0.xy, r5.xy) + (c1.w);
    r0.y = (r1.z) * (c2.x) + (c2.y);
    r3 = tex2D(s14, v0.zw);
    r1.xy = (r3.xy) * (c3.xx);
    r1.z = (r4.w) * (r0.y);
    r0.xy = (r4.xz) * (r1.yy);
    r1.z = saturate((r0.z) * (r1.z) + (r1.z));
    r2.w = (r3.y) * (c3.x) + (-(r0.x));
    r0.xz = (r0.xy) * (c3.yy);
    r0.y = (r4.z) * (-(r1.y)) + (r2.w);
    r0.y = (r0.y) + (r0.y);
    r2.xy = (r2.xz) * (r1.xx);
    r0.xyz = (r1.zzz) * (r0.xyz);
    r1.z = (r3.x) * (c3.x) + (-(r2.x));
    r8.xz = (r2.xy) * (c3.yy);
    r1.z = (r2.z) * (-(r1.x)) + (r1.z);
    r8.y = (r1.z) + (r1.z);
    r2 = v1;
    r1.xyz = (r5.xxx) * (v4.xyz) + (r2.xyz);
    r0.xyz = (r8.xyz) * (r4.www) + (r0.xyz);
    r2.xyz = (r5.yyy) * (v3.xyz) + (r1.xyz);
    r13.yw = c1.yw;
    r1.xyz = float3(((r5.w) >= 0.0f ? (c[20].x) : (r13.w)), ((r5.w) >= 0.0f ? (c[20].y) : (r13.w)), ((r5.w) >= 0.0f ? (c[20].w) : (r13.w)));
    r9.xyz = normalize(r2.xyz);
    r11.xyz = lerp(r1.xyz, c[21].xyw, r7.zzz);
    r7.w = saturate(dot(r9.xyz, c[17].xyz));
    r1.xyz = (r0.xyz) * (r11.yyy);
    r0.xyz = (r7.www) * (c[18].xyz);
    r3 = tex2D(s12, v0.zw);
    r2.xyz = (r3.yyy) * (r0.xyz) + (r1.xyz);
    r4 = tex2D(s4, v0.xy);
    r6 = float4(((r5.w) >= 0.0f ? (r4.x) : (-(c1.w))), ((r5.w) >= 0.0f ? (r4.y) : (-(c1.w))), ((r5.w) >= 0.0f ? (r4.z) : (-(c1.w))), ((r5.w) >= 0.0f ? (r4.w) : (-(c1.y))));
    r5 = tex2D(s5, v6.zw);
    r0.z = dot(v5.xyz, v5.xyz);
    r4 = lerp(r6, r5, r7.zzzz);
    r0.x = rsqrt(r0.z);
    r3.x = (r4.w) * (c2.w);
    r1.xyz = (r0.xxx) * (v5.xyz);
    r0.y = (r4.w) * (-(c4.x)) + (c4.y);
    r3.z = saturate(dot(r9.xyz, -(r1.xyz)));
    r0.z = (r7.w) * (r0.y) + (r3.x);
    r0.y = (r3.z) * (r0.y) + (r3.x);
    r5.xyz = (v5.xyz) * (-(r0.xxx)) + (c[17].xyz);
    r0.z = (r0.z) * (r0.y) + (c3.w);
    r3.w = 1.0f / (r0.z);
    r0.xyz = normalize(r5.xyz);
    r3.w = (r7.w) * (r3.w);
    r5.w = saturate(dot(r9.xyz, r0.xyz));
    r0.z = saturate(dot(r0.xyz, c[17].xyz));
    r5.xy = (r4.ww) * (c12.xy) + (c12.zw);
    r0.y = exp2(r5.y);
    r0.z = (-(r0.z)) + (-(c1.y));
    r3.x = pow(abs(r5.w), r0.y);
    r0.x = (r0.z) * (r0.z);
    r0.y = (r0.y) * (c4.z) + (c4.w);
    r0.x = (r0.x) * (r0.x);
    r0.y = (r3.x) * (r0.y);
    r0.z = (r0.z) * (r0.x);
    r10.xyz = (r4.xyz) * (r4.xyz);
    r7.xyz = (r4.xyz) * (-(r4.xyz)) + (-(c1.yyy));
    r3.w = (r3.w) * (r0.y);
    r0.xyz = (r7.xyz) * (r0.zzz) + (r10.xyz);
    r4.z = (-(r3.z)) + (-(c1.y));
    r0.xyz = (r3.www) * (r0.xyz);
    r8.w = 1.0f / (r5.x);
    r0.xyz = (r11.zzz) * (r0.xyz);
    r3.w = (r4.w) * (c2.z);
    r0.xyz = (r0.xyz) * (c[19].xyz);
    r3.xyz = (r3.yyy) * (r0.xyz);
    r4.w = dot(r1.xyz, r9.xyz);
    r0.xyz = (r12.xyz) * (r12.xyz);
    r4.w = (r4.w) + (r4.w);
    r2.xyz = (r0.xyz) * (r2.xyz) + (r3.xyz);
    r3.xyz = (r9.xyz) * (-(r4.www)) + (r1.xyz);
    r3 = texCUBElod(s15, r3);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r1.xyz = (r11.xxx) * (r1.xyz);
    r3.w = (r4.z) * (r4.z);
    r7.w = (r4.z) * (r3.w);
    r6 = (-(v5.yyyy)) + (c[6]);
    r3 = (r6) * (r6);
    r5 = (-(v5.xxxx)) + (c[5]);
    r3 = (r5) * (r5) + (r3);
    r4 = (-(v5.zzzz)) + (c[7]);
    r7.w = (r8.w) * (r7.w);
    r3 = (r4) * (r4) + (r3);
    r10.xyz = (r7.xyz) * (r7.www) + (r10.xyz);
    r7.x = rsqrt(r3.x);
    r7.y = rsqrt(r3.y);
    r7.z = rsqrt(r3.z);
    r7.w = rsqrt(r3.w);
    r1.xyz = (r1.xyz) * (r10.xyz);
    r6 = (r6) * (r7);
    r6 = (r9.yyyy) * (r6);
    r5 = (r5) * (r7);
    r4 = (r4) * (r7);
    r5 = (r5) * (r9.xxxx) + (r6);
    r3 = saturate((r3) * (c[8]) + (-(r13.yyyy)));
    r4 = saturate((r4) * (r9.zzzz) + (r5));
    r1.xyz = (r8.xyz) * (r1.xyz);
    r3 = (r3) * (r4);
    r2.xyz = (r1.xyz) * (c2.zzz) + (r2.xyz);
    r1.x = dot(c[9], r3);
    r1.y = dot(c[10], r3);
    r1.z = dot(c[11], r3);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r0.xyz = (r2.www) * (r0.xyz) + (v2.xyz);
    r1.w = (-(r1.w)) + (-(c1.y));
    r0.xyz = max(((r0.xyz) * (c[22].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = (r0.w) * (-(v6.y)) + (-(c1.y));
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = (r1.w) * (-(r0.w)) + (-(c1.y));
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
