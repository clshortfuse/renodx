// Mechanically reconstructed from 0x78AC4393.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c2 = float4(1.0f, -0.0f, -1.0f, 8.0f);
    const float4 c3 = float4(31.875f, 0.797884583f, 1.0f, 0.0009765625f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c11 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s0, v1.xy);
    r1.xyz = (r0.xyz) * (v0.xyz);
    r2.xy = (v1.xy) + (c1.xx);
    r0.z = c1.y;
    r0.xy = (r2.xy) * (c[9].xx) + (r0.zz);
    r2.xy = (r2.xy) * (c[8].xx) + (r0.zz);
    r0 = tex2D(s4, r0.xy);
    r0.xyz = (r1.xyz) * (-(r1.xyz)) + (r0.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.w = (r0.w) + (c2.z);
    r2 = tex2D(s3, r2.xy);
    r0.xyz = (r0.xyz) * (r2.yyy);
    r4.w = (v7.w) * (c1.z) + (-(r2.x));
    r1.w = c2.x;
    r3.w = saturate((r4.w) * (c1.w));
    r2 = tex2D(s1, v1.xy);
    r4.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r2 = tex2D(s5, v1.xy);
    r3.xy = (r2.wy) * (c0.xy) + (c0.zw);
    r0 = (r3.wwww) * (r0) + (r1);
    r2.xy = lerp(r4.xy, r3.xy, r3.ww);
    r1 = v2;
    r1.xyz = (r2.xxx) * (v5.xyz) + (r1.xyz);
    r2.w = dot(v6.xyz, v6.xyz);
    r1.xyz = (r2.yyy) * (v4.xyz) + (r1.xyz);
    r4.z = rsqrt(r2.w);
    r7.xyz = normalize(r1.xyz);
    r8.xyz = (r4.zzz) * (v6.xyz);
    r5.w = (-(r3.w)) + (c2.x);
    r3.y = saturate(dot(r7.xyz, -(r8.xyz)));
    r2 = tex2D(s2, v1.xy);
    r1.y = (r2.w) * (c3.y);
    r1.z = (r2.w) * (-(c3.y)) + (c3.z);
    r3.w = saturate(dot(r7.xyz, c[17].xyz));
    r3.x = (r3.y) * (r1.z) + (r1.y);
    r3.z = (r3.w) * (r1.z) + (r1.y);
    r1.xyz = (v6.xyz) * (-(r4.zzz)) + (c[17].xyz);
    r3.z = (r3.z) * (r3.x) + (c3.w);
    r7.w = (-(r3.y)) + (c2.x);
    r3.z = 1.0f / (r3.z);
    r4.z = (r3.w) * (r3.z);
    r3.xyz = normalize(r1.xyz);
    r1.xyz = (r3.www) * (c[18].xyz);
    r4.x = saturate(dot(r7.xyz, r3.xyz));
    r3.w = saturate(dot(r3.xyz, c[17].xyz));
    r3.xy = (r2.ww) * (c4.xy) + (c4.zw);
    r4.y = exp2(r3.y);
    r6.w = 1.0f / (r3.x);
    r3.x = pow(abs(r4.x), r4.y);
    r3.z = (-(r3.w)) + (c2.x);
    r3.w = (r4.y) * (c11.x) + (c11.y);
    r3.y = (r3.z) * (r3.z);
    r3.w = (r3.x) * (r3.w);
    r3.y = (r3.y) * (r3.y);
    r3.w = (r4.z) * (r3.w);
    r3.z = (r3.z) * (r3.y);
    r5.xyz = (r2.xyz) * (r2.xyz);
    r4.xyz = (r2.xyz) * (-(r2.xyz)) + (c2.xxx);
    r2.w = (r2.w) * (c2.w);
    r2.xyz = (r4.xyz) * (r3.zzz) + (r5.xyz);
    r2.xyz = (r3.www) * (r2.xyz);
    r6.z = max(abs(r7.y), abs(r7.z));
    r2.xyz = (r2.xyz) * (c[6].www);
    r3.w = max(abs(r7.x), r6.z);
    r3.w = 1.0f / (r3.w);
    r3.xyz = (r7.xyz) * (c[5].xyz);
    r2.xyz = (r2.xyz) * (c[19].xyz);
    r3.xyz = (r3.xyz) * (r3.www) + (v7.xyz);
    r3 = tex3D(s11, r3.xyz);
    r6.xyz = (r2.xyz) * (r3.www);
    r2.xyz = (r3.xyz) * (r3.xyz);
    r2.xyz = (r2.xyz) * (c[6].yyy);
    r3.z = dot(r8.xyz, r7.xyz);
    r2.xyz = (r2.xyz) * (c3.xxx);
    r3.z = (r3.z) + (r3.z);
    r1.xyz = (r3.www) * (r1.xyz) + (r2.xyz);
    r2.xyz = (r7.xyz) * (-(r3.zzz)) + (r8.xyz);
    r2 = texCUBElod(s15, r2);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r6.xyz);
    r3.xyz = (r2.xyz) * (c2.www);
    r2 = tex3D(s11, v7.xyz);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r2.w = (r7.w) * (r7.w);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r2.w = (r7.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c3.xxx);
    r2.w = (r6.w) * (r2.w);
    r2.xyz = (r4.xyz) * (r2.www) + (r5.xyz);
    r2.w = ((r4.w) >= 0.0f ? (r5.w) : (c2.y));
    r0.xyz = (r0.xyz) * (r2.xyz);
    r2.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c[6].xxx) + (r1.xyz);
    r1.z = (r2.w) * (c[10].w);
    r0.xyz = (c[10].xyz) * (r1.zzz) + (r0.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[7].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
