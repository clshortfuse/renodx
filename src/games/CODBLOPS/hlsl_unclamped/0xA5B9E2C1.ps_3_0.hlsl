// Mechanically reconstructed from 0xA5B9E2C1.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-1.0f, 1.0f, -0.0f, 8.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c3 = float4(1.0f, 0.5f, -0.0f, 4.0f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c13 = float4(-2.0f, 3.0f, 0.0f, 0.0f);
    const float4 c14 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c3.xy);
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v1.zw) * (c3.xy) + (c3.zy);
    r2 = tex2D(s13, r1.xy);
    r0.w = r2.y;
    r3.xy = (r0.yw) * (c14.xx) + (c14.yy);
    r1 = tex2D(s1, v1.xy);
    r7.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r0.y = dot(r3.xy, r3.xy) + (c1.z);
    r0.w = dot(r7.xy, r7.xy) + (c1.z);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c2.x) + (c2.y);
    r0.w = (r0.w) * (c2.x) + (c2.y);
    r2.w = (r0.y) * (r0.w);
    r0.y = dot(r3.xy, r7.xy) + (c1.z);
    r1 = tex2D(s14, v1.zw);
    r4.xy = (r1.xy) * (c2.ww);
    r1.z = saturate((r0.y) * (r2.w) + (r2.w));
    r2.xy = (r2.xz) * (r4.yy);
    r0.y = (r1.y) * (c2.w) + (-(r2.x));
    r3.xz = (r2.xy) * (c3.ww);
    r1.w = (r2.z) * (-(r4.y)) + (r0.y);
    r0.xy = (r0.xz) * (r4.xx);
    r3.y = (r1.w) + (r1.w);
    r1.w = (r1.x) * (c2.w) + (-(r0.x));
    r1.xyz = (r1.zzz) * (r3.xyz);
    r0.z = (r0.z) * (-(r4.x)) + (r1.w);
    r3.xz = (r0.xy) * (c3.ww);
    r3.y = (r0.z) + (r0.z);
    r2.xyz = (r3.xyz) * (r0.www) + (r1.xyz);
    r0.xy = (v6.ww) * (c[35].xy) + (v6.xy);
    r0.zw = v6.zw;
    r0 = tex2Dproj(s2, r0);
    r1.xy = (v6.ww) * (-(c[35].xy)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.y = r1.x;
    r1.xy = (v6.ww) * (c[35].zw) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.z = r1.x;
    r1.xy = (v6.ww) * (-(c[35].zw)) + (v6.xy);
    r1.zw = v6.zw;
    r1 = tex2Dproj(s2, r1);
    r0.w = r1.x;
    r6.xyz = (r2.xyz) * (c[37].yyy);
    r2.w = dot(r0, c4.zzzz);
    r0 = tex2D(s12, v1.zw);
    r5.xyz = (-(v7.xyz)) + (c[28].xyz);
    r1.y = dot(r5.xyz, r5.xyz);
    r3.w = rsqrt(r1.y);
    r4.xyz = (r5.xyz) * (r3.www);
    r0.w = dot(r4.xyz, c[31].xyz);
    r0.w = saturate((r0.w) * (c[32].x) + (c[32].y));
    r0.w = (r0.w) * (c[32].w);
    r1.x = 1.0f / (r3.w);
    r1.w = lerp(r0.y, r2.w, r0.w);
    r0.xy = saturate((r1.xx) * (c[34].xy) + (c[34].zw));
    r1.z = dot(c[33].yz, r1.xy) + (c[33].x);
    r1.xy = (r0.xy) * (r0.xy);
    r2.xy = (r0.xy) * (c13.xx) + (c13.yy);
    r0 = v2;
    r0.xyz = (r7.xxx) * (v5.xyz) + (r0.xyz);
    r1.xy = (r1.xy) * (r2.xy);
    r0.xyz = (r7.yyy) * (v4.xyz) + (r0.xyz);
    r1.z = (r1.z) * (r1.x);
    r10.xyz = normalize(r0.xyz);
    r0.z = (r1.y) * (r1.z);
    r8.z = saturate(dot(r10.xyz, r4.xyz));
    r4.w = (r1.w) * (r0.z);
    r0.xyz = (r8.zzz) * (c[29].xyz);
    r2 = tex2D(s0, v1.xy);
    r1 = tex2D(s4, v8.xy);
    r1.xyz = (r1.xyz) * (v0.yyy);
    r0.xyz = (r4.www) * (r0.xyz) + (r6.xyz);
    r6.xyz = (r1.xyz) * (r1.www) + (r2.xyz);
    r1 = tex2D(s5, v8.zw);
    r1.xyz = (r1.xyz) + (c1.xxx);
    r2.xyz = (v0.zzz) * (r1.xyz) + (c1.yyy);
    r11.xyz = normalize(v7.xyz);
    r1.xyz = (r6.xyz) * (r2.xyz);
    r5.xyz = (r5.xyz) * (r3.www) + (-(r11.xyz));
    r7.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = normalize(r5.xyz);
    r5.w = saturate(dot(r10.xyz, r1.xyz));
    r4.z = saturate(dot(r1.xyz, r4.xyz));
    r1 = tex2D(s6, v1.xy);
    r4.xy = (r1.ww) * (c12.xy) + (c12.zw);
    r10.w = exp2(r4.y);
    r2.w = 1.0f / (r4.x);
    r3.w = pow(abs(r5.w), r10.w);
    r9.w = (r10.w) * (c4.y) + (c4.z);
    r4.y = (r3.w) * (r9.w);
    r8.w = (r1.w) * (c2.z);
    r7.w = (r1.w) * (-(c14.z)) + (c14.w);
    r3.w = saturate(dot(r10.xyz, -(r11.xyz)));
    r4.x = (r8.z) * (r7.w) + (r8.w);
    r6.w = (r3.w) * (r7.w) + (r8.w);
    r4.x = (r4.x) * (r6.w) + (c4.x);
    r4.z = (-(r4.z)) + (c1.y);
    r4.x = 1.0f / (r4.x);
    r5.w = (r4.z) * (r4.z);
    r4.x = (r8.z) * (r4.x);
    r5.w = (r5.w) * (r5.w);
    r4.z = (r4.z) * (r5.w);
    r1.xyz = (r2.xyz) * (r1.xyz);
    r9.xyz = (r1.xyz) * (r1.xyz);
    r8.xyz = (r1.xyz) * (-(r1.xyz)) + (c1.yyy);
    r2.z = (r4.y) * (r4.x);
    r1.xyz = (r8.xyz) * (r4.zzz) + (r9.xyz);
    r3.w = (-(r3.w)) + (c1.y);
    r1.xyz = (r2.zzz) * (r1.xyz);
    r1.w = (r1.w) * (c1.w);
    r1.xyz = (r1.xyz) * (c[37].www);
    r1.xyz = (r1.xyz) * (c[30].xyz);
    r2.z = dot(r11.xyz, r10.xyz);
    r1.xyz = (r4.www) * (r1.xyz);
    r4.w = (r2.z) + (r2.z);
    r2.xyz = (r7.xyz) * (r0.xyz) + (r1.xyz);
    r1.xyz = (r10.xyz) * (-(r4.www)) + (r11.xyz);
    r1 = texCUBElod(s15, r1);
    r1.w = (r3.w) * (r3.w);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r3.w) * (r1.w);
    r0.xyz = (r0.xyz) * (c[37].xxx);
    r1.w = (r2.w) * (r1.w);
    r4.xyz = (r8.xyz) * (r1.www) + (r9.xyz);
    r1 = (v7.yyyy) * (c[25]);
    r0.xyz = (r0.xyz) * (r4.xyz);
    r1 = (v7.xxxx) * (c[24]) + (r1);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r1 = (v7.zzzz) * (c[26]) + (r1);
    r6.xyz = (r0.xyz) * (c1.www) + (r2.xyz);
    r1 = (r1) + (c[27]);
    r4.xy = (r1.ww) * (c[36].xy) + (r1.xy);
    r4.zw = r1.zw;
    r2 = tex2Dproj(s2, r4);
    r3.zw = r4.zw;
    r1.zw = r3.zw;
    r5.xy = (r4.ww) * (-(c[36].zw)) + (r1.xy);
    r5.zw = r1.zw;
    r5 = tex2Dproj(s2, r5);
    r2.w = r5.x;
    r3.xy = (r4.ww) * (-(c[36].xy)) + (r1.xy);
    r1.xy = (r4.ww) * (c[36].zw) + (r1.xy);
    r3 = tex2Dproj(s2, r3);
    r2.y = r3.x;
    r3 = tex2Dproj(s2, r1);
    r1 = (v7.xyzx) * (c1.yyyz) + (c1.zzzy);
    r0.z = dot(r1, c[21]);
    r2.z = r3.x;
    r0.z = 1.0f / (r0.z);
    r0.x = dot(r1, c[10]);
    r0.y = dot(r1, c[11]);
    r2.w = dot(r2, c4.zzzz);
    r0.xy = (r0.zz) * (r0.xy);
    r2.x = dot(r1, c[20]);
    r0.xy = (r0.xy) * (c3.yy) + (c3.yy);
    r1 = tex2D(s3, r0.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[8].yz, r2.xy) + (c[8].x);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.z = saturate(1.0f / (r1.w));
    r3.xy = saturate((r2.xx) * (c[9].xy) + (c[9].zw));
    r3.w = ((-abs(r1.w)) >= 0.0f ? (c1.z) : (r1.z));
    r1.xy = (r3.xy) * (r3.xy);
    r2.xyz = (-(v7.xyz)) + (c[5].xyz);
    r3.xy = (r3.xy) * (c13.xx) + (c13.yy);
    r1.w = dot(r2.xyz, r2.xyz);
    r3.z = (r1.x) * (r3.x);
    r1.w = rsqrt(r1.w);
    r3.y = (r1.y) * (-(r3.y)) + (c1.y);
    r1.xyz = (r2.xyz) * (r1.www);
    r3.w = (r3.w) * (r3.z);
    r3.z = dot(r1.xyz, c[22].xyz);
    r3.w = (r3.y) * (r3.w);
    r4.w = saturate((r3.z) * (c[23].x) + (c[23].y));
    r3.xyz = (r2.xyz) * (r1.www) + (-(r11.xyz));
    r1.w = (r4.w) * (r4.w);
    r4.w = (r4.w) * (c13.x) + (c13.y);
    r2.xyz = normalize(r3.xyz);
    r3.z = (r1.w) * (r4.w);
    r1.w = saturate(dot(r10.xyz, r2.xyz));
    r2.z = saturate(dot(r2.xyz, r1.xyz));
    r2.y = pow(abs(r1.w), r10.w);
    r1.w = saturate(dot(r1.xyz, r10.xyz));
    r1.y = (r9.w) * (r2.y);
    r1.x = (r1.w) * (r7.w) + (r8.w);
    r1.z = (-(r2.z)) + (c1.y);
    r1.x = (r1.x) * (r6.w) + (c4.x);
    r2.z = (r1.z) * (r1.z);
    r1.x = 1.0f / (r1.x);
    r2.z = (r2.z) * (r2.z);
    r1.x = (r1.w) * (r1.x);
    r1.z = (r1.z) * (r2.z);
    r2.y = (r1.y) * (r1.x);
    r1.xyz = (r8.xyz) * (r1.zzz) + (r9.xyz);
    r2.z = (r3.w) * (r3.z);
    r1.xyz = (r2.yyy) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r2.zzz);
    r2.xyz = (r1.xyz) * (c[7].xyz);
    r1.xyz = (r1.www) * (c[6].xyz);
    r2.xyz = (r2.xyz) * (c[37].www);
    r0.xyz = (r2.www) * (r0.xyz);
    r1.xyz = (r7.xyz) * (r1.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r6.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[38].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.y;

    return oC0;
}
