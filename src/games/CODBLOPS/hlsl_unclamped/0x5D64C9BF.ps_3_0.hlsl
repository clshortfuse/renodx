// Mechanically reconstructed from 0x5D64C9BF.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD4;
    float4 v5 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(-0.5f, 1.0f, 0.0f, 8.0f);
    const float4 c1 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 0.125f, 0.25f);
    const float4 c4 = float4(0.00048828125f, -0.000122070312f, 0.0f, -3.0f);
    const float4 c12 = float4(0.000244140625f, 0.0f, -0.000244140625f, 0.75f);
    const float4 c13 = float4(4.0f, -3.0f, -2.0f, 3.0f);
    const float4 c14 = float4(0.797884583f, 1.0f, 0.5f, 0.0f);
    const float4 c15 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v5.xyz, v5.xyz);
    r1.xyz = (-(v5.xyz)) + (c[5].xyz);
    r9.w = rsqrt(r0.w);
    r0.w = dot(r1.xyz, r1.xyz);
    r0.w = rsqrt(r0.w);
    r2.xyz = (r9.www) * (v5.xyz);
    r0.xyz = (r1.xyz) * (r0.www) + (-(r2.xyz));
    r5.xyz = (r1.xyz) * (r0.www);
    r6.xyz = normalize(r0.xyz);
    r0.w = dot(r5.xyz, c[22].xyz);
    r0.z = saturate(dot(r6.xyz, r5.xyz));
    r0.w = saturate((r0.w) * (c[23].x) + (c[23].y));
    r7.z = (r0.w) * (c13.z) + (c13.w);
    r1.w = (-(r0.z)) + (c0.y);
    r6.w = (r0.w) * (r0.w);
    r1.z = (r1.w) * (r1.w);
    r0 = tex2D(s0, v1.xy);
    r7.y = (r0.w) * (v0.w) + (c0.x);
    r0 = (r0.xyzx) * (c0.yyyz) + (c0.zzzy);
    r1.z = (r1.z) * (r1.z);
    r0 = float4(((r7.y) >= 0.0f ? (r0.x) : (c0.z)), ((r7.y) >= 0.0f ? (r0.y) : (c0.z)), ((r7.y) >= 0.0f ? (r0.z) : (c0.z)), ((r7.y) >= 0.0f ? (r0.w) : (c0.z)));
    r7.x = (r1.w) * (r1.z);
    r4.xyz = (r0.xyz) * (v0.xyz);
    r1 = tex2D(s4, v1.xy);
    r1 = float4(((r7.y) >= 0.0f ? (r1.x) : (c0.z)), ((r7.y) >= 0.0f ? (r1.y) : (c0.z)), ((r7.y) >= 0.0f ? (r1.z) : (c0.z)), ((r7.y) >= 0.0f ? (r1.w) : (c0.y)));
    r11.xyz = (r1.xyz) * (-(r1.xyz)) + (c0.yyy);
    r12.xyz = (r1.xyz) * (r1.xyz);
    r7.w = (r1.w) * (-(c14.x)) + (c14.y);
    r3.xyz = normalize(v2.xyz);
    r1.z = saturate(dot(r3.xyz, r6.xyz));
    r8.w = (r1.w) * (c14.x);
    r5.z = saturate(dot(r5.xyz, r3.xyz));
    r2.w = saturate(dot(r3.xyz, -(r2.xyz)));
    r0.z = (r5.z) * (r7.w) + (r8.w);
    r5.w = (r2.w) * (r7.w) + (r8.w);
    r0.z = (r0.z) * (r5.w) + (c1.w);
    r5.xy = (r1.ww) * (c15.xy) + (c15.zw);
    r0.y = 1.0f / (r0.z);
    r4.w = exp2(r5.y);
    r0.z = pow(abs(r1.z), r4.w);
    r3.w = (r4.w) * (c3.z) + (c3.w);
    r1.y = (r5.z) * (r0.y);
    r1.z = (r0.z) * (r3.w);
    r0.xyz = (r11.xyz) * (r7.xxx) + (r12.xyz);
    r1.z = (r1.y) * (r1.z);
    r8.xyz = (r4.xyz) * (r4.xyz);
    r0.xyz = (r0.xyz) * (r1.zzz);
    r0.xyz = (r0.xyz) * (c[7].xyz);
    r1.y = c0.z;
    r10.xyz = float3(((r7.y) >= 0.0f ? (c[29].x) : (r1.y)), ((r7.y) >= 0.0f ? (c[29].y) : (r1.y)), ((r7.y) >= 0.0f ? (c[29].w) : (r1.y)));
    r1.xyz = (r0.xyz) * (r10.zzz);
    r0.xyz = (r5.zzz) * (c[6].xyz);
    r6.w = (r7.z) * (r6.w);
    r6.xyz = (r8.xyz) * (r0.xyz) + (r1.xyz);
    r4.z = 1.0f / (r5.x);
    r0.y = (-(r2.w)) + (c0.y);
    r0.x = (r0.y) * (r0.y);
    r0.z = dot(r2.xyz, r3.xyz);
    r2.w = (r0.y) * (r0.x);
    r0.z = (r0.z) + (r0.z);
    r1.w = (r1.w) * (c0.w);
    r1.xyz = (r3.xyz) * (-(r0.zzz)) + (r2.xyz);
    r1 = texCUBElod(s15, r1);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r4.z) * (r2.w);
    r0.xyz = (r10.xxx) * (r0.xyz);
    r4.xyz = (r11.xyz) * (r1.www) + (r12.xyz);
    r1.xy = (v1.zw) * (c14.yz);
    r1 = tex2D(s13, r1.xy);
    r2 = tex2D(s14, v1.zw);
    r9.xy = (r2.xy) * (c1.xx);
    r7.xyz = (r0.xyz) * (r4.xyz);
    r5.xy = (r1.xz) * (r9.xx);
    r0.x = r1.y;
    r0.z = (r2.x) * (c1.x) + (-(r5.x));
    r2.w = (r1.z) * (-(r9.x)) + (r0.z);
    r1.xy = (v1.zw) * (c14.yz) + (c14.wz);
    r1 = tex2D(s13, r1.xy);
    r4.xy = (r9.yy) * (r1.xz);
    r0.y = r1.y;
    r0.z = (r2.y) * (c1.x) + (-(r4.x));
    r0.xy = (r0.xy) * (c1.yy) + (c1.zz);
    r1.w = (r1.z) * (-(r9.y)) + (r0.z);
    r0.z = dot(r0.xy, r0.xy) + (c0.z);
    r9.y = (r2.w) + (r2.w);
    r0.z = exp2(-(r0.z));
    r1.y = (r1.w) + (r1.w);
    r1.w = saturate((r0.z) * (c3.x) + (c3.y));
    r9.xz = (r5.xy) * (c1.yy);
    r2.xyz = (v5.xyz) * (-(r9.www)) + (c[17].xyz);
    r1.xz = (r4.xy) * (c1.yy);
    r0.xyz = normalize(r2.xyz);
    r1.xyz = (r1.xyz) * (r1.www) + (r9.xyz);
    r1.w = saturate(dot(r0.xyz, c[17].xyz));
    r14.xyz = (r10.yyy) * (r1.xyz);
    r1.w = (-(r1.w)) + (c0.y);
    r1.z = (r1.w) * (r1.w);
    r2.w = saturate(dot(r3.xyz, c[17].xyz));
    r1.z = (r1.z) * (r1.z);
    r1.x = (r2.w) * (r7.w) + (r8.w);
    r1.y = saturate(dot(r3.xyz, r0.xyz));
    r0.y = (r1.x) * (r5.w) + (c1.w);
    r0.z = pow(abs(r1.y), r4.w);
    r0.y = 1.0f / (r0.y);
    r0.z = (r3.w) * (r0.z);
    r0.y = (r2.w) * (r0.y);
    r8.w = (r1.w) * (r1.z);
    r7.w = (r0.z) * (r0.y);
    r1 = tex2D(s12, v1.zw);
    r0.z = ((-abs(r1.y)) >= 0.0f ? (c0.y) : (c0.z));
    r13.xyz = (r2.www) * (c[18].xyz);
    if ((r0.z) != (-(r0.z)))
    {
        r0.z = r1.y;
        r2.w = r0.z;
    }
    else
    {
        if ((c0.y) >= (v4.w))
        {
            r2 = (v4.xyzx) * (c0.yyyz);
            r1 = (r2) + (-(c4.xyzz));
            r1 = tex2Dlod(s1, r1);
            r1.w = r1.x;
            r3 = (r2) + (c12.xxyy);
            r3 = tex2Dlod(s1, r3);
            r1.x = r3.x;
            r3 = (r2) + (c12.zzyy);
            r3 = tex2Dlod(s1, r3);
            r1.y = r3.x;
            r2 = (r2) + (c4.xyzz);
            r2 = tex2Dlod(s1, r2);
            r1.z = r2.x;
            r0.z = dot(r1, c3.wwww);
            if ((c12.w) < (v4.w))
            {
                r0.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
                r1.xy = (r0.xy) + (c12.xx);
                r1.zw = (v4.zx) * (c0.yz);
                r1 = tex2Dlod(s1, r1);
                r2.xy = (r0.xy) + (c12.zz);
                r2.zw = (v4.zx) * (c0.yz);
                r4 = tex2Dlod(s1, r2);
                r2.xy = (r0.xy) + (c4.xy);
                r2.zw = (v4.zx) * (c0.yz);
                r3 = tex2Dlod(s1, r2);
                r2.xy = (r0.xy) + (-(c4.xy));
                r2.zw = (v4.zx) * (c0.yz);
                r2 = tex2Dlod(s1, r2);
                r1.y = r4.x;
                r1.z = r3.x;
                r1.w = r2.x;
                r0.y = dot(r1, c3.wwww);
                r0.x = (-(r0.z)) + (r0.y);
                r0.y = (v4.w) * (c13.x) + (c13.y);
                r2.w = (r0.y) * (r0.x) + (r0.z);
            }
            else
            {
                r2.w = r0.z;
            }
        }
        else
        {
            r0.z = (v4.w) + (-(c1.y));
            r0.z = ((r0.z) >= 0.0f ? (c0.z) : (c0.y));
            if ((r0.z) != (-(r0.z)))
            {
                r0.xy = (v4.xy) * (c[2].ww) + (c[2].xy);
                r2.xy = (r0.xy) + (c12.xx);
                r2.zw = (v4.zz) * (c0.yz);
                r2 = tex2Dlod(s1, r2);
                r3.xy = (r0.xy) + (c12.zz);
                r3.zw = (v4.zz) * (c0.yz);
                r5 = tex2Dlod(s1, r3);
                r3.xy = (r0.xy) + (c4.xy);
                r3.zw = (v4.zz) * (c0.yz);
                r4 = tex2Dlod(s1, r3);
                r3.xy = (r0.xy) + (-(c4.xy));
                r3.zw = (v4.zz) * (c0.yz);
                r3 = tex2Dlod(s1, r3);
                r2.y = r5.x;
                r2.z = r4.x;
                r2.w = r3.x;
                r0.x = dot(r2, c3.wwww);
                r0.z = saturate((v4.w) + (c4.w));
                r0.y = (r1.y) + (-(r0.x));
                r0.z = (r0.z) * (r0.y) + (r0.x);
            }
            else
            {
                r0.z = r1.y;
            }
            r2.w = r0.z;
        }
    }
    r0.xyz = (r2.www) * (r13.xyz) + (r14.xyz);
    r2.xyz = (r11.xyz) * (r8.www) + (r12.xyz);
    r1 = (v5.yyyy) * (c[25]);
    r2.xyz = (r7.www) * (r2.xyz);
    r1 = (v5.xxxx) * (c[24]) + (r1);
    r2.xyz = (r10.zzz) * (r2.xyz);
    r1 = (v5.zzzz) * (c[26]) + (r1);
    r2.xyz = (r2.xyz) * (c[19].xyz);
    r4 = (r1) + (c[27]);
    r1.xyz = (r2.www) * (r2.xyz);
    r3.zw = r4.zw;
    r8.xyz = (r8.xyz) * (r0.xyz) + (r1.xyz);
    r5.zw = r3.zw;
    r7.xyz = (r7.xyz) * (r9.xyz);
    r2.zw = r5.zw;
    r1.xy = (r4.ww) * (-(c[28].zw)) + (r4.xy);
    r1.zw = r2.zw;
    r1 = tex2Dproj(s2, r1);
    r1.w = r1.x;
    r5.xy = (r3.ww) * (-(c[28].xy)) + (r4.xy);
    r5 = tex2Dproj(s2, r5);
    r1.y = r5.x;
    r3.xy = (r3.ww) * (c[28].xy) + (r4.xy);
    r5 = tex2Dproj(s2, r3);
    r1.x = r5.x;
    r2.xy = (r3.ww) * (c[28].zw) + (r4.xy);
    r3 = tex2Dproj(s2, r2);
    r2 = (v5.xyzx) * (c0.yyyz) + (c0.zzzy);
    r1.z = r3.x;
    r0.z = dot(r2, c[21]);
    r3.w = 1.0f / (r0.z);
    r3.x = dot(r2, c[20]);
    r0.x = dot(r2, c[10]);
    r3.y = (r3.x) * (r3.x);
    r0.y = dot(r2, c[11]);
    r0.z = dot(c[8].yz, r3.xy) + (c[8].x);
    r2.w = saturate(1.0f / (r0.z));
    r2.xy = saturate((r3.xx) * (c[9].xy) + (c[9].zw));
    r3.xy = (r2.xy) * (c13.zz) + (c13.ww);
    r2.xy = (r2.xy) * (r2.xy);
    r0.z = ((-abs(r0.z)) >= 0.0f ? (c0.z) : (r2.w));
    r2.w = (r3.x) * (r2.x);
    r0.z = (r0.z) * (r2.w);
    r2.w = (r2.y) * (-(r3.y)) + (c0.y);
    r0.xy = (r3.ww) * (r0.xy);
    r0.z = (r0.z) * (r2.w);
    r3.w = (r6.w) * (r0.z);
    r0.xy = (r0.xy) * (-(c0.xx)) + (-(c0.xx));
    r2 = tex2D(s3, r0.xy);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r1.w = dot(r1, c3.wwww);
    r0.xyz = (r3.www) * (r0.xyz);
    r1.xyz = (r7.xyz) * (c0.www) + (r8.xyz);
    r0.xyz = (r1.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (r6.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r1.x = v2.w;
    r0.xyz = (r1.xxx) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[30].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = r0.w;

    return oC0;
}
