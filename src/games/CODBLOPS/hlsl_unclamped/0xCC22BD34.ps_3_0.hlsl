// Mechanically reconstructed from 0xCC22BD34.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(-0.5f, 0.200000003f, 1.0f, 0.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(8.0f, 31.875f, 0.797884583f, 0.25f);
    const float4 c3 = float4(0.797884583f, 1.0f, 0.0009765625f, 0.0f);
    const float4 c4 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, -2.0f, 3.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[33]);
    r0 = (v6.xxxx) * (c[32]) + (r0);
    r0 = (v6.zzzz) * (c[34]) + (r0);
    r1 = (r0) + (c[35]);
    r3.xy = (r1.ww) * (c[36].xy) + (r1.xy);
    r3.zw = r1.zw;
    r0 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r1.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[36].zw)) + (r1.xy);
    r4.zw = r1.zw;
    r4 = tex2Dproj(s2, r4);
    r0.w = r4.x;
    r2.xy = (r3.ww) * (-(c[36].xy)) + (r1.xy);
    r1.xy = (r3.ww) * (c[36].zw) + (r1.xy);
    r2 = tex2Dproj(s2, r2);
    r0.y = r2.x;
    r2 = tex2Dproj(s2, r1);
    r1 = (v6.xyzx) * (c0.zzzw) + (c0.wwwz);
    r0.z = dot(r1, c[29]);
    r2.w = 1.0f / (r0.z);
    r3.x = dot(r1, c[26]);
    r3.y = dot(r1, c[27]);
    r0.z = r2.x;
    r3.xy = (r2.ww) * (r3.xy);
    r2.x = dot(r1, c[28]);
    r1.xy = (r3.xy) * (-(c0.xx)) + (-(c0.xx));
    r1 = tex2D(s3, r1.xy);
    r2.y = (r2.x) * (r2.x);
    r1.w = dot(c[24].yz, r2.xy) + (c[24].x);
    r3.xy = saturate((r2.xx) * (c[25].xy) + (c[25].zw));
    r2.w = saturate(1.0f / (r1.w));
    r1.w = ((-abs(r1.w)) >= 0.0f ? (c0.w) : (r2.w));
    r6.xyz = (-(v6.xyz)) + (c[21].xyz);
    r2.xy = (r3.xy) * (r3.xy);
    r2.w = dot(r6.xyz, r6.xyz);
    r3.xy = (r3.xy) * (c12.zz) + (c12.ww);
    r8.w = rsqrt(r2.w);
    r2.z = (r2.x) * (r3.x);
    r5.xyz = (r6.xyz) * (r8.www);
    r2.y = (r2.y) * (-(r3.y)) + (c0.z);
    r2.w = dot(r5.xyz, c[30].xyz);
    r1.w = (r1.w) * (r2.z);
    r2.z = saturate((r2.w) * (c[31].x) + (c[31].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c12.z) + (c12.w);
    r1.w = (r2.y) * (r1.w);
    r2.w = (r2.w) * (r2.z);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r1.w) * (r2.w);
    r0.w = dot(r0, c2.wwww);
    r0.xyz = (r1.xyz) * (r1.www);
    r7.xyz = (r0.www) * (r0.xyz);
    r0 = tex3D(s11, v7.xyz);
    r4.xyz = (r0.xyz) * (r0.xyz);
    r0.xy = (v1.xy) * (c[37].xy);
    r0 = tex2D(s5, r0.xy);
    r1.xyz = (r0.xyz) + (c0.xxx);
    r0 = tex2D(s0, v1.xy);
    r9.xyz = saturate((r1.xyz) * (r0.www) + (r0.xyz));
    r5.w = (r0.w) * (v0.w) + (c0.x);
    r3 = tex2D(s4, v1.xy);
    r0 = tex2D(s1, v1.xy);
    r0.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r2.xy = float2(((r5.w) >= 0.0f ? (r0.x) : (c0.w)), ((r5.w) >= 0.0f ? (r0.y) : (c0.w)));
    r1 = v2;
    r1.xyz = (r2.xxx) * (v5.xyz) + (r1.xyz);
    r0.xyz = lerp(r9.xyz, c0.yyy, r3.xxx);
    r1.xyz = (r2.yyy) * (v4.xyz) + (r1.xyz);
    r10.xyz = normalize(r1.xyz);
    r8.xyz = normalize(v6.xyz);
    r0.w = r3.w;
    r1.z = dot(r8.xyz, r10.xyz);
    r2 = float4(((r5.w) >= 0.0f ? (r0.x) : (c0.w)), ((r5.w) >= 0.0f ? (r0.y) : (c0.w)), ((r5.w) >= 0.0f ? (r0.z) : (c0.w)), ((r5.w) >= 0.0f ? (r0.w) : (c0.z)));
    r0.z = (r1.z) + (r1.z);
    r0.w = (r2.w) * (c2.x);
    r0.xyz = (r10.xyz) * (-(r0.zzz)) + (r8.xyz);
    r0 = texCUBElod(s15, r0);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r9.xyz) * (r3.xxx);
    r1.xyz = (r1.xyz) * (c2.xxx);
    r7.w = ((r5.w) >= 0.0f ? (r3.y) : (c0.w));
    r1.xyz = (r4.xyz) * (r1.xyz);
    r1.xyz = (r1.xyz) * (c2.yyy);
    r4.w = saturate(dot(r10.xyz, -(r8.xyz)));
    r4.xyz = (r2.xyz) * (r2.xyz);
    r0.w = (-(r4.w)) + (c0.z);
    r3.xy = (r2.ww) * (c4.xy) + (c4.zw);
    r3.z = (r0.w) * (r0.w);
    r3.w = 1.0f / (r3.x);
    r0.w = (r0.w) * (r3.z);
    r2.xyz = (r2.xyz) * (-(r2.xyz)) + (c0.zzz);
    r0.w = (r3.w) * (r0.w);
    r6.w = exp2(r3.y);
    r3.xyz = (r2.xyz) * (r0.www) + (r4.xyz);
    r6.xyz = (r6.xyz) * (r8.www) + (-(r8.xyz));
    r1.xyz = (r1.xyz) * (r3.xyz);
    r8.xyz = (r7.www) * (r1.xyz);
    r0.w = c0.z;
    r0 = float4(((r5.w) >= 0.0f ? (r0.x) : (c0.w)), ((r5.w) >= 0.0f ? (r0.y) : (c0.w)), ((r5.w) >= 0.0f ? (r0.z) : (c0.w)), ((r5.w) >= 0.0f ? (r0.w) : (c0.w)));
    r3.w = max(abs(r10.y), abs(r10.z));
    r0.xyz = (r0.xyz) * (v0.xyz);
    r1.z = max(abs(r10.x), r3.w);
    r3.w = 1.0f / (r1.z);
    r1.xyz = (r10.xyz) * (c[5].xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.xyz = (r1.xyz) * (r3.www) + (v7.xyz);
    r3 = tex3D(s11, r1.xyz);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r3.y = (r2.w) * (c2.z);
    r3.w = (r2.w) * (-(c3.x)) + (c3.y);
    r2.w = saturate(dot(r5.xyz, r10.xyz));
    r3.z = (r4.w) * (r3.w) + (r3.y);
    r3.w = (r2.w) * (r3.w) + (r3.y);
    r1.xyz = (r7.www) * (r1.xyz);
    r3.w = (r3.w) * (r3.z) + (c3.z);
    r1.xyz = (r0.xyz) * (r1.xyz);
    r3.w = 1.0f / (r3.w);
    r8.xyz = (r1.xyz) * (c2.yyy) + (r8.xyz);
    r4.w = (r2.w) * (r3.w);
    r1.xyz = (r2.www) * (c[22].xyz);
    r3.xyz = normalize(r6.xyz);
    r5.w = (r6.w) * (c12.x) + (c12.y);
    r2.w = saturate(dot(r3.xyz, r5.xyz));
    r5.z = saturate(dot(r10.xyz, r3.xyz));
    r2.w = (-(r2.w)) + (c0.z);
    r3.z = pow(abs(r5.z), r6.w);
    r3.w = (r2.w) * (r2.w);
    r3.z = (r5.w) * (r3.z);
    r3.w = (r3.w) * (r3.w);
    r6.w = (r4.w) * (r3.z);
    r2.w = (r2.w) * (r3.w);
    r6.xyz = (r2.xyz) * (r2.www) + (r4.xyz);
    r5 = (-(v6.yyyy)) + (c[7]);
    r2 = (r5) * (r5);
    r4 = (-(v6.xxxx)) + (c[6]);
    r2 = (r4) * (r4) + (r2);
    r3 = (-(v6.zzzz)) + (c[8]);
    r6.xyz = (r6.www) * (r6.xyz);
    r2 = (r3) * (r3) + (r2);
    r9.xyz = (r6.xyz) * (c[23].xyz);
    r6.x = rsqrt(r2.x);
    r6.y = rsqrt(r2.y);
    r6.z = rsqrt(r2.z);
    r6.w = rsqrt(r2.w);
    r9.xyz = (r7.www) * (r9.xyz);
    r5 = (r5) * (r6);
    r5 = (r10.yyyy) * (r5);
    r4 = (r4) * (r6);
    r3 = (r3) * (r6);
    r4 = (r4) * (r10.xxxx) + (r5);
    r5.y = c0.z;
    r2 = saturate((r2) * (c[9]) + (r5.yyyy));
    r3 = saturate((r3) * (r10.zzzz) + (r4));
    r1.xyz = (r0.xyz) * (r1.xyz) + (r9.xyz);
    r2 = (r2) * (r3);
    r3.xyz = (r7.xyz) * (r1.xyz) + (r8.xyz);
    r1.x = dot(c[10], r2);
    r1.y = dot(c[11], r2);
    r1.z = dot(c[20], r2);
    r2.xyz = (r0.xyz) * (r1.xyz) + (r3.xyz);
    r2.w = c0.z;
    r0.x = dot(r2, c[39]);
    r0.y = dot(r2, c[40]);
    r0.z = dot(r2, c[41]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[38].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
