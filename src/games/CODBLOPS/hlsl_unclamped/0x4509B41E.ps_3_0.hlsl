// Mechanically reconstructed from 0x4509B41E.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    const float4 c0 = float4(4.0f, -2.0f, 0.600000024f, 0.400000006f);
    const float4 c1 = float4(1.0f, 0.5f, 0.0f, 31.875f);
    const float4 c2 = float4(0.25f, -2.0f, 3.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v4.yyyy) * (c[31]);
    r0 = (v4.xxxx) * (c[30]) + (r0);
    r0 = (v4.zzzz) * (c[32]) + (r0);
    r0 = (r0) + (c[33]);
    r3.xy = (r0.ww) * (c[34].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s1, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[34].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s1, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[34].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[34].zw) + (r0.xy);
    r2 = tex2Dproj(s1, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s1, r0);
    r0 = (v4.xyzx) * (c1.xxxz) + (c1.zzzx);
    r2.w = dot(r0, c[27]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[24]);
    r2.y = dot(r0, c[25]);
    r1.w = dot(r1, c2.xxxx);
    r2.xy = (r2.ww) * (r2.xy);
    r1.x = dot(r0, c[26]);
    r0.xy = (r2.xy) * (c1.yy) + (c1.yy);
    r0 = tex2D(s2, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = dot(c[22].yz, r1.xy) + (c[22].x);
    r1.xy = saturate((r1.xx) * (c[23].xy) + (c[23].zw));
    r1.z = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c1.z) : (r1.z));
    r3.xy = (r1.xy) * (r1.xy);
    r4.xy = (r1.xy) * (c2.yy) + (c2.zz);
    r2.xyz = (-(v4.xyz)) + (c[20].xyz);
    r3.w = (r3.x) * (r4.x);
    r1.xyz = normalize(r2.xyz);
    r2.y = (r3.y) * (-(r4.y)) + (c1.x);
    r2.w = dot(r1.xyz, c[28].xyz);
    r0.w = (r0.w) * (r3.w);
    r2.z = saturate((r2.w) * (c[29].x) + (c[29].y));
    r2.w = (r2.z) * (r2.z);
    r2.z = (r2.z) * (c2.y) + (c2.z);
    r0.w = (r2.y) * (r0.w);
    r2.w = (r2.w) * (r2.z);
    r0.w = (r0.w) * (r2.w);
    r11.xyz = normalize(v2.xyz);
    r0.xyz = (r0.xyz) * (r0.www);
    r0.w = saturate(dot(r1.xyz, r11.xyz));
    r4.xyz = (r1.www) * (r0.xyz);
    r2.xyz = (r0.www) * (c[21].xyz);
    r0 = tex2D(s0, v1.xy);
    r0 = (r0.wxyz) * (v0.wxyz);
    r1.xy = (v1.zw) * (c1.xy);
    r3 = tex2D(s13, r1.xy);
    r1.xy = (v1.zw) * (c1.xy) + (c1.zy);
    r1 = tex2D(s13, r1.xy);
    r3.w = r1.y;
    r7.xyz = (r0.yzw) * (r0.yzw);
    r6.xy = (r3.yw) * (c0.xx) + (c0.yy);
    r5.xyz = (r2.xyz) * (r7.xyz);
    r0.w = dot(r6.xy, r6.xy) + (c1.z);
    r2 = tex2D(s14, v1.zw);
    r6.xy = (r2.xy) * (c1.ww);
    r0.w = exp2(-(r0.w));
    r3.xy = (r3.xz) * (r6.xx);
    r0.w = saturate((r0.w) * (c0.z) + (c0.w));
    r0.z = (r2.x) * (c1.w) + (-(r3.x));
    r2.xz = (r3.xy) * (c0.xx);
    r1.xy = (r1.xz) * (r6.yy);
    r0.y = (r3.z) * (-(r6.x)) + (r0.z);
    r0.z = (r2.y) * (c1.w) + (-(r1.x));
    r2.y = (r0.y) + (r0.y);
    r0.z = (r1.z) * (-(r6.y)) + (r0.z);
    r1.xz = (r1.xy) * (c0.xx);
    r1.y = (r0.z) + (r0.z);
    r8.xyz = (r4.xyz) * (r5.xyz);
    r10.xyz = (r1.xyz) * (r0.www) + (r2.xyz);
    r3 = tex2D(s12, v1.zw);
    r5 = (-(v4.yyyy)) + (c[6]);
    r1 = (r5) * (r5);
    r4 = (-(v4.xxxx)) + (c[5]);
    r1 = (r4) * (r4) + (r1);
    r2 = (-(v4.zzzz)) + (c[7]);
    r1 = (r2) * (r2) + (r1);
    r0.w = saturate(dot(c[17].xyz, r11.xyz));
    r6.x = rsqrt(r1.x);
    r6.y = rsqrt(r1.y);
    r6.z = rsqrt(r1.z);
    r6.w = rsqrt(r1.w);
    r9.xyz = (r0.www) * (c[18].xyz);
    r5 = (r5) * (r6);
    r5 = (r11.yyyy) * (r5);
    r4 = (r4) * (r6);
    r2 = (r2) * (r6);
    r4 = (r4) * (r11.xxxx) + (r5);
    r0.w = c1.x;
    r1 = saturate((r1) * (c[8]) + (r0.wwww));
    r2 = saturate((r2) * (r11.zzzz) + (r4));
    r3.xyz = (r3.yyy) * (r9.xyz) + (r10.xyz);
    r1 = (r1) * (r2);
    r3.xyz = (r7.xyz) * (r3.xyz) + (r8.xyz);
    r2.x = dot(c[9], r1);
    r2.y = dot(c[10], r1);
    r2.z = dot(c[11], r1);
    r1.xyz = (r7.xyz) * (r2.xyz) + (r3.xyz);
    r2.xyz = (r0.xxx) * (v3.xyz);
    r1.xyz = (r1.xyz) * (r0.xxx) + (-(r2.xyz));
    r1.xyz = (v2.www) * (r1.xyz) + (r2.xyz);
    r1.xyz = max(((r1.xyz) * (c[35].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.x);
    r0.x = rsqrt(r1.x);
    r0.y = rsqrt(r1.y);
    r0.z = rsqrt(r1.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
