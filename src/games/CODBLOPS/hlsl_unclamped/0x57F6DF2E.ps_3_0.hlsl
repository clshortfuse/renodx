// Mechanically reconstructed from 0x57F6DF2E.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
samplerCUBE s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);

struct PS_INPUT
{
    float4 v0 : TEXCOORD1;
    float4 v1 : TEXCOORD2;
    float4 v2 : TEXCOORD3;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
    float2 vPosInput : VPOS;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 vPos = float4(input.vPosInput.xy, 0.0f, 0.0f);
    const float4 c0 = float4(0.00333333341f, -2.0f, -0.5f, 0.0f);
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(300.0f, -64.0301971f, 9.40301991f, 60.0f);
    const float4 c3 = float4(1.0f, 0.0f, -2.0f, 3.0f);
    const float4 c12 = float4(0.999511719f, 9.99999994e-09f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v2.xyzx) * (c3.xxxy) + (c3.yyyx);
    r1.w = dot(r0, c[20]);
    r1.w = 1.0f / (r1.w);
    r1.x = dot(r0, c[9]);
    r1.y = dot(r0, c[10]);
    r2.xy = (r1.ww) * (r1.xy);
    r1.x = dot(r0, c[11]);
    r0.xy = (r2.xy) * (-(c0.zz)) + (-(c0.zz));
    r0 = tex2D(s0, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[7].yz, r1.xy) + (c[7].x);
    r1.xy = saturate((r1.xx) * (c[8].xy) + (c[8].zw));
    r1.w = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c0.w) : (r1.w));
    r2.xy = (r1.xy) * (r1.xy);
    r3.xy = (r1.xy) * (c3.zz) + (c3.ww);
    r1.xyz = (-(v2.xyz)) + (c[5].xyz);
    r2.w = (r2.x) * (r3.x);
    r8.xyz = normalize(r1.xyz);
    r1.y = (r2.y) * (-(r3.y)) + (c3.x);
    r1.w = dot(r8.xyz, c[21].xyz);
    r0.w = (r0.w) * (r2.w);
    r1.z = saturate((r1.w) * (c[22].x) + (c[22].y));
    r1.w = (r1.z) * (r1.z);
    r1.z = (r1.z) * (c3.z) + (c3.w);
    r0.w = (r1.y) * (r0.w);
    r1.w = (r1.w) * (r1.z);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r0.w) * (r1.w);
    r2.xyz = (r0.xyz) * (r0.www);
    r0 = tex2D(s4, v5.xy);
    r0.w = saturate(r0.y);
    r0.xyz = c[33].xyz;
    r0.xyz = (-(r0.yzx)) + (c[34].yzx);
    r4.xyz = (r0.www) * (r0.xyz) + (c[33].yzx);
    r0.w = c0.x;
    r0.w = (r0.w) * (c[27].w);
    r0.z = frac(r4.y);
    r0.w = frac(abs(r0.w));
    r1.w = (r4.y) + (-(r0.z));
    r0.w = ((c[27].w) >= 0.0f ? (r0.w) : (-(r0.w)));
    r2.w = (r0.z) + (c0.z);
    r0 = (r0.wwww) * (c2);
    r1.w = (r1.w) * (c0.y);
    r1.xy = (c[31].xy) * (r0.xx) + (v5.xy);
    r3.w = exp2(r1.w);
    r3.xy = (r1.xy) * (c[32].xx);
    r1.x = (r3.x) * (r3.w) + (r0.w);
    r1.z = (r3.w) * (r3.y);
    r1 = tex2D(s5, r1.xz);
    r5.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1.xy = (r3.xy) * (r3.ww) + (r0.yz);
    r1 = tex2D(s5, r1.xy);
    r0.x = ((r2.w) >= 0.0f ? (-(c0.z)) : (-(c0.y)));
    r1.xy = (r1.wy) * (c1.xy) + (c1.zw);
    r1.w = (r3.w) * (r0.x);
    r1.xy = (r5.xy) * (r1.xy);
    r0.x = (r3.x) * (r1.w) + (r0.w);
    r6.xy = (r3.xy) * (r1.ww) + (r0.yz);
    r0.z = (r3.y) * (r1.w);
    r0 = tex2D(s5, r0.xz);
    r5.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s5, r6.xy);
    r0.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0.w = (abs(r2.w)) + (abs(r2.w));
    r0.xy = (r0.xy) * (r5.xy) + (-(r1.xy));
    r0.xy = (r0.ww) * (r0.xy) + (r1.xy);
    r0.w = (r4.x) * (c[36].x);
    r6.xyz = (r2.xyz) * (c[6].xyz);
    r4.xy = (r0.xy) * (r0.ww);
    r0 = tex2D(s3, r3.xy);
    r1 = tex2D(s6, r3.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = v0;
    r2.xyz = (r0.yzx) * (v3.zxy);
    r3.xy = (c[35].xx) * (r3.xy) + (r4.xy);
    r0.xyz = (v3.yzx) * (r0.zxy) + (-(r2.xyz));
    r0.xyz = (r3.yyy) * (-(r0.xyz));
    r2.xyz = (r3.xxx) * (v0.xyz) + (r0.xyz);
    r0.xyz = (-(v2.xyz)) + (c[23].xyz);
    r3.xyz = (r2.xyz) + (v3.xyz);
    r7.y = dot(r0.xyz, r0.xyz);
    r2.xyz = normalize(r3.xyz);
    r1.w = rsqrt(r7.y);
    r3.xyz = (r0.xyz) * (r1.www);
    r0.z = dot(-(v2.xyz), -(v2.xyz));
    r7.x = 1.0f / (r1.w);
    r1.w = rsqrt(r0.z);
    r5.xyz = (-(v2.xyz)) * (r1.www) + (r3.xyz);
    r4.xy = saturate((r7.xx) * (c[26].xy) + (c[26].zw));
    r0.xy = (r4.xy) * (r4.xy);
    r4.xy = (r4.xy) * (c3.zz) + (c3.ww);
    r2.w = dot(c[25].yz, r7.xy) + (c[25].x);
    r4.xy = (r0.xy) * (r4.xy);
    r0.xyz = normalize(r5.xyz);
    r2.w = (r2.w) * (r4.x);
    r0.y = saturate(dot(r2.xyz, r0.xyz));
    r0.z = (r4.y) * (r2.w);
    r3.w = pow(abs(r0.y), c2.x);
    r0.xyz = (r0.zzz) * (c[24].xyz);
    r2.w = saturate(dot(r2.xyz, r3.xyz));
    r5.xyz = (r3.www) * (r0.xyz);
    r0.xyz = (r0.xyz) * (r2.www);
    r9.xyz = (-(v2.xyz)) * (r1.www) + (r8.xyz);
    r3.xyz = (r1.www) * (-(v2.xyz));
    r7.xyz = normalize(r9.xyz);
    r2.w = saturate(dot(r2.xyz, r8.xyz));
    r1.w = saturate(dot(r2.xyz, r7.xyz));
    r3.w = pow(abs(r1.w), c2.x);
    r1.w = dot(-(r3.xyz), r2.xyz);
    r5.xyz = (r3.www) * (r6.xyz) + (r5.xyz);
    r1.w = (r1.w) + (r1.w);
    r0.xyz = (r2.www) * (r6.xyz) + (r0.xyz);
    r2.xyz = (r2.xyz) * (-(r1.www)) + (-(r3.xyz));
    r2 = texCUBE(s2, r2.xyz);
    r3.xy = (vPos.xy) * (c[28].zw);
    r3 = tex2D(s1, r3.xy);
    r2.w = (-abs(r3.x)) + (c12.x);
    r1.w = max(c12.y, r2.w);
    r2.w = 1.0f / (r1.w);
    r1.w = c12.x;
    r1.w = (r1.w) * (c[4].x);
    r2.xyz = (r2.xyz) * (c[30].xxx) + (r5.xyz);
    r2.w = (r1.w) * (r2.w) + (-(v1.z));
    r2.xyz = (r4.zzz) * (r2.xyz);
    r1.w = max(r2.w, c0.w);
    r1.w = (r1.w) * (c[37].x);
    r2.w = 1.0f / (c[38].x);
    r0.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r1.x = saturate((r1.w) * (r2.w));
    r0.xyz = (r0.xyz) * (r1.xxx) + (-(v4.xyz));
    r0.xyz = (r0.www) * (r0.xyz) + (v4.xyz);
    r0.xyz = max(((r0.xyz) * (c[29].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = r1.x;
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
