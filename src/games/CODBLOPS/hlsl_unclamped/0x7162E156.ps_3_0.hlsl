// Mechanically reconstructed from 0x7162E156.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD3;
    float4 v2 : TEXCOORD5;
    float4 v3 : TEXCOORD6;
    float4 v4 : TEXCOORD7;
    float4 v5 : TEXCOORD8;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(2.75f, -2.0f, 0.5f, 2.0f);
    const float4 c1 = float4(75.0f, 0.5f, -0.5f, 3.68000007f);
    const float4 c2 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c3 = float4(0.00333333341f, 300.0f, -64.0301971f, 9.40301991f);
    const float4 c4 = float4(60.0f, 1.0f, 0.0f, 31.875f);
    const float4 c12 = float4(100.0f, 8.0f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v2.xyzx) * (c4.yyyz) + (c4.zzzy);
    r0.w = dot(r1, c[30]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[27]);
    r0.y = dot(r1, c[28]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[31].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[32].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[32].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[29]);
    r0.z = (r2.x) * (c[33].x);
    r0.y = (r2.y) * (c[33].y) + (-(r0.z));
    r0.z = c[33].y;
    r0.z = (r2.y) * (r0.z) + (-(c[32].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[25];
    r1 = saturate((r2.zyxz) * (r1) + (c[26]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[33].zzzz) * (r3) + (c[33].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[32].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[24].yz, r2.zw) + (c[24].x);
    r0.z = (r0.y) * (r0.z);
    r4.w = (r1.x) * (r0.z);
    r1 = c[34];
    r1.x = dot(r0.xw, r1.xy) + (c[23].x);
    r1.y = dot(r0.xw, r1.zw) + (c[23].z);
    r0 = tex2D(s0, r1.xy);
    r1 = tex2D(s2, v5.xy);
    r0.w = saturate(r1.y);
    r1.xyz = c[39].xyz;
    r1.xyz = (-(r1.yzx)) + (c[40].yzx);
    r5.xyz = (r0.www) * (r1.xyz) + (c[39].yzx);
    r0.w = frac(r5.y);
    r1.w = (r5.y) + (-(r0.w));
    r3.w = (r0.w) + (c1.z);
    r0.w = c3.x;
    r0.w = (r0.w) * (c[35].w);
    r1.w = (r1.w) * (c0.y);
    r0.w = frac(abs(r0.w));
    r3.z = exp2(r1.w);
    r0.w = ((c[35].w) >= 0.0f ? (r0.w) : (-(r0.w)));
    r2.y = (r0.w) * (c4.x);
    r11.xy = (v5.xy) * (c[38].xx);
    r1.x = (r11.x) * (r3.z) + (r2.y);
    r1.z = (r3.z) * (r11.y);
    r1 = tex2D(s3, r1.xz);
    r2.xzw = (r0.www) * (c3.yzw);
    r3.xy = (r1.wy) * (c2.xy) + (c2.zw);
    r1.xy = (r11.xy) * (r3.zz) + (r2.zw);
    r1 = tex2D(s3, r1.xy);
    r1.xy = (r1.wy) * (c2.xy) + (c2.zw);
    r1.w = ((r3.w) >= 0.0f ? (c0.z) : (c0.w));
    r4.xy = (r3.xy) * (r1.xy);
    r1.w = (r3.z) * (r1.w);
    r4.z = (abs(r3.w)) + (abs(r3.w));
    r1.x = (r11.x) * (r1.w) + (r2.y);
    r1.z = (r11.y) * (r1.w);
    r3.xy = (r11.xy) * (r1.ww) + (r2.zw);
    r1 = tex2D(s3, r1.xz);
    r6.xy = (r1.wy) * (c2.xy) + (c2.zw);
    r1 = tex2D(s3, r3.xy);
    r3.xy = (r11.xy) * (c[41].xx);
    r7.xy = (c[42].xy) * (r2.xx) + (r3.xy);
    r3.xy = (c[43].xx) * (r2.xx) + (r3.xy);
    r2 = tex2D(s4, r7.xy);
    r3 = tex2D(s4, r3.xy);
    r1.z = (r2.x) + (r3.x);
    r1.z = (r1.z) + (r1.z);
    r0.w = (r0.w) * (-(c1.x)) + (r1.z);
    r0.w = (r0.w) + (c1.y);
    r0.w = frac(r0.w);
    r0.w = (r0.w) + (c1.z);
    r0.w = (abs(r0.w)) * (c1.w);
    r1.xy = (r1.wy) * (c2.xy) + (c2.zw);
    r1.w = pow(abs(r0.w), c0.x);
    r1.xy = (r1.xy) * (r6.xy) + (-(r4.xy));
    r0.w = lerp(c[44].x, c[44].y, r1.w);
    r3.xy = (r4.zz) * (r1.xy) + (r4.xy);
    r0.w = (r5.x) * (r0.w);
    r1 = tex2D(s1, r11.xy);
    r4.xy = (r1.wy) * (c2.xy) + (c2.zw);
    r1.xyz = v3.xyz;
    r2.xyz = (r1.zxy) * (v1.yzx);
    r3.xy = (r0.ww) * (r3.xy) + (r4.xy);
    r1.xyz = (r1.yzx) * (v1.zxy) + (-(r2.xyz));
    r1.xyz = (r3.yyy) * (-(r1.xyz));
    r1.xyz = (r3.xxx) * (v1.xyz) + (r1.xyz);
    r1.xyz = (r1.xyz) + (v3.xyz);
    r4.xyz = normalize(r1.xyz);
    r1.w = max(abs(r4.y), abs(r4.z));
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.w = max(abs(r4.x), r1.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r4.xyz) * (c[5].xyz);
    r2.xyz = (r4.www) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (v0.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.z = dot(-(v2.xyz), -(v2.xyz));
    r1.w = rsqrt(r0.z);
    r1.xyz = (r1.www) * (-(v2.xyz));
    r2.w = dot(-(r1.xyz), r4.xyz);
    r0.xyz = (r2.xyz) * (r0.www);
    r0.w = (r2.w) + (r2.w);
    r6.xyz = (r0.xyz) * (c[22].xyz);
    r0.xyz = (r4.xyz) * (-(r0.www)) + (-(r1.xyz));
    r5.w = saturate(dot(r4.xyz, r1.xyz));
    r0.w = c[37].x;
    r0 = texCUBElod(s15, r0);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (-(v2.xyz)) + (c[21].xyz);
    r7.xyz = (r1.xyz) * (c12.yyy);
    r9.xyz = normalize(r0.xyz);
    r10.xyz = (-(v2.xyz)) * (r1.www) + (r9.xyz);
    r3 = (-(v2.yyyy)) + (c[7]);
    r0 = (r3) * (r3);
    r2 = (-(v2.xxxx)) + (c[6]);
    r0 = (r2) * (r2) + (r0);
    r1 = (-(v2.zzzz)) + (c[8]);
    r8.xyz = normalize(r10.xyz);
    r0 = (r1) * (r1) + (r0);
    r5.y = saturate(dot(r4.xyz, r8.xyz));
    r4.x = rsqrt(r0.x);
    r4.y = rsqrt(r0.y);
    r4.z = rsqrt(r0.z);
    r4.w = rsqrt(r0.w);
    r3 = (r3) * (r4);
    r8.xyz = normalize(v3.xyz);
    r2 = (r2) * (r4);
    r3 = (r3) * (r8.yyyy);
    r1 = (r1) * (r4);
    r2 = (r2) * (r8.xxxx) + (r3);
    r3.z = c4.y;
    r0 = saturate((r0) * (c[9]) + (r3.zzzz));
    r1 = saturate((r1) * (r8.zzzz) + (r2));
    r2.w = pow(abs(r5.y), c12.x);
    r0 = (r0) * (r1);
    r1.x = dot(c[10], r0);
    r2.z = max(abs(r8.y), abs(r8.z));
    r1.y = dot(c[11], r0);
    r1.w = max(abs(r8.x), r2.z);
    r1.w = 1.0f / (r1.w);
    r2.xyz = (r8.xyz) * (c[5].xyz);
    r1.z = dot(c[20], r0);
    r0.xyz = (r2.xyz) * (r1.www) + (v0.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = saturate(dot(r8.xyz, r9.xyz));
    r0.xyz = (r0.xyz) * (c4.www) + (r1.xyz);
    r2.xyz = (r2.www) * (r6.xyz) + (r7.xyz);
    r3.xyz = (r0.www) * (r6.xyz) + (r0.xyz);
    r1 = tex2D(s6, r11.xy);
    r0 = tex2D(s5, r11.xy);
    r0.w = (-(r5.w)) + (c4.y);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.z = (r0.w) * (r0.w);
    r1.xyz = (r3.xyz) * (r1.xyz);
    r0.z = (r0.z) * (r0.z);
    r2.w = (-(r0.x)) + (c4.y);
    r1.w = (r0.w) * (r0.z);
    r0.xyz = (r2.xyz) * (r5.zzz) + (-(r1.xyz));
    r0.w = saturate(lerp(r2.w, c4.y, r1.w));
    r0.xyz = (r0.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v4.xyz));
    r0.w = v1.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v4.xyz);
    r0.xyz = max(((r0.xyz) * (c[36].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c4.y;

    return oC0;
}
