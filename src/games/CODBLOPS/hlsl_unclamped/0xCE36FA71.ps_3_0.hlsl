// Mechanically reconstructed from 0xCE36FA71.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(1.0f, 0.0f, -1.0f, 8.0f);
    const float4 c2 = float4(1.0f, 0.5f, 0.0f, 4.0f);
    const float4 c3 = float4(0.600000024f, 0.400000006f, 0.797884583f, 31.875f);
    const float4 c4 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c12 = float4(4.0f, -2.0f, 0.797884583f, 1.0f);
    const float4 c13 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v0.zw) * (c2.xy);
    r0 = tex2D(s13, r0.xy);
    r1.xy = (v0.zw) * (c2.xy) + (c2.zy);
    r3 = tex2D(s13, r1.xy);
    r0.w = r3.y;
    r1 = tex2D(s2, v6.zw);
    r2.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s1, v6.zw);
    r4.w = (r1.w) * (v6.y);
    r4.xy = (r0.yw) * (c12.xx) + (c12.yy);
    r5.xy = (r2.xy) * (r4.ww);
    r0.y = dot(r4.xy, r4.xy) + (c1.y);
    r0.w = dot(r5.xy, r5.xy) + (c1.y);
    r0.y = exp2(-(r0.y));
    r0.w = exp2(-(r0.w));
    r0.y = (r0.y) * (c3.x) + (c3.y);
    r0.w = (r0.w) * (c3.x) + (c3.y);
    r3.w = (r0.y) * (r0.w);
    r2 = tex2D(s14, v0.zw);
    r6.xy = (r2.xy) * (c3.ww);
    r0.y = dot(r4.xy, r5.xy) + (c1.y);
    r3.xy = (r3.xz) * (r6.yy);
    r2.w = saturate((r0.y) * (r3.w) + (r3.w));
    r0.y = (r2.y) * (c3.w) + (-(r3.x));
    r4.xz = (r3.xy) * (c2.ww);
    r0.y = (r3.z) * (-(r6.y)) + (r0.y);
    r4.y = (r0.y) + (r0.y);
    r0.xy = (r0.xz) * (r6.xx);
    r3.xyz = (r2.www) * (r4.xyz);
    r2.w = (r2.x) * (c3.w) + (-(r0.x));
    r9.xz = (r0.xy) * (c2.ww);
    r0.z = (r0.z) * (-(r6.x)) + (r2.w);
    r9.y = (r0.z) + (r0.z);
    r11.xz = c1.xz;
    r2.xyz = (r11.zzz) + (c[20].xyw);
    r0.xyz = (r9.xyz) * (r0.www) + (r3.xyz);
    r3.xyz = (r4.www) * (r2.xyz) + (c1.xxx);
    r7.xyz = (r0.xyz) * (r3.yyy);
    r0 = tex2D(s12, v0.zw);
    r2.xyz = v1.xyz;
    r2.xyz = (r5.xxx) * (v4.xyz) + (r2.xyz);
    r2.xyz = (r5.yyy) * (v3.xyz) + (r2.xyz);
    r8.xyz = normalize(r2.xyz);
    r0.x = saturate(dot(r8.xyz, c[17].xyz));
    r0.w = dot(v5.xyz, v5.xyz);
    r5.xyz = (r0.xxx) * (c[18].xyz);
    r0.w = rsqrt(r0.w);
    r4.xyz = (r0.www) * (v5.xyz);
    r10.xyz = (v5.xyz) * (-(r0.www)) + (c[17].xyz);
    r2 = tex2D(s3, v6.zw);
    r2 = (r2) + (c1.yyyz);
    r0.z = saturate(dot(r8.xyz, -(r4.xyz)));
    r0.w = (r4.w) * (r2.w) + (c1.x);
    r3.y = (r0.w) * (c3.z);
    r2.w = (r0.w) * (-(c12.z)) + (c12.w);
    r3.w = (r0.z) * (r2.w) + (r3.y);
    r2.w = (r0.x) * (r2.w) + (r3.y);
    r2.xyz = (r4.www) * (r2.xyz);
    r2.w = (r2.w) * (r3.w) + (c4.x);
    r2.w = 1.0f / (r2.w);
    r6.xyz = normalize(r10.xyz);
    r3.w = (r0.x) * (r2.w);
    r3.y = saturate(dot(r8.xyz, r6.xyz));
    r2.w = saturate(dot(r6.xyz, c[17].xyz));
    r12.xy = (r0.ww) * (c13.xy) + (c13.zw);
    r0.x = exp2(r12.y);
    r2.w = (-(r2.w)) + (c1.x);
    r5.w = pow(abs(r3.y), r0.x);
    r3.y = (r2.w) * (r2.w);
    r0.x = (r0.x) * (c4.y) + (c4.z);
    r3.y = (r3.y) * (r3.y);
    r0.x = (r5.w) * (r0.x);
    r2.w = (r2.w) * (r3.y);
    r10.xyz = (r2.xyz) * (r2.xyz);
    r6.xyz = (r2.xyz) * (-(r2.xyz)) + (c1.xxx);
    r0.x = (r3.w) * (r0.x);
    r2.xyz = (r6.xyz) * (r2.www) + (r10.xyz);
    r7.xyz = (r0.yyy) * (r5.xyz) + (r7.xyz);
    r2.xyz = (r0.xxx) * (r2.xyz);
    r3.y = (-(r0.z)) + (c1.x);
    r2.xyz = (r3.zzz) * (r2.xyz);
    r0.w = (r0.w) * (c1.w);
    r2.xyz = (r2.xyz) * (c[19].xyz);
    r3.w = 1.0f / (r12.x);
    r0.xyz = (r0.yyy) * (r2.xyz);
    r2 = tex2D(s0, v0.xy);
    r5.xyz = lerp(r2.xyz, r1.xyz, r4.www);
    r5.w = (r2.w) * (-(v6.x)) + (c1.x);
    r6.w = (r1.w) * (-(v6.y)) + (c1.x);
    r1.w = dot(r4.xyz, r8.xyz);
    r5.xyz = (r5.xyz) * (r5.xyz);
    r1.w = (r1.w) + (r1.w);
    r7.xyz = (r5.xyz) * (r7.xyz) + (r0.xyz);
    r0.xyz = (r8.xyz) * (-(r1.www)) + (r4.xyz);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r3.y) * (r3.y);
    r4.xyz = (r3.xxx) * (r0.xyz);
    r0.w = (r3.y) * (r0.w);
    r4.w = (r3.w) * (r0.w);
    r3 = (-(v5.yyyy)) + (c[6]);
    r0 = (r3) * (r3);
    r2 = (-(v5.xxxx)) + (c[5]);
    r0 = (r2) * (r2) + (r0);
    r1 = (-(v5.zzzz)) + (c[7]);
    r6.xyz = (r6.xyz) * (r4.www) + (r10.xyz);
    r0 = (r1) * (r1) + (r0);
    r6.xyz = (r4.xyz) * (r6.xyz);
    r4.x = rsqrt(r0.x);
    r4.y = rsqrt(r0.y);
    r4.z = rsqrt(r0.z);
    r4.w = rsqrt(r0.w);
    r6.xyz = (r9.xyz) * (r6.xyz);
    r3 = (r3) * (r4);
    r3 = (r8.yyyy) * (r3);
    r2 = (r2) * (r4);
    r1 = (r1) * (r4);
    r2 = (r2) * (r8.xxxx) + (r3);
    r0 = saturate((r0) * (c[8]) + (r11.xxxx));
    r1 = saturate((r1) * (r8.zzzz) + (r2));
    r2.xyz = (r6.xyz) * (c1.www) + (r7.xyz);
    r0 = (r0) * (r1);
    r1.x = dot(c[9], r0);
    r1.y = dot(c[10], r0);
    r1.z = dot(c[11], r0);
    r0.w = (r5.w) * (-(r6.w)) + (c1.x);
    r0.xyz = (r5.xyz) * (r1.xyz) + (r2.xyz);
    r1.xyz = (r0.www) * (v2.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (-(r1.xyz));
    r0.xyz = (v1.www) * (r0.xyz) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[21].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
