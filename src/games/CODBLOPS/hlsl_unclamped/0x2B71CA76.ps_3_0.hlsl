// Mechanically reconstructed from 0x2B71CA76.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c1 = float4(0.959999979f, 0.0399999991f, -2.0f, 3.0f);
    const float4 c2 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c3 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c4 = float4(1.0f, 0.0f, 0.5f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v4.yyyy) * (c[26]);
    r0 = (v4.xxxx) * (c[25]) + (r0);
    r0 = (v4.zzzz) * (c[27]) + (r0);
    r0 = (r0) + (c[28]);
    r3.xy = (r0.ww) * (c[34].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s1, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[34].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s1, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[34].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[34].zw) + (r0.xy);
    r2 = tex2Dproj(s1, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s1, r0);
    r0 = (v4.xyzx) * (c4.xxxy) + (c4.yyyx);
    r2.w = dot(r0, c[22]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[11]);
    r2.y = dot(r0, c[20]);
    r1.w = dot(r1, c3.zzzz);
    r1.xy = (r2.ww) * (r2.xy);
    r2.x = dot(r0, c[21]);
    r0.xy = (r1.xy) * (c4.zz) + (c4.zz);
    r0 = tex2D(s2, r0.xy);
    r2.y = (r2.x) * (r2.x);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.w = dot(c[9].yz, r2.xy) + (c[9].x);
    r2.xy = saturate((r2.xx) * (c[10].xy) + (c[10].zw));
    r0.z = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c3.w) : (r0.z));
    r8.xyz = (-(v4.xyz)) + (c[6].xyz);
    r0.xy = (r2.xy) * (r2.xy);
    r0.z = dot(r8.xyz, r8.xyz);
    r2.xy = (r2.xy) * (c1.zz) + (c1.ww);
    r7.w = rsqrt(r0.z);
    r0.x = (r0.x) * (r2.x);
    r7.xyz = (r8.xyz) * (r7.www);
    r0.y = (r0.y) * (-(r2.y)) + (c0.z);
    r0.z = dot(r7.xyz, c[23].xyz);
    r0.w = (r0.w) * (r0.x);
    r0.z = saturate((r0.z) * (c[24].x) + (c[24].y));
    r0.w = (r0.y) * (r0.w);
    r2.w = (r0.z) * (r0.z);
    r2.z = (r0.z) * (c1.z) + (c1.w);
    r0.xyz = (-(v4.xyz)) + (c[29].xyz);
    r2.w = (r2.w) * (r2.z);
    r4.y = dot(r0.xyz, r0.xyz);
    r2.w = (r0.w) * (r2.w);
    r0.w = rsqrt(r4.y);
    r1.xyz = (r1.xyz) * (r2.www);
    r4.x = 1.0f / (r0.w);
    r2.xyz = (r1.www) * (r1.xyz);
    r3.xy = saturate((r4.xx) * (c[33].xy) + (c[33].zw));
    r1.w = dot(c[32].yz, r4.xy) + (c[32].x);
    r1.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c1.zz) + (c1.ww);
    r4.xyz = normalize(v2.xyz);
    r3.xy = (r1.xy) * (r3.xy);
    r3.w = max(abs(r4.y), abs(r4.z));
    r2.w = (r1.w) * (r3.x);
    r1.w = max(abs(r4.x), r3.w);
    r1.w = 1.0f / (r1.w);
    r1.xyz = (r4.xyz) * (c[5].xyz);
    r2.w = (r3.y) * (r2.w);
    r1.xyz = (r1.xyz) * (r1.www) + (v5.xyz);
    r1 = tex3D(s11, r1.xyz);
    r9.xyz = normalize(v4.xyz);
    r5.xyz = (r0.xyz) * (r0.www) + (-(r9.xyz));
    r3.xyz = (r0.xyz) * (r0.www);
    r0.xyz = normalize(r5.xyz);
    r0.w = saturate(dot(r0.xyz, r3.xyz));
    r0.w = (-(r0.w)) + (c0.z);
    r8.w = (r2.w) * (r1.w);
    r1.w = (r0.w) * (r0.w);
    r3.z = saturate(dot(r4.xyz, r3.xyz));
    r1.w = (r1.w) * (r1.w);
    r5.y = saturate(dot(r4.xyz, r0.xyz));
    r5.z = (r0.w) * (r1.w);
    r0 = tex2D(s3, v1.xy);
    r4.w = (r0.w) * (c0.w);
    r3.w = (r0.w) * (-(c0.w)) + (c0.z);
    r0.x = saturate(dot(r4.xyz, -(r9.xyz)));
    r0.z = (r3.z) * (r3.w) + (r4.w);
    r2.w = (r0.x) * (r3.w) + (r4.w);
    r0.z = (r0.z) * (r2.w) + (c3.x);
    r3.xy = (r0.ww) * (c2.xy) + (c2.zw);
    r5.w = 1.0f / (r0.z);
    r6.w = exp2(r3.y);
    r1.w = pow(abs(r5.y), r6.w);
    r0.z = (r6.w) * (c3.y) + (c3.z);
    r5.w = (r3.z) * (r5.w);
    r1.w = (r1.w) * (r0.z);
    r3.y = (r5.z) * (c1.x) + (c1.y);
    r1.w = (r5.w) * (r1.w);
    r10.xyz = (r1.xyz) * (r1.xyz);
    r1.w = (r3.y) * (r1.w);
    r0.x = (-(r0.x)) + (c0.z);
    r1.w = (r0.y) * (r1.w);
    r5.xyz = (r3.zzz) * (c[30].xyz);
    r1.xyz = (r1.www) * (c[31].xyz);
    r5.w = 1.0f / (r3.x);
    r6.xyz = (r8.www) * (r1.xyz);
    r1 = tex2D(s0, v1.xy);
    r1.xyz = (r1.xyz) * (v0.xyz);
    r3.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r0.yyy) * (r10.xyz);
    r1.xyz = (r1.xyz) * (c0.yyy);
    r1.w = dot(r9.xyz, r4.xyz);
    r5.xyz = (r8.www) * (r5.xyz) + (r1.xyz);
    r1.w = (r1.w) + (r1.w);
    r1.xyz = (r4.xyz) * (-(r1.www)) + (r9.xyz);
    r1.w = (r0.w) * (c0.x);
    r1 = texCUBElod(s15, r1);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r6.xyz = (r3.xyz) * (r5.xyz) + (r6.xyz);
    r5.xyz = (r1.xyz) * (c0.xxx);
    r1 = tex3D(s11, v5.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r8.xyz = (r8.xyz) * (r7.www) + (-(r9.xyz));
    r1.xyz = (r5.xyz) * (r1.xyz);
    r1.xyz = (r1.xyz) * (c0.yyy);
    r0.w = (r0.x) * (r0.x);
    r0.x = (r0.x) * (r0.w);
    r0.w = saturate(dot(r7.xyz, r4.xyz));
    r0.x = (r5.w) * (r0.x);
    r1.w = (r0.w) * (r3.w) + (r4.w);
    r1.w = (r1.w) * (r2.w) + (c3.x);
    r5.xyz = normalize(r8.xyz);
    r2.w = 1.0f / (r1.w);
    r1.w = saturate(dot(r5.xyz, r7.xyz));
    r2.w = (r0.w) * (r2.w);
    r1.w = (-(r1.w)) + (c0.z);
    r5.w = saturate(dot(r4.xyz, r5.xyz));
    r3.w = (r1.w) * (r1.w);
    r4.w = pow(abs(r5.w), r6.w);
    r3.w = (r3.w) * (r3.w);
    r0.z = (r0.z) * (r4.w);
    r1.w = (r1.w) * (r3.w);
    r0.z = (r2.w) * (r0.z);
    r1.w = (r1.w) * (c1.x) + (c1.y);
    r0.x = (r0.x) * (c1.x) + (c1.y);
    r0.z = (r0.z) * (r1.w);
    r1.xyz = (r1.xyz) * (r0.xxx);
    r5.xyz = (r0.zzz) * (c[8].xyz);
    r4.xyz = (r0.www) * (c[7].xyz);
    r5.xyz = (r0.yyy) * (r5.xyz);
    r1.xyz = (r1.xyz) * (r0.yyy) + (r6.xyz);
    r0.xyz = (r3.xyz) * (r4.xyz) + (r5.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[35].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.z;

    return oC0;
}
