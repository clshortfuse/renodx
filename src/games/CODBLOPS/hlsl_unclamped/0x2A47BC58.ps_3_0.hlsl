// Mechanically reconstructed from 0x2A47BC58.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD4;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(-0.5f, 0.200000003f, 8.0f, 31.875f);
    const float4 c1 = float4(1.0f, 0.797884583f, 0.0f, 0.0009765625f);
    const float4 c2 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c3 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = (v5.xyzx) * (c1.xxxz) + (c1.zzzx);
    r0.w = dot(r1, c[32]);
    r0.w = 1.0f / (r0.w);
    r0.x = dot(r1, c[29]);
    r0.y = dot(r1, c[30]);
    r0 = (r0.wwww) * (r0.xxyy);
    r2 = (r0) * (c[33].zwxy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.z = log2(abs(r2.z));
    r2.w = log2(abs(r2.w));
    r2 = (r2) * (c[34].xxxx);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = exp2(r2.z);
    r2.w = exp2(r2.w);
    r2.xy = (r2.zw) + (r2.xy);
    r2.x = log2(abs(r2.x));
    r2.y = log2(abs(r2.y));
    r2.xy = (r2.xy) * (c[34].yy);
    r2.x = exp2(r2.x);
    r2.y = exp2(r2.y);
    r2.z = dot(r1, c[31]);
    r0.z = (r2.x) * (c[35].x);
    r0.y = (r2.y) * (c[35].y) + (-(r0.z));
    r0.z = c[35].y;
    r0.z = (r2.y) * (r0.z) + (-(c[34].z));
    r0.y = 1.0f / (r0.y);
    r2.xy = abs(r0.xw);
    r3.w = saturate((r0.z) * (r0.y));
    r1 = c[27];
    r1 = saturate((r2.zyxz) * (r1) + (c[28]));
    r3.x = (r1.w) * (r1.x);
    r3.yz = r1.yz;
    r1 = (r3) * (r3);
    r3 = (c[35].zzzz) * (r3) + (c[35].wwww);
    r1 = (r1) * (r3);
    r0.y = (r1.z) * (r1.y);
    r0.z = abs(c[34].w);
    r2.w = (r2.z) * (r2.z);
    r0.y = ((-(r0.z)) >= 0.0f ? (r0.y) : (r1.w));
    r0.z = dot(c[26].yz, r2.zw) + (c[26].x);
    r0.z = (r0.y) * (r0.z);
    r1.w = (r1.x) * (r0.z);
    r1.xz = c[25].xz;
    r1.x = dot(r0.xw, c[36].xy) + (r1.x);
    r1.y = dot(r0.xw, c[36].zw) + (r1.z);
    r0 = tex2D(s2, r1.xy);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r1.www) * (r0.xyz);
    r0.xy = (v4.ww) * (c[37].xy) + (v4.xy);
    r0.zw = v4.zw;
    r0 = tex2Dproj(s1, r0);
    r1.xy = (v4.ww) * (-(c[37].xy)) + (v4.xy);
    r1.zw = v4.zw;
    r1 = tex2Dproj(s1, r1);
    r0.y = r1.x;
    r1.xy = (v4.ww) * (c[37].zw) + (v4.xy);
    r1.zw = v4.zw;
    r1 = tex2Dproj(s1, r1);
    r0.z = r1.x;
    r1.xy = (v4.ww) * (-(c[37].zw)) + (v4.xy);
    r1.zw = v4.zw;
    r1 = tex2Dproj(s1, r1);
    r10.xyz = normalize(v2.xyz);
    r2.w = max(abs(r10.y), abs(r10.z));
    r0.w = r1.x;
    r1.w = max(abs(r10.x), r2.w);
    r2.w = 1.0f / (r1.w);
    r1.xyz = (r10.xyz) * (c[5].xyz);
    r1.w = dot(r0, c3.yyyy);
    r0.xyz = (r1.xyz) * (r2.www) + (v6.xyz);
    r0 = tex3D(s11, r0.xyz);
    r4.xyz = (-(v5.xyz)) + (c[21].xyz);
    r1.z = dot(r4.xyz, r4.xyz);
    r3.w = rsqrt(r1.z);
    r4.w = lerp(r0.w, r1.w, c[25].w);
    r3.xyz = (r4.xyz) * (r3.www);
    r6.xyz = (r0.xyz) * (r0.xyz);
    r2.w = saturate(dot(r10.xyz, r3.xyz));
    r1 = tex2D(s3, v1.xy);
    r0.x = (r1.w) * (c1.y);
    r5.xyz = normalize(v5.xyz);
    r0.y = (r1.w) * (-(c1.y)) + (c1.x);
    r0.z = saturate(dot(r10.xyz, -(r5.xyz)));
    r0.w = (r2.w) * (r0.y) + (r0.x);
    r0.y = (r0.z) * (r0.y) + (r0.x);
    r2.xyz = (r2.xyz) * (r4.www);
    r0.w = (r0.w) * (r0.y) + (c1.w);
    r5.w = (-(r0.z)) + (c1.x);
    r0.w = 1.0f / (r0.w);
    r4.w = (r2.w) * (r0.w);
    r4.xyz = (r4.xyz) * (r3.www) + (-(r5.xyz));
    r0.xyz = normalize(r4.xyz);
    r7.xy = (r1.ww) * (c2.xy) + (c2.zw);
    r3.w = saturate(dot(r10.xyz, r0.xyz));
    r1.z = exp2(r7.y);
    r0.y = saturate(dot(r0.xyz, r3.xyz));
    r0.z = pow(abs(r3.w), r1.z);
    r0.w = (r1.z) * (c3.x) + (c3.y);
    r1.z = (-(r0.y)) + (c1.x);
    r3.w = (r0.z) * (r0.w);
    r0.w = (r1.z) * (r1.z);
    r6.w = (r0.w) * (r0.w);
    r0.xy = (v1.xy) * (c[38].xy);
    r0 = tex2D(s4, r0.xy);
    r3.xyz = (r0.xyz) + (c0.xxx);
    r0 = tex2D(s0, v1.xy);
    r4.xyz = saturate((r3.xyz) * (r0.www) + (r0.xyz));
    r1.z = (r1.z) * (r6.w);
    r0.xyz = lerp(r4.xyz, c0.yyy, r1.xxx);
    r11.xyz = (r0.xyz) * (r0.xyz);
    r9.xyz = (r0.xyz) * (-(r0.xyz)) + (c1.xxx);
    r0.w = (r4.w) * (r3.w);
    r0.xyz = (r9.xyz) * (r1.zzz) + (r11.xyz);
    r1.z = 1.0f / (r7.x);
    r0.xyz = (r0.www) * (r0.xyz);
    r0.xyz = (r1.yyy) * (r0.xyz);
    r7.xy = c[24].xy;
    r3.xyz = (r7.yyy) * (c[23].xyz);
    r4.xyz = (r1.xxx) * (r4.xyz);
    r0.xyz = (r0.xyz) * (r3.xyz);
    r3.xyz = (r2.xyz) * (r0.xyz);
    r0.xyz = (r6.xyz) * (r1.yyy);
    r6.xyz = (r0.xyz) * (c0.www);
    r0.xyz = (r7.xxx) * (c[22].xyz);
    r0.xyz = (r2.www) * (r0.xyz);
    r0.w = dot(r5.xyz, r10.xyz);
    r2.xyz = (r2.xyz) * (r0.xyz) + (r6.xyz);
    r0.w = (r0.w) + (r0.w);
    r0.xyz = (r10.xyz) * (-(r0.www)) + (r5.xyz);
    r0.w = (r1.w) * (c0.z);
    r0 = texCUBElod(s15, r0);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r5.xyz = (r4.xyz) * (v0.xyz);
    r4.xyz = (r0.xyz) * (c0.zzz);
    r0 = tex3D(s11, v6.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r6.xyz = (r5.xyz) * (r5.xyz);
    r0.xyz = (r4.xyz) * (r0.xyz);
    r7.xyz = (r6.xyz) * (r2.xyz) + (r3.xyz);
    r8.xyz = (r0.xyz) * (c0.www);
    r1.w = (r5.w) * (r5.w);
    r4 = (-(v5.yyyy)) + (c[7]);
    r0 = (r4) * (r4);
    r3 = (-(v5.xxxx)) + (c[6]);
    r0 = (r3) * (r3) + (r0);
    r2 = (-(v5.zzzz)) + (c[8]);
    r1.w = (r5.w) * (r1.w);
    r0 = (r2) * (r2) + (r0);
    r1.w = (r1.z) * (r1.w);
    r5.x = rsqrt(r0.x);
    r5.y = rsqrt(r0.y);
    r5.z = rsqrt(r0.z);
    r5.w = rsqrt(r0.w);
    r9.xyz = (r9.xyz) * (r1.www) + (r11.xyz);
    r4 = (r4) * (r5);
    r4 = (r10.yyyy) * (r4);
    r3 = (r3) * (r5);
    r2 = (r2) * (r5);
    r3 = (r3) * (r10.xxxx) + (r4);
    r1.w = c1.x;
    r0 = saturate((r0) * (c[9]) + (r1.wwww));
    r2 = saturate((r2) * (r10.zzzz) + (r3));
    r3.xyz = (r8.xyz) * (r9.xyz);
    r0 = (r0) * (r2);
    r2.xyz = (r3.xyz) * (r1.yyy) + (r7.xyz);
    r1.x = dot(c[10], r0);
    r1.y = dot(c[11], r0);
    r1.z = dot(c[20], r0);
    r0.xyz = (r6.xyz) * (r1.xyz) + (r2.xyz);
    r0.w = c1.x;
    r1.x = dot(r0, c[40]);
    r1.y = dot(r0, c[41]);
    r1.z = dot(r0, c[42]);
    r0.xyz = (r1.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[39].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c1.x;

    return oC0;
}
