// Mechanically reconstructed from 0xD2583759.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD4;
    float4 v7 : TEXCOORD5;
    float4 v8 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 0.5f, 1.10000002f, 10.0f);
    const float4 c3 = float4(1.0f, 0.0f, -1.0f, 8.0f);
    const float4 c4 = float4(31.875f, 0.797884583f, 1.0f, 0.0009765625f);
    const float4 c11 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c12 = float4(0.125f, 0.25f, 0.000244140625f, 0.0f);
    const float4 c13 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.75f);
    const float4 c14 = float4(4.0f, -3.0f, -4.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r0.w = dot(v7.xyz, v7.xyz);
    r4.w = rsqrt(r0.w);
    r0.xyz = (v7.xyz) * (-(r4.www)) + (c[17].xyz);
    r3.xyz = normalize(r0.xyz);
    r0.w = saturate(dot(r3.xyz, c[17].xyz));
    r0.w = (-(r0.w)) + (c3.x);
    r0.z = (r0.w) * (r0.w);
    r0.z = (r0.z) * (r0.z);
    r1.xy = (v1.xy) + (c1.xx);
    r3.w = (r0.w) * (r0.z);
    r1.z = c1.y;
    r4.xy = (r1.xy) * (c[9].xx) + (r1.zz);
    r0 = tex2D(s0, v1.xy);
    r2.xyz = (r0.xyz) * (v0.xyz);
    r0 = tex2D(s5, r4.xy);
    r0.xyz = (r2.xyz) * (-(r2.xyz)) + (r0.xyz);
    r0.w = (r0.w) + (c3.z);
    r1.xy = (r1.xy) * (c[8].xx) + (r1.zz);
    r1 = tex2D(s4, r1.xy);
    r0.xyz = (r0.xyz) * (r1.yyy);
    r8.w = (v8.w) * (c1.z) + (-(r1.x));
    r1 = tex2D(s1, v1.xy);
    r5.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r9.w = saturate((r8.w) * (c1.w));
    r1 = tex2D(s6, v1.xy);
    r1.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r2.w = c3.x;
    r4.xy = lerp(r5.xy, r1.xy, r9.ww);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r1 = v2;
    r1.xyz = (r4.xxx) * (v5.xyz) + (r1.xyz);
    r0 = (r9.wwww) * (r0) + (r2);
    r1.xyz = (r4.yyy) * (v4.xyz) + (r1.xyz);
    r10.xyz = normalize(r1.xyz);
    r11.xyz = (r4.www) * (v7.xyz);
    r4.w = saturate(dot(r10.xyz, r3.xyz));
    r10.w = saturate(dot(r10.xyz, -(r11.xyz)));
    r2 = tex2D(s3, v1.xy);
    r8.xyz = (r2.xyz) * (-(r2.xyz)) + (c3.xxx);
    r9.xyz = (r2.xyz) * (r2.xyz);
    r2.xyz = (r8.xyz) * (r3.www) + (r9.xyz);
    r1.z = (r2.w) * (-(c4.y)) + (c4.z);
    r3.z = (r2.w) * (c4.y);
    r14.xy = (r2.ww) * (c11.xy) + (c11.zw);
    r1.y = (r10.w) * (r1.z) + (r3.z);
    r1.x = exp2(r14.y);
    r3.w = pow(abs(r4.w), r1.x);
    r4.w = saturate(dot(r10.xyz, c[17].xyz));
    r1.x = (r1.x) * (c12.x) + (c12.y);
    r1.z = (r4.w) * (r1.z) + (r3.z);
    r3.z = (r3.w) * (r1.x);
    r1.z = (r1.z) * (r1.y) + (c4.w);
    r1.z = 1.0f / (r1.z);
    r3.w = max(abs(r10.y), abs(r10.z));
    r3.y = (r4.w) * (r1.z);
    r1.z = max(abs(r10.x), r3.w);
    r3.w = 1.0f / (r1.z);
    r1.xyz = (r10.xyz) * (c[5].xyz);
    r4.z = (r3.z) * (r3.y);
    r1.xyz = (r1.xyz) * (r3.www) + (v8.xyz);
    r3 = tex3D(s11, r1.xyz);
    r1.xyz = (r3.xyz) * (r3.xyz);
    r2.xyz = (r2.xyz) * (r4.zzz);
    r1.xyz = (r1.xyz) * (c[6].yyy);
    r13.xyz = (r1.xyz) * (c4.xxx);
    r12.xyz = (r4.www) * (c[18].xyz);
    if ((c3.x) >= (v6.w))
    {
        r4 = (v6.xyzx) * (c3.xxxy);
        r3 = (r4) + (-(c13.xyzz));
        r3 = tex2Dlod(s2, r3);
        r3.w = r3.x;
        r5 = (r4) + (c12.zzww);
        r5 = tex2Dlod(s2, r5);
        r3.x = r5.x;
        r5 = (r4) + (-(c12.zzww));
        r5 = tex2Dlod(s2, r5);
        r3.y = r5.x;
        r4 = (r4) + (c13.xyzz);
        r4 = tex2Dlod(s2, r4);
        r3.z = r4.x;
        r1.z = dot(r3, c12.yyyy);
        if ((c13.w) < (v6.w))
        {
            r1.xy = (v6.xy) * (c[2].ww) + (c[2].xy);
            r3.xy = (r1.xy) + (c12.zz);
            r3.zw = (v6.zx) * (c3.xy);
            r3 = tex2Dlod(s2, r3);
            r4.xy = (r1.xy) + (-(c12.zz));
            r4.zw = (v6.zx) * (c3.xy);
            r6 = tex2Dlod(s2, r4);
            r4.xy = (r1.xy) + (c13.xy);
            r4.zw = (v6.zx) * (c3.xy);
            r5 = tex2Dlod(s2, r4);
            r4.xy = (r1.xy) + (-(c13.xy));
            r4.zw = (v6.zx) * (c3.xy);
            r4 = tex2Dlod(s2, r4);
            r3.y = r6.x;
            r3.z = r5.x;
            r3.w = r4.x;
            r1.y = dot(r3, c12.yyyy);
            r1.x = (-(r1.z)) + (r1.y);
            r1.y = (v6.w) * (c14.x) + (c14.y);
            r3.w = (r1.y) * (r1.x) + (r1.z);
        }
        else
        {
            r3.w = r1.z;
        }
    }
    else
    {
        r1.z = (v6.w) + (c14.z);
        r1.z = ((r1.z) >= 0.0f ? (c3.y) : (c3.x));
        if ((r1.z) != (-(r1.z)))
        {
            r1.xy = (v6.xy) * (c[2].ww) + (c[2].xy);
            r4.xy = (r1.xy) + (c12.zz);
            r4.zw = (v6.zz) * (c3.xy);
            r4 = tex2Dlod(s2, r4);
            r5.xy = (r1.xy) + (-(c12.zz));
            r5.zw = (v6.zz) * (c3.xy);
            r7 = tex2Dlod(s2, r5);
            r5.xy = (r1.xy) + (c13.xy);
            r5.zw = (v6.zz) * (c3.xy);
            r6 = tex2Dlod(s2, r5);
            r5.xy = (r1.xy) + (-(c13.xy));
            r5.zw = (v6.zz) * (c3.xy);
            r5 = tex2Dlod(s2, r5);
            r4.y = r7.x;
            r4.z = r6.x;
            r4.w = r5.x;
            r1.x = dot(r4, c12.yyyy);
            r1.z = saturate((v6.w) + (c14.y));
            r1.y = (r3.w) + (-(r1.x));
            r1.z = (r1.z) * (r1.y) + (r1.x);
        }
        else
        {
            r1.z = r3.w;
        }
        r3.w = r1.z;
    }
    r1.xyz = (r2.xyz) * (c[6].www);
    r2.xyz = (r1.xyz) * (c[19].xyz);
    r1.xyz = (r3.www) * (r12.xyz) + (r13.xyz);
    r2.xyz = (r3.www) * (r2.xyz);
    r1.xyz = (r0.xyz) * (r1.xyz) + (r2.xyz);
    r4.w = 1.0f / (r14.x);
    r3.w = (-(r10.w)) + (c3.x);
    r0.z = dot(r11.xyz, r10.xyz);
    r3.z = (r3.w) * (r3.w);
    r0.z = (r0.z) + (r0.z);
    r2.w = (r2.w) * (c3.w);
    r2.xyz = (r10.xyz) * (-(r0.zzz)) + (r11.xyz);
    r2 = texCUBElod(s15, r2);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r3.w = (r3.w) * (r3.z);
    r3.xyz = (r0.xyz) * (c3.www);
    r2 = tex3D(s11, v8.xyz);
    r0.xyz = (r2.xyz) * (r2.xyz);
    r2.z = (r4.w) * (r3.w);
    r0.xyz = (r3.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c4.xxx);
    r2.w = (-(r9.w)) + (c3.x);
    r2.xyz = (r8.xyz) * (r2.zzz) + (r9.xyz);
    r2.w = ((r8.w) >= 0.0f ? (r2.w) : (c3.y));
    r0.xyz = (r0.xyz) * (r2.xyz);
    r2.w = (r2.w) * (r2.w);
    r0.xyz = (r0.xyz) * (c[6].xxx) + (r1.xyz);
    r1.z = (r2.w) * (c[10].w);
    r0.xyz = (c[10].xyz) * (r1.zzz) + (r0.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[7].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    r0.w = rsqrt(r0.w);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
