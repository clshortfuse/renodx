// Mechanically reconstructed from 0xC505E47B.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s12 : register(s12);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : TEXCOORD0;
    float4 v1 : TEXCOORD1;
    float4 v2 : TEXCOORD8;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(2.0f, -1.0f, -0.5f, -0.0f);
    const float4 c2 = float4(0.600000024f, 0.400000006f, 8.0f, 0.797884583f);
    const float4 c3 = float4(31.875f, 4.0f, -2.0f, 0.0009765625f);
    const float4 c4 = float4(0.797884583f, 1.0f, 0.125f, 0.25f);
    const float4 c12 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 r13 = 0.0f;
    float4 r14 = 0.0f;
    float4 oC0 = 0.0f;

    r1 = tex2D(s0, v0.xy);
    r0 = (r1.xyzx) * (-(c1.yyyw)) + (-(c1.wwwy));
    r8.z = (r1.w) * (v6.x) + (c1.z);
    r2 = float4(((r8.z) >= 0.0f ? (r0.x) : (c1.w)), ((r8.z) >= 0.0f ? (r0.y) : (c1.w)), ((r8.z) >= 0.0f ? (r0.z) : (c1.w)), ((r8.z) >= 0.0f ? (r0.w) : (c1.w)));
    r0 = tex2D(s3, v6.zw);
    r3 = (v5.xyzx) * (-(c1.yyyw)) + (-(c1.wwwy));
    r1.w = dot(r3, c[23]);
    r1.w = 1.0f / (r1.w);
    r1.x = dot(r3, c[20]);
    r1.y = dot(r3, c[21]);
    r1 = (r1.wwww) * (r1.xxyy);
    r4 = (r1) * (c[24].zwxy);
    r4.x = log2(abs(r4.x));
    r4.y = log2(abs(r4.y));
    r4.z = log2(abs(r4.z));
    r4.w = log2(abs(r4.w));
    r4 = (r4) * (c[25].xxxx);
    r4.x = exp2(r4.x);
    r4.y = exp2(r4.y);
    r4.z = exp2(r4.z);
    r4.w = exp2(r4.w);
    r4.xy = (r4.zw) + (r4.xy);
    r4.x = log2(abs(r4.x));
    r4.y = log2(abs(r4.y));
    r4.xy = (r4.xy) * (c[25].yy);
    r4.x = exp2(r4.x);
    r4.y = exp2(r4.y);
    r4.z = dot(r3, c[22]);
    r1.z = (r4.x) * (c[26].x);
    r1.y = (r4.y) * (c[26].y) + (-(r1.z));
    r1.z = c[26].y;
    r1.z = (r4.y) * (r1.z) + (-(c[25].z));
    r1.y = 1.0f / (r1.y);
    r4.xy = abs(r1.xw);
    r5.w = saturate((r1.z) * (r1.y));
    r3 = c[10];
    r3 = saturate((r4.zyxz) * (r3) + (c[11]));
    r5.x = (r3.w) * (r3.x);
    r5.yz = r3.yz;
    r3 = (r5) * (r5);
    r5 = (c[26].zzzz) * (r5) + (c[26].wwww);
    r3 = (r3) * (r5);
    r1.y = (r3.z) * (r3.y);
    r1.z = abs(c[25].w);
    r4.w = (r4.z) * (r4.z);
    r1.y = ((-(r1.z)) >= 0.0f ? (r1.y) : (r3.w));
    r1.z = dot(c[9].yz, r4.zw) + (c[9].x);
    r1.z = (r1.y) * (r1.z);
    r4.w = (r3.x) * (r1.z);
    r3 = c[27];
    r3.x = dot(r1.xw, r3.xy) + (c[8].x);
    r3.y = dot(r1.xw, r3.zw) + (c[8].z);
    r1 = tex2D(s2, r3.xy);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r3.xyz = (r4.www) * (r1.xyz);
    r1 = tex2D(s12, v0.zw);
    r12.xyz = (r3.xyz) * (r1.yyy);
    r1 = tex2D(s1, v0.xy);
    r3.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = tex2D(s4, v6.zw);
    r4.xy = (r1.wy) * (c0.xy) + (c0.zw);
    r1 = (c1.xxxx) * (v7) + (c1.yyyy);
    r3.xy = float2(((r8.z) >= 0.0f ? (r3.x) : (c1.w)), ((r8.z) >= 0.0f ? (r3.y) : (c1.w)));
    r1.x = dot(r4.xy, r1.xy) + (c1.w);
    r1.y = dot(r4.xy, r1.zw) + (c1.w);
    r8.w = (r0.w) * (v6.y);
    r14.xy = lerp(r3.xy, r1.xy, r8.ww);
    r7.xyz = (-(v5.xyz)) + (c[5].xyz);
    r1 = v1;
    r1.xyz = (r14.xxx) * (v4.xyz) + (r1.xyz);
    r3.w = dot(r7.xyz, r7.xyz);
    r1.xyz = (r14.yyy) * (v3.xyz) + (r1.xyz);
    r7.w = rsqrt(r3.w);
    r11.xyz = normalize(r1.xyz);
    r6.xyz = (r7.xyz) * (r7.www);
    r1.xyz = lerp(r2.xyz, r0.xyz, r8.www);
    r6.w = saturate(dot(r11.xyz, r6.xyz));
    r3 = tex2D(s5, v0.xy);
    r5 = float4(((r8.z) >= 0.0f ? (r3.x) : (-(c1.w))), ((r8.z) >= 0.0f ? (r3.y) : (-(c1.w))), ((r8.z) >= 0.0f ? (r3.z) : (-(c1.w))), ((r8.z) >= 0.0f ? (r3.w) : (-(c1.y))));
    r0.x = c1.w;
    r0.xyz = float3(((r8.z) >= 0.0f ? (c[28].x) : (r0.x)), ((r8.z) >= 0.0f ? (c[28].y) : (r0.x)), ((r8.z) >= 0.0f ? (c[28].w) : (r0.x)));
    r4 = tex2D(s6, v6.zw);
    r3 = lerp(r5, r4, r8.wwww);
    r10.xyz = lerp(r0.xyz, c[29].xyw, r8.www);
    r4.z = (r3.w) * (c2.w);
    r0.xyz = normalize(v5.xyz);
    r2.y = (r3.w) * (-(c4.x)) + (c4.y);
    r2.x = saturate(dot(r11.xyz, -(r0.xyz)));
    r2.z = (r6.w) * (r2.y) + (r4.z);
    r2.y = (r2.x) * (r2.y) + (r4.z);
    r8.w = (-(r2.x)) + (-(c1.y));
    r2.z = (r2.z) * (r2.y) + (c3.w);
    r2.z = 1.0f / (r2.z);
    r4.xyz = (r7.xyz) * (r7.www) + (-(r0.xyz));
    r4.w = (r6.w) * (r2.z);
    r2.xyz = normalize(r4.xyz);
    r13.xyz = (r6.www) * (c[6].xyz);
    r5.w = saturate(dot(r11.xyz, r2.xyz));
    r2.z = saturate(dot(r2.xyz, r6.xyz));
    r4.xy = (r3.ww) * (c12.xy) + (c12.zw);
    r2.y = exp2(r4.y);
    r2.z = (-(r2.z)) + (-(c1.y));
    r4.z = pow(abs(r5.w), r2.y);
    r2.x = (r2.z) * (r2.z);
    r2.y = (r2.y) * (c4.z) + (c4.w);
    r2.x = (r2.x) * (r2.x);
    r2.y = (r4.z) * (r2.y);
    r2.z = (r2.z) * (r2.x);
    r9.xyz = (r3.xyz) * (r3.xyz);
    r8.xyz = (r3.xyz) * (-(r3.xyz)) + (-(c1.yyy));
    r3.z = (r4.w) * (r2.y);
    r2.xyz = (r8.xyz) * (r2.zzz) + (r9.xyz);
    r7.w = 1.0f / (r4.x);
    r2.xyz = (r3.zzz) * (r2.xyz);
    r3.w = (r3.w) * (c2.z);
    r2.xyz = (r10.zzz) * (r2.xyz);
    r2.xyz = (r2.xyz) * (c[7].xyz);
    r3.xy = (v0.zw) * (-(c1.yz));
    r4 = tex2D(s13, r3.xy);
    r3.xy = (v0.zw) * (-(c1.yz)) + (-(c1.wz));
    r6 = tex2D(s13, r3.xy);
    r4.w = r6.y;
    r7.xyz = (r12.xyz) * (r2.xyz);
    r2.xy = (r4.yw) * (c3.yy) + (c3.zz);
    r3.z = dot(r14.xy, r14.xy) + (c1.w);
    r2.z = dot(r2.xy, r2.xy) + (c1.w);
    r3.z = exp2(-(r3.z));
    r2.z = exp2(-(r2.z));
    r4.w = (r3.z) * (c2.x) + (c2.y);
    r3.z = (r2.z) * (c2.x) + (c2.y);
    r2.z = dot(r2.xy, r14.xy) + (c1.w);
    r2.y = (r4.w) * (r3.z);
    r5 = tex2D(s14, v0.zw);
    r14.xy = (r5.xy) * (c3.xx);
    r3.z = saturate((r2.z) * (r2.y) + (r2.y));
    r2.xy = (r6.xz) * (r14.yy);
    r3.y = (r5.y) * (c3.x) + (-(r2.x));
    r2.xz = (r2.xy) * (c3.yy);
    r2.y = (r6.z) * (-(r14.y)) + (r3.y);
    r4.xy = (r4.xz) * (r14.xx);
    r2.y = (r2.y) + (r2.y);
    r5.w = (r5.x) * (c3.x) + (-(r4.x));
    r3.xyz = (r3.zzz) * (r2.xyz);
    r2.y = (r4.z) * (-(r14.x)) + (r5.w);
    r2.xz = (r4.xy) * (c3.yy);
    r2.y = (r2.y) + (r2.y);
    r3.xyz = (r2.xyz) * (r4.www) + (r3.xyz);
    r4.w = dot(r0.xyz, r11.xyz);
    r3.xyz = (r10.yyy) * (r3.xyz);
    r4.w = (r4.w) + (r4.w);
    r4.xyz = (r12.xyz) * (r13.xyz) + (r3.xyz);
    r3.xyz = (r11.xyz) * (-(r4.www)) + (r0.xyz);
    r3 = texCUBElod(s15, r3);
    r0.z = (r8.w) * (r8.w);
    r3.w = (r8.w) * (r0.z);
    r0.xyz = (r3.xyz) * (r3.xyz);
    r3.w = (r7.w) * (r3.w);
    r0.xyz = (r10.xxx) * (r0.xyz);
    r3.xyz = (r8.xyz) * (r3.www) + (r9.xyz);
    r1.xyz = (r1.xyz) * (r1.xyz);
    r0.xyz = (r0.xyz) * (r3.xyz);
    r1.xyz = (r1.xyz) * (r4.xyz) + (r7.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c2.zzz) + (r1.xyz);
    r0.xyz = (r0.xyz) + (-(v2.xyz));
    r1.z = (-(r2.w)) + (-(c1.y));
    r0.xyz = (r1.www) * (r0.xyz) + (v2.xyz);
    r0.w = (r0.w) * (-(v6.y)) + (-(c1.y));
    r0.xyz = max(((r0.xyz) * (c[31].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = (r1.z) * (-(r0.w)) + (-(c1.y));
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.w = (r0.w) * (c[30].w);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);

    return oC0;
}
