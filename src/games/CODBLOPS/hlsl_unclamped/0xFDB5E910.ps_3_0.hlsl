// Mechanically reconstructed from 0xFDB5E910.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler2D s5 : register(s5);
sampler2D s6 : register(s6);
sampler2D s7 : register(s7);
sampler2D s8 : register(s8);
sampler2D s13 : register(s13);
sampler2D s14 : register(s14);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD2;
    float4 v4 : TEXCOORD3;
    float4 v5 : TEXCOORD5;
    float4 v6 : TEXCOORD6;
    float4 v7 : TEXCOORD7;
    float4 v8 : COLOR1;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    float4 v8 = input.v8;
    const float4 c1 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c2 = float4(2.0f, -1.0f, 0.0f, 1.0f);
    const float4 c3 = float4(0.200000003f, 0.600000024f, 0.400000006f, 8.0f);
    const float4 c4 = float4(3.5f, 1.0f, 0.5f, 0.0f);
    const float4 c10 = float4(31.875f, 4.0f, -2.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 oC0 = 0.0f;

    r0.xy = (v1.zw) * (c4.yz);
    r1 = tex2D(s13, r0.xy);
    r0.xy = (v1.zw) * (c4.yz) + (c4.wz);
    r4 = tex2D(s13, r0.xy);
    r0 = tex2D(s1, v1.xy);
    r3.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = tex2D(s5, v6.xy);
    r2.xy = (r0.wy) * (c1.xy) + (c1.zw);
    r0 = (c2.xxxx) * (v8) + (c2.yyyy);
    r0.x = dot(r2.xy, r0.xy) + (c2.z);
    r0.y = dot(r2.xy, r0.zw) + (c2.z);
    r2 = tex2D(s2, v6.xy);
    r6.w = (r2.w) * (v0.y);
    r1.w = r4.y;
    r5.xy = lerp(r3.xy, r0.xy, r6.ww);
    r0 = tex2D(s4, v7.xy);
    r2.w = (r0.w) * (v0.w);
    r3.xy = (r1.yw) * (c10.yy) + (c10.zz);
    r6.xy = (r2.ww) * (-(r5.xy)) + (r5.xy);
    r1.w = dot(r3.xy, r3.xy) + (c2.z);
    r0.w = dot(r6.xy, r6.xy) + (c2.z);
    r1.w = exp2(-(r1.w));
    r0.w = exp2(-(r0.w));
    r1.w = (r1.w) * (c3.y) + (c3.z);
    r0.w = (r0.w) * (c3.y) + (c3.z);
    r1.y = (r1.w) * (r0.w);
    r1.w = dot(r3.xy, r6.xy) + (c2.z);
    r3 = tex2D(s14, v1.zw);
    r7.xy = (r3.xy) * (c10.xx);
    r3.w = saturate((r1.w) * (r1.y) + (r1.y));
    r4.xy = (r4.xz) * (r7.yy);
    r1.w = (r3.y) * (c10.x) + (-(r4.x));
    r5.xz = (r4.xy) * (c10.yy);
    r1.w = (r4.z) * (-(r7.y)) + (r1.w);
    r1.xy = (r1.xz) * (r7.xx);
    r5.y = (r1.w) + (r1.w);
    r1.w = (r3.x) * (c10.x) + (-(r1.x));
    r3.xyz = (r3.www) * (r5.xyz);
    r1.w = (r1.z) * (-(r7.x)) + (r1.w);
    r8.xz = (r1.xy) * (c10.yy);
    r8.y = (r1.w) + (r1.w);
    r4.xyz = (r8.xyz) * (r0.www) + (r3.xyz);
    r3 = tex2D(s6, v1.xy);
    r1 = tex2D(s7, v6.xy);
    r5.xyz = v2.xyz;
    r5.xyz = (r6.xxx) * (v4.xyz) + (r5.xyz);
    r4.w = lerp(r3.y, r1.y, r6.w);
    r1.xyz = (r6.yyy) * (v3.xyz) + (r5.xyz);
    r10.xyz = normalize(r1.xyz);
    r9.xyz = normalize(v5.xyz);
    r5.w = lerp(r3.w, r1.w, r6.w);
    r0.w = dot(r9.xyz, r10.xyz);
    r11.xy = lerp(r4.ww, c[8].xy, r2.ww);
    r0.w = (r0.w) + (r0.w);
    r6.xyz = (r4.xyz) * (r11.yyy);
    r3.xyz = (r10.xyz) * (-(r0.www)) + (r9.xyz);
    r4 = tex2D(s8, v7.xy);
    r1 = tex2D(s3, v6.zw);
    r1.xyz = (r1.xyz) + (c2.yyy);
    r7.xyz = (v0.zzz) * (r1.xyz) + (c2.www);
    r5.xyz = (r7.xyz) * (c3.xxx);
    r7.w = saturate(dot(r10.xyz, -(r9.xyz)));
    r1 = lerp(r5, r4, r2.wwww);
    r0.w = dot(c[5].xyz, r9.xyz);
    r3.w = (r1.w) * (c3.w);
    r3 = texCUBElod(s15, r3);
    r4.xyz = (r3.xyz) * (r3.xyz);
    r3.z = (-(r7.w)) + (c2.w);
    r3.y = (r3.z) * (r3.z);
    r3.w = (r1.w) * (c4.x) + (c4.y);
    r1.w = (r3.z) * (r3.y);
    r3.w = 1.0f / (r3.w);
    r1.w = (r1.w) * (r3.w);
    r3.xyz = (r1.xyz) * (-(r1.xyz)) + (c2.www);
    r5.xyz = (r11.xxx) * (r4.xyz);
    r9.xyz = (r1.www) * (r3.xyz);
    r3 = tex2D(s0, v1.xy);
    r4.xyz = lerp(r3.xyz, r2.xyz, r6.www);
    r1.xyz = (r1.xyz) * (r1.xyz) + (r9.xyz);
    r0.xyz = (r4.xyz) * (-(r7.xyz)) + (r0.xyz);
    r1.xyz = (r5.xyz) * (r1.xyz);
    r0.xyz = (r2.www) * (r0.xyz);
    r1.xyz = (r8.xyz) * (r1.xyz);
    r0.xyz = (r4.xyz) * (r7.xyz) + (r0.xyz);
    r2.xyz = (r1.xyz) * (c3.www);
    r1.xyz = (r0.xyz) * (r0.xyz);
    r0.w = saturate((c[7].y) * (r0.w) + (c[7].x));
    r0.xyz = c[0].xyz;
    r0.xyz = (-(r0.xyz)) + (c[6].xyz);
    r1.xyz = (r1.xyz) * (r6.xyz) + (r2.xyz);
    r0.xyz = (r0.www) * (r0.xyz) + (c[0].xyz);
    r1.xyz = (r0.xyz) * (-(c[0].www)) + (r1.xyz);
    r1.xyz = (r1.xyz) * (v2.www);
    r0.xyz = (r0.xyz) * (c[0].www) + (r1.xyz);
    r0.xyz = max(((r0.xyz) * (c[9].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c2.w;

    return oC0;
}
