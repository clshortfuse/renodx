// Mechanically reconstructed from 0xC3EBE1F5.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(8.0f, 31.875f, 1.0f, 0.797884583f);
    const float4 c1 = float4(0.0009765625f, 0.125f, 0.25f, 0.0f);
    const float4 c2 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex2D(s0, v1.xy);
    r0.w = dot(v4.xyz, v4.xyz);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r2.y = rsqrt(r0.w);
    r8.xyz = (r2.yyy) * (v4.xyz);
    r1.xyz = normalize(v2.xyz);
    r3.xyz = (r0.xyz) * (r0.xyz);
    r2.w = saturate(dot(r1.xyz, -(r8.xyz)));
    r0 = tex2D(s1, v1.xy);
    r2.x = (r0.w) * (c0.w);
    r1.w = (r0.w) * (-(c0.w)) + (c0.z);
    r5.w = saturate(dot(r1.xyz, c[17].xyz));
    r2.z = (r2.w) * (r1.w) + (r2.x);
    r1.w = (r5.w) * (r1.w) + (r2.x);
    r4.xyz = (v4.xyz) * (-(r2.yyy)) + (c[17].xyz);
    r1.w = (r1.w) * (r2.z) + (c1.x);
    r3.w = (-(r2.w)) + (c0.z);
    r1.w = 1.0f / (r1.w);
    r6.w = (r5.w) * (r1.w);
    r2.xyz = normalize(r4.xyz);
    r4.z = saturate(dot(r1.xyz, r2.xyz));
    r4.xy = (r0.ww) * (c2.xy) + (c2.zw);
    r1.w = saturate(dot(r2.xyz, c[17].xyz));
    r4.w = exp2(r4.y);
    r2.w = 1.0f / (r4.x);
    r1.w = (-(r1.w)) + (c0.z);
    r2.x = pow(abs(r4.z), r4.w);
    r2.z = (r1.w) * (r1.w);
    r2.y = (r4.w) * (c1.y) + (c1.z);
    r2.z = (r2.z) * (r2.z);
    r4.w = (r2.x) * (r2.y);
    r1.w = (r1.w) * (r2.z);
    r5.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r0.xyz) * (-(r0.xyz)) + (c0.zzz);
    r4.w = (r6.w) * (r4.w);
    r0.xyz = (r2.xyz) * (r1.www) + (r5.xyz);
    r1.w = (r0.w) * (c0.x);
    r0.xyz = (r4.www) * (r0.xyz);
    r4.xyz = (r0.xyz) * (c[7].www);
    r4.w = max(abs(r1.y), abs(r1.z));
    r9.xy = c[6].xy;
    r6.xyz = (r9.yyy) * (c[19].xyz);
    r0.w = max(abs(r1.x), r4.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r1.xyz) * (c[5].xyz);
    r4.xyz = (r4.xyz) * (r6.xyz);
    r0.xyz = (r0.xyz) * (r0.www) + (v5.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r4.xyz = (r4.xyz) * (r0.www);
    r0.xyz = (r0.xyz) * (c[7].yyy);
    r7.xyz = (r0.xyz) * (c0.yyy);
    r4.w = dot(r8.xyz, r1.xyz);
    r0.xyz = (r9.xxx) * (c[18].xyz);
    r4.w = (r4.w) + (r4.w);
    r6.xyz = (r5.www) * (r0.xyz);
    r1.xyz = (r1.xyz) * (-(r4.www)) + (r8.xyz);
    r1 = texCUBElod(s15, r1);
    r0.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r0.www) * (r6.xyz) + (r7.xyz);
    r6.xyz = (r0.xyz) * (c0.xxx);
    r0 = tex3D(s11, v5.xyz);
    r0.w = (r3.w) * (r3.w);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.w = (r3.w) * (r0.w);
    r0.xyz = (r6.xyz) * (r0.xyz);
    r0.w = (r2.w) * (r0.w);
    r0.xyz = (r0.xyz) * (c0.yyy);
    r2.xyz = (r2.xyz) * (r0.www) + (r5.xyz);
    r1.xyz = (r3.xyz) * (r1.xyz) + (r4.xyz);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r0.xyz = (r0.xyz) * (c[7].xxx) + (r1.xyz);
    r0.w = c0.z;
    r1.x = dot(r0, c[9]);
    r1.y = dot(r0, c[10]);
    r1.z = dot(r0, c[11]);
    r0.xyz = (r1.xyz) + (-(v3.xyz));
    r0.w = v2.w;
    r0.xyz = (r0.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[8].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = c0.z;

    return oC0;
}
