// Mechanically reconstructed from 0x0CE70A79.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler2D s3 : register(s3);
sampler2D s4 : register(s4);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD2;
    float4 v5 : TEXCOORD3;
    float4 v6 : TEXCOORD5;
    float4 v7 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    float4 v7 = input.v7;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(-0.5f, 1.0f, 0.0f, 0.200000003f);
    const float4 c2 = float4(8.0f, 31.875f, 0.797884583f, 1.0f);
    const float4 c3 = float4(3.5f, -13.0f, 1.0f, 13.0f);
    const float4 c4 = float4(0.125f, 0.25f, 0.0f, 0.0f);
    const float4 c12 = float4(-2.0f, 3.0f, 0.0009765625f, 0.25f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 r8 = 0.0f;
    float4 r9 = 0.0f;
    float4 r10 = 0.0f;
    float4 r11 = 0.0f;
    float4 r12 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = (v6.yyyy) * (c[26]);
    r0 = (v6.xxxx) * (c[25]) + (r0);
    r0 = (v6.zzzz) * (c[27]) + (r0);
    r0 = (r0) + (c[28]);
    r3.xy = (r0.ww) * (c[34].xy) + (r0.xy);
    r3.zw = r0.zw;
    r1 = tex2Dproj(s2, r3);
    r2.zw = r3.zw;
    r0.zw = r2.zw;
    r4.xy = (r3.ww) * (-(c[34].zw)) + (r0.xy);
    r4.zw = r0.zw;
    r4 = tex2Dproj(s2, r4);
    r1.w = r4.x;
    r2.xy = (r3.ww) * (-(c[34].xy)) + (r0.xy);
    r0.xy = (r3.ww) * (c[34].zw) + (r0.xy);
    r2 = tex2Dproj(s2, r2);
    r1.y = r2.x;
    r2 = tex2Dproj(s2, r0);
    r0 = (v6.xyzx) * (c1.yyyz) + (c1.zzzy);
    r2.w = dot(r0, c[22]);
    r1.z = r2.x;
    r2.w = 1.0f / (r2.w);
    r2.x = dot(r0, c[11]);
    r2.y = dot(r0, c[20]);
    r4.w = dot(r1, c12.wwww);
    r2.xy = (r2.ww) * (r2.xy);
    r1.x = dot(r0, c[21]);
    r0.xy = (r2.xy) * (-(c1.xx)) + (-(c1.xx));
    r0 = tex2D(s3, r0.xy);
    r1.y = (r1.x) * (r1.x);
    r0.w = dot(c[9].yz, r1.xy) + (c[9].x);
    r2.xy = saturate((r1.xx) * (c[10].xy) + (c[10].zw));
    r1.w = saturate(1.0f / (r0.w));
    r0.w = ((-abs(r0.w)) >= 0.0f ? (c1.z) : (r1.w));
    r7.xyz = (-(v6.xyz)) + (c[6].xyz);
    r1.xy = (r2.xy) * (r2.xy);
    r1.w = dot(r7.xyz, r7.xyz);
    r2.xy = (r2.xy) * (c12.xx) + (c12.yy);
    r12.z = rsqrt(r1.w);
    r1.z = (r1.x) * (r2.x);
    r4.xyz = (r7.xyz) * (r12.zzz);
    r1.y = (r1.y) * (-(r2.y)) + (c1.y);
    r1.w = dot(r4.xyz, c[23].xyz);
    r0.w = (r0.w) * (r1.z);
    r1.z = saturate((r1.w) * (c[24].x) + (c[24].y));
    r1.w = (r1.z) * (r1.z);
    r1.z = (r1.z) * (c12.x) + (c12.y);
    r0.w = (r1.y) * (r0.w);
    r1.w = (r1.w) * (r1.z);
    r2.xyz = (r0.xyz) * (r0.xyz);
    r2.w = (r0.w) * (r1.w);
    r0 = tex2D(s1, v1.xy);
    r1.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r3 = tex2D(s4, v1.xy);
    r1.zw = (r3.ww) * (c1.zy) + (c1.wz);
    r0 = tex2D(s0, v1.xy);
    r3.w = (r0.w) * (v0.w) + (c1.x);
    r9.xyz = (r2.xyz) * (r2.www);
    r2 = float4(((r3.w) >= 0.0f ? (r1.x) : (c1.z)), ((r3.w) >= 0.0f ? (r1.y) : (c1.z)), ((r3.w) >= 0.0f ? (r1.z) : (c1.z)), ((r3.w) >= 0.0f ? (r1.w) : (c1.y)));
    r10.w = (r2.w) * (c2.z);
    r5.xyz = (-(v6.xyz)) + (c[29].xyz);
    r9.w = (r2.w) * (-(c2.z)) + (c2.w);
    r12.y = dot(r5.xyz, r5.xyz);
    r3.x = rsqrt(r12.y);
    r1 = v2;
    r1.xyz = (r2.xxx) * (v5.xyz) + (r1.xyz);
    r10.xyz = (r5.xyz) * (r3.xxx);
    r1.xyz = (r2.yyy) * (v4.xyz) + (r1.xyz);
    r6.xyz = normalize(r1.xyz);
    r8.xyz = normalize(v6.xyz);
    r2.y = saturate(dot(r6.xyz, r10.xyz));
    r3.z = saturate(dot(r6.xyz, -(r8.xyz)));
    r0.w = (r2.y) * (r9.w) + (r10.w);
    r8.w = (r3.z) * (r9.w) + (r10.w);
    r1.xyz = (r4.www) * (r9.xyz);
    r2.x = (r0.w) * (r8.w) + (c12.z);
    r0 = (r0.xyzx) * (c1.yyyz) + (c1.zzzy);
    r2.x = 1.0f / (r2.x);
    r12.w = (-(r3.z)) + (c1.y);
    r3.z = (r2.y) * (r2.x);
    r9.xyz = (r2.yyy) * (c[30].xyz);
    r2.xy = (r2.ww) * (c3.xy) + (c3.zw);
    r11.w = exp2(r2.y);
    r11.xyz = (r5.xyz) * (r3.xxx) + (-(r8.xyz));
    r6.w = 1.0f / (r2.x);
    r5.xyz = normalize(r11.xyz);
    r12.x = 1.0f / (r3.x);
    r2.y = saturate(dot(r5.xyz, r10.xyz));
    r3.x = saturate(dot(r6.xyz, r5.xyz));
    r2.y = (-(r2.y)) + (c1.y);
    r2.x = pow(abs(r3.x), r11.w);
    r3.x = (r2.y) * (r2.y);
    r10.z = (r11.w) * (c4.x) + (c4.y);
    r3.x = (r3.x) * (r3.x);
    r2.x = (r2.x) * (r10.z);
    r2.y = (r2.y) * (r3.x);
    r7.w = (r2.z) * (r2.z);
    r5.w = (r2.z) * (-(r2.z)) + (c1.y);
    r2.z = (r3.z) * (r2.x);
    r2.y = (r5.w) * (r2.y) + (r7.w);
    r2.z = (r2.z) * (r2.y);
    r4.w = ((r3.w) >= 0.0f ? (r3.y) : (c1.z));
    r2.w = (r2.w) * (c2.x);
    r2.z = (r2.z) * (r4.w);
    r0 = float4(((r3.w) >= 0.0f ? (r0.x) : (c1.z)), ((r3.w) >= 0.0f ? (r0.y) : (c1.z)), ((r3.w) >= 0.0f ? (r0.z) : (c1.z)), ((r3.w) >= 0.0f ? (r0.w) : (c1.z)));
    r5.xyz = (r2.zzz) * (c[31].xyz);
    r2.z = dot(c[32].yz, r12.xy) + (c[32].x);
    r3.xy = saturate((r12.xx) * (c[33].xy) + (c[33].zw));
    r2.xy = (r3.xy) * (r3.xy);
    r3.xy = (r3.xy) * (c12.xx) + (c12.yy);
    r3.xy = (r2.xy) * (r3.xy);
    r3.w = max(abs(r6.y), abs(r6.z));
    r3.z = (r2.z) * (r3.x);
    r2.z = max(abs(r6.x), r3.w);
    r3.w = 1.0f / (r2.z);
    r2.xyz = (r6.xyz) * (c[5].xyz);
    r10.y = (r3.y) * (r3.z);
    r2.xyz = (r2.xyz) * (r3.www) + (v7.xyz);
    r3 = tex3D(s11, r2.xyz);
    r2.xyz = (r3.xyz) * (r3.xyz);
    r3.w = (r10.y) * (r3.w);
    r2.xyz = (r4.www) * (r2.xyz);
    r5.xyz = (r5.xyz) * (r3.www);
    r2.xyz = (r2.xyz) * (c2.yyy);
    r3.xyz = (r3.www) * (r9.xyz) + (r2.xyz);
    r2.z = dot(r8.xyz, r6.xyz);
    r0.xyz = (r0.xyz) * (v0.xyz);
    r2.z = (r2.z) + (r2.z);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r6.xyz) * (-(r2.zzz)) + (r8.xyz);
    r2 = texCUBElod(s15, r2);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r5.xyz = (r0.xyz) * (r3.xyz) + (r5.xyz);
    r3.xyz = (r2.xyz) * (c2.xxx);
    r2 = tex3D(s11, v7.xyz);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r7.xyz = (r7.xyz) * (r12.zzz) + (-(r8.xyz));
    r2.xyz = (r3.xyz) * (r2.xyz);
    r2.xyz = (r2.xyz) * (c2.yyy);
    r2.w = (r12.w) * (r12.w);
    r2.w = (r12.w) * (r2.w);
    r3.xyz = normalize(r7.xyz);
    r6.w = (r6.w) * (r2.w);
    r2.w = saturate(dot(r6.xyz, r3.xyz));
    r3.z = saturate(dot(r3.xyz, r4.xyz));
    r3.w = pow(abs(r2.w), r11.w);
    r2.w = saturate(dot(r4.xyz, r6.xyz));
    r3.w = (r10.z) * (r3.w);
    r3.y = (r2.w) * (r9.w) + (r10.w);
    r3.z = (-(r3.z)) + (c1.y);
    r3.y = (r3.y) * (r8.w) + (c12.z);
    r3.x = (r3.z) * (r3.z);
    r3.y = 1.0f / (r3.y);
    r3.x = (r3.x) * (r3.x);
    r3.y = (r2.w) * (r3.y);
    r3.z = (r3.z) * (r3.x);
    r3.w = (r3.w) * (r3.y);
    r3.y = (r5.w) * (r3.z) + (r7.w);
    r3.z = (r5.w) * (r6.w) + (r7.w);
    r3.w = (r3.w) * (r3.y);
    r2.xyz = (r2.xyz) * (r3.zzz);
    r4.xyz = (r3.www) * (c[8].xyz);
    r3.xyz = (r2.www) * (c[7].xyz);
    r4.xyz = (r4.www) * (r4.xyz);
    r2.xyz = (r2.xyz) * (r4.www) + (r5.xyz);
    r0.xyz = (r0.xyz) * (r3.xyz) + (r4.xyz);
    r0.xyz = (r1.xyz) * (r0.xyz) + (r2.xyz);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r0.xyz = (r1.www) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[35].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
