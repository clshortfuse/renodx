// Mechanically reconstructed from 0xF7926D86.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler3D s11 : register(s11);
samplerCUBE s15 : register(s15);

struct PS_INPUT
{
    float4 v0 : COLOR0;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD1;
    float4 v3 : TEXCOORD8;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    const float4 c0 = float4(-0.5f, 1.0f, 0.0f, 8.0f);
    const float4 c1 = float4(31.875f, 3.5f, 1.0f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 oC0 = 0.0f;

    r0 = tex3D(s11, v5.xyz);
    r3.xyz = normalize(v4.xyz);
    r4.xyz = normalize(v2.xyz);
    r0.w = dot(r3.xyz, r4.xyz);
    r0.w = (r0.w) + (r0.w);
    r5.xyz = (r0.xyz) * (r0.xyz);
    r2.xyz = (r4.xyz) * (-(r0.www)) + (r3.xyz);
    r1 = tex2D(s1, v1.xy);
    r0 = tex2D(s0, v1.xy);
    r3.w = (r0.w) * (v0.w) + (c0.x);
    r3.z = saturate(dot(r4.xyz, -(r3.xyz)));
    r1 = float4(((r3.w) >= 0.0f ? (r1.x) : (c0.z)), ((r3.w) >= 0.0f ? (r1.y) : (c0.z)), ((r3.w) >= 0.0f ? (r1.z) : (c0.z)), ((r3.w) >= 0.0f ? (r1.w) : (c0.y)));
    r0 = (r0.xyzx) * (c0.yyyz) + (c0.zzzy);
    r2.w = (r1.w) * (c0.w);
    r2 = texCUBElod(s15, r2);
    r2.xyz = (r2.xyz) * (r2.xyz);
    r2.xyz = (r2.xyz) * (c0.www);
    r3.z = (-(r3.z)) + (c0.y);
    r3.y = (r3.z) * (r3.z);
    r2.w = (r1.w) * (c1.y) + (c1.z);
    r1.w = (r3.z) * (r3.y);
    r2.w = 1.0f / (r2.w);
    r1.w = (r1.w) * (r2.w);
    r3.xyz = (r1.xyz) * (-(r1.xyz)) + (c0.yyy);
    r2.xyz = (r5.xyz) * (r2.xyz);
    r3.xyz = (r1.www) * (r3.xyz);
    r2.xyz = (r2.xyz) * (c1.xxx);
    r1.xyz = (r1.xyz) * (r1.xyz) + (r3.xyz);
    r3.xyz = (r2.xyz) * (r1.xyz);
    r2.w = max(abs(r4.y), abs(r4.z));
    r1.y = c0.z;
    r5.xy = float2(((r3.w) >= 0.0f ? (c[6].x) : (r1.y)), ((r3.w) >= 0.0f ? (c[6].y) : (r1.y)));
    r1.w = max(abs(r4.x), r2.w);
    r1.xyz = (r4.xyz) * (c[5].xyz);
    r1.w = 1.0f / (r1.w);
    r0 = float4(((r3.w) >= 0.0f ? (r0.x) : (c0.z)), ((r3.w) >= 0.0f ? (r0.y) : (c0.z)), ((r3.w) >= 0.0f ? (r0.z) : (c0.z)), ((r3.w) >= 0.0f ? (r0.w) : (c0.z)));
    r1.xyz = (r1.xyz) * (r1.www) + (v5.xyz);
    r1 = tex3D(s11, r1.xyz);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r0.xyz) * (v0.xyz);
    r0.xyz = (r5.yyy) * (r2.xyz);
    r2.xyz = (r1.xyz) * (r1.xyz);
    r1.xyz = (r3.xyz) * (r5.xxx);
    r0.xyz = (r0.xyz) * (r2.xyz);
    r1.xyz = (r0.xyz) * (c1.xxx) + (r1.xyz);
    r1.w = c0.y;
    r0.x = dot(r1, c[8]);
    r0.y = dot(r1, c[9]);
    r0.z = dot(r1, c[10]);
    r0.xyz = (r0.xyz) + (-(v3.xyz));
    r1.x = v2.w;
    r0.xyz = (r1.xxx) * (r0.xyz) + (v3.xyz);
    r0.xyz = max(((r0.xyz) * (c[7].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.w = rsqrt(r0.w);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = 1.0f / (r0.w);

    return oC0;
}
