// Mechanically reconstructed from 0xA1EB5A44.ps_3_0.cso.
// HDR edit: terminal RGB upper clamp removed; zero floor retained.
// Original terminal path: saturate(linear RGB) -> sqrt encode.
// Modified terminal path: max(linear RGB, 0) -> sqrt encode.
// Intermediate clamps, alpha clamps, masks, depth, and control flow are unchanged.
// Entry point: main    Target: ps_3_0

float4 c[224] : register(c0);
sampler2D s0 : register(s0);
sampler2D s1 : register(s1);
sampler2D s2 : register(s2);
sampler3D s11 : register(s11);

struct PS_INPUT
{
    float4 v0 : TEXCOORD1;
    float4 v1 : TEXCOORD0;
    float4 v2 : TEXCOORD2;
    float4 v3 : TEXCOORD4;
    float4 v4 : TEXCOORD5;
    float4 v5 : TEXCOORD6;
    float4 v6 : TEXCOORD7;
};

float4 main(PS_INPUT input) : COLOR0
{
    float4 v0 = input.v0;
    float4 v1 = input.v1;
    float4 v2 = input.v2;
    float4 v3 = input.v3;
    float4 v4 = input.v4;
    float4 v5 = input.v5;
    float4 v6 = input.v6;
    const float4 c0 = float4(4.07999992f, 4.06451607f, -2.07999992f, -2.06451607f);
    const float4 c1 = float4(0.5f, 0.449999988f, 0.330000013f, 0.0900000036f);
    const float4 c3 = float4(0.00048828125f, -0.000122070312f, 0.0f, 0.25f);
    const float4 c4 = float4(0.75f, 4.0f, -3.0f, -4.0f);
    const float4 c10 = float4(1.0f, 0.0f, 0.000244140625f, -0.000244140625f);
    const float4 c11 = float4(31.875f, 9.99999975e-05f, 0.100000001f, 0.0f);
    float4 r0 = 0.0f;
    float4 r1 = 0.0f;
    float4 r2 = 0.0f;
    float4 r3 = 0.0f;
    float4 r4 = 0.0f;
    float4 r5 = 0.0f;
    float4 r6 = 0.0f;
    float4 r7 = 0.0f;
    float4 oC0 = 0.0f;

    r6.xy = (v6.xy) * (c[7].xy);
    r0 = tex2D(s1, r6.xy);
    r2.xy = (r0.wy) * (c0.xy) + (c0.zw);
    r0.xyz = v4.xyz;
    r1.xyz = (r0.zxy) * (v2.yzx);
    r2.xy = (r2.xy) * (c[8].xx);
    r0.xyz = (r0.yzx) * (v2.zxy) + (-(r1.xyz));
    r0.xyz = (r2.yyy) * (-(r0.xyz));
    r0.xyz = (r2.xxx) * (v2.xyz) + (r0.xyz);
    r5.xyz = normalize(-(v3.xyz));
    r1.xyz = (r0.xyz) + (v4.xyz);
    r0.xyz = normalize(r1.xyz);
    r1.xyz = normalize(c[17].xyz);
    r1.w = saturate(dot(r0.xyz, r1.xyz));
    r1.z = dot(r1.xyz, r5.xyz);
    r0.w = (c[9].x) * (c[9].x);
    r1.y = c[9].x;
    r2.xy = (r1.yy) * (r1.yy) + (c1.zw);
    r1.xy = (r0.ww) * (c1.xy);
    r2.x = 1.0f / (r2.x);
    r2.y = 1.0f / (r2.y);
    r6.z = (r1.y) * (r2.y);
    r0.w = saturate(dot(r0.xyz, r5.xyz));
    r1.z = saturate((r1.w) * (-(r0.w)) + (r1.z));
    r1.y = 1.0f / (r0.w);
    r1.z = (r6.z) * (r1.z);
    r1.y = saturate((r1.w) * (r1.y));
    r7.w = (r1.x) * (-(r2.x)) + (c10.x);
    r1.z = (r1.z) * (r1.y);
    r6.w = (r7.w) * (r1.w) + (r1.z);
    r1.z = (r0.w) * (-(r0.w)) + (c10.x);
    r2.w = max(abs(r0.y), abs(r0.z));
    r1.w = (r0.w) * (r0.w);
    r1.y = max(abs(r0.x), r2.w);
    r0.w = 1.0f / (r1.y);
    r0.xyz = (r0.xyz) * (c[5].xyz);
    r5.w = ((-(r1.w)) >= 0.0f ? (c10.x) : (r1.z));
    r0.xyz = (r0.xyz) * (r0.www) + (v1.xyz);
    r0 = tex3D(s11, r0.xyz);
    if ((c10.x) >= (v0.w))
    {
        r2 = (v0.xyzx) * (c10.xxxy);
        r1 = (r2) + (-(c3.xyzz));
        r1 = tex2Dlod(s0, r1);
        r1.w = r1.x;
        r3 = (r2) + (c10.zzyy);
        r3 = tex2Dlod(s0, r3);
        r1.x = r3.x;
        r3 = (r2) + (c10.wwyy);
        r3 = tex2Dlod(s0, r3);
        r1.y = r3.x;
        r2 = (r2) + (c3.xyzz);
        r2 = tex2Dlod(s0, r2);
        r1.z = r2.x;
        r0.w = dot(r1, c3.wwww);
        if ((c4.x) < (v0.w))
        {
            r7.xy = (v0.xy) * (c[2].ww) + (c[2].xy);
            r1.xy = (r7.xy) + (c10.zz);
            r1.zw = (v0.zx) * (c10.xy);
            r1 = tex2Dlod(s0, r1);
            r2.xy = (r7.xy) + (c10.ww);
            r2.zw = (v0.zx) * (c10.xy);
            r4 = tex2Dlod(s0, r2);
            r2.xy = (r7.xy) + (c3.xy);
            r2.zw = (v0.zx) * (c10.xy);
            r3 = tex2Dlod(s0, r2);
            r2.xy = (r7.xy) + (-(c3.xy));
            r2.zw = (v0.zx) * (c10.xy);
            r2 = tex2Dlod(s0, r2);
            r1.y = r4.x;
            r1.z = r3.x;
            r1.w = r2.x;
            r1.w = dot(r1, c3.wwww);
            r1.z = (-(r0.w)) + (r1.w);
            r1.w = (v0.w) * (c4.y) + (c4.z);
            r1.w = (r1.w) * (r1.z) + (r0.w);
        }
        else
        {
            r1.w = r0.w;
        }
    }
    else
    {
        r1.w = (v0.w) + (c4.w);
        r1.w = ((r1.w) >= 0.0f ? (c10.y) : (c10.x));
        if ((r1.w) != (-(r1.w)))
        {
            r7.xy = (v0.xy) * (c[2].ww) + (c[2].xy);
            r1.xy = (r7.xy) + (c10.zz);
            r1.zw = (v0.zz) * (c10.xy);
            r1 = tex2Dlod(s0, r1);
            r2.xy = (r7.xy) + (c10.ww);
            r2.zw = (v0.zz) * (c10.xy);
            r4 = tex2Dlod(s0, r2);
            r2.xy = (r7.xy) + (c3.xy);
            r2.zw = (v0.zz) * (c10.xy);
            r3 = tex2Dlod(s0, r2);
            r2.xy = (r7.xy) + (-(c3.xy));
            r2.zw = (v0.zz) * (c10.xy);
            r2 = tex2Dlod(s0, r2);
            r1.y = r4.x;
            r1.z = r3.x;
            r1.w = r2.x;
            r1.z = dot(r1, c3.wwww);
            r1.w = saturate((v0.w) + (c4.z));
            r0.w = (r0.w) + (-(r1.z));
            r0.w = (r1.w) * (r0.w) + (r1.z);
        }
        r1.w = r0.w;
    }
    r0.xyz = (r0.xyz) * (r0.xyz);
    r2.w = max(abs(r5.y), abs(r5.z));
    r1.xyz = (r7.www) * (r0.xyz);
    r0.w = max(abs(r5.x), r2.w);
    r0.w = 1.0f / (r0.w);
    r0.xyz = (r5.xyz) * (c[5].xyz);
    r2.xyz = (r1.xyz) * (c11.xxx);
    r0.xyz = (r0.xyz) * (r0.www) + (v1.xyz);
    r0 = tex3D(s11, r0.xyz);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r0.xyz = (r2.xyz) * (r0.xyz);
    r0.xyz = (r0.xyz) * (c11.xxx);
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    r0.x = 1.0f / (r0.x);
    r0.y = 1.0f / (r0.y);
    r0.z = 1.0f / (r0.z);
    r1.xyz = (r1.www) * (c[18].xyz);
    r0.xyz = (r6.zzz) * (r0.xyz);
    r1.xyz = (r1.xyz) * (r6.www) + (r2.xyz);
    r2.xyz = normalize(v4.xyz);
    r1.xyz = (r0.xyz) * (r5.www) + (r1.xyz);
    r0 = tex2D(s2, r6.xy);
    r0.xyz = (r0.xyz) * (r0.xyz);
    r1.w = dot(r5.xyz, r2.xyz);
    r0.xyz = (r1.xyz) * (r0.xyz) + (-(v5.xyz));
    r2.w = max(c11.y, r1.w);
    r1.x = v2.w;
    r0.xyz = (r1.xxx) * (r0.xyz) + (v5.xyz);
    r1.w = pow(abs(r2.w), c11.z);
    r0.xyz = max(((r0.xyz) * (c[6].xxx)), 0.0f); // HDR: removed only the 1.0 ceiling
    r0.x = rsqrt(r0.x);
    r0.y = rsqrt(r0.y);
    r0.z = rsqrt(r0.z);
    r0.w = (r0.w) + (-(c10.x));
    oC0.x = 1.0f / (r0.x);
    oC0.y = 1.0f / (r0.y);
    oC0.z = 1.0f / (r0.z);
    oC0.w = (r1.w) * (r0.w) + (c10.x);

    return oC0;
}
