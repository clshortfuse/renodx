...
float4 envMapParms      : register(c31);
float4 colorDetailScale : register(c32);
float4 hdrControl0      : register(c33);

float4 heroLightingR : register(c34);
float4 heroLightingG : register(c35);
float4 heroLightingB : register(c36);
...
float4 main(PS_INPUT input) : COLOR0
{
    ...
    // Original assembly:
    //
    //   mul_sat_pp r0.xyz, r0, c33.x
    //
    // UNCLAMP_HIGHLIGHTS removes only the upper 1.0 clamp from this final
    // operation. Negative values are still rejected before sqrt.
