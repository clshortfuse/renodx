...
float3 applyUserTonemap(float3 untonemapped, Texture3D lutTexture, SamplerState lutSampler, float3 vanilla, float3 bloom, float midGray) {
  ...
  if (injectedData.toneMapType == 0.f) {
  ...
  } else if (injectedData.toneMapType == 4.f) {  // Reinhard+
    float peak = injectedData.toneMapPeakNits / injectedData.toneMapGameNits;
    if (injectedData.toneMapGammaCorrection != 0.f) {
      peak = renodx::color::correct::Gamma(peak, injectedData.toneMapGammaCorrection > 0.f, injectedData.toneMapGammaCorrection == 1.f ? 2.2f : 2.4f);
    }
    ...
    float previous_lut_config_strength = lut_config.strength;
    lut_config.strength = 1.f;
    float3 sdrColor = applyReinhardPlus(outputColor, config, true) + bloom;
    float3 hdrColor = applyReinhardPlus(outputColor, config) + bloom;
    hdrColor = min(peak, hdrColor);
    sdrColor = min(1.f, sdrColor);
    float3 lutColor = renodx::lut::Sample(lutTexture, lut_config, sdrColor);
    outputColor = injectedData.upgradePerChannel !=0.f ? UpgradeToneMapPerChannel(hdrColor, sdrColor, lutColor, previous_lut_config_strength)
    : UpgradeToneMapByLuminance(hdrColor, sdrColor, lutColor, previous_lut_config_strength);
  } else {
