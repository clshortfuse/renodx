// ---- Created with 3Dmigoto v1.3.16 on Tue Sep 09 14:18:33 2025
#include "./!common.hlsl"
...
SamplerState tonemapBloomTexture_s : register(s3);
SamplerState filmGrainTexture_s : register(s4);
SamplerState blurTexture_s : register(s5);
SamplerState colorGradingTexture_s : register(s6);
...
Texture2D<float4> filmGrainTexture : register(t5);
Texture3D<float4> colorGradingTexture : register(t6);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.x = HexDOF_Params.x * r0.x + HexDOF_Params.y;
  r0.x = max(HexDOF_Params.w, r0.x);
  r0.x = saturate(abs(r0.x) * HexDOF_Params.z + -HexDOF_Params2.x);
  r1.xyzw = mainTexture.Sample(mainTexture_s, v1.xy).xyzw;
  r0.y = saturate(255 * r1.w);
  r0.y = 1 + -r0.y;
  r2.xyzw = blurTexture.Sample(blurTexture_s, v1.xy).wxyz;
  r2.x = saturate(r2.x);
  r0.y = r2.x * r0.y;
  r3.xyz = r2.yzw + -r1.xyz;
  ...
  r1.xy = v1.xy * filmGrainTextureScaleAndOffset.xy + filmGrainTextureScaleAndOffset.zw;
  r0.w = filmGrainTexture.Sample(filmGrainTexture_s, r1.xy).x;
  r0.w = -0.5 + r0.w;
  r1.xyz = filmGrainColorScale.xyz * r0.www;
  r0.xyz = r0.xyz * colorScale.xyz + r1.xyz;
