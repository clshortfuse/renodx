...
VSOutput main(uint vertex_id : SV_VertexID) {
  ...
  // TEXCOORD0 is normalized [0,1]+ now so the decode stage can reconstruct
  // filtering instead of using raw integer pixel coordinates.
  if (vertex_id == 0u) {
    ...
    output.texcoord = float3(2.f, 0.f, 0.f);
  } else {
    output.position = float4(-1.f, -3.f, 0.f, 1.f);
    output.texcoord = float3(0.f, 2.f, 0.f);
  }
