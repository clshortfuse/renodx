...
void main(
  ...
  r0.y = saturate(r0.x * 0.083333f);
  r0.x = saturate((r0.x - cb4[25].x) * cb4[25].y);
  r0.x *= cb4[25].z;
  ...
  r0.z = dot(cb4[23].xyz, r1.xyz);
  r2.y = log(abs(r0.z));
  r2.x = (r2.y == asfloat(0xff800000u)) ? 1.f : 0.f;
  r0.z = (r2.x != 0.f) ? kHugeNeg : r2.y;

  r0.zw = r0.z * float2(0.12f, 0.7f);
  r0.w = exp(r0.w);
  r0.z = exp(r0.z);
  r0.w = r0.w - r0.z;
  r0.y = max(r0.w * r0.y, 0.f);
  r0.y = saturate(cb4[136].x * r0.y + r0.z);
  r0.y = 1.f - r0.y;
  r0.y *= max(cb4[137].x, 0.f);

  r1.xyz = saturate(r0.yyy * cb4[138].xyz);
  r2.xyz = cb4[24].xyz - r1.xyz;
  r3.xyz = r0.xxx * r2.xyz + r1.xyz;

  r4.y = (0.f < abs(v2.w)) ? 1.f : 0.f;
  r4.x = rcp(v2.w);
  r0.x = (r4.y != 0.f) ? r4.x : kHuge;
  o1.xyz = r0.xxx * v2.zzz;

  r1 = BitNorm(t0.Sample(s0_s, v5.xy), cb3[44], cb3[45]);
  r0.x = max(r1.w * v3.w, 0.f);
  r0.z = r1.w - 0.1f;
  r0.w = abs(r0.x) * abs(r0.x);
  r0.w *= r0.w;
  r0.x = r0.w * abs(r0.x);
  r0.w = frac(-r0.z);
  r0.z = saturate(r0.w + r0.z);
  r4.x = (-r0.z >= 0.f) ? 1.f : 0.f;
  r0.x = (r4.x != 0.f) ? 1.f : r0.x;
