...
float4 DecodeBloomSample(float4 raw_sample, float4 mask, float4 set) {
  float4 decoded = BitNorm(raw_sample, mask, set);

  float raw_luma = dot(max(raw_sample.rgb, 0.f.xxx), 0.33333334f.xxx);
  float decoded_luma = dot(max(decoded.rgb, 0.f.xxx), 0.33333334f.xxx);
  bool decoded_negative = any(decoded.rgb < 0.f.xxx);
  bool decoded_missing = decoded_luma < 1e-5f && raw_luma > 1e-4f;
  bool decoded_exploding = decoded_luma > raw_luma * 64.f && raw_luma > 0.f;

  return (decoded_negative || decoded_missing || decoded_exploding) ? raw_sample : decoded;
}
...
void main(
  ...
  r1.x = 1.f;
  r1.z = 0.f;
  r3 = v6.xyzx * float4(r1.x, r1.x, r1.x, r1.z) - cb4[20];
  r1.y = dot(r3, r3);
  ...
  r3 = DecodeBloomSample(r3, cb3[44], cb3[45]);

  r3.x = dot(r0.yzw, cb4[139].xyz);
  r3.y = dot(r0.yzw, cb4[140].xyz);
  r4 = t1.Sample(s1_s, r3.xy);
  r4 = DecodeBloomSample(r4, cb3[46], cb3[47]);
  ...
  r1.x = cb4[151].x;
  r0.y = max(r1.x, 0.f) * max(cb4[153].x, 0.f);
  r0.y = r0.y * r1.y;
