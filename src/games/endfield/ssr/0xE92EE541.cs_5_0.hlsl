// ---- Created with 3Dmigoto v1.4.1 on Tue Feb  3 19:30:39 2026
#include "../shared.h"
...
void main(uint3 vThreadID : SV_DispatchThreadID)
{
  ...
  // r0.x is the vanilla SSR blend factor (0 = full IBL, 1 = full SSR/no IBL)
  // To reduce IBL intensity, we increase this value 
  float ssr_blend = r0.x;
  
  if (shader_injection.metallic_ibl_intensity == 1.f) {
    // set minimum blend of 0.75
    ssr_blend = max(ssr_blend, 0.75);
  }
  // else: metallic_ibl_intensity == 0.f = Vanilla (no change)
