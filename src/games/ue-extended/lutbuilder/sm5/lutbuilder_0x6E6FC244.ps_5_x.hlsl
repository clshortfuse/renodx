...
void main(
  ...
  float4 lutweights[2] = {float4(asfloat(cb0[38].x), asfloat(cb0[39].x), asfloat(cb0[40].x), asfloat(cb0[41].x)), float4(asfloat(cb0[42].x), 0.f, 0.f, 0.f)};
  cb_config.ue_lutweights = lutweights;
  o0 = ProcessLutbuilder(float3(untonemapped_ap1), s0_s, s1_s, s2_s, s3_s, t0, t1, t2, t3, cb_config, o0, asuint(cb0[65].z));
  return;
