...
void main(
  ...
  cb_config.ue_bluecorrection = asfloat(cb0[29].x);
  cb_config.ue_colorscale = asfloat(cb0[33].yzw);
  float4 lutweights[2] = {float4(asfloat(cb0[29].x), asfloat(cb0[30].x), asfloat(cb0[31].x), 0.f), float4(0.f, 0.f, 0.f, 0.f)};
  cb_config.ue_lutweights = lutweights;
  o0 = ProcessLutbuilder(float3(r0.xyz), s0_s, s1_s, t0, t1, cb_config, o0, asuint(cb0[40].w));
  return;
