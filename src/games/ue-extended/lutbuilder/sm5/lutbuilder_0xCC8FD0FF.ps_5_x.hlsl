...
// ---- Created with 3Dmigoto v1.3.16 on Wed Dec 25 01:33:54 2024
...
void main(
  ...
  // Film curve not used in this shader — defaults from CreateCbufferConfig() are correct
  cb_config.ue_tonecurveammount = 1.f; // no ToneCurveAmount fingerprint found in dead code
  cb_config.ue_mappingpolynomial = asfloat(cb0[26].xyz);
  cb_config.ue_overlaycolor = asfloat(cb0[43].xyzw);
  cb_config.ue_bluecorrection = 0.f; // no blue correction matrix fingerprint found (cb0[66] is output device switch)
  cb_config.ue_colorscale = asfloat(cb0[42].yzw);
  o0 = ProcessLutbuilder(float3(r0.xyz), cb_config, o0, asuint(cb0[65].z));
  return;
