...
void main(
  ...
  // Grading ends in linear BT.709, before the AP0 red modifier and film.
  // Product of the original BT.709->AP0 and AP0->AP1 matrices.
  float3 untonemapped_ap1 = mul(float3x3(
      0.613191471883, 0.339512051665, 0.047366337548,
      0.070206905327, 0.916335818846, 0.013450018324,
      0.020618870846, 0.109567293668, 0.869606726779), r0.xyz);

  // The mod intentionally replaces the game's old/film selector with its
  // selected tone mapper. HDR must enter the extended curve and LUT range
  // restoration before final display mapping, even when cb0[28].w is zero.
  ...
  float4 lutweights[2] = {float4(cb0[31].x, cb0[32].x, 0.f, 0.f), float4(0.f, 0.f, 0.f, 0.f)};
  cb_config.ue_lutweights = lutweights;

  // No blue-correction blend or tone-curve attenuation exists in this bytecode.
  // These disable added operations; they are not measured cbuffer fields.
  cb_config.ue_bluecorrection = 0.f;
  cb_config.ue_tonecurveammount = 1.f;

  o0 = ProcessLutbuilder(float3(untonemapped_ap1), s0_s, t0, cb_config, o0, asuint(cb0[42].w));
  return;
