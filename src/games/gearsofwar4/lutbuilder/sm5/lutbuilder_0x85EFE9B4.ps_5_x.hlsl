  ...
  out float4 o0 : SV_Target0)
{
  ...
  cb_config.ue_tonecurveammount = 1.f; // shader has no ToneCurveAmount cbuffer member (cb0[71] out of bounds)
  ...
  o0 = ProcessLutbuilder(float3(untonemapped_ap1), s0_s, t0, cb_config, o0, asuint(cb0[70].z));
  return;


  // r2.x = saturate(dot(float3(1.70505154,-0.621790707,-0.0832583979), r0.xyz));
  // r2.y = saturate(dot(float3(-0.130257145,1.14080286,-0.0105485283), r0.xyz));
  // r2.z = saturate(dot(float3(-0.0240032747,-0.128968775,1.15297174), r0.xyz));
