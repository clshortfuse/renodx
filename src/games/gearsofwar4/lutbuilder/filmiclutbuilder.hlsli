...
float3 SampleLUTSRGBInSRGBOut(Texture2D<float4> lut_texture, SamplerState lut_sampler, float3 color_input, UECbufferConfig cb_config) {
  ...
  float3 lut_output_color = SamplePacked1DLut(lut_input_color, lut_sampler, lut_texture, cb_config);
  float3 color_output = renodx::lut::LinearOutput(lut_output_color, lut_config);

  [branch]
  if (lut_config.scaling != 0.f) {
    float3 lut_black = SamplePacked1DLut(float3(0, 0, 0), lut_sampler, lut_texture, cb_config);
    ...
    if (lut_black_y > 0.f) {
      ...
      float3 lut_mid = SamplePacked1DLut(lut_black, lut_sampler, lut_texture, cb_config);
