...
//       -> gamma 2.0 encode
//       -> original filtering, DOF blending, tint, and color bias
//       -> gamma 2.0 decode
...
sampler2D floatZSampler    : register(s6);
...
float4 main(PS_INPUT input) : COLOR0
{
    ...
    // Filtering, DOF composition, tint, and color bias were evaluated in the
    // reconstructed gamma 2.0 space. Decode the completed graded result back
    // to scene-linear RGB before passing it to either HDR tonemapper.
    linearOutput =
        GammaDecode2(
            gradedOutput
        );
    ...
    // RenderIntermediatePass applies the configured intermediate encoding.
    ...
    if (!IsVanillaMode())
    {
        toneMappedOutput = RestorePostTonemapBloomHeadroom(linearOutput, toneMappedOutput);
    }
