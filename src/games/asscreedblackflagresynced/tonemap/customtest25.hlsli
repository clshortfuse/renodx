...
float3 CompressPsychoV25ReferenceScaleHull(
  ...
  // Adjust saturation only after Fast60 and source-direction recovery have
  // authored the hue, but before Reference Scale gamut containment.
  [branch]
  if (post_saturation != 1.f) {
    float saturation_scale = max(post_saturation, 0.f);
    authored_radius *= saturation_scale;
    authored_offset = authored_direction * authored_radius;
    authored_mb.xy = adapted_neutral_mb + authored_offset;
    source_offset *= saturation_scale;
    source_mb.xy = adapted_neutral_mb + source_offset;
  }
