...
float3 psycho30_ApplyAnchoredTonalGrading(
  ...
  if (shadow_contrast != 1.f) {
    ...
    graded_normalized *= pow(1.f + flat_distance, shadow_contrast - 1.f);
  }
...
float3 psychotm_custom_test30(
  ...
  float3 tonal_input_lms = source_lms;
  [branch]
  if (purity_scale != 1.f || highlight_saturation != 1.f || dechroma != 0.f) {
    ...
    if (dechroma != 0.f || highlight_saturation != 1.f) {
      ...
      float source_relative_yf = max(
          renodx::color::yf::from::LMS(source_lms / anchor_in_lms),
          0.f);
      float neutral_relative_yf = renodx::color::yf::from::LMS(
          float3(1.f, 1.f, 1.f));
    ...
    tonal_input_lms = psycho30_ApplyAdaptiveLMSPurity(source_lms, anchor_in_lms, effective_purity_scale);
  }
  ...
  float3 graded_lms = psycho30_ApplyAnchoredTonalGrading(
      tonal_input_lms,
      anchor_in_lms,
      anchor_out_lms,
      contrast,
  ...
  if (gamut_compression != 0.f) {
    uint solve_valid;
    float3 solved_coord = psycho30_ApplyCustomSoftRadialGamutCompression(
        desired_coord,
        response_yf,
        gamut_compression_mode,
        solve_valid);
