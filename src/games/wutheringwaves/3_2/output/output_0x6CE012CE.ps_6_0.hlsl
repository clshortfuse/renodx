...
float4 main(
  ...
  float4 _439 = t0.Sample(s0, float2(min(max(_427, cb0_053z), cb0_054x), min(max(_428, cb0_053w), cb0_054y)));
  float _445 = saturate(cb0_099x);
  float _447 = saturate(cb0_099y);
  if (_445 > 0.0f) {
    float _458 = saturate(((_439.x * 0.29899999499320984f) + (_439.y * 0.5870000123977661f)) + (_439.z * 0.11400000005960464f));
    float _464 = ((_458 * _458) * (saturate(1.0f - cb0_099z) * 2.0f)) * (3.0f - (_458 * 2.0f));
    float _471 = max(sqrt(((_439.x * _439.x) + (_439.y * _439.y)) + (_439.z * _439.z)), 0.014999999664723873f);
    float _472 = _439.x / _471;
    float _473 = _439.y / _471;
    float _474 = _439.z / _471;
    float _484 = log2(_464);
    float _503 = ((_295 * _215) * _332) * _445;
    _514 = (((((lerp(_472, 1.0f, 0.25f)) * (((_447 * 0.17000000178813934f) + 0.029999999329447746f) + _464)) - _439.x) * _503) + _439.x);
    _515 = (((((((_447 * 0.18000000715255737f) + 0.05000000074505806f) + exp2(_484 * 0.8999999761581421f)) * (lerp(_473, 1.0f, 0.25f))) - _439.y) * _503) + _439.y);
    _516 = (((((((_447 * 0.17999999225139618f) + 0.07999999821186066f) + exp2(_484 * 0.949999988079071f)) * (lerp(_474, 1.0f, 0.25f))) - _439.z) * _503) + _439.z);
  } else {
    _514 = _439.x;
    _515 = _439.y;
    _516 = _439.z;
  }
  float _519 = saturate(cb0_100x);
  float _520 = _388 * _519;
  if (_519 > 0.0f) {
    float _531 = (sin(((_117 * 640.0f) + 640.0f) - (View_2376 * 10.0f)) * 0.5f) + 1.0f;
    _545 = ((((_531 * _514) - _514) * _520) + _514);
    _546 = ((((_531 * _515) - _515) * _520) + _515);
    _547 = ((((_531 * _516) - _516) * _520) + _516);
  } else {
    _545 = _514;
    _546 = _515;
    _547 = _516;
  }
  float4 _572 = t1.Sample(s1, float2(min(max(((cb0_068z * _427) + cb0_069x), cb0_060z), cb0_061x), min(max(((cb0_068w * _428) + cb0_069y), cb0_060w), cb0_061y)));
  APPLY_BLOOM(_572);
  [branch]
  if (!(cb0_085z == 0)) {
    bool _601 = (cb0_086x != 0);
    float4 _604 = t2.Sample(s2, float2(select(_601, _427, min(max(((cb0_076z * _427) + cb0_077x), cb0_075z), cb0_076x)), select(_601, _428, min(max(((cb0_076w * _428) + cb0_077y), cb0_075w), cb0_076y))));
    _612 = (_604.x + _572.x);
    _613 = (_604.y + _572.y);
    _614 = (_604.z + _572.z);
  } else {
  ...
  [branch]
  if (!(cb0_085w == 0)) {
    float4 _618 = t3.Sample(s3, float2(_427, _428));
    _626 = (_618.x + _612);
    _627 = (_618.y + _613);
    _628 = (_618.z + _614);
  } else {
  ...
  float _653 = TEXCOORD_1.z + -1.0f;
  float _655 = TEXCOORD_1.w + -1.0f;
  float _658 = ((_653 + (cb0_091x * 2.0f)) * cb0_089z) * cb0_089x;
  float _660 = ((_655 + (cb0_091y * 2.0f)) * cb0_089w) * cb0_089x;
  float _667 = 1.0f / ((((saturate(cb0_090w) * 9.0f) + 1.0f) * dot(float2(_658, _660), float2(_658, _660))) + 1.0f);
  float _668 = _667 * _667;
  float _669 = cb0_091z + 1.0f;
  float _697 = ((_653 + (cb0_094x * 2.0f)) * cb0_092z) * cb0_092x;
  float _699 = ((_655 + (cb0_094y * 2.0f)) * cb0_092w) * cb0_092x;
  float _706 = 1.0f / ((((saturate(cb0_093w) * 9.0f) + 1.0f) * dot(float2(_697, _699), float2(_697, _699))) + 1.0f);
  float _707 = _706 * _706;
  float _708 = cb0_094z + 1.0f;
  float _730 = ((((_626 + ((_545 * TEXCOORD_1.x) * cb0_087x)) * _385) * ((_668 * (_669 - cb0_090x)) + cb0_090x)) * ((_707 * (_708 - cb0_093x)) + cb0_093x)) * ((cb0_096x * _339) + cb0_096y);
  float _734 = ((((_627 + ((_546 * TEXCOORD_1.x) * cb0_087y)) * _385) * ((_668 * (_669 - cb0_090y)) + cb0_090y)) * ((_707 * (_708 - cb0_093y)) + cb0_093y)) * ((cb0_096x * _362) + cb0_096y);
  float _738 = ((((_628 + ((_547 * TEXCOORD_1.x) * cb0_087z)) * _385) * ((_668 * (_669 - cb0_090z)) + cb0_090z)) * ((_707 * (_708 - cb0_093z)) + cb0_093z)) * ((cb0_096x * _363) + cb0_096y);

  CAPTURE_UNTONEMAPPED(float3(_730, _734, _738));
  [branch]
  if (WUWA_TM_IS(1)) {
    _771 = ((((_730 * 1.3600000143051147f) + 0.04699999839067459f) * _730) / ((((_730 * 0.9599999785423279f) + 0.5600000023841858f) * _730) + 0.14000000059604645f));
    _772 = ((((_734 * 1.3600000143051147f) + 0.04699999839067459f) * _734) / ((((_734 * 0.9599999785423279f) + 0.5600000023841858f) * _734) + 0.14000000059604645f));
    _773 = ((((_738 * 1.3600000143051147f) + 0.04699999839067459f) * _738) / ((((_738 * 0.9599999785423279f) + 0.5600000023841858f) * _738) + 0.14000000059604645f));
  } else {
    _771 = _730;
    _772 = _734;
    _773 = _738;
  ...
  [branch]
  if (WUWA_TM_IS(2)) {
    float _783 = 1.0049500465393066f - (0.16398000717163086f / (_771 + -0.19505000114440918f));
    float _784 = 1.0049500465393066f - (0.16398000717163086f / (_772 + -0.19505000114440918f));
    float _785 = 1.0049500465393066f - (0.16398000717163086f / (_773 + -0.19505000114440918f));
    _805 = (((_771 - _783) * select((_771 > 0.6000000238418579f), 0.0f, 1.0f)) + _783);
    _806 = (((_772 - _784) * select((_772 > 0.6000000238418579f), 0.0f, 1.0f)) + _784);
    _807 = (((_773 - _785) * select((_773 > 0.6000000238418579f), 0.0f, 1.0f)) + _785);
  } else {
    _805 = _771;
    _806 = _772;
    _807 = _773;
  ...
  [branch]
  if (WUWA_TM_IS(3)) {
    float _813 = cb0_037y * _805;
    float _814 = cb0_037y * _806;
    float _815 = cb0_037y * _807;
    float _818 = cb0_037z * cb0_037w;
    float _828 = cb0_038y * cb0_038x;
    float _839 = cb0_038z * cb0_038x;
    float _846 = cb0_038y / cb0_038z;
    _854 = (((((_818 + _813) * _805) + _828) / (_839 + ((_813 + cb0_037z) * _805))) - _846);
    _855 = (((((_818 + _814) * _806) + _828) / (_839 + ((_814 + cb0_037z) * _806))) - _846);
    _856 = (((((_818 + _815) * _807) + _828) / (_839 + ((_815 + cb0_037z) * _807))) - _846);
  } else {
    _854 = _805;
    _855 = _806;
    _856 = _807;
  ...
  if (!(cb0_106w == 0)) {
    if (!(cb0_107x == 1.0f)) {
      float _866 = (cb0_107x * 0.699999988079071f) + 0.30000001192092896f;
      _871 = (_866 * _854);
      _872 = (_866 * _855);
      _873 = (_866 * _856);
    } else {
  ...
  CLAMP_IF_SDR3(_871, _872, _873);
  CAPTURE_TONEMAPPED(float3(_871, _872, _873));
  float _894 = (saturate((log2(_871 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _895 = (saturate((log2(_872 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _896 = (saturate((log2(_873 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float4 _897 = t7.Sample(s7, float3(_894, _895, _896));
  [branch]
  if (!(cb0_108w == 0)) {
    float4 _921 = t6.Sample(s6, float2(min(max(((cb0_084z * _427) + cb0_085x), cb0_083z), cb0_084x), min(max(((cb0_084w * _428) + cb0_085y), cb0_083w), cb0_084y)));
    float _931 = select((((uint)(uint(float((uint)((int)((uint)(uint(round(_921.w * 255.0f))) & 15))))) & -4) == 12), 1.0f, 0.0f);
    float4 _932 = t8.Sample(s8, float3(_894, _895, _896));
    _946 = (lerp(_932.x, _897.x, _931));
    _947 = (lerp(_932.y, _897.y, _931));
    _948 = (lerp(_932.z, _897.z, _931));
  } else {
    _946 = _897.x;
    _947 = _897.y;
    _948 = _897.z;
  }
  HANDLE_LUT_OUTPUT3_FADE(_946, _947, _948, t7, s7);
  float _949 = _948 * 1.0499999523162842f;
  float _950 = _947 * 1.0499999523162842f;
  float _951 = _946 * 1.0499999523162842f;
  float _959 = ((_339 * 0.00390625f) + -0.001953125f) + _951;
  float _960 = ((_362 * 0.00390625f) + -0.001953125f) + _950;
  float _961 = ((_363 * 0.00390625f) + -0.001953125f) + _949;
  [branch]
  if (!(cb0_107w == 0)) {
    float _973 = (pow(_959, 0.012683313339948654f));
    float _974 = (pow(_960, 0.012683313339948654f));
    float _975 = (pow(_961, 0.012683313339948654f));
    float _1008 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_973 + -0.8359375f)) / (18.8515625f - (_973 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    float _1009 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_974 + -0.8359375f)) / (18.8515625f - (_974 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    float _1010 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_975 + -0.8359375f)) / (18.8515625f - (_975 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    _1036 = min((_1008 * 12.920000076293945f), ((exp2(log2(max(_1008, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _1037 = min((_1009 * 12.920000076293945f), ((exp2(log2(max(_1009, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _1038 = min((_1010 * 12.920000076293945f), ((exp2(log2(max(_1010, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _1036 = _959;
    _1037 = _960;
    _1038 = _961;
  ...
  SV_Target.y = _1037;
  SV_Target.z = _1038;
  SV_Target.w = dot(float3(_951, _950, _949), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
