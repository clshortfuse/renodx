...
float4 main(
  ...
  float4 _423 = t0.Sample(s0, float2(_421, _422));
  float _429 = saturate(cb0_099x);
  float _431 = saturate(cb0_099y);
  if (_429 > 0.0f) {
    float _442 = saturate(((_423.x * 0.29899999499320984f) + (_423.y * 0.5870000123977661f)) + (_423.z * 0.11400000005960464f));
    float _448 = ((_442 * _442) * (saturate(1.0f - cb0_099z) * 2.0f)) * (3.0f - (_442 * 2.0f));
    float _455 = max(sqrt(((_423.x * _423.x) + (_423.y * _423.y)) + (_423.z * _423.z)), 0.014999999664723873f);
    float _456 = _423.x / _455;
    float _457 = _423.y / _455;
    float _458 = _423.z / _455;
    float _468 = log2(_448);
    float _487 = ((_300 * _220) * _337) * _429;
    _498 = (((((lerp(_456, 1.0f, 0.25f)) * (((_431 * 0.17000000178813934f) + 0.029999999329447746f) + _448)) - _423.x) * _487) + _423.x);
    _499 = (((((((_431 * 0.18000000715255737f) + 0.05000000074505806f) + exp2(_468 * 0.8999999761581421f)) * (lerp(_457, 1.0f, 0.25f))) - _423.y) * _487) + _423.y);
    _500 = (((((((_431 * 0.17999999225139618f) + 0.07999999821186066f) + exp2(_468 * 0.949999988079071f)) * (lerp(_458, 1.0f, 0.25f))) - _423.z) * _487) + _423.z);
  } else {
    _498 = _423.x;
    _499 = _423.y;
    _500 = _423.z;
  }
  float _502 = saturate(cb0_100x);
  float _503 = _393 * _502;
  if (_502 > 0.0f) {
    float _514 = (sin(((_122 * 640.0f) + 640.0f) - (View_2376 * 10.0f)) * 0.5f) + 1.0f;
    _528 = ((((_514 * _498) - _498) * _503) + _498);
    _529 = ((((_514 * _499) - _499) * _503) + _499);
    _530 = ((((_514 * _500) - _500) * _503) + _500);
  } else {
    _528 = _498;
    _529 = _499;
    _530 = _500;
  }
  float _539 = log2(max(dot(float3(_528, _529, _530), float3(cb0_043x, cb0_043y, cb0_043z)), cb0_042x));
  float4 _557 = t7.Sample(s7, float3((cb0_046z * TEXCOORD_4.x), (cb0_046w * TEXCOORD_4.y), ((((cb0_041z * _539) + cb0_041w) * 0.96875f) + 0.015625f)));
  float4 _561 = t8.Sample(s8, float2(TEXCOORD_4.x, TEXCOORD_4.y));
  float _564 = select((_557.y < 0.0010000000474974513f), _561.x, (_557.x / _557.y));
  float _567 = log2(TEXCOORD_1.x);
  float _569 = (_564 + _567) + ((_561.x - _564) * cb0_046x);
  float _574 = _567 + _539;
  float _576 = _569 - log2((TEXCOORD_1.y * 0.18000000715255737f) * cb0_046y);
  bool _577 = (_576 > 0.0f);
  if (_577) {
    _589 = max(0.0f, (_576 - cb0_047x));
  } else {
    _589 = min(0.0f, (cb0_047y + _576));
  }
  float4 _621 = t1.Sample(s1, float2(min(max(((cb0_068z * _421) + cb0_069x), cb0_060z), cb0_061x), min(max(((cb0_068w * _422) + cb0_069y), cb0_060w), cb0_061y)));
  APPLY_BLOOM(_621);
  [branch]
  if (!(cb0_085z == 0)) {
    bool _650 = (cb0_086x != 0);
    float4 _653 = t2.Sample(s2, float2(select(_650, _421, min(max(((cb0_076z * _421) + cb0_077x), cb0_075z), cb0_076x)), select(_650, _422, min(max(((cb0_076w * _422) + cb0_077y), cb0_075w), cb0_076y))));
    _661 = (_653.x + _621.x);
    _662 = (_653.y + _621.y);
    _663 = (_653.z + _621.z);
  } else {
  ...
  [branch]
  if (!(cb0_085w == 0)) {
    float4 _667 = t3.Sample(s3, float2(_421, _422));
    _675 = (_667.x + _661);
    _676 = (_667.y + _662);
    _677 = (_667.z + _663);
  } else {
  ...
  float _678 = exp2((((_569 - _574) + ((_574 - _569) * cb0_045w)) - _589) + (_589 * select(_577, cb0_045y, cb0_045z))) * TEXCOORD_1.x;
  float _703 = TEXCOORD_1.z + -1.0f;
  float _705 = TEXCOORD_1.w + -1.0f;
  float _708 = ((_703 + (cb0_091x * 2.0f)) * cb0_089z) * cb0_089x;
  float _710 = ((_705 + (cb0_091y * 2.0f)) * cb0_089w) * cb0_089x;
  float _717 = 1.0f / ((((saturate(cb0_090w) * 9.0f) + 1.0f) * dot(float2(_708, _710), float2(_708, _710))) + 1.0f);
  float _718 = _717 * _717;
  float _719 = cb0_091z + 1.0f;
  float _747 = ((_703 + (cb0_094x * 2.0f)) * cb0_092z) * cb0_092x;
  float _749 = ((_705 + (cb0_094y * 2.0f)) * cb0_092w) * cb0_092x;
  float _756 = 1.0f / ((((saturate(cb0_093w) * 9.0f) + 1.0f) * dot(float2(_747, _749), float2(_747, _749))) + 1.0f);
  float _757 = _756 * _756;
  float _758 = cb0_094z + 1.0f;
  float _770 = (((_675 + ((_678 * _528) * cb0_087x)) * _390) * ((_718 * (_719 - cb0_090x)) + cb0_090x)) * ((_757 * (_758 - cb0_093x)) + cb0_093x);
  float _773 = (((_676 + ((_678 * _529) * cb0_087y)) * _390) * ((_718 * (_719 - cb0_090y)) + cb0_090y)) * ((_757 * (_758 - cb0_093y)) + cb0_093y);
  float _776 = (((_677 + ((_678 * _530) * cb0_087z)) * _390) * ((_718 * (_719 - cb0_090z)) + cb0_090z)) * ((_757 * (_758 - cb0_093z)) + cb0_093z);

  CAPTURE_UNTONEMAPPED(float3(_770, _773, _776));
  [branch]
  if (WUWA_TM_IS(1)) {
    _809 = ((((_770 * 1.3600000143051147f) + 0.04699999839067459f) * _770) / ((((_770 * 0.9599999785423279f) + 0.5600000023841858f) * _770) + 0.14000000059604645f));
    _810 = ((((_773 * 1.3600000143051147f) + 0.04699999839067459f) * _773) / ((((_773 * 0.9599999785423279f) + 0.5600000023841858f) * _773) + 0.14000000059604645f));
    _811 = ((((_776 * 1.3600000143051147f) + 0.04699999839067459f) * _776) / ((((_776 * 0.9599999785423279f) + 0.5600000023841858f) * _776) + 0.14000000059604645f));
  } else {
    _809 = _770;
    _810 = _773;
    _811 = _776;
  ...
  [branch]
  if (WUWA_TM_IS(2)) {
    float _821 = 1.0049500465393066f - (0.16398000717163086f / (_809 + -0.19505000114440918f));
    float _822 = 1.0049500465393066f - (0.16398000717163086f / (_810 + -0.19505000114440918f));
    float _823 = 1.0049500465393066f - (0.16398000717163086f / (_811 + -0.19505000114440918f));
    _843 = (((_809 - _821) * select((_809 > 0.6000000238418579f), 0.0f, 1.0f)) + _821);
    _844 = (((_810 - _822) * select((_810 > 0.6000000238418579f), 0.0f, 1.0f)) + _822);
    _845 = (((_811 - _823) * select((_811 > 0.6000000238418579f), 0.0f, 1.0f)) + _823);
  } else {
    _843 = _809;
    _844 = _810;
    _845 = _811;
  ...
  [branch]
  if (WUWA_TM_IS(3)) {
    float _851 = cb0_037y * _843;
    float _852 = cb0_037y * _844;
    float _853 = cb0_037y * _845;
    float _856 = cb0_037z * cb0_037w;
    float _866 = cb0_038y * cb0_038x;
    float _877 = cb0_038z * cb0_038x;
    float _884 = cb0_038y / cb0_038z;
    _892 = (((((_856 + _851) * _843) + _866) / (_877 + ((_851 + cb0_037z) * _843))) - _884);
    _893 = (((((_856 + _852) * _844) + _866) / (_877 + ((_852 + cb0_037z) * _844))) - _884);
    _894 = (((((_856 + _853) * _845) + _866) / (_877 + ((_853 + cb0_037z) * _845))) - _884);
  } else {
    _892 = _843;
    _893 = _844;
    _894 = _845;
  ...
  if (!(cb0_106w == 0)) {
    if (!(cb0_107x == 1.0f)) {
      float _904 = (cb0_107x * 0.699999988079071f) + 0.30000001192092896f;
      _909 = (_904 * _892);
      _910 = (_904 * _893);
      _911 = (_904 * _894);
    } else {
  ...
  CLAMP_IF_SDR3(_909, _910, _911);
  CAPTURE_TONEMAPPED(float3(_909, _910, _911));
  float _932 = (saturate((log2(_909 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  ...
  float4 _935 = t9.Sample(s9, float3(_932, _933, _934));
  [branch]
  if (!(cb0_108w == 0)) {
    float4 _959 = t6.Sample(s6, float2(min(max(((cb0_084z * _421) + cb0_085x), cb0_083z), cb0_084x), min(max(((cb0_084w * _422) + cb0_085y), cb0_083w), cb0_084y)));
    float _969 = select((((uint)(uint(float((uint)((int)((uint)(uint(round(_959.w * 255.0f))) & 15))))) & -4) == 12), 1.0f, 0.0f);
    float4 _970 = t10.Sample(s10, float3(_932, _933, _934));
    _984 = (lerp(_970.x, _935.x, _969));
    _985 = (lerp(_970.y, _935.y, _969));
    _986 = (lerp(_970.z, _935.z, _969));
  } else {
    _984 = _935.x;
    _985 = _935.y;
    _986 = _935.z;
  }
  HANDLE_LUT_OUTPUT3_FADE(_984, _985, _986, t9, s9);
  float _987 = _986 * 1.0499999523162842f;
  float _988 = _985 * 1.0499999523162842f;
  float _989 = _984 * 1.0499999523162842f;
  float _997 = ((_344 * 0.00390625f) + -0.001953125f) + _989;
  float _998 = ((_367 * 0.00390625f) + -0.001953125f) + _988;
  float _999 = ((_368 * 0.00390625f) + -0.001953125f) + _987;
  [branch]
  if (!(cb0_107w == 0)) {
    float _1011 = (pow(_997, 0.012683313339948654f));
    float _1012 = (pow(_998, 0.012683313339948654f));
    float _1013 = (pow(_999, 0.012683313339948654f));
    float _1046 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1011 + -0.8359375f)) / (18.8515625f - (_1011 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    float _1047 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1012 + -0.8359375f)) / (18.8515625f - (_1012 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    float _1048 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1013 + -0.8359375f)) / (18.8515625f - (_1013 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_107z));
    _1074 = min((_1046 * 12.920000076293945f), ((exp2(log2(max(_1046, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _1075 = min((_1047 * 12.920000076293945f), ((exp2(log2(max(_1047, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _1076 = min((_1048 * 12.920000076293945f), ((exp2(log2(max(_1048, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _1074 = _997;
    _1075 = _998;
    _1076 = _999;
  ...
  SV_Target.y = _1075;
  SV_Target.z = _1076;
  SV_Target.w = dot(float3(_989, _988, _987), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
