...
float4 main(float4 position : SV_POSITION, float2 uv : TEXCOORD0) : SV_TARGET {
  ...
  const float3 debanded = DebandVideo(source_texture, source_sampler, uv, texel_size, CUSTOM_RANDOM);
  const float hans_local_map = hans_map.SampleLevel(source_sampler, uv, 0.0f).r;
  const float3 result = AutoHDRVideo(debanded, uv, hans_local_map);
