...
float4 main(
  ...
  CAPTURE_UNTONEMAPPED(float3(_304, _305, _306));
  ...
  const float3 inverted = renodx::draw::InvertIntermediatePass(float3(_537, _538, _539));
  _537 = inverted.r; _538 = inverted.g; _539 = inverted.b;

  float _548 = ((((_538 * 587.0f) + (_537 * 299.0f)) + (_539 * 114.0f)) * 0.0010000000474974513f) - cb0_090z;
  float _555 = saturate(float(((int)(uint)((bool)(_548 > 0.0f))) - ((int)(uint)((bool)(_548 < 0.0f)))));
  float _562 = cb0_091x - _537;
  float _563 = cb0_091y - _538;
  float _564 = cb0_091z - _539;

  const float peak_scaling = RENODX_PEAK_NITS / RENODX_GAME_NITS;
  // float _569 = cb0_092x - _537;
  // float _570 = cb0_092y - _538;
  // float _571 = cb0_092z - _539;
  float _569 = peak_scaling * cb0_092x - _537;
  float _570 = peak_scaling * cb0_092y - _538;
  float _571 = peak_scaling * cb0_092z - _539;
  ...
  SV_Target.x = ((cb0_090x * (lerp(_588, _591, _555))) + _537);
  SV_Target.y = ((cb0_090x * (lerp(_589, _592, _555))) + _538);
  SV_Target.z = (((lerp(_590, _593, _555)) * cb0_090x) + _539);
