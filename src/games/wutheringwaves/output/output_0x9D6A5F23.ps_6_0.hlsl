...
float4 main(
  ...
  float4 _432 = t0.Sample(s0, float2(min(max(_420, cb0_053z), cb0_054x), min(max(_421, cb0_053w), cb0_054y)));
  float _438 = saturate(cb0_090x);
  float _440 = saturate(cb0_090y);
  if (_438 > 0.0f) {
    float _451 = saturate(((_432.x * 0.29899999499320984f) + (_432.y * 0.5870000123977661f)) + (_432.z * 0.11400000005960464f));
    float _452 = _451 * 2.0f;
    float _456 = ((_451 * saturate(1.0f - cb0_090z)) * _452) * (3.0f - _452);
    float _463 = max(sqrt(((_432.x * _432.x) + (_432.y * _432.y)) + (_432.z * _432.z)), 0.014999999664723873f);
    float _464 = _432.x / _463;
    float _465 = _432.y / _463;
    float _466 = _432.z / _463;
    float _476 = log2(_456);
    float _495 = ((_292 * _212) * _328) * _438;
    _506 = (((((lerp(_464, 1.0f, 0.25f)) * (((_440 * 0.17000000178813934f) + 0.029999999329447746f) + _456)) - _432.x) * _495) + _432.x);
    _507 = (((((((_440 * 0.18000000715255737f) + 0.05000000074505806f) + exp2(_476 * 0.8999999761581421f)) * (lerp(_465, 1.0f, 0.25f))) - _432.y) * _495) + _432.y);
    _508 = (((((((_440 * 0.17999999225139618f) + 0.07999999821186066f) + exp2(_476 * 0.949999988079071f)) * (lerp(_466, 1.0f, 0.25f))) - _432.z) * _495) + _432.z);
  } else {
    _506 = _432.x;
    _507 = _432.y;
    _508 = _432.z;
  }
  float _511 = saturate(cb0_091x);
  float _512 = _511 * _382;
  if (_511 > 0.0f) {
    float _523 = (sin(((_113 * 640.0f) + 640.0f) - (View_2376 * 10.0f)) * 0.5f) + 1.0f;
  ...
  float4 _564 = t1.Sample(s1, float2(min(max(((cb0_068z * _420) + cb0_069x), cb0_060z), cb0_061x), min(max(((cb0_068w * _421) + cb0_069y), cb0_060w), cb0_061y)));
  APPLY_BLOOM(_564);

  [branch]
  if (!((uint)(cb0_077z) == 0)) {
    bool _592 = ((uint)(cb0_077w) != 0);
    float4 _595 = t2.Sample(s2, float2(select(_592, _420, min(max(((cb0_076z * _420) + cb0_077x), cb0_075z), cb0_076x)), select(_592, _421, min(max(((cb0_076w * _421) + cb0_077y), cb0_075w), cb0_076y))));
    _603 = (_595.x + _564.x);
    _604 = (_595.y + _564.y);
    _605 = (_595.z + _564.z);
  } else {
  ...
  float _630 = TEXCOORD_1.z + -1.0f;
  float _632 = TEXCOORD_1.w + -1.0f;
  float _635 = (((cb0_082x * 2.0f) + _630) * cb0_080z) * cb0_080x;
  float _637 = (((cb0_082y * 2.0f) + _632) * cb0_080w) * cb0_080x;
  float _644 = 1.0f / ((((saturate(cb0_081w) * 9.0f) + 1.0f) * dot(float2(_635, _637), float2(_635, _637))) + 1.0f);
  float _645 = _644 * _644;
  float _646 = cb0_082z + 1.0f;
  float _674 = (((cb0_085x * 2.0f) + _630) * cb0_083z) * cb0_083x;
  float _676 = (((cb0_085y * 2.0f) + _632) * cb0_083w) * cb0_083x;
  float _683 = 1.0f / ((((saturate(cb0_084w) * 9.0f) + 1.0f) * dot(float2(_674, _676), float2(_674, _676))) + 1.0f);
  float _684 = _683 * _683;
  float _685 = cb0_085z + 1.0f;
  float _707 = ((((_603 + ((_537 * TEXCOORD_1.x) * cb0_078x)) * _379) * ((_645 * (_646 - cb0_081x)) + cb0_081x)) * ((_684 * (_685 - cb0_084x)) + cb0_084x)) * ((cb0_087x * _335) + cb0_087y);
  float _711 = ((((_604 + ((_538 * TEXCOORD_1.x) * cb0_078y)) * _379) * ((_645 * (_646 - cb0_081y)) + cb0_081y)) * ((_684 * (_685 - cb0_084y)) + cb0_084y)) * ((cb0_087x * _356) + cb0_087y);
  float _715 = ((((_605 + ((_539 * TEXCOORD_1.x) * cb0_078z)) * _379) * ((_645 * (_646 - cb0_081z)) + cb0_081z)) * ((_684 * (_685 - cb0_084z)) + cb0_084z)) * ((cb0_087x * _357) + cb0_087y);

  CAPTURE_UNTONEMAPPED(float3(_707, _711, _715));

  [branch]
  if (WUWA_TM_IS(1)) {
    _748 = ((((_707 * 1.3600000143051147f) + 0.04699999839067459f) * _707) / ((((_707 * 0.9599999785423279f) + 0.5600000023841858f) * _707) + 0.14000000059604645f));
    _749 = ((((_711 * 1.3600000143051147f) + 0.04699999839067459f) * _711) / ((((_711 * 0.9599999785423279f) + 0.5600000023841858f) * _711) + 0.14000000059604645f));
    _750 = ((((_715 * 1.3600000143051147f) + 0.04699999839067459f) * _715) / ((((_715 * 0.9599999785423279f) + 0.5600000023841858f) * _715) + 0.14000000059604645f));
  } else {
    _748 = _707;
    _749 = _711;
    _750 = _715;
  ...
  [branch]
  if (WUWA_TM_IS(2)) {
    float _760 = 1.0049500465393066f - (0.16398000717163086f / (_748 + -0.19505000114440918f));
    float _761 = 1.0049500465393066f - (0.16398000717163086f / (_749 + -0.19505000114440918f));
    float _762 = 1.0049500465393066f - (0.16398000717163086f / (_750 + -0.19505000114440918f));
    _782 = (((_748 - _760) * select((_748 > 0.6000000238418579f), 0.0f, 1.0f)) + _760);
    _783 = (((_749 - _761) * select((_749 > 0.6000000238418579f), 0.0f, 1.0f)) + _761);
    _784 = (((_750 - _762) * select((_750 > 0.6000000238418579f), 0.0f, 1.0f)) + _762);
  } else {
    _782 = _748;
    _783 = _749;
    _784 = _750;
  ...
  [branch]
  if (WUWA_TM_IS(3)) {
    float _790 = cb0_037y * _782;
    float _791 = cb0_037y * _783;
    float _792 = cb0_037y * _784;
    float _795 = cb0_037z * cb0_037w;
    float _805 = cb0_038y * cb0_038x;
    float _816 = cb0_038z * cb0_038x;
    float _823 = cb0_038y / cb0_038z;
    _831 = (((((_795 + _790) * _782) + _805) / (((_790 + cb0_037z) * _782) + _816)) - _823);
    _832 = (((((_795 + _791) * _783) + _805) / (((_791 + cb0_037z) * _783) + _816)) - _823);
    _833 = (((((_795 + _792) * _784) + _805) / (((_792 + cb0_037z) * _784) + _816)) - _823);
  } else {
    _831 = _782;
    _832 = _783;
    _833 = _784;
  ...
  if (!((uint)(cb0_097w) == 0)) {
    if (!(cb0_098x == 1.0f)) {
      float _842 = dot(float3(_831, _832, _833), float3(0.2125999927520752f, 0.7152000069618225f, 0.0722000002861023f));
      if (!(_842 <= 9.999999747378752e-05f)) {
        float _851 = (pow(_831, 0.1593017578125f));
        float _852 = (pow(_832, 0.1593017578125f));
        float _853 = (pow(_833, 0.1593017578125f));
        float _875 = exp2(log2(((_851 * 18.8515625f) + 0.8359375f) / ((_851 * 18.6875f) + 1.0f)) * 78.84375f);
        float _876 = exp2(log2(((_852 * 18.8515625f) + 0.8359375f) / ((_852 * 18.6875f) + 1.0f)) * 78.84375f);
        float _877 = exp2(log2(((_853 * 18.8515625f) + 0.8359375f) / ((_853 * 18.6875f) + 1.0f)) * 78.84375f);
        if ((_842 * 200.0f) > 1.0f) {
          float _881 = 1.0f - cb0_098x;
          do {
            if (_875 > 0.44028136134147644f) {
              float _884 = _875 + -0.44028136134147644f;
              _890 = ((_884 / ((_884 * _881) + 1.0f)) + 0.44028136134147644f);
            } else {
              _890 = _875;
            }
            do {
              if (_876 > 0.44028136134147644f) {
                float _1072 = _876 + -0.44028136134147644f;
                _1078 = ((_1072 / ((_1072 * _881) + 1.0f)) + 0.44028136134147644f);
                if (_877 > 0.44028136134147644f) {
                  float _1081 = _877 + -0.44028136134147644f;
                  _893 = ((_1081 / ((_1081 * _881) + 1.0f)) + 0.44028136134147644f);
                  _894 = _1078;
                  _895 = _890;
                } else {
                  _893 = _877;
              ...
              while(true) {
                float _902 = (pow(_895, 0.012683313339948654f));
                float _903 = (pow(_894, 0.012683313339948654f));
                float _904 = (pow(_893, 0.012683313339948654f));
                _930 = exp2(log2(max((_902 + -0.8359375f), 0.0f) / (18.8515625f - (_902 * 18.6875f))) * 6.277394771575928f);
                _931 = exp2(log2(max((_903 + -0.8359375f), 0.0f) / (18.8515625f - (_903 * 18.6875f))) * 6.277394771575928f);
                _932 = exp2(log2(max((_904 + -0.8359375f), 0.0f) / (18.8515625f - (_904 * 18.6875f))) * 6.277394771575928f);
                break;
              }
            } while (false);
          } while (false);
  ...
  CLAMP_IF_SDR3(_930, _931, _932);
  CAPTURE_TONEMAPPED(float3(_930, _931, _932));

  float _953 = (saturate((log2(_930 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _954 = (saturate((log2(_931 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _955 = (saturate((log2(_932 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float4 _956 = t5.Sample(s5, float3(_953, _954, _955));

  HANDLE_LUT_OUTPUT(_956);

  [branch]
  if (false) {
    float _963 = select(((_160 * _141.x) > 0.0f), 0.0f, 1.0f);
    float4 _964 = t6.Sample(s6, float3(_953, _954, _955));
    _978 = (lerp(_956.x, _964.x, _963));
    _979 = (lerp(_956.y, _964.y, _963));
    _980 = (lerp(_956.z, _964.z, _963));
  } else {
    _978 = _956.x;
    _979 = _956.y;
    _980 = _956.z;
  }
  float _981 = _978 * 1.0499999523162842f;
  float _982 = _979 * 1.0499999523162842f;
  float _983 = _980 * 1.0499999523162842f;
  float _991 = ((_335 * 0.00390625f) + -0.001953125f) + _981;
  float _992 = ((_356 * 0.00390625f) + -0.001953125f) + _982;
  float _993 = ((_357 * 0.00390625f) + -0.001953125f) + _983;
  [branch]
  if (!((uint)(cb0_098w) == 0)) {
    float _1005 = (pow(_991, 0.012683313339948654f));
    float _1006 = (pow(_992, 0.012683313339948654f));
    float _1007 = (pow(_993, 0.012683313339948654f));
    float _1040 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1005 + -0.8359375f)) / (18.8515625f - (_1005 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_098z));
    float _1041 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1006 + -0.8359375f)) / (18.8515625f - (_1006 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_098z));
    float _1042 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1007 + -0.8359375f)) / (18.8515625f - (_1007 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_098z));
    _1068 = min((_1040 * 12.920000076293945f), ((exp2(log2(max(_1040, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  ...
  SV_Target.w = (dot(float3(_981, _982, _983), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f)));
