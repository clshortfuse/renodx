...
float4 main(
  ...
  CAPTURE_UNTONEMAPPED(untonemapped, float3(_630, _632, _634));
  ...
  CAPTURE_TONEMAPPED(tonemapped, float3(_750, _751, _752));
  ...
  const float3 inverted = renodx::draw::InvertIntermediatePass(float3(_865, _866, _867));
  _865 = inverted.r; _866 = inverted.g; _867 = inverted.b;

  float _876 = ((((_866 * 587.0f) + (_865 * 299.0f)) + (_867 * 114.0f)) * 0.0010000000474974513f) - cb0_090z;
  float _883 = saturate(float(((int)(uint)((bool)(_876 > 0.0f))) - ((int)(uint)((bool)(_876 < 0.0f)))));
  float _890 = cb0_091x - _865;
  float _891 = cb0_091y - _866;
  float _892 = cb0_091z - _867;

  const float peak_scaling = RENODX_PEAK_NITS / RENODX_GAME_NITS;
  // float _897 = cb0_092x - _865;
  // float _898 = cb0_092y - _866;
  // float _899 = cb0_092z - _867;
  float _897 = peak_scaling * cb0_092x - _865;
  float _898 = peak_scaling * cb0_092y - _866;
  float _899 = peak_scaling * cb0_092z - _867;
  ...
  SV_Target.x = ((cb0_090x * (lerp(_916, _919, _883))) + _865);
  SV_Target.y = ((cb0_090x * (lerp(_917, _920, _883))) + _866);
  SV_Target.z = (((lerp(_918, _921, _883)) * cb0_090x) + _867);
