...
float4 main(
  ...
  float4 _418 = t0.Sample(s0, float2(_416, _417));
  float _424 = saturate(cb0_098x);
  float _426 = saturate(cb0_098y);
  if (_424 > 0.0f) {
    float _437 = saturate(((_418.x * 0.29899999499320984f) + (_418.y * 0.5870000123977661f)) + (_418.z * 0.11400000005960464f));
    float _438 = _437 * 2.0f;
    float _442 = ((_437 * saturate(1.0f - cb0_098z)) * _438) * (3.0f - _438);
    float _449 = max(sqrt(((_418.x * _418.x) + (_418.y * _418.y)) + (_418.z * _418.z)), 0.014999999664723873f);
    float _450 = _418.x / _449;
    float _451 = _418.y / _449;
    float _452 = _418.z / _449;
    float _462 = log2(_442);
    float _481 = ((_298 * _218) * _335) * _424;
    _492 = (((((lerp(_450, 1.0f, 0.25f)) * (((_426 * 0.17000000178813934f) + 0.029999999329447746f) + _442)) - _418.x) * _481) + _418.x);
    _493 = (((((((_426 * 0.18000000715255737f) + 0.05000000074505806f) + exp2(_462 * 0.8999999761581421f)) * (lerp(_451, 1.0f, 0.25f))) - _418.y) * _481) + _418.y);
    _494 = (((((((_426 * 0.17999999225139618f) + 0.07999999821186066f) + exp2(_462 * 0.949999988079071f)) * (lerp(_452, 1.0f, 0.25f))) - _418.z) * _481) + _418.z);
  } else {
    _492 = _418.x;
    _493 = _418.y;
    _494 = _418.z;
  }
  float _496 = saturate(cb0_099x);
  float _497 = _496 * _389;
  if (_496 > 0.0f) {
    float _508 = (sin(((_120 * 640.0f) + 640.0f) - (View_2376 * 10.0f)) * 0.5f) + 1.0f;
    _522 = ((((_508 * _492) - _492) * _497) + _492);
    _523 = ((((_508 * _493) - _493) * _497) + _493);
    _524 = ((((_508 * _494) - _494) * _497) + _494);
  } else {
    _522 = _492;
    _523 = _493;
    _524 = _494;
  }
  float _533 = log2(max(dot(float3(_522, _523, _524), float3(cb0_043x, cb0_043y, cb0_043z)), cb0_042x));
  float4 _551 = t6.Sample(s6, float3((cb0_046z * TEXCOORD_4.x), (cb0_046w * TEXCOORD_4.y), ((((cb0_041z * _533) + cb0_041w) * 0.96875f) + 0.015625f)));
  float4 _555 = t7.Sample(s7, float2(TEXCOORD_4.x, TEXCOORD_4.y));
  float _558 = select((_551.y < 0.0010000000474974513f), _555.x, (_551.x / _551.y));
  float _561 = log2(TEXCOORD_1.x);
  float _563 = (_558 + _561) + ((_555.x - _558) * cb0_046x);
  float _568 = _561 + _533;
  float _570 = _563 - log2((TEXCOORD_1.y * 0.18000000715255737f) * cb0_046y);
  bool _571 = (_570 > 0.0f);
  if (_571) {
    _583 = max(0.0f, (_570 - cb0_047x));
  } else {
    _583 = min(0.0f, (_570 + cb0_047y));
  }
  float4 _615 = t1.Sample(s1, float2(min(max(((cb0_068z * _416) + cb0_069x), cb0_060z), cb0_061x), min(max(((cb0_068w * _417) + cb0_069y), cb0_060w), cb0_061y)));
  APPLY_BLOOM(_615);
  [branch]
  if (!(cb0_085z == 0)) {
    bool _643 = (cb0_085w != 0);
    float4 _646 = t2.Sample(s2, float2(select(_643, _416, min(max(((cb0_076z * _416) + cb0_077x), cb0_075z), cb0_076x)), select(_643, _417, min(max(((cb0_076w * _417) + cb0_077y), cb0_075w), cb0_076y))));
    _654 = (_646.x + _615.x);
    _655 = (_646.y + _615.y);
    _656 = (_646.z + _615.z);
  } else {
  ...
  float _657 = exp2((((_563 - _568) + ((_568 - _563) * cb0_045w)) - _583) + (_583 * select(_571, cb0_045y, cb0_045z))) * TEXCOORD_1.x;
  float _682 = TEXCOORD_1.z + -1.0f;
  float _684 = TEXCOORD_1.w + -1.0f;
  float _687 = (((cb0_090x * 2.0f) + _682) * cb0_088z) * cb0_088x;
  float _689 = (((cb0_090y * 2.0f) + _684) * cb0_088w) * cb0_088x;
  float _696 = 1.0f / ((((saturate(cb0_089w) * 9.0f) + 1.0f) * dot(float2(_687, _689), float2(_687, _689))) + 1.0f);
  float _697 = _696 * _696;
  float _698 = cb0_090z + 1.0f;
  float _726 = (((cb0_093x * 2.0f) + _682) * cb0_091z) * cb0_091x;
  float _728 = (((cb0_093y * 2.0f) + _684) * cb0_091w) * cb0_091x;
  float _735 = 1.0f / ((((saturate(cb0_092w) * 9.0f) + 1.0f) * dot(float2(_726, _728), float2(_726, _728))) + 1.0f);
  float _736 = _735 * _735;
  float _737 = cb0_093z + 1.0f;
  float _749 = (((_654 + ((_657 * _522) * cb0_086x)) * _386) * ((_697 * (_698 - cb0_089x)) + cb0_089x)) * ((_736 * (_737 - cb0_092x)) + cb0_092x);
  float _752 = (((_655 + ((_657 * _523) * cb0_086y)) * _386) * ((_697 * (_698 - cb0_089y)) + cb0_089y)) * ((_736 * (_737 - cb0_092y)) + cb0_092y);
  float _755 = (((_656 + ((_657 * _524) * cb0_086z)) * _386) * ((_697 * (_698 - cb0_089z)) + cb0_089z)) * ((_736 * (_737 - cb0_092z)) + cb0_092z);
  [branch]
  if (!(cb0_111w == 0)) {
    float _769 = ((((cb0_111z + 1.0f) * 0.009900989942252636f) * (cb0_111x - cb0_111y)) + cb0_111y) * -1.4426950216293335f;
    _780 = (1.0f - exp2(_769 * _749));
    _781 = (1.0f - exp2(_769 * _752));
    _782 = (1.0f - exp2(_769 * _755));
  } else {
    _780 = _749;
    _781 = _752;
    _782 = _755;
  ...
  CAPTURE_UNTONEMAPPED(float3(_780, _781, _782));
  [branch]
  if (WUWA_TM_IS(1)) {
    _815 = ((((_780 * 1.3600000143051147f) + 0.04699999839067459f) * _780) / ((((_780 * 0.9599999785423279f) + 0.5600000023841858f) * _780) + 0.14000000059604645f));
    _816 = ((((_781 * 1.3600000143051147f) + 0.04699999839067459f) * _781) / ((((_781 * 0.9599999785423279f) + 0.5600000023841858f) * _781) + 0.14000000059604645f));
    _817 = ((((_782 * 1.3600000143051147f) + 0.04699999839067459f) * _782) / ((((_782 * 0.9599999785423279f) + 0.5600000023841858f) * _782) + 0.14000000059604645f));
  } else {
    _815 = _780;
    _816 = _781;
    _817 = _782;
  ...
  [branch]
  if (WUWA_TM_IS(2)) {
    float _827 = 1.0049500465393066f - (0.16398000717163086f / (_815 + -0.19505000114440918f));
    float _828 = 1.0049500465393066f - (0.16398000717163086f / (_816 + -0.19505000114440918f));
    float _829 = 1.0049500465393066f - (0.16398000717163086f / (_817 + -0.19505000114440918f));
    _849 = (((_815 - _827) * select((_815 > 0.6000000238418579f), 0.0f, 1.0f)) + _827);
    _850 = (((_816 - _828) * select((_816 > 0.6000000238418579f), 0.0f, 1.0f)) + _828);
    _851 = (((_817 - _829) * select((_817 > 0.6000000238418579f), 0.0f, 1.0f)) + _829);
  } else {
    _849 = _815;
    _850 = _816;
    _851 = _817;
  ...
  [branch]
  if (WUWA_TM_IS(3)) {
    float _857 = cb0_037y * _849;
    float _858 = cb0_037y * _850;
    float _859 = cb0_037y * _851;
    float _862 = cb0_037z * cb0_037w;
    float _872 = cb0_038y * cb0_038x;
    float _883 = cb0_038z * cb0_038x;
    float _890 = cb0_038y / cb0_038z;
    _898 = (((((_862 + _857) * _849) + _872) / (((_857 + cb0_037z) * _849) + _883)) - _890);
    _899 = (((((_862 + _858) * _850) + _872) / (((_858 + cb0_037z) * _850) + _883)) - _890);
    _900 = (((((_862 + _859) * _851) + _872) / (((_859 + cb0_037z) * _851) + _883)) - _890);
  } else {
    _898 = _849;
    _899 = _850;
    _900 = _851;
  ...
  if (!(cb0_105w == 0)) {
    if (!(cb0_106x == 1.0f)) {
      float _910 = (cb0_106x * 0.699999988079071f) + 0.30000001192092896f;
      _915 = (_910 * _898);
      _916 = (_910 * _899);
      _917 = (_910 * _900);
    } else {
  ...
  CLAMP_IF_SDR3(_915, _916, _917);
  CAPTURE_TONEMAPPED(float3(_915, _916, _917));
  float _932 = (saturate((log2(_917 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _936 = (saturate((log2(_916 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _940 = (saturate((log2(_915 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  [branch]
  if (!(cb0_107w == 0)) {
    float4 _961 = t5.Sample(s5, float2(min(max(((cb0_084z * _416) + cb0_085x), cb0_083z), cb0_084x), min(max(((cb0_084w * _417) + cb0_085y), cb0_083w), cb0_084y)));
    float _971 = select((((uint)(uint(float((uint)((int)((uint)(uint(round(_961.w * 255.0f))) & 15))))) & -4) == 12), 1.0f, 0.0f);
    float4 _972 = t9.Sample(s9, float3(_940, _936, _932));
    float4 _979 = t8.Sample(s8, float3(_940, _936, _932));
    _1004 = ((((_979.x - _972.x) * 1.0499999523162842f) * _971) + (_972.x * 1.0499999523162842f));
    _1005 = ((((_979.y - _972.y) * 1.0499999523162842f) * _971) + (_972.y * 1.0499999523162842f));
    _1006 = ((((_979.z - _972.z) * 1.0499999523162842f) * _971) + (_972.z * 1.0499999523162842f));
  } else {
    float4 _996 = t8.Sample(s8, float3(_940, _936, _932));
    _1004 = (_996.x * 1.0499999523162842f);
    _1005 = (_996.y * 1.0499999523162842f);
    _1006 = (_996.z * 1.0499999523162842f);
  }
  HANDLE_LUT_OUTPUT3_FADE(_1004, _1005, _1006, t8, s8);
  float _1014 = ((_342 * 0.00390625f) + -0.001953125f) + _1004;
  float _1015 = ((_363 * 0.00390625f) + -0.001953125f) + _1005;
  float _1016 = ((_364 * 0.00390625f) + -0.001953125f) + _1006;
  [branch]
  if (!(cb0_106w == 0)) {
    float _1028 = (pow(_1014, 0.012683313339948654f));
    float _1029 = (pow(_1015, 0.012683313339948654f));
    float _1030 = (pow(_1016, 0.012683313339948654f));
    float _1063 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1028 + -0.8359375f)) / (18.8515625f - (_1028 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_106z));
    float _1064 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1029 + -0.8359375f)) / (18.8515625f - (_1029 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_106z));
    float _1065 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_1030 + -0.8359375f)) / (18.8515625f - (_1030 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_106z));
    _1091 = min((_1063 * 12.920000076293945f), ((exp2(log2(max(_1063, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _1092 = min((_1064 * 12.920000076293945f), ((exp2(log2(max(_1064, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  ...
  SV_Target.y = _1092;
  SV_Target.z = _1093;
  SV_Target.w = dot(float3(_1004, _1005, _1006), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
