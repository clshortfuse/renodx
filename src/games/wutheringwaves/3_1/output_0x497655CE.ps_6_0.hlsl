...
float4 main(
  ...
  float4 _42 = t5.SampleLevel(s5, float2(_40, _41), 0.0f);
  float _54 = ((1.0f / ((View_1040.z * _42.x) - View_1040.w)) + View_1040.y) + (View_1040.x * _42.x);
  float4 _55 = t4.Sample(s4, float2(_40, _41));
  float _63 = 1.0f / ((_54 + View_1040.w) * View_1040.z);
  float _76 = (TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z;
  float _79 = frac(sin(_76) * 493013.0f);
  float _101;
  float _102;
  float _161;
  float _162;
  ...
  float _746;
  if (cb0_096x > 0.0f) {
    _101 = (((frac((sin(_76 + 33.9900016784668f) * 493013.0f) + 7.177000045776367f) - _79) * cb0_096x) + _79);
    _102 = (((frac((sin(_76 + 66.98999786376953f) * 493013.0f) + 14.298999786376953f) - _79) * cb0_096x) + _79);
  } else {
    _101 = _79;
    _102 = _79;
  }
  ...
  float _144 = float((int)(((int)(uint)((bool)(_124 > 0.0f))) - ((int)(uint)((bool)(_124 < 0.0f))))) * saturate(abs(_124) - cb0_117z);
  float _146 = float((int)(((int)(uint)((bool)(_125 > 0.0f))) - ((int)(uint)((bool)(_125 < 0.0f))))) * saturate(abs(_125) - cb0_117z);
  float _151 = _125 - (_146 * _112);
  float _153 = _125 - (_146 * _113);
  bool _154 = (cb0_118x > 0.0f);
  if (_154) {
    _161 = (_151 - (cb0_118w * 0.4000000059604645f));
    _162 = (_153 - (cb0_118w * 0.20000000298023224f));
  } else {
    _161 = _151;
    _162 = _153;
  }
  float4 _196 = t0.Sample(s0, float2(_40, _41));
  float4 _200 = t0.Sample(s0, float2((((((cb0_115z * (_124 - (_144 * select(_114, _112, cb0_117x)))) + cb0_115x) * cb0_048z) + cb0_049x) * cb0_048x), (((((cb0_115w * _161) + cb0_115y) * cb0_048w) + cb0_049y) * cb0_048y)));
  float4 _202 = t0.Sample(s0, float2((((((cb0_115z * (_124 - (_144 * select(_114, _113, cb0_117y)))) + cb0_115x) * cb0_048z) + cb0_049x) * cb0_048x), (((((cb0_115w * _162) + cb0_115y) * cb0_048w) + cb0_049y) * cb0_048y)));
  if (_154) {
    float _212 = saturate(((((_196.y * 0.5870000123977661f) - cb0_118y) + (_196.x * 0.29899999499320984f)) + (_196.z * 0.11400000005960464f)) * 10.0f);
    float _216 = (_212 * _212) * (3.0f - (_212 * 2.0f));
    _230 = ((((_196.x - _200.x) + (_216 * (_200.x - _196.x))) * cb0_118x) + _200.x);
    _231 = ((((_196.y - _202.y) + (_216 * (_202.y - _196.y))) * cb0_118x) + _202.y);
  } else {
    _230 = _200.x;
    _231 = _202.y;
  }
  float4 _256 = t1.Sample(s1, float2(min(max(((cb0_068z * _40) + cb0_069x), cb0_060z), cb0_061x), min(max(((cb0_068w * _41) + cb0_069y), cb0_060w), cb0_061y)));
  APPLY_BLOOM(_256);

  [branch]
  if (!(cb0_085z == 0)) {
    bool _284 = (cb0_085w != 0);
    float4 _287 = t2.Sample(s2, float2(select(_284, _40, min(max(((cb0_076z * _40) + cb0_077x), cb0_075z), cb0_076x)), select(_284, _41, min(max(((cb0_076w * _41) + cb0_077y), cb0_075w), cb0_076y))));
    _295 = (_287.x + _256.x);
    _296 = (_287.y + _256.y);
    _297 = (_287.z + _256.z);
  } else {
  ...
  float _322 = TEXCOORD_1.z + -1.0f;
  float _324 = TEXCOORD_1.w + -1.0f;
  float _327 = (((cb0_090x * 2.0f) + _322) * cb0_088z) * cb0_088x;
  float _329 = (((cb0_090y * 2.0f) + _324) * cb0_088w) * cb0_088x;
  float _336 = 1.0f / ((((saturate(cb0_089w) * 9.0f) + 1.0f) * dot(float2(_327, _329), float2(_327, _329))) + 1.0f);
  float _337 = _336 * _336;
  float _338 = cb0_090z + 1.0f;
  float _366 = (((cb0_093x * 2.0f) + _322) * cb0_091z) * cb0_091x;
  float _368 = (((cb0_093y * 2.0f) + _324) * cb0_091w) * cb0_091x;
  float _375 = 1.0f / ((((saturate(cb0_092w) * 9.0f) + 1.0f) * dot(float2(_366, _368), float2(_366, _368))) + 1.0f);
  float _376 = _375 * _375;
  float _377 = cb0_093z + 1.0f;
  float _388 = (((_337 * (_338 - cb0_089x)) + cb0_089x) * (_295 + ((_230 * TEXCOORD_1.x) * cb0_086x))) * ((_376 * (_377 - cb0_092x)) + cb0_092x);
  float _390 = (((_337 * (_338 - cb0_089y)) + cb0_089y) * (_296 + ((_231 * TEXCOORD_1.x) * cb0_086y))) * ((_376 * (_377 - cb0_092y)) + cb0_092y);
  float _392 = (((_337 * (_338 - cb0_089z)) + cb0_089z) * (_297 + ((_196.z * TEXCOORD_1.x) * cb0_086z))) * ((_376 * (_377 - cb0_092z)) + cb0_092z);
  CAPTURE_UNTONEMAPPED(float3(_388, _390, _392));

  [branch]
  if (!(cb0_111w == 0)) {
    float _406 = ((((cb0_111z + 1.0f) * 0.009900989942252636f) * (cb0_111x - cb0_111y)) + cb0_111y) * -1.4426950216293335f;
    _417 = (1.0f - exp2(_406 * _388));
    _418 = (1.0f - exp2(_406 * _390));
    _419 = (1.0f - exp2(_406 * _392));
  } else {
    _417 = _388;
    _418 = _390;
    _419 = _392;
  ...
  [branch]
  if (WUWA_TM_IS(1)) {
    _452 = ((((_417 * 1.3600000143051147f) + 0.04699999839067459f) * _417) / ((((_417 * 0.9599999785423279f) + 0.5600000023841858f) * _417) + 0.14000000059604645f));
    _453 = ((((_418 * 1.3600000143051147f) + 0.04699999839067459f) * _418) / ((((_418 * 0.9599999785423279f) + 0.5600000023841858f) * _418) + 0.14000000059604645f));
    _454 = ((((_419 * 1.3600000143051147f) + 0.04699999839067459f) * _419) / ((((_419 * 0.9599999785423279f) + 0.5600000023841858f) * _419) + 0.14000000059604645f));
  } else {
    _452 = _417;
    _453 = _418;
    _454 = _419;
  ...
  [branch]
  if (WUWA_TM_IS(2)) {
    float _464 = 1.0049500465393066f - (0.16398000717163086f / (_452 + -0.19505000114440918f));
    float _465 = 1.0049500465393066f - (0.16398000717163086f / (_453 + -0.19505000114440918f));
    float _466 = 1.0049500465393066f - (0.16398000717163086f / (_454 + -0.19505000114440918f));
    _486 = (((_452 - _464) * select((_452 > 0.6000000238418579f), 0.0f, 1.0f)) + _464);
    _487 = (((_453 - _465) * select((_453 > 0.6000000238418579f), 0.0f, 1.0f)) + _465);
  ...
  [branch]
  if (WUWA_TM_IS(3)) {
    float _494 = cb0_037y * _486;
    float _495 = cb0_037y * _487;
    float _496 = cb0_037y * _488;
    float _499 = cb0_037z * cb0_037w;
    float _509 = cb0_038y * cb0_038x;
    float _520 = cb0_038z * cb0_038x;
    float _527 = cb0_038y / cb0_038z;
    _535 = (((((_499 + _494) * _486) + _509) / (((_494 + cb0_037z) * _486) + _520)) - _527);
    _536 = (((((_499 + _495) * _487) + _509) / (((_495 + cb0_037z) * _487) + _520)) - _527);
    _537 = (((((_499 + _496) * _488) + _509) / (((_496 + cb0_037z) * _488) + _520)) - _527);
  } else {
    _535 = _486;
    _536 = _487;
    _537 = _488;
  ...
  if (!(cb0_105w == 0)) {
    if (!(cb0_106x == 1.0f)) {
      float _547 = (cb0_106x * 0.699999988079071f) + 0.30000001192092896f;
      _552 = (_547 * _535);
      _553 = (_547 * _536);
      _554 = (_547 * _537);
    } else {
  ...
  CLAMP_IF_SDR3(_552, _553, _554);
  CAPTURE_TONEMAPPED(float3(_552, _553, _554));

  float _569 = (saturate((log2(_554 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _573 = (saturate((log2(_553 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  float _577 = (saturate((log2(_552 + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f;
  [branch]
  if (!(cb0_107w == 0)) {
    float4 _598 = t6.Sample(s6, float2(min(max(((cb0_084z * _40) + cb0_085x), cb0_083z), cb0_084x), min(max(((cb0_084w * _41) + cb0_085y), cb0_083w), cb0_084y)));
    float _608 = select((((uint)(uint(float((uint)((int)((uint)(uint(round(_598.w * 255.0f))) & 15))))) & -4) == 12), 1.0f, 0.0f);
    float4 _609 = t8.Sample(s8, float3(_577, _573, _569));
    float4 _616 = t7.Sample(s7, float3(_577, _573, _569));
    _657 = ((((_616.x - _609.x) * 1.0499999523162842f) * _608) + (_609.x * 1.0499999523162842f));
    _658 = ((((_616.y - _609.y) * 1.0499999523162842f) * _608) + (_609.y * 1.0499999523162842f));
    _659 = ((((_616.z - _609.z) * 1.0499999523162842f) * _608) + (_609.z * 1.0499999523162842f));
  } else {
    float _635 = select(((select((((_63 - _55.x) + ((((View_448[3].z) - _63) + ((View_448[2].z) * _54)) * select(((View_448[3].w) < 1.0f), 0.0f, 1.0f))) > 0.0005000000237487257f), 0.0f, 1.0f) * _55.x) > 0.0f), 0.0f, 1.0f);
    float4 _636 = t7.Sample(s7, float3(_577, _573, _569));
    float4 _640 = t3.Sample(s3, float3(_577, _573, _569));
    _657 = ((lerp(_640.x, _636.x, _635)) * 1.0499999523162842f);
    _658 = ((lerp(_640.y, _636.y, _635)) * 1.0499999523162842f);
    _659 = ((lerp(_640.z, _636.z, _635)) * 1.0499999523162842f);
  }
  HANDLE_LUT_OUTPUT3_FADE(_657, _658, _659, t7, s7);

  float _667 = ((_79 * 0.00390625f) + -0.001953125f) + _657;
  float _668 = ((_101 * 0.00390625f) + -0.001953125f) + _658;
  float _669 = ((_102 * 0.00390625f) + -0.001953125f) + _659;
  [branch]
  if (!(cb0_106w == 0)) {
    ...
    float _718 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_683 + -0.8359375f)) / (18.8515625f - (_683 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_106z));
