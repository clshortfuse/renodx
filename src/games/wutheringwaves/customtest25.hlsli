// Local vendored copy of a friend's custom PsychoV25, kept verbatim so it stays
// diffable against their original. The only local adaptation is the include
// below (Elite Dangerous layout used "../../common.hlsli"). The guard does NOT
// protect against the shared library shipping its own test25: that file would
// carry a different guard, so both would be included — the psycho25_ symbol
// prefixes are what keep them apart for now. Do not rename symbols casually.
