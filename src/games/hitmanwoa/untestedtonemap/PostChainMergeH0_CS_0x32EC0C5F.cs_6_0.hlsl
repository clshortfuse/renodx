...
void comp_main() {
  ...
  float3 tonemapped = ((((CUSTOM_BLOOM * _26_m0[9u].x * _110.xyz) + _104.xyz) * _26_m0[7u].xyz) * _26_m0[10u].y * _138);

  // add vignette and grain
  tonemapped = (_26_m0[4u].xyz * _78) + (((((_26_m0[5u].xyz - 0.5f) * _77) + 0.5f) * 2.0f) * tonemapped * _168);
...
#else
  // float4 _207 = _12.SampleLevel(_29, float3((sqrt(clamp((_26_m0[4u].x * _78) + ((((((_26_m0[5u].x + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp(((((_26_m0[9u].x * _110.x) + _104.x) * _26_m0[7u].x) * _26_m0[10u].y) * _138, 0.0f, 1.0f)) * _168), 0.0f, 1.0f)) * 0.9375f) + 0.03125f, (sqrt(clamp((_26_m0[4u].y * _78) + ((((((_26_m0[5u].y + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp(((((_26_m0[9u].x * _110.y) + _104.y) * _26_m0[7u].y) * _26_m0[10u].y) * _138, 0.0f, 1.0f)) * _168), 0.0f, 1.0f)) * 0.9375f) + 0.03125f, (sqrt(clamp((_26_m0[4u].z * _78) + ((((((_26_m0[5u].z + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp(((((_26_m0[9u].x * _110.z) + _104.z) * _26_m0[7u].z) * _26_m0[10u].y) * _138, 0.0f, 1.0f)) * _168), 0.0f, 1.0f)) * 0.9375f) + 0.03125f), 0.0f);
  float _223 = (frac(sin(dot(float2(_57, _58), float2(12.98980045318603515625f, 78.233001708984375f))) * 43758.546875f) * 0.0039215688593685626983642578125f) + (-0.00196078442968428134918212890625f);
  _20[uint2(gl_GlobalInvocationID.x, gl_GlobalInvocationID.y)] = float4((_223 + _207.x) * _26_m0[11u].x, (_223 + _207.y) * _26_m0[11u].x, (_223 + _207.z) * _26_m0[11u].x, 0.0f);
#endif
}
