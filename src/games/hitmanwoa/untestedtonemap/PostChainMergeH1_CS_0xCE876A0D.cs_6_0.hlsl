...
#if 1
  float3 combined_color = float3(_139, _140, _141);
...
#else
  float4 _213 = _12.SampleLevel(_29, float3((sqrt(clamp((_26_m0[4u].x * _78) + ((((((_26_m0[5u].x + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp((_139 / (_139 + 1.0f)) * _26_m0[10u].y, 0.0f, 1.0f)) * _174), 0.0f, 1.0f)) * 0.9375f) + 0.03125f, (sqrt(clamp((_26_m0[4u].y * _78) + ((((((_26_m0[5u].y + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp((_140 / (_140 + 1.0f)) * _26_m0[10u].y, 0.0f, 1.0f)) * _174), 0.0f, 1.0f)) * 0.9375f) + 0.03125f, (sqrt(clamp((_26_m0[4u].z * _78) + ((((((_26_m0[5u].z + (-0.5f)) * _77) + 0.5f) * 2.0f) * clamp((_141 / (_141 + 1.0f)) * _26_m0[10u].y, 0.0f, 1.0f)) * _174), 0.0f, 1.0f)) * 0.9375f) + 0.03125f), 0.0f);
  float _229 = (frac(sin(dot(float2(_57, _58), float2(12.98980045318603515625f, 78.233001708984375f))) * 43758.546875f) * 0.0039215688593685626983642578125f) + (-0.00196078442968428134918212890625f);
  _20[uint2(gl_GlobalInvocationID.x, gl_GlobalInvocationID.y)] = float4((_229 + _213.x) * _26_m0[11u].x, (_229 + _213.y) * _26_m0[11u].x, (_229 + _213.z) * _26_m0[11u].x, 0.0f);
#endif
}
