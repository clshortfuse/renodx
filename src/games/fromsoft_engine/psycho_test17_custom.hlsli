...
#ifndef RENODX_SHADERS_TONEMAP_PSYCHO_TEST17_HLSLI_
...
float3 psycho17_RestoreHueAdaptive(
  ...
  float hue_sensitivity = psycho17_AdaptiveHueSensitivityFromTClip(target_t_clip);
...
Config Create(
    ...
    bool apply_maxch_displaymap = true,
    float peak_value = 1000.f / 203.f,
    float exposure = 1.f,
    float gamma = 1.f,
    float highlights = 1.f,
...
float3 ApplyTest17BT2020(float3 color_bt2020, float3 color_hue_shift_source_bt2020, config17::Config psycho_config) {
  ...
  if (psycho_config.apply_maxch_displaymap) {
