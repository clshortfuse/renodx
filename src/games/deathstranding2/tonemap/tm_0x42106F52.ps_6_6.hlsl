...
void frag_main() {
  ...
  uint4 _169 = asuint(_40_m0[21u]);
  uint _170 = _169.y;
  float _202 = (((_40_m0[6u].z * ((_40_m0[0u].z + (-0.5f)) + (_40_m0[0u].x * TEXCOORD.x))) + 0.5f) * 2.0f) + (-1.0f);
  float _204 = (((_40_m0[6u].z * ((_40_m0[0u].w + (-0.5f)) + (_40_m0[0u].y * TEXCOORD.y))) + 0.5f) * 2.0f) + (-1.0f);
  float _205 = _202 * _40_m0[8u].x;
  float _206 = _204 * _40_m0[8u].y;
  float _207 = dot(float2(_205, _206), float2(_205, _206));
  float _210 = _207 * _207;
  float _211 = _40_m0[6u].x * 2.0f;
  float _212 = _40_m0[6u].y * 4.0f;
  float _219 = _40_m0[6u].x * 3.0f;
  float _221 = _40_m0[6u].y * 5.0f;
  float _237 = (dot(float2(_207, _210), float2(_211, _212)) + 1.0f) / (((dot(1.0f.xx, float2(_211, _212)) + 1.0f) / (dot(1.0f.xx, float2(_219, _221)) + 1.0f)) * (dot(float2(_207, _210), float2(_219, _221)) + 1.0f));
  float _244 = log2(abs((_40_m0[6u].w != 0.0f) ? _202 : _204));
  float _255 = ((((_40_m0[7u].x * 0.5f) * (1.0f - _237)) * (exp2(_244 * _40_m0[7u].z) + exp2(_244 * _40_m0[7u].y))) + _237) * 0.5f;
  float _258 = (_255 * _202) + 0.5f;
  float _259 = (_255 * _204) + 0.5f;
  uint _262_dummy_parameter;
  uint2 _262 = spvTextureSize(_18, 0u, _262_dummy_parameter);
  float _269 = (1.0f / _40_m0[0u].x) * _258;
  float _270 = _269 * _40_m0[21u].z;
  float _271 = (1.0f / _40_m0[0u].y) * _259;
  ...
  float _287 = _284.y;
  float _288 = _284.z;
  float _291 = float(int(_142.x));
  float _292 = float(int(_142.y));
  float _298 = float(int(uint(int(uint(clamp(_273, 0.0f, 1.0f) * float(int(_262.x)))) >> int(_64.z & 31u))));
  float _299 = float(int(uint(int(uint(clamp(_274, 0.0f, 1.0f) * float(int(_262.y)))) >> int(_64.w & 31u))));
  uint4 _311 = _12.Load(int3(uint2(uint(int(max(min(_298, _291), min(max(_298, _291), float(int(_142.z)))))), uint(int(max(min(_299, _292), min(max(_299, _292), float(int(_142.w))))))), 0u));
  uint _313 = _311.x;
  float _315;
  float _320;
  float _325;
  if (_313 == 0u) {
    ...
    if ((_313 & 536870912u) == 0u) {
      ...
      if ((_313 & 268435456u) == 0u) {
        ...
        float _775 = _774 * 24.0f;
        float _940;
        float _941;
        float _942;
        if ((_534 < 1.0f) && ((_529 < 1.0f) && (_775 > 0.25f))) {
        ...
        if ((_958.z > 0.0f) || ((_958.x > 0.0f) || (_958.y > 0.0f))) {
          ...
          frontier_phi_1_2_ladder_5_ladder_23_ladder = _329;
          frontier_phi_1_2_ladder_5_ladder_23_ladder_1 = _324;
          frontier_phi_1_2_ladder_5_ladder_23_ladder_2 = _319;
        }
        frontier_phi_1_2_ladder_5_ladder = frontier_phi_1_2_ladder_5_ladder_23_ladder;
    ...
    _315 = frontier_phi_1_2_ladder_2;
    _320 = frontier_phi_1_2_ladder_1;
    _325 = frontier_phi_1_2_ladder;
  }
  float _332 = (_258 - _40_m0[0u].z) / _40_m0[0u].x;
  ...
  float _358 = _325 * _354;
  float _371 = (clamp(dot(float3(_356, _357, _358), float3(0.2125999927520751953125f, 0.715200006961822509765625f, 0.072200000286102294921875f)) + 9.9999999747524270787835121154785e-07f, 0.0f, 1.0f) * (1.0f - _40_m0[17u].w)) + _40_m0[17u].w;
  float _387 = (exp2(log2(max(_356, 0.0f)) * _371) / _354) * _40_m0[17u].w;
  float _388 = (exp2(log2(max(_357, 0.0f)) * _371) / _354) * _40_m0[17u].w;
  float _389 = (exp2(log2(max(_358, 0.0f)) * _371) / _354) * _40_m0[17u].w;
  float _390 = (_341 * _341) / _40_m0[19u].z;
  float _391 = (_342 * _342) / _40_m0[19u].z;
  float _392 = (_343 * _343) / _40_m0[19u].z;
  float _395 = (_336 * 4.0f) / (_336 + 0.25f);
  float _399 = max(_336, 1.0000000031710768509710513471353e-30f);
  float _412 = 1.0f / ((_336 + 1.0f) + _395);
  float _426 = (((_391 + _388) + (sqrt((_391 * _388) / _399) * _395)) * _412) + (_346.y * _40_m0[19u].x);
  float _432 = ((_426 * (_40_m0[17u].x - _40_m0[17u].y)) + (((_412 * ((_390 + _387) + (sqrt((_390 * _387) / _399) * _395))) + (_346.x * _40_m0[19u].x)) * _40_m0[17u].x)) * _40_m0[19u].z;
  float _434 = (_40_m0[19u].z * _40_m0[17u].y) * _426;
  float _436 = (_40_m0[19u].z * _40_m0[17u].z) * ((((_392 + _389) + (sqrt((_392 * _389) / _399) * _395)) * _412) + (_346.z * _40_m0[19u].x));
  float _442;
  float _444;
  float _446;
  if ((_170 & 1u) == 0u) {
  ...
  if ((_170 & 32u) == 0u) {
    ...
    float _497 = (_258 * 2.0f) + (-1.0f);
    float _498 = (_259 * 2.0f) + (-1.0f);
    ...
    float _512 = (_412 * _40_m0[19u].z) * _507;
    _483 = (_508 * _442) + (_512 * _40_m0[1u].x);
    _485 = (_508 * _444) + (_512 * _40_m0[1u].y);
    _487 = (_508 * _446) + (_512 * _40_m0[1u].z);
  }
  ...
  if ((_170 & 16u) == 0u) {
    ...
    if (asuint(_40_m0[12u]).w == 0u) {
      ...
      float _652 = min(_646 * _646, _40_m0[2u].x);
      ...
      frontier_phi_11_12_ladder = exp2(log2(abs((_746 + (-0.8359375f)) / (18.8515625f - (_746 * 18.6875f)))) * 6.798340320587158203125f) * 100.0f;
      frontier_phi_11_12_ladder_1 = exp2(log2(abs((_745 + (-0.8359375f)) / (18.8515625f - (_745 * 18.6875f)))) * 6.798340320587158203125f) * 100.0f;
      frontier_phi_11_12_ladder_2 = exp2(log2(abs((_744 + (-0.8359375f)) / (18.8515625f - (_744 * 18.6875f)))) * 6.798340320587158203125f) * 100.0f;
    }
    _560 = frontier_phi_11_12_ladder_2;
    _563 = frontier_phi_11_12_ladder_1;
    _566 = frontier_phi_11_12_ladder;
  }
  float _576;
  float _578;
  float _580;
  ...
  float _1043;
  if (_582 == 1u) {
    float _791 = exp2(log2(abs(_576)) * _40_m0[18u].x);
    float _792 = exp2(log2(abs(_578)) * _40_m0[18u].x);
    float _793 = exp2(log2(abs(_580)) * _40_m0[18u].x);
    float _1040;
    if (_791 < 0.00310000008903443813323974609375f) {
      _1040 = _791 * 12.9200000762939453125f;
    } else {
      _1040 = (exp2(log2(abs(_791)) * 0.4166666567325592041015625f) * 1.05499994754791259765625f) + (-0.054999999701976776123046875f);
    }
    float _1042;
    if (_792 < 0.00310000008903443813323974609375f) {
      _1042 = _792 * 12.9200000762939453125f;
    } else {
      _1042 = (exp2(log2(abs(_792)) * 0.4166666567325592041015625f) * 1.05499994754791259765625f) + (-0.054999999701976776123046875f);
    }
    float frontier_phi_27_32_ladder;
    float frontier_phi_27_32_ladder_1;
    float frontier_phi_27_32_ladder_2;
    if (_793 < 0.00310000008903443813323974609375f) {
      frontier_phi_27_32_ladder = _1040;
      frontier_phi_27_32_ladder_1 = _793 * 12.9200000762939453125f;
      frontier_phi_27_32_ladder_2 = _1042;
    } else {
      frontier_phi_27_32_ladder = _1040;
      frontier_phi_27_32_ladder_1 = (exp2(log2(abs(_793)) * 0.4166666567325592041015625f) * 1.05499994754791259765625f) + (-0.054999999701976776123046875f);
      frontier_phi_27_32_ladder_2 = _1042;
    }
    _1039 = frontier_phi_27_32_ladder;
    _1041 = frontier_phi_27_32_ladder_2;
    _1043 = frontier_phi_27_32_ladder_1;
  } else {
    float frontier_phi_27_21_ladder;
    ...
    if (_582 == 2u) {
      ...
      frontier_phi_27_21_ladder_2 = _578;
    }
    _1039 = frontier_phi_27_21_ladder;
    _1041 = frontier_phi_27_21_ladder_2;
    _1043 = frontier_phi_27_21_ladder_1;
  }
  float _1055 = max(((_1039 + (-0.5f)) * _40_m0[5u].x) + 0.5f, 0.0f);
  float _1056 = max(((_1041 + (-0.5f)) * _40_m0[5u].x) + 0.5f, 0.0f);
  float _1057 = max(((_1043 + (-0.5f)) * _40_m0[5u].x) + 0.5f, 0.0f);
  SV_Target.x = _1055;
  SV_Target.y = _1056;
  SV_Target.z = _1057;
  SV_Target.w = 1.0f;
  SV_Target_1 = dot(float3(_1055, _1056, _1057), float3(0.2125999927520751953125f, 0.715200006961822509765625f, 0.072200000286102294921875f));
}
