...
// Screen-space post-process shader extracted via 3Dmigoto.
// Applies optional horizontal mirroring, bloom-like neighborhood blend,
// tone mapping, and dithering before outputting HDR-corrected color.
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  // Optional horizontal mirror of UV.x driven by cb1[1].w flag.
  r0.x = cmp(0.5 < cb1[1].w);
  r0.y = 1 + -v1.x;
  r0.x = r0.x ? r0.y : v1.x;
  r0.y = v1.y;
  // Sample base color from main scene texture.
  r1.xyz = t0.Sample(s1_s, r0.xy, int2(0, 0)).xyz;
  // Determine blend strength using cb1[4] parameters.
  r0.z = max(cb1[4].x, cb1[4].y);
  r0.w = cmp(0 < r0.z);
  if (r0.w != 0) {
    // Build rotated offsets using sin/cos for bloom sample positions.
    ...
    // Blend between two rotated taps to create bloom/glow contribution.
    r1.xyzw = r2.xxxx * r4.xyzw + r3.xyzw;
  } else {
    r1.w = 1;
  }
  // Alpha falloff based on blend radius.
  r0.w = cmp(0 < r0.z);
  ...
  r0.z = r1.w * r0.z;
  o0.w = r0.w ? r0.z : r1.w;
  // Retrieve MSAA sample count for t1 (replacement for sampleinfo_uint).
  t1.GetDimensions(uiDest.x, uiDest.y, uiDest.z);
  r0.z = (float)uiDest.z;
  r0.w = cmp(0 < cb1[7].w);
  r1.w = 0.00999999978 * cb0[44].x;
  // Procedural noise to jitter per-scanline sampling based on uv and constants.
  r2.x = cb1[8].y * v1.y;
  r2.y = ceil(r2.x);
  ...
  r0.w = 0;
  // Accumulate multi-sample texture t1 (likely MSAA buffer) with gamma correction.
  while (true) {
  ...
  // Convert sample count to float reciprocal.
  r0.z = 1 / r0.z;
  r2.xyzw = r3.xyzw * r0.zzzz;
  // Convert scene sample to PQ-like space for HDR tone mapping.
  ...
  r1.w = sqrt(r2.w);
  // Blend HDR neighborhood average with base color using luminance gates.
  ...
  // Apply exposure from cb1[1].z and bring HDR back to linear domain.
  r1.xyz = r0.zzz * r2.xyz;
  r1.xyz = exp2(r1.xyz);
  // Calculate blue-noise style dithering offset using pixel indices.
