...
OutputSignature main(
  ...
  float _51 = float(((int)(uint)((bool)(_39 > 0.0f))) - ((int)(uint)((bool)(_39 < 0.0f))));
  float _52 = float(((int)(uint)((bool)(_40 > 0.0f))) - ((int)(uint)((bool)(_40 < 0.0f))));
  ...
  float _217 = (_27 * View.OneOverPreExposure) * (_205 * _205);
  float4 _259 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * saturate((log2((((((BloomDirtMaskTint.x * _182.x) + 1.0f) * _163.x) * _217) + 0.002667719265446067f) + (((ColorScale0.x * _115.x) * _217) * _213)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.y * _182.y) + 1.0f) * _163.y) * _217) + 0.002667719265446067f) + (((ColorScale0.y * _128.y) * _217) * _214)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.z * _182.z) + 1.0f) * _163.z) * _217) + 0.002667719265446067f) + (((ColorScale0.z * _141.z) * _217) * _215)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset)));

  float3 untonemapped = float3(_115.x, _128.y, _141.z);
  float3 midgray = (((ColorScale0.rgb * 0.18f) * SceneColorApplyParamaters[0].rgb) * _217);
  float midgray_lum = renodx::color::y::from::BT709(midgray);
  return LutToneMap(untonemapped, _259.rgb, TEXCOORD, midgray_lum);

  float _263 = _259.x * 1.0499999523162842f;
  float _264 = _259.y * 1.0499999523162842f;
  float _265 = _259.z * 1.0499999523162842f;
  float _340;
  float _341;
  float _342;
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _277 = (pow(_263, 0.012683313339948654f));
    float _278 = (pow(_264, 0.012683313339948654f));
    float _279 = (pow(_265, 0.012683313339948654f));
    float _312 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_277 + -0.8359375f)) / (18.8515625f - (_277 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _313 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_278 + -0.8359375f)) / (18.8515625f - (_278 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _314 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_279 + -0.8359375f)) / (18.8515625f - (_279 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _340 = min((_312 * 12.920000076293945f), ((exp2(log2(max(_312, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _341 = min((_313 * 12.920000076293945f), ((exp2(log2(max(_313, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _342 = min((_314 * 12.920000076293945f), ((exp2(log2(max(_314, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _340 = _263;
    _341 = _264;
    _342 = _265;
  }
  float _349 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _352 = min(max((_349 * +1.#INF), -1.0f), 1.0f);
  float _361 = (_352 - (sqrt(saturate(1.0f - abs(_349))) * _352)) * BackbufferQuantizationDithering;
  SV_Target.x = (_361 + _340);
  SV_Target.y = (_361 + _341);
  SV_Target.z = (_361 + _342);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_263, _264, _265), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
