...
OutputSignature main(
  ...
  float4 _159 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * saturate((log2((((((BloomDirtMaskTint.x * _82.x) + 1.0f) * _63.x) * _117) + 0.002667719265446067f) + (((ColorScale0.x * _39.x) * _117) * _113)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.y * _82.y) + 1.0f) * _63.y) * _117) + 0.002667719265446067f) + (((ColorScale0.y * _39.y) * _117) * _114)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset), ((LUTScale * saturate((log2((((((BloomDirtMaskTint.z * _82.z) + 1.0f) * _63.z) * _117) + 0.002667719265446067f) + (((ColorScale0.z * _39.z) * _117) * _115)) * 0.0714285746216774f) + 0.6107269525527954f)) + LUTOffset)));
  ...
  return LutToneMap(untonemapped, _159.rgb, TEXCOORD, midgray_lum);
  ...
  if (!((uint)(bOutputInHDR) == 0)) {
    float _177 = (pow(_163, 0.012683313339948654f));
    float _178 = (pow(_164, 0.012683313339948654f));
    float _179 = (pow(_165, 0.012683313339948654f));
    float _212 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_177 + -0.8359375f)) / (18.8515625f - (_177 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _213 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_178 + -0.8359375f)) / (18.8515625f - (_178 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _214 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_179 + -0.8359375f)) / (18.8515625f - (_179 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _240 = min((_212 * 12.920000076293945f), ((exp2(log2(max(_212, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _241 = min((_213 * 12.920000076293945f), ((exp2(log2(max(_213, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _242 = min((_214 * 12.920000076293945f), ((exp2(log2(max(_214, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _240 = _163;
    _241 = _164;
    _242 = _165;
  }
  float _249 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _252 = min(max((_249 * 0x7FF0000000000000), -1.0f), 1.0f);
  float _261 = (_252 - (sqrt(saturate(1.0f - abs(_249))) * _252)) * BackbufferQuantizationDithering;
  SV_Target.x = (_261 + _240);
  SV_Target.y = (_261 + _241);
  SV_Target.z = (_261 + _242);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_163, _164, _165), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
