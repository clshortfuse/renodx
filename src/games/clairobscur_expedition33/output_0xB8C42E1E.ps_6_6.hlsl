...
OutputSignature main(
  ...
  float3 midgray = (((ColorScale0.rgb * 0.18f) * SceneColorApplyParamaters[0].rgb) * _117);
  float midgray_lum = renodx::color::y::from::BT709(midgray);
  return LutToneMap(untonemapped, _183.rgb, TEXCOORD, midgray_lum);
  ...
  if (!((uint)(bOutputInHDR) == 0)) {
    ...
    float _202 = (pow(_188, 0.012683313339948654f));
    float _203 = (pow(_189, 0.012683313339948654f));
    ...
    float _238 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_203 + -0.8359375f)) / (18.8515625f - (_203 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    ...
    _266 = min((_238 * 12.920000076293945f), ((exp2(log2(max(_238, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _264 = _187;
    _265 = _188;
    _266 = _189;
  }
  float _273 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _276 = min(max((_273 * 0x7FF0000000000000), -1.0f), 1.0f);
  float _285 = (_276 - (sqrt(saturate(1.0f - abs(_273))) * _276)) * BackbufferQuantizationDithering;
  SV_Target.x = (_285 + _264);
  SV_Target.y = (_285 + _265);
  SV_Target.z = (_285 + _266);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_187, _188, _189), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
