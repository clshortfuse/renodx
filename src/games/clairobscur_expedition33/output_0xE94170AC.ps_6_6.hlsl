...
OutputSignature main(
  ...
  float _212 = exp2(log2(((((ColorScale0.x * _44.x) * _107) * _184) + ((_68.x * _183) * ((BloomDirtMaskTint.x * _87.x) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _213 = exp2(log2(((((ColorScale0.y * _44.y) * _108) * _184) + ((_68.y * _183) * ((BloomDirtMaskTint.y * _87.y) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float _214 = exp2(log2(((((ColorScale0.z * _44.z) * _109) * _184) + ((_68.z * _183) * ((BloomDirtMaskTint.z * _87.z) + 1.0f))) * 0.009999999776482582f) * 0.1593017578125f);
  float4 _253 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * exp2(log2((1.0f / ((_212 * 18.6875f) + 1.0f)) * ((_212 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_213 * 18.6875f) + 1.0f)) * ((_213 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_214 * 18.6875f) + 1.0f)) * ((_214 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset)));
  ...
  return LutToneMap(untonemapped, _253.rgb, TEXCOORD, midgray_lum);
  
  float _257 = _253.x * 1.0499999523162842f;
  float _258 = _253.y * 1.0499999523162842f;
  float _259 = _253.z * 1.0499999523162842f;
  [branch]
  if (!((uint)(bOutputInHDR) == 0)) {
    float _271 = (pow(_257, 0.012683313339948654f));
    float _272 = (pow(_258, 0.012683313339948654f));
    float _273 = (pow(_259, 0.012683313339948654f));
    float _306 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_271 + -0.8359375f)) / (18.8515625f - (_271 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _307 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_272 + -0.8359375f)) / (18.8515625f - (_272 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
  ...
  float _343 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  float _346 = min(max((_343 * 0x7FF0000000000000), -1.0f), 1.0f);
  float _355 = (_346 - (sqrt(saturate(1.0f - abs(_343))) * _346)) * BackbufferQuantizationDithering;
  SV_Target.x = (_355 + _334);
  SV_Target.y = (_355 + _335);
  SV_Target.z = (_355 + _336);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_257, _258, _259), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
