...
#define LUT_EXTENSION_SAMPLE(color)                                            \
  SampleHDRLUT((color), sampleLinear_s, g_tHdrLut)
...
float3 HueAndChrominanceOKLab(float3 incorrect_color, float3 reference_color,
  ...
  if (hue_correct_strength != 0.0 || chrominance_correct_strength != 0.0 ||
    ...
    const float3 reference_oklab =
        renodx::color::oklab::from::BT709(reference_color);
    ...
    if (hue_correct_strength != 0.0) {
      ...
      perceptual_new.yz =
          lerp(perceptual_new.yz, reference_oklab.yz, hue_correct_strength);
      const float chrominancePost = length(perceptual_new.yz);
      chrominance_ratio =
          renodx::math::SafeDivision(chrominance_pre, chrominancePost, 1);
      chrominance_current = chrominancePost;
    }

    if (chrominance_correct_strength != 0.0) {
      const float reference_chrominance = length(reference_oklab.yz);
      float target_chrominance_ratio = renodx::math::SafeDivision(
          reference_chrominance, chrominance_current, 1);
      chrominance_ratio = lerp(chrominance_ratio, target_chrominance_ratio,
                               chrominance_correct_strength);
    }
...
void main(float4 v0 : SV_Position0, float2 v1 : TEXCOORD0,
  ...
  const float4 icb[] = {{0, 0, 1.000000, 0},        {0, 1.000000, 0, 0},
                        {1.000000, 0, 0, 0},        {1.000000, 0, 0, 0},
                        {0, 1.000000, 0, 0},        {0, 0, 1.000000, 0},
...
#else
  r0.rgb = untonemapped;
  float3 vanilla = SampleHDRLUT(untonemapped, sampleLinear_s, g_tHdrLut);
  if (RENODX_TONE_MAP_TYPE) {
    ...
    r0.rgb = ApplyExposureContrastFlareHighlightsShadowsByLuminance(
        r0.rgb, untonemapped_lum, cg_config);

    // HDR LUT has grading + curve, but the curve is different from SDR (grading
    // seems similar) Workaround is to correct luminance and manually match
    r0.rgb = SampleHDRLUTShoulderExtended(r0.rgb);

    const float y_tonemapped = max(0, renodx::color::y::from::BT709(r0.rgb));
    const float y_untonemapped =
        max(0, renodx::color::y::from::BT709(untonemapped));

    r0.rgb = max(0, renodx::color::correct::Luminance(
                        r0.rgb, y_tonemapped,
                        lerp(y_tonemapped, y_untonemapped, 0.07f)));

    r0.rgb = ApplySaturationBlowoutHueCorrectionHighlightSaturation(
        r0.rgb, untonemapped_lum, cg_config);
  } else {
    r0.rgb = vanilla;
  }
  ...
  // o0.xyz = DrawLUTShoulderDebug(v0.xy, o0.xyz);

  o0.w = 1;

  o0 = ProcessColor(o0, v0);
