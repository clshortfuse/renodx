#include "./shared.h"
#include "./eyeadaptation/transport.hlsl"
...
// at the adapting background (heuristic tie between detectability and cone
// gain).
float3 CastleDechroma_CVVDPStyle_NakaRushton(float3 rgb_lin,
  ...
  float3x3 XYZ_TO_LMS_PROPOSED_2023 =
      float3x3(0.185083290265044, 0.584080232530060, -0.0240724126371618,
  ...
  // CastleCSF sensitivity at background (achromatic) -> contrast threshold
  // proxy.
  ...
  float S_ach = renodx::color::castlecsf::Eq27_29_MechSens(
                    rho, omega, ecc, vis_field, area, Lbkg_nits)
                    .x;
  float c_thr = 1.0f / max(S_ach, 1e-6f);

  float r_target = clamp(nr_response_at_thr, 1e-3f, 0.999f);
  float sigma_scale =
      pow((1.0f - r_target) / r_target, 1.0f / max(nr_n, 1e-3f));
  ...
  float3 lms_rel_nr =
      float3(NR(lms_rel.x, sigma_rel, nr_n), NR(lms_rel.y, sigma_rel, nr_n),
  ...
  float3 testout =
      mul(XYZ_TO_BT709, mul(LMS_TO_XYZ_2006, lms_stim_nr)) / diffuse_white;
  float luminance_out = renodx::color::y::from::BT709(testout);

  return testout * renodx::math::DivideSafe(luminance_in, luminance_out);
}

float3 Unclamp(float3 original_gamma, float3 black_gamma, float3 mid_gray_gamma,
               float3 neutral_gamma) {
  const float3 added_gamma = black_gamma;

  const float mid_gray_average =
      (mid_gray_gamma.r + mid_gray_gamma.g + mid_gray_gamma.b) / 3.f;

  // Remove from 0 to mid-gray
  const float shadow_length = mid_gray_average;
  const float shadow_stop =
      max(neutral_gamma.r, max(neutral_gamma.g, neutral_gamma.b));
  const float3 floor_remove =
      added_gamma * max(0, shadow_length - shadow_stop) / shadow_length;
...
float3 ApplyExposureContrastFlareHighlightsShadowsByLuminance(
    float3 untonemapped, float y, renodx::color::grade::Config config,
  ...
  // gamma
  float lum_gamma_adjusted =
      renodx::math::Select(y < 1.f, pow(y, RENODX_TONE_MAP_GAMMA), y);
  ...
  // contrast & flare
  float flare =
      renodx::math::DivideSafe(y_normalized + config.flare, y_normalized, 1.f);
  float exponent = config.contrast * flare;
  const float y_contrasted = pow(y_normalized, exponent);

  // highlights
  float y_highlighted = pow(y_contrasted, config.highlights);
  y_highlighted = lerp(y_contrasted, y_highlighted,
                       saturate(y_contrasted / highlight_mask));

  // shadows
  float y_shadowed = pow(y_highlighted, -1.f * (config.shadows - 2.f));
  y_shadowed =
      lerp(y_shadowed, y_highlighted, saturate(y_highlighted / shadow_mask));
...
float3 ApplySaturationBlowoutHueCorrectionHighlightSaturation(
    float3 tonemapped, float y, renodx::color::grade::Config config) {
  ...
  if (config.saturation != 1.f || config.dechroma != 0.f || config.blowout != 0.f) {
    ...
    if (config.dechroma != 0.f) {
      perceptual_new.yz *=
          lerp(1.f, 0.f,
               saturate(pow(y / (10000.f / 100.f), (1.f - config.dechroma))));
    }
    ...
    if (config.blowout != 0.f) {
      ...
      float blowout_change =
          pow(1.f - percent_max, blowout_strength * abs(config.blowout));
      if (config.blowout < 0) {
...
float3 SampleScaledLUT(Texture3D<float4> lut, renodx::lut::Config lut_config,
                       float3 color) {
  ...
  if (CUSTOM_LUT_SCALING != 0.f) {
    ...
    float3 unclamped_gamma = renodx::lut::Unclamp(
        output_gamma, black_gamma, midgray_gamma, peak_gamma, input_gamma);

    float3 recolored = renodx::lut::RecolorUnclamped(
        color, renodx::color::srgb::DecodeSafe(unclamped_gamma));
...
float3 SampleHDRLUT(float3 untonemapped, SamplerState sampleLinear_s,
                    Texture3D<float4> g_tHdrLut) {
  float3 color = untonemapped;
...
float3 SampleSDRLUT(float3 lut_input_color, SamplerState sampleLinear_s,
                    Texture3D<float4> g_tLdrLut) {
...
float4 ProcessColor(float3 untonemapped, float3 graded) {
  ...
  if (RENODX_TONE_MAP_TYPE != 0.f) {
    color.rgb = renodx::tonemap::UpgradeToneMap(
        untonemapped, renodx::tonemap::neutwo::MaxChannel(untonemapped),
        color.rgb, 1);
  }
...
float4 ProcessColor(float4 color, float4 sv_position) {
  float peak_ratio = RENODX_PEAK_NITS / RENODX_GAME_NITS;
  [branch] if (RENODX_GAMMA_CORRECTION != 0.f) {
  ...
  // // SF says psycho should only apply to untonemapped
  // color.rgb = renodx::tonemap::psychov::psychotm_test17(
  //     color.rgb, peak_ratio, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f,
  //     RENODX_RENO_DRT_WHITE_CLIP, 1.f, 1.f, 0, 1.f, eye_adaptation.xxx,
  //     eye_adaptation.yyy);

  // RenodxResolveEyeAdaptationTransport(sv_position);
  color.rgb = renodx::color::bt2020::from::BT709(color);
  color.rgb = renodx::tonemap::neutwo::MaxChannel(color.rgb, peak_ratio,
                                                  RENODX_RENO_DRT_WHITE_CLIP);
  color.rgb = renodx::color::bt709::from::BT2020(color);
...
float4 ProcessColor(float4 color) {
  return ProcessColor(color, float4(1.f, 1.f, 0.f, 0.f));
}