...
// Preserve the original effective behavior: the later definition wins in the
// source you provided, so use color_grade_strength directly without a redefinition.
#undef CUSTOM_GRADING_STRENGTH
#define CUSTOM_GRADING_STRENGTH shader_injection.color_grade_strength

// Must be 32-bit aligned.
// Keep the total float count a multiple of 4 so the injected constant-buffer
// payload remains 16-byte aligned.
struct ShaderInjectData {
  ...
  // PsychoV24 controls. The previous four Psycho controls remain in-place.
  // V24 adds two more parameters; trailing reserved floats keep the complete
  // CPU/HLSL payload size on a 16-byte boundary without moving earlier fields.
  ...
  float psychov24_reserved0;
  float psychov24_reserved1;
  float psychov24_reserved2;
};
...
#endif  // AssassinsCreedLiberation_SHARED_H_
