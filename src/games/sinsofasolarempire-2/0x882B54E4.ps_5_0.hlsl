...
// General-purpose mesh PBR shader: parallax occlusion, clustered lighting, optional retro filter.
// Comments identify the logical blocks in the decompiled register operations.
void main(
  ...
  out float4 o1 : SV_TARGET1)
{
  ...
  // Vertex payload array reused for shadow casters later in the shader.
  float4 v[4] = { v0,v1,v2,v3 };
  // --- Parallax occlusion branch driven by mask texture.
  ...
  r0.z = r0.w ? r0.z : 0;
  if (r0.z != 0) {
    // Evaluate local height neighbourhood to get UV offsets and bent normals.
    ...
    // Fallback: sample all material textures at mesh UV.
    r0.x = 0;
  ...
  if (r2.x != 0) {
    // Retro post-process path creates warbly base colour input before lighting.
  ...
  // --- View vector + PBR scalar prep (roughness, Fresnel, translucent mask).
  // --- View vector and Cook-Torrance scalar setup.
  ...
  // --- Clustered light selection and optional shadow filter.
  r0.x = log2(v1.z);
  ...
  r4.w = r0.x ? r4.w : 0;
  if (r4.w == 0) {
    // Run percentage-closer filtering on the cascaded shadow map.
    r5.w = ~(int)r0.x;
  ...
  // --- Loop visible lights and integrate diffuse/specular contributions.
  ...
  while (true) {
    ...
    if (r15.x == 0) {
      ...
      if (r15.x == 0) {
        // Point light attenuation branch.
        ...
        if (r15.y != 0) {
          ...
          // Spot (type 2) and tube (type 3) light handling.
  ...
  if (r2.y != 0) {
    ...
    // --- Environment lighting evaluation using irradiance/DFG lookup.
