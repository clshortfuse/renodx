...
// Physically based surface shader compiled by 3Dmigoto. Registers rN carry the usual PBR terms.
// We annotate the major stages: parallax occlusion, normal reconstruction, clustered lighting,
// screen-space shadows, and environment lighting.
void main(
  ...
  out float4 o1 : SV_TARGET1)
{
  ...
  // Vertex payload replicated into an array because of the decompiler; indices >3 address shadow data.
  float4 v[4] = { v0,v1,v2,v3 };
  // --- Parallax occlusion mapping driven by the blue mask channel. Computes offset UVs if mask is active.
  ...
  if (r0.z != 0) {
    ...
    // No parallax: use original UVs straight from the mesh.
    r0.x = 0;
  ...
  r0.x = r0.z ? r0.x : r0.y;
  // --- Reconstruct tangent-space normal and apply blend with geometric basis.
  ...
  if (r2.x != 0) {
    // Retro mode: add time varying dithering and low resolution sampling before the main PBR pass.
  ...
  // --- View vector, roughness and Fresnel terms for Cook-Torrance lighting.
  ...
  r3.xyzw = r3.wxyz * base_color_factor.wxyz + float4(-0.99000001,-0.0399999991,-0.0399999991,-0.0399999991);
  r0.x = cmp(r3.x < 0);
  if (r0.x != 0) discard;
  ...
  // --- Clustered forward lighting setup: pick froxel, pull the light list and decide if PCF shadows are on.
  r0.x = log2(v1.z);
  ...
  r4.w = r0.x ? r4.w : 0;
  if (r4.w == 0) {
    // --- Percentage-closer filtering over cascaded shadow map. The nested loops march a 5x5 kernel.
    r5.w = ~(int)r0.x;
  ...
  // --- Loop over clustered lights, accumulate diffuse and specular terms per light type.
    ...
    if (r15.x == 0) {
      ...
      if (r15.x == 0) {
        ...
        if (r15.y != 0) {
          ...
          // Spot lights and line lights share the same branch, check the encoded type id.
  ...
  if (r2.y != 0) {
    ...
    // --- Environment lighting path when no direct lights contributed.
    ...
    r0.xyz = r0.xyz * r4.xxx + r4.yyy; // Split-sum approximation using preintegrated DFG.
