...
float4 main(
  ...
  float3 uiTexture = float3(_40, _41, _42);
  ...
  uiTexture = renodx::color::bt2020::from::BT709(uiTexture);

  float uiControl = $Globals_007w;
  float uiLevel = $Globals_007z;
  ...
  // merging UI and game
  float3 output = uiTexture.rgb * uiLevel;
  output.rgb = output.rgb + float3(_123, _124, _125);
  output = renodx::color::pq::Encode(output.rgb, injectedData.toneMapGameNits);
  return float4(output, 1.f);

  float _141 = exp2(((log2((((_119 * _122) + (((uiControl) * (mad(0.04330150783061981f, _42, (mad(0.32926714420318604f, _41, (_40 * 0.6274880170822144f)))))) * (uiLevel))) * 9.999999747378752e-05f))) * 0.1593017578125f));
  float _142 = exp2(((log2((((_120 * _122) + (((uiControl) * (mad(0.011359544470906258f, _42, (mad(0.9195171594619751f, _41, (_40 * 0.06910824030637741f)))))) * (uiLevel))) * 9.999999747378752e-05f))) * 0.1593017578125f));
  float _143 = exp2(((log2((((_121 * _122) + (((uiControl) * (mad(0.8954997062683105f, _42, (mad(0.08802297711372375f, _41, (_40 * 0.016396233811974525f)))))) * (uiLevel))) * 9.999999747378752e-05f))) * 0.1593017578125f));
