...
float4 main(
  ...
  float3 ap1_graded_color = float3(_574, _575, _576);  // CustomEdit
  ...
  float3 ap1_aces_colored = float3(_705, _706, _707);  // CustomEdit
  ap1_aces_colored.rgb = ((ap1_aces_colored.rgb - _708) * 0.9599999785423279f) + _708;
  ...
  if (shouldTonemap) {
    ...
    sdr_ap1_color = float3(_855, _856, _857);
    sdr_ap1_color = max(0.0f, (((sdr_ap1_color - _858) * 0.9300000071525574f) + _858));
    // end of FilmToneMap
  }
  ...
  float _878 = ((cb0_039x) * (sdr_ap1_color.r - _574)) + _574;
  float _879 = ((cb0_039x) * (sdr_ap1_color.g - _575)) + _575;
  float _880 = ((cb0_039x) * (sdr_ap1_color.b - _576)) + _576;
  ...
  float _971 = exp2(((log2((max(0.0f, _945)))) * (gamma)));
  float _972 = exp2(((log2((max(0.0f, _946)))) * (gamma)));
  float _973 = exp2(((log2((max(0.0f, _947)))) * (gamma)));

  // CustomEdit
  float3 film_graded_color = float3(_971, _972, _973);
  ...
  if (shouldTonemap) {
    ...
    final_color = final_color * 0.9523810148239136f;
  }
