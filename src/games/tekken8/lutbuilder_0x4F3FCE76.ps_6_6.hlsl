...
float4 main(
  ...
  float3 ap1_graded_color = float3(_827, _828, _829);  // CustomEdit
  ...
  float3 ap1_aces_colored = float3(_958, _959, _960);  // CustomEdit
  ap1_aces_colored = ((ap1_aces_colored - _961) * 0.9599999785423279f) + _961;
  ...
  if (shouldTonemap) {
    ...
    sdr_ap1_color = float3(_1108, _1109, _1110);
    sdr_ap1_color = (max(0.0f, (((sdr_ap1_color - _1111) * 0.9300000071525574f) + _1111)));
    // end of FilmToneMap
  }
  ...
  float _1131 = ((cb0_039x) * (sdr_ap1_color.r - _827)) + _827;
  float _1132 = ((cb0_039x) * (sdr_ap1_color.g - _828)) + _828;
  float _1133 = ((cb0_039x) * (sdr_ap1_color.b - _829)) + _829;
  ...
  float _1224 = exp2(((log2((max(0.0f, _1198)))) * (gamma)));
  float _1225 = exp2(((log2((max(0.0f, _1199)))) * (gamma)));
  float _1226 = exp2(((log2((max(0.0f, _1200)))) * (gamma)));
  // CustomEdit
  float3 film_graded_color = float3(_1224, _1225, _1226);
