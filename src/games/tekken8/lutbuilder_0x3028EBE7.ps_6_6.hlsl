...
float4 main(
  ...
  float3 untonemapped_ap1 = float3(_829, _830, _831);  // CustomEdit
  ...
  float _1332 = exp2(((log2((max(0.0f, _1306)))) * (gamma)));
  float _1333 = exp2(((log2((max(0.0f, _1307)))) * (gamma)));
  float _1334 = exp2(((log2((max(0.0f, _1308)))) * (gamma)));
  // CustomEdit
  float3 film_graded_color = float3(_1332, _1333, _1334);
  if (shouldTonemap) {
    return LutBuilderToneMap(untonemapped_ap1, film_graded_color);
  }
