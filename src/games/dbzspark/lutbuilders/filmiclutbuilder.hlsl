...
// THIS FILE HAS CUSTOM VALUES AND CODE FOR DBZ! 
...
float GetUnrealFilmicMidGrayScale() {
  float film_black_clip = cb0[37].w;
  float film_toe = cb0[37].y;
  float film_white_clip = cb0[38].x;
  float film_shoulder = cb0[37].z;
  float film_slope = cb0[37].x;
  ...
  float _1067 = (_1007 < _998) ? ((_1016 / (exp2((_1019 * 1.4426950216293335f) * _1018) + 1.0f)) - film_black_clip) : _1013;
  float _1076 = _1003 - _998;
  float _1080 = saturate(_1019 / _1076);
  bool _1083 = (_1003 < _998);
  float _1087 = _1083 ? (1.0f - _1080) : _1080;
  float _1108 = (((_1087 * _1087) * (((_1007 > _1003) ? (_978 - (_1040 / (exp2(((_1007 - _1003) * 1.4426950216293335f) * _1042) + 1.0f))) : _1013) - _1067)) * (3.0f - (_1087 * 2.0f))) + _1067;
...
#define UNREALFILMIC_GENERATOR(T)                                                                                                                                                                                                                                                        \
  T ApplyUnrealFilmicToneMap(T untonemapped) {                                                                                                                                                                                                                                           \
    ...
    float _1040 = _980 * 2.0f;                                                                                                                                                                                                                                                           \
    float _1042 = (film_slope * 2.0f) / _980;                                                                                                                                                                                                                                            \
    T _1067_1068_1069 = ((_1007_1008_1009 < _998) ? ((_1016 / (exp2((_1019_1020_1021 * 1.4426950216293335f) * _1018) + 1.0f)) - film_black_clip) : _1013_1014_1015);                                                                                                                     \
    float _1076 = _1003 - _998;                                                                                                                                                                                                                                                          \
    T _1080_1081_1082 = saturate(_1019_1020_1021 / _1076);                                                                                                                                                                                                                               \
    bool _1083 = (_1003 < _998);                                                                                                                                                                                                                                                         \
    T _1087_1088_1089 = (_1083 ? (1.0f - _1080_1081_1082) : _1080_1081_1082);                                                                                                                                                                                                            \
    T _1108_1109_1110 = (((_1087_1088_1089 * _1087_1088_1089) * (((_1007_1008_1009 > _1003) ? (_978 - (_1040 / (exp2(((_1007_1008_1009 - _1003) * 1.4426950216293335f) * _1042) + 1.0f))) : _1013_1014_1015) - _1067_1068_1069)) * (3.0f - (_1087_1088_1089 * 2.0f))) + _1067_1068_1069; \
                                                                                                                                                                                                                                                                                         \
...
float3 LerpToneMapStrength(float3 tonemapped, float3 preRRT) {
  float tone_map_curve = cb0[36].w;
  // tone_map_curve = 1.f; // Specific to DBZ!
...
float3 ApplyBlueCorrection(float3 tonemapped) {
  float blue_correction = cb0[36].y;
...
void ApplyFilmicToneMap(
    inout float3 input_color,
    float3 preRRT) {
  float3 tonemapped;
  float3 untonemapped_pre_grade = input_color;
  ...
  if (RENODX_TONE_MAP_TYPE == 2.f) {  // ACES
    ...
    tonemapped = LerpToneMapStrength(untonemapped, preRRT);
  } else {
    tonemapped = ApplyVanillaToneMap(untonemapped, preRRT);
  ...
  if (RENODX_TONE_MAP_TYPE != 4.f) {
    tonemapped = ApplyExposureContrastFlareHighlightsShadowsByLuminance(tonemapped, untonemapped_lum, cg_config);

    tonemapped = renodx::color::ap1::from::BT709(ApplySaturationBlowoutHueCorrectionHighlightSaturation(renodx::color::bt709::from::AP1(tonemapped), saturate(renodx::color::bt709::from::AP1(preRRT)), untonemapped_lum, cg_config));
  }

  input_color = tonemapped;
  return;
}
...
float3 SamplePacked1DLut(
  ...
  const float lut_weights_x = cb0[5].x;
  const float lut_weights_y = cb0[5].y;
  const float lut_weights_z = cb0[5].z;
  const float lut_weights_w = cb0[5].w;
...
void SampleLUTUpgradeToneMap(float3 color_lut_input, SamplerState lut_sampler, Texture2D<float4> lut_texture, inout float3 output) {
...
void SampleLUTUpgradeToneMap(float3 color_lut_input, SamplerState lut_sampler_1, SamplerState lut_sampler_2, Texture2D<float4> lut_texture_1, Texture2D<float4> lut_texture_2, inout float3 output) {
  float3 color_output = color_lut_input;
...
void SampleLUTUpgradeToneMap(float3 color_lut_input, SamplerState lut_sampler_1, SamplerState lut_sampler_2, SamplerState lut_sampler_3, Texture2D<float4> lut_texture_1, Texture2D<float4> lut_texture_2, Texture2D<float4> lut_texture_3, inout float3 output) {
  float3 color_output = color_lut_input;
