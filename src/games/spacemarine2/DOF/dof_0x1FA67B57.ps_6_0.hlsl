...
cbuffer CB_SCREEN_RECT_DATA : register(b2) {
  float CB_SCREEN_RECT_DATA_000x : packoffset(c000.x);
};
...
float4 main(
  ...
  float4 SV_Target;
  float _10 = (CB_SCREEN_RECT_DATA_000x) * (CB_PASS_DOF_000x);
  float _11 = _10 * 3.200000047683716f;
  float _12 = (TEXCOORD.x) - _11;
  float4 _14 = DOF_BUFFER_0.Sample(PS_SAMPLERS[4], float2(_12, (TEXCOORD.y)));
  float _19 = _10 * 1.3333333730697632f;
  float _20 = (TEXCOORD.x) - _19;
  float4 _22 = DOF_BUFFER_0.Sample(PS_SAMPLERS[4], float2(_20, (TEXCOORD.y)));
  float4 _28 = DOF_BUFFER_0.Sample(PS_SAMPLERS[4], float2((TEXCOORD.x), (TEXCOORD.y)));
  float _33 = _19 + (TEXCOORD.x);
  float4 _35 = DOF_BUFFER_0.Sample(PS_SAMPLERS[4], float2(_33, (TEXCOORD.y)));
  float _40 = _11 + (TEXCOORD.x);
  float4 _42 = DOF_BUFFER_0.Sample(PS_SAMPLERS[4], float2(_40, (TEXCOORD.y)));
