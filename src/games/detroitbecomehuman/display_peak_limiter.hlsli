...
vec3 LimitDisplayLightPeak(vec3 display_light_bt709)
{
    if (!CUSTOM_HDR_ACTIVE || shader_injection.tone_map_type == 0.0)
    {
        return display_light_bt709;
    }
    ...
    const float output_display_peak = max(
        max(display_light_bt709.x, display_light_bt709.y),
        display_light_bt709.z);
    if (output_display_peak > configured_display_peak)
    {
        display_light_bt709 *= configured_display_peak /
