...
float3 LinearOutput(float3 color, Config lut_config) {
  if (lut_config.type_output == config::type::SRGB) {
    ...
    color = renodx::math::CopySign(color) * renodx::color::arri::logc::c800::Decode(abs(color));
  } else if (lut_config.type_output == config::type::ARRI_C1000) {
    color = renodx::math::CopySign(color) * renodx::color::arri::logc::c1000::Decode(abs(color));
  } else if (lut_config.type_output == config::type::ARRI_C800_NO_CUT) {
    color = renodx::math::CopySign(color) * renodx::color::arri::logc::c800::Decode(abs(color), false);
  } else if (lut_config.type_output == config::type::ARRI_C1000_NO_CUT) {
    color = renodx::math::CopySign(color) * renodx::color::arri::logc::c1000::Decode(abs(color), false);
  }
  return color;
}
