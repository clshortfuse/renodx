...
float3 srgbPostProcess(float3 srgb, float3 a, float3 b, float bits = 8.f) {
  ...
  return srgb + (((a - 0.5f) + (min(min(1.0f, srgb * maxValueX2),
                                    maxValueX2 - (srgb * maxValueX2)) *
                                (b - 0.5f))) /
                 maxValue);
}
...
float4 main(PSSceneIn input) : SV_TARGET {
  ...
  const float4 inputColor =
    _8.Load(int3(uint2(uint(gl_FragCoord.x), uint(gl_FragCoord.y)), 0u));
  ...
  float _119 =
    dot(float3(_112, _113, _114),
        float3(_113 + 33.3300018310546875f, _112 + 33.3300018310546875f, _118));
  ...
  float _144 =
    dot(float3(_140, _141, _114),
        float3(_141 + 33.3300018310546875f, _140 + 33.3300018310546875f, _118));
  ...
  const float3x3 rgbColorMatrix =
    float3x3(pixConsts.pqMatrix[0][0], pixConsts.pqMatrix[1][0],
             pixConsts.pqMatrix[2][0], pixConsts.pqMatrix[0][1],
             pixConsts.pqMatrix[1][1], pixConsts.pqMatrix[2][1],
             pixConsts.pqMatrix[0][2], pixConsts.pqMatrix[1][2],
             pixConsts.pqMatrix[2][2]);
  ...
  switch (pixConsts.outputTypeEnum) {
    case OUTPUT_TYPE_SRGB8: {
      ...
      float3 postProcessedSRGB =
        srgbPostProcess(srgbColor.rgb, minSRGB, maxSRGB, 8.f);
    ...
    case OUTPUT_TYPE_PQ: {
      ...
      float3 newShiftedColor =
        ((matrixedColor - rec2020) * pixConsts.pqSaturation) + rec2020;
    ...
    case OUTPUT_TYPE_SRGB10: {
      ...
      float3 postProcessedSRGB =
        srgbPostProcess(srgbColor.rgb, minSRGB, maxSRGB, 10.f);
      outputColor.rgb = postProcessedSRGB.rgb;
    } break;
    default:
      // Unknown default case
      outputColor.rgb =
        (correctedColor.rgb * paperWhiteScaling) + pixConsts.blackFloorAdjust;
  }
