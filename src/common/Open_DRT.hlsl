...
float3 open_drt_transform(
  ...
  const static float dch = 0.40f;

  // Chroma contrast
  const static float chc_p = 1.2f;  // amount of contrast
  const static float chc_m = 0.5f;  // pivot of contrast curve

  // Tonescale parameters
  const float c = 1.1f * contrast;  // contrast
  ...
  */
  ...
  const static float3 chc_L_weights = float3(0.23f, 0.69f, 0.08f);  // P3 Weights
...
float3 open_drt_transform_bt709(
  float3 rgb,
  float Lp = 100.f,
  float gb = 0.12,
  float contrast = 1.f,
  ...
  weights = float3(BT709_2_XYZ_MAT[1].rgb);
  ...
  */
  ...
  // float chc_L = 0.23f*rats.x + 0.69f*rats.y + 0.08f*rats.z; // Roughly P3 weights, doesn't matter
  // const static float3 chc_L_weights = float3(0.23f, 0.69f, 0.08f);  // P3 Weights
  float chc_L = dot(rats, BT709_2_XYZ_MAT[1].rgb);
