...
static const float3x3 BT72020_2_AP0_MAT = mul(XYZ_2_AP0_MAT, mul(D65_2_D60_CAT, BT2020_2_XYZ_MAT));
...
float3 main(float4 pos : SV_Position, float2 texcoord : TexCoord ) : COLOR
{
	...
	float3 outputColor = aces_rrt_odt(
		inputColor,
		yMin,
		48.f * (peakNits / paperWhite),
		clampMatrix
	);
	outputColor = mul(outputMatrix, odtFinal);
