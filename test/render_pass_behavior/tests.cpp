...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "usage: render_pass_behavior_tests <app> <addon> <reshade-dir>\n";
    return 2;
  }
  ...
  const fs::path runtime = fs::temp_directory_path()
                           / ("renodx_render_pass_" + std::to_string(GetCurrentProcessId()));
  std::error_code error;
  ...
  const fs::path app = runtime / "render_pass_behavior_app.exe";
  ...
  SetEnvironmentVariableW(L"RENODX_RENDER_PASS_RESULT", result_path.c_str());
  ...
  const bool addon_loaded = log.find(addon.filename().string()) != std::string::npos
                            || log.find("RenoDX RenderPass Behavior Test") != std::string::npos;
